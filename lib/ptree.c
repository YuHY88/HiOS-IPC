...
/* Free route tree. */
void ptree_free (struct ptree *rt)
{
...
/* Remove route tree. */
void ptree_finish (struct ptree *rt)
{
    ptree_free (rt);
}
...
static struct ptree_node *ptree_node_create (u_int16_t key_len)
{
 ...
 */
int ptree_key_match1 (u_char *np, u_int16_t n_len, u_char *pp, u_int16_t p_len)
{
    int shift;
    int offset;
...
static struct ptree_node *ptree_node_set (struct ptree *tree, u_char *key, u_int16_t key_len)
{
    struct ptree_node *node;
...
static struct ptree_node *ptree_node_common (struct ptree_node *n, u_char *pp, u_int16_t p_len)
{
    int i;
    int j;
    u_char diff;
...
static void ptree_set_link (struct ptree_node *node, struct ptree_node *new)
{
    int bit;
...
struct ptree_node *ptree_node_match1 (struct ptree *tree, u_char *key, u_int16_t key_len)
{
    ...
    struct ptree_node *matched1;
    uint32_t key_tmp = *((uint32_t *)key);
    u_char *pkey = NULL;

    pkey = key;
    if (tree->max_key_len == IPV4_MAX_PREFIXLEN)
    {
                                   ...
                                   node->key_len, pkey, key_len))
    {
        ...
        matched1 = node;
        node = node->link[ptree_check_bit (tree, pkey, node->key_len)];

        if (matched1->key_len >= key_len)
        {
            break;
        }
    }
...
struct ptree_node *ptree_node_match_exclude (struct ptree *pt, u_char *key1, u_int16_t key_len1,
 ...
 */
//struct ptree_node *ptree_node_sub_tree_lookup (struct ptree *tree,
...
struct ptree_node *ptree_node_get (struct ptree *tree, u_char *key, u_int16_t key_len)
{
    ...
    node = tree->top;
    while (node && node->key_len <= key_len &&
         ptree_key_match (PTREE_NODE_KEY (node), node->key_len, pkey, key_len))
    {
    ...
    if (node == NULL)
    {
        new = ptree_node_set (tree, pkey, key_len);
        if (match)
        {
    ...
    {
        new = ptree_node_common (node, pkey, key_len);
...
void ptree_node_delete_all (struct ptree *rt)
{
    struct ptree_node *tmp_node;
    struct ptree_node *node;
...
struct ptree_node *ptree_next_until (struct ptree_node *node, struct ptree_node *limit)
{
    struct ptree_node *next, *start;
