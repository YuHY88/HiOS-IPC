...
static int 
cpu_record_hash_cmp (const struct cpu_thread_history *a, const struct cpu_thread_history *b)
{
...
cpu_record_hash_alloc (struct cpu_thread_history *a)
{
  ...
  new = XCALLOC (MTYPE_THREAD, sizeof (struct cpu_thread_history));
  new->func = a->func;
...
cpu_record_hash_free (void *a)
{
  ...
  XFREE (MTYPE_THREAD, hist);
}

static void 
vty_out_cpu_thread_history(struct vty* vty, struct cpu_thread_history *a)
{
#ifdef HAVE_RUSAGE
...
int thread_timer_cmp(void *a, void *b)
{
...
struct thread_master *thread_master_create ()
{
    ...
    rv = XCALLOC (MTYPE_THREAD, sizeof (struct thread_master));
...
thread_master_free (struct thread_master *m)
{
  ...
  thread_list_free (m, &m->event_normal);
  thread_list_free (m, &m->ready);
  thread_list_free (m, &m->unuse);
  thread_queue_free (m, m->background);
  
  XFREE (MTYPE_THREAD, m);
	    ...
	    int (*func) (struct thread *), void *arg, debugargdef)
{
  ...
  if(m->func)
  	thread->cmd_excute = m->func;
	else
		thread->cmd_excute = NULL;

	if(m->config)
	  thread->config_finish = m->config;
	else	
...
/* Add simple event thread. */
struct thread *
funcname_thread_add_event_normal (struct thread_master *m,
		  int (*func) (struct thread *), void *arg, int val,
		  debugargdef)
{
  struct thread *thread;
  ...
  thread = thread_get (m, THREAD_EVENT_NORMAL, func, arg, debugargpass);
  thread->u.val = val;
  thread_list_add (&m->event_normal, thread);
...
thread_cancel (struct thread *thread)
{
  ...
  switch (thread->type)
    ...
    case THREAD_EVENT:
      ...
      break;
	case THREAD_EVENT_NORMAL:
      list = &thread->master->event_normal;
      break;
    case THREAD_READY:
...
/* Delete all events which has argument value arg. */
unsigned int
thread_cancel_event_normal (struct thread_master *m, void *arg)
{
  unsigned int ret = 0;
  struct thread *thread;

  thread = m->event_normal.head;
  while (thread)
    {
      struct thread *t;
      ...
      if (t->arg == arg)
        {
          ret++;
          thread_list_delete (&m->event_normal, t);
          t->type = THREAD_UNUSED;
          thread_add_unuse (m, t);
        }
    }
...
//struct timeval *timer_wait_bg;
  ...
  while (1)
    {
      int num = 0;
#if 0 //defined HAVE_SNMP && defined SNMP_AGENTX close by suxq close by suxq 2019 03 22
      ...
      if(m->ready.count == 0)
      {
          timer_val.tv_sec = 1;   /*timeout 1s*/
      }

#else //     close by suxq 2019 03 22
...
#endif       
#if 0 //defined HAVE_SNMP && defined SNMP_AGENTX close by suxq 2019 03 22
...
#if 0  //disable timer and timer event by suxq
      time_get_relative_time (NULL);
...
#if 0  //disable all timer by suxq      
...
/* Execute thread */
struct thread *
funcname_thread_execute_normal (struct thread_master *m,
                int (*func)(struct thread *), 
                void *arg,
                int val,
		debugargdef)
{
  ...
  dummy.type = THREAD_EVENT_NORMAL;
  dummy.add_type = THREAD_EXECUTE;
  dummy.master = NULL;
  dummy.func = func;
  dummy.arg = arg;
