...
int work_queue_should_yield (struct work_queue * wq)
{
  time_get_relative_time (NULL);
  return (timeval_elapsed(relative_time, wq->real) >
...
work_queue_schedule (struct work_queue *wq, unsigned int delay)
{
  ...
  if ( CHECK_FLAG (wq->flags, WQ_UNPLUGGED)
       ...
       && (listcount (wq->items) > 0) )
    {
	  ...
	  wq->queue_id =  high_pre_timer_add("work_queue_run", LIB_TIMER_TYPE_NOLOOP, work_queue_run, (void *)wq, delay);
