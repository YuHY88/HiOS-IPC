...
enum node_type
{
  ...
  POOL_NODE,        /* dhcp pool */
  AAA_NODE, 		/* AAA node. */
  AAA_LOCAL_USER_NODE, 		/* AAA local-user node. */
  SNMPD_NODE, 		/* SNMPD node. */
  OPENFLOW_NODE,
  ...
  /**/
  VRF_NODE,			/* VRF node. */
  ARP_NODE,			/* arp connection node. */
  NDP_NODE,         /* NDP node*/
  ROUTE_NODE,		/* Static ip route node. */
  ...
  OSPF_DCN_NODE,	/* ospf dcn node(add by zzl)*/
  AREA_NODE,		/* ospf area */
  OSPF6_NODE,
  ...
  /* mpls */
  MPLS_NODE,		/* Mpls node */
  MPLSTP_OAM_MEG_NODE,  /* MPLSTP_OAM meg*/
  MPLSTP_OAM_SESS_NODE, /*MPLSTP_OAM node*/
  BFD_TEMPLATE_NODE,
  BFD_SESSION_NODE, /*BFD node*/
  VSI_NODE,         /* Vsi node Under the Mpls node */
  LSP_NODE,			/* Staitc-lsp node Under the Mpls node */
  PW_NODE,			/* Pw node Under the Mpls node */
  APS_NODE,
  MSTP_NODE,
  L3VPN_NODE,		/* L3vpn node Under the Mpls node */

  /**/
  INTERFACE_NODE,
  LOOPBACK_IF_NODE,
  LLDP_NODE,			/* lldp connection node */
  VLANIF_NODE,
  TRUNK_IF_NODE,
  TRUNK_SUBIF_NODE,
  TUNNEL_IF_NODE,
  ...
  DEBUG_NODE,			/* Debug node. *//*debug command transfer to config*/
  ...
  RMON_NODE ,
  TRUNK_GLOABLE_NODE,/*trunk gloable config node*/
...
struct cmd_node
{
  ...
  /* Node's configuration write factory function */
  int (*func_factory) (struct vty *);			//add by lipf, 2018/12/17, for factory settings
...
/*if cmd need to sync to slave slot?*/
enum sync_type
{
	CMD_LOCAL,
	CMD_SYNC,
...
struct cmd_element
{
  ...
  long long daemon;                   /* Daemon to which this command belong. */
  pthread_mutex_t daemon_lock;
  
  vector tokens;		/* Vector of cmd_tokens */
  u_char attr;			/* Command attributes */
	struct list *order;		/*Order of deamons*/
	int level;
	enum sync_type sync;				/*whether cmd need sync?*/
};

enum cmd_privilege_level
{
	/*visit level: 0*/
	VISIT_LEVE = 0,

	/*Monitoring level 1-3*/
	MONITOR_LEVE_1,
	MONITOR_LEVE_2,
	MONITOR_LEVE_3,

	/*Configuration level 4-13*/
	CONFIG_LEVE_1,
	CONFIG_LEVE_2,
	CONFIG_LEVE_3,
	CONFIG_LEVE_4,
...
struct match_flag
{
	int ipv4_check_faile;
	int ipv4_prefix_check_faile;
	int mac_check_faile;
	int ifname_check_faile;
	int range_check_faile;
	int string_check_faile;
	int name_check_faile;
	int match_fail_cnt;
};
...
struct cmd_token
{
  ...
  /* Used for type == brackets */
  vector brackets; /* vector of vector of cmd_tokens */
...
#define CMD_WAIT				12
#define CMD_REPLAY				13
#define CMD_LEVE_CHECK			14
#define CMD_TEXT				13
#define CMD_ERR_NO_MATCH2       15
...
#define VTYSH_DAEMON_INDEX(DEAMON, I)  \
	while((((long long)1 << (I)++) & DEAMON) != DEAMON);

/*liufy add*/
/* Added by Pranjal Kumar Dutta on 14/04/2003 for zMPLS */
...
#define RIP_STR "RIP (Routing Information Protocol)\n"
#define RIPNG_STR "RIP (Routing Information Protocol next generation)\n"
#define BGP_STR "Border Gateway Protocol(BGP)\n"
...
#define V4NOTATION_STR "specify by IPv4 address notation(e.g. 1.0.0.0)\n"
...
#define OSPF6_INSTANCE "Ospf6 instance ID <1-255>\n"
...
#define LLDP_STR	"Link Layer Discovery Protocol\n"
#define BFD_STR "Specify BFD(Bidirectional Forwarding Detection) configuration information\n"
#define SLA_STR	"Service Level Agreement\n"
#define SAC_STR	"Service Acceptance Criteria\n"
#define PIM_STR "PIM(Protocol Independent Multicast) Configuration\n"
#define MSTP_STR "Multiple Spanning Tree Protocol (MSTP) mode\n"
...
/* Prototypes. */
extern void install_node_factory (struct cmd_node *node, int (*func) (struct vty *));

extern void install_node (struct cmd_node *, int (*) (struct vty *));
extern void install_default (enum node_type);
extern void install_element (enum node_type, struct cmd_element *, enum sync_type sync_flag);
extern void install_element_daemon_order (enum node_type ntype, struct cmd_element *cmd, enum sync_type sync_flag, int deamon_num, ...);
extern void install_element_level (enum node_type ntype, struct cmd_element *cmd, enum cmd_privilege_level level, enum sync_type sync_flag);
extern void install_element_daemon_order_level (enum node_type ntype, struct cmd_element *cmd, enum cmd_privilege_level level, enum sync_type sync_flag, int deamon_num, ...);
...
extern char *cmd_prompt (enum node_type);
...
extern struct match_flag validity_check;
...
extern char *zencrypt (const char *);
