...
#define STREAM_WARN_OFFSETS(S) \
  zlog_debug (0, "&(struct stream): %p, size: %lu, getp: %lu, endp: %lu\n", \
             (void *)(S), \
...
#define STREAM_BOUND_WARN(S, WHAT) \
  do { \
    zlog_debug (0, "%s: Attempt to %s out of bounds", __func__, (WHAT)); \
    STREAM_WARN_OFFSETS(S); \
    assert (0); \
  } while (0)
...
stream_new (size_t size)
{
	struct stream *s;
  	...
  	if (size == 0)
    {
		zlog_err ("stream_new(): called with 0 size!");
      	return NULL;
    }
...
struct stream *
stream_dupcat_offset (struct stream *s1, struct stream *s2, size_t offset)
{
  struct stream *new;
...
struct stream *
stream_dupcat (struct stream *s1, struct stream *s2)
{
  struct stream *new;
  ...
  memcpy (new->data, s1->data, s1->endp);
  memcpy (new->data + s1->endp, s2->data, s2->endp);
  new->endp = s1->endp + s2->endp;
  return new;
}
...
stream_resize (struct stream *s, size_t newsize)
{
  ...
  newdata = XREALLOC (MTYPE_STREAM, s->data, newsize);
...
void
stream_set_endp (struct stream *s, unsigned long pos)
{
  STREAM_VERIFY_SANE(s);
...
stream_fifo_new (void)
{
  ...
  new = XCALLOC (MTYPE_STREAM, sizeof (struct stream_fifo));
  return new;
}
