 ...
 */
extern void high_pre_timer_call(void);
  ...
  for (i = 0; i < sigmaster.sigc; i++)
    {
      ...
      if (sig->signal == signo)
      {
          //sig->caught = 1;             modi by suxq 20180813
          if(signo == SIGALRM)  
          {
               sig->caught ++;
             ...
             //printf("quagga_signal_handler signo = %d\r\n", signo);
          }
          else sig->caught = 1;
      }
    }
...
{
  struct quagga_signal_t *sig = NULL;
  int i = 0;
  int count = 0;
...
static int signal_set (int signo)
{
  ...
  sigemptyset(&sNewSet);  
  sigaddset(&sNewSet, signo);  
  sigprocmask(SIG_UNBLOCK, &sNewSet, NULL);
  ...
  ret = sigaction (signo, &sig, &osig);
                                   /*ALARM 信号在指定的线程打开*/
  if(signo == SIGALRM) sigprocmask(SIG_BLOCK, &sNewSet, NULL);
             ...
             struct quagga_signal_t signals[])
{
  ...
  sigprocmask(SIG_BLOCK, &sNewSet, &sOldSet); 
