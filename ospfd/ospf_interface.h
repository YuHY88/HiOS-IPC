...
#define OSPF_IF_PASSIVE_STATUS(O) \
    ...
    DECLARE_IF_PARAM (u_char, mode);               /* interface mode */

	/* ospf bfd cfg-info at interface*/
	DECLARE_IF_PARAM (u_char, bfd_flag);               /* bfd flag */
#define OSPF_IF_BFD_DISABLE		0
#define OSPF_IF_BFD_ENABLE		1

	DECLARE_IF_PARAM (u_int32_t, if_bfd_recv_interval);
#define OSPF_IF_BFD_RECV_INTERVAL_DEFAULT	10
	DECLARE_IF_PARAM (u_int32_t, if_bfd_send_interval);
#define OSPF_IF_BFD_SEND_INTERVAL_DEFAULT	10
	DECLARE_IF_PARAM (u_int16_t, if_bfd_detect_multiplier);
#define OSPF_IF_BFD_DETECT_MULT_DEFAULT		3

	/* OSPF LDP at interface*/
    DECLARE_IF_PARAM (u_char,ldp_enable);            /* ospf ldp_enable */
    DECLARE_IF_PARAM (u_int32_t, v_holddown);       /* ospf ldp hold down Interval */
    DECLARE_IF_PARAM (u_int32_t, v_maxcost);       /* ospf ldp max cost Interval */
...
struct ospf_interface
{
    ...
    /* Interface data from IFM. */
    struct interface *ifp;
    struct ospf_vl_data *vl_data;     /* Data for Virtual Link */

    /* Packet send buffer. */
	struct ospf_fifo *obuf_highest_priority;           /* Output queue highest priority*/
...
#define PRIORITY(I)     ((I)->nbr_self->priority)
    ...
    u_char ldp_enable;                  /* OSPF LDP ENABLE  */
    ...
    struct thread *t_hold;                /* timer */
    struct thread *t_max_cost;            /* timer */
#endif
	TIMERID			t_hello;               /* timer */
    TIMERID			t_wait;                /* timer */
    TIMERID			t_ls_ack;              /* timer */
    TIMERID			t_hold;                /* timer */
    TIMERID			t_max_cost;            /* timer */
    struct thread 	*t_ls_ack_direct;      /* event */
    struct thread 	*t_ls_upd_event;       /* event */
    //struct thread 	*t_opaque_lsa_self;    /* Type-9 Opaque-LSAs */
	TIMERID			t_opaque_lsa_self;    /* Type-9 Opaque-LSAs */
...
extern void ospf_sub_interface_free(struct interface *);
extern int ospf_sub_interface_look_master(struct interface *ifp);
...
extern struct ospf_vl_data *ospf_vl_lookup (struct ospf *, struct ospf_area *,
        struct in_addr);
extern struct ospf_vl_data *ospf_vl_lookup_for_culc_nexthop (struct ospf *ospf, 
	struct in_addr vl_peer, struct in_addr link_data);
...
extern void ospf_auth_md5_delete_delete_all(struct ospf_if_params *);
extern void ospf_area_auth_md5_delete_all(struct ospf_area *);


extern u_char ospf_default_iftype (struct interface *ifp);
//extern struct connected *get_ospf_out_if_address(uint32_t );
...
extern void ospf_param_if_default(struct interface *);
