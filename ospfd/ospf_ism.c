        ...
        if ((nbr = rn->info) != NULL)
        {
            ...
            if (nbr->router_id.s_addr != 0)
            {
                ...
                if (nbr->priority > 0)
                {
                    ...
                    if (nbr->state >= NSM_TwoWay && nbr->ef_flags == 0)
                    {
...
int ospf_dr_election (struct ospf_interface *oi)
{
    ...
    zlog_debug (OSPF_DBG_ISM_EVENTS, "DR-Election[1st]: Backup %s", inet_ntoa (BDR (oi)));
    zlog_debug (OSPF_DBG_ISM_EVENTS, "DR-Election[1st]: DR     %s", inet_ntoa (DR (oi)));
            ...
            !(new_state == ISM_DROther && old_state < ISM_DROther))
    {
        ...
        ospf_elect_dr (oi, el_list);
        new_state = ospf_ism_state (oi);
        zlog_debug (OSPF_DBG_ISM_EVENTS, "DR-Election[2nd]: Backup %s", inet_ntoa (BDR (oi)));
        zlog_debug (OSPF_DBG_ISM_EVENTS, "DR-Election[2nd]: DR     %s", inet_ntoa (DR (oi)));
    }
    list_delete (el_list);
	el_list = NULL;
...
int ospf_hello_timer (void *arg)
{
    struct ospf_interface *oi;

	if(NULL == arg)
	{
		zlog_warn("%s[%d] arg is NULL !!!\n", __FUNCTION__, __LINE__);
		return 0;
	}
    ...
    if (IS_DEBUG_OSPF (ism, ISM_TIMERS))
	{
	    /*zlog (NULL, LOG_DEBUG, "ISM[%s]: Timer (Hello timer expire)",
	          IF_NAME (oi));*/
		zlog_debug (OSPF_DBG_ISM_TIMERS, "%s[%d]-->ISM[%s]: Timer (Hello timer expire)",
	          		__FUNCTION__, __LINE__, IF_NAME (oi));
	}
...
int ospf_wait_timer (void *thread)
{
    ...
    if (IS_DEBUG_OSPF (ism, ISM_TIMERS))
    {
        /*zlog (NULL, LOG_DEBUG, "ISM[%s]: Timer (Wait timer expire)",
              IF_NAME (oi));*/
		zlog_debug (OSPF_DBG_ISM_TIMERS, "ISM[%s]: Timer (Wait timer expire)",
              IF_NAME (oi));
    }
...
ospf_ldp_waiting (void *arg)
{
    ...
    OSPF_ISM_TIMER_MSEC_ON (oi->t_hello, ospf_hello_timer, 10);
...
void 
ospf_ldp_session_up(struct ospf_interface *oi)
{
    u_int32_t newcost;
    if(oi->t_hold)
    {
        OSPF_ISM_TIMER_OFF (oi->t_hold);
    ...
    if(oi->t_max_cost)
    {
        OSPF_ISM_TIMER_OFF (oi->t_max_cost);
...
void 
ospf_ldp_session_down(struct ospf_interface *oi)
{
    oi->output_cost = 65535;
    ospf_router_lsa_update_area (oi->area);
    OSPF_ISM_TIMER_ON (oi->t_max_cost, ospf_ldp_max_cost, OSPF_IF_PARAM (oi, v_maxcost));
}
...
void ism_timer_set (struct ospf_interface *oi)
{
    ...
    case ISM_Waiting:
        ...
        if(oi->ldp_enable == OSPF_IF_LDP_SYNC_DISABLE)
        {
            OSPF_ISM_TIMER_MSEC_ON (oi->t_hello, ospf_hello_timer, 10);
            OSPF_ISM_TIMER_ON (oi->t_wait, ospf_wait_timer,
                               OSPF_IF_PARAM (oi, v_wait));
        ...
        {
            OSPF_ISM_TIMER_ON (oi->t_hold, ospf_ldp_waiting, OSPF_IF_PARAM (oi, v_holddown));
        }
        break;
    case ISM_PointToPoint:
        ...
        else
        {
            ...
            OSPF_ISM_TIMER_ON (oi->t_hold, ospf_ldp_p2p, OSPF_IF_PARAM (oi, v_holddown));
        }
        break;
    case ISM_DROther:
...
ism_ignore (struct ospf_interface *oi)
{
    if (IS_DEBUG_OSPF (ism, ISM_EVENTS))
    {
		...
		zlog_debug (OSPF_DBG_ISM_EVENTS, "ISM[%s]: ism_ignore called", IF_NAME (oi));
    }
    return 0;
}
...
void ism_change_state (struct ospf_interface *oi, int state)
{
    ...
    if (IS_DEBUG_OSPF (ism, ISM_STATUS))
    {
        ...
        zlog_debug (OSPF_DBG_ISM_EVENTS, "ISM[%s]: State change %s -> %s", IF_NAME (oi),
              LOOKUP (ospf_ism_state_msg, oi->state),
              LOOKUP (ospf_ism_state_msg, state));
...
ospf_ism_event (struct thread *thread)
{
    ...
    if (IS_DEBUG_OSPF (ism, ISM_EVENTS))
    {
        zlog_debug(OSPF_DBG_ISM_EVENTS, "%s[%d]:ISM[%s]: %s (%s) oi->type = %d",__FUNCTION__, __LINE__, IF_NAME (oi),
              LOOKUP (ospf_ism_state_msg, oi->state),
              ospf_ism_event_str[event], oi->type);
    }
    /* If state is changed. */
