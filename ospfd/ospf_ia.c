                       ...
                       struct ospf_route *abr_or)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_ia_network_route(): processing summary route to %s/%d cost = %d",
                    inet_ntoa (p->prefix), p->prefixlen, new_or->cost);
    ...
    if ((rn1 = route_node_lookup (rt, (struct prefix *) p)))
    {
        ...
        if ((or = rn1->info))
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_network_route(): "
            ...
            if ((res = ospf_route_cmp (ospf, new_or, or)) < 0)
            {
                ...
                {
                	zlog_debug (OSPF_DBG_EVENT, "ospf_ia_network_route(): "
                            "summary route(%d) better than old(%d)", new_or->cost, or->cost);
            	}
            ...
            else if (res == 0)
            {
                ...
                {
                	zlog_debug (OSPF_DBG_EVENT, "ospf_ia_network_route(): "
                            "summary route == old route; cost = %d", or->cost);
            	}
    ...
    else
    {
        /* no route */
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_ia_network_route(): add new route to %s/%d cost = %d",
                        inet_ntoa (p->prefix), p->prefixlen, new_or->cost);
                      ...
                      struct ospf_route *new_or, struct ospf_route *abr_or)
{
    ...
    int ret;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_ia_router_route(): considering %s/%d",
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_ia_router_route(): "
        ...
        else if (ret == 0)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_router_route(): merging the new route");
            }
            ospf_route_copy_nexthops (or, abr_or->paths);
            ospf_route_free (new_or);
			new_or = NULL;
        ...
        else
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_router_route(): skipping the new route");
            }
            ospf_route_free (new_or);
			new_or = NULL;
            return;
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_ia_router_route(): adding the new route");
    }
                     ...
                     struct route_table *rtrs, struct ospf_lsa *lsa)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "process_summary_lsa(): LS ID: %s", inet_ntoa (sl->header.id));
		zlog_debug (OSPF_DBG_EVENT, "process_summary_lsa(): LS adv_router: %s", inet_ntoa (sl->header.adv_router));
    }
                           ...
                           struct ospf_area *area)
{
    ...
    if (abr_or == NULL)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): can't find a route to the ABR");
        }
        return;
    }
    ...
    if (! rn)
    {
        ...
        {
            return;
        } /* Standard ABR can update only already installed
                   backbone paths                                       */
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): "
	...
	if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): or->cost = %d, new_cost = %d", or->cost, cost);
    }
            ...
            or->path_type != OSPF_PATH_INTER_AREA)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): ERR: path type is wrong");
        }
        return;
    }
    if (ospf->abr_type == OSPF_ABR_SHORTCUT)
    {
        ...
        {
            if (IS_DEBUG_OSPF_EVENT)
                zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): Shortcut: "
    ...
    if (or->cost < cost)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_update_network_route(): new route is worse");
        }
        return;
    }
    if (or->cost == cost)
                          ...
                          struct ospf_area *area)
{
    ...
    if (abr_or == NULL)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_update_router_route(): can't find a route to the ABR");
        }
        return;
    }
    cost = abr_or->cost + GET_METRIC (lsa->metric);
    ...
    if (!(or->u.std.flags & ROUTER_LSA_EXTERNAL))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_upd_router_route(): the remote router is not an ASBR");
        }
        return;
    }
    if (or->path_type != OSPF_PATH_INTRA_AREA &&
                             ...
                             struct route_table *rtrs, struct ospf_lsa *lsa)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): LS ID: %s",
                    inet_ntoa (lsa->data->id));
		zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): LS adv_router: %s",
                    inet_ntoa (lsa->data->adv_router));
		zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): At area: %s",
    ...
    if (metric == OSPF_LS_INFINITY)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): metric is infinity, skip");
        }
        return 0;
    }
    if (IS_LSA_MAXAGE (lsa))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): This LSA is too old");
        }
        return 0;
    }
    if (ospf_lsa_is_self_originated (area->ospf, lsa))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "process_transit_summaries(): This LSA is mine, skip");
        }
        return 0;
    }
    p.family = AF_INET;
                 ...
                 struct route_table *rtrs)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():start");
    }
    if (IS_OSPF_ABR (ospf))
    {
        ...
        case OSPF_ABR_STAND:
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():Standard ABR");
            }
            if ((area = ospf->backbone))
            {
                if (IS_DEBUG_OSPF_EVENT)
                {
                    zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():backbone area found");
                    zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():examining summaries");
                }
            ...
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():backbone area NOT found");
            }
            break;
        case OSPF_ABR_IBM:
        case OSPF_ABR_CISCO:
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():Alternative Cisco/IBM ABR");
            }
            ...
            if (area && ospf_act_bb_connection (ospf))
            {
                ...
                {
                    zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing(): backbone area found");
                    zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing(): examining BB summaries");
                }
                OSPF_EXAMINE_SUMMARIES_ALL (area, rt, rtrs);
                for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
                {
            ...
            else
            {
                ...
                if (IS_DEBUG_OSPF_EVENT)
                {
                    zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing(): Active BB connection not found");
                }
                for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
                {
                    OSPF_EXAMINE_SUMMARIES_ALL (area, rt, rtrs);
        ...
        case OSPF_ABR_SHORTCUT:
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():Alternative Shortcut");
            }
            area = ospf->backbone; /* Find the BB */
            /* If we have an active BB connection */
            if (area && ospf_act_bb_connection (ospf))
    ...
    else
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_ia_routing():not ABR, considering all areas");
        }
        for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
        {
            OSPF_EXAMINE_SUMMARIES_ALL (area, rt, rtrs);
