...
/* 
OSPF DCN 网元信息TLV字段(T)Prototypes. 
*/
#define OPAQUE_TLV_TYPE_MANUID		0x8000
#define OPAQUE_TLV_TYPE_DEVID		0x8001
#define OPAQUE_TLV_TYPE_DEVMAC		0x8002
#define OPAQUE_TLV_TYPE_NEID		0x8003
#define OPAQUE_TLV_TYPE_NEIPV4		0x8004
#define OPAQUE_TLV_TYPE_NEIPV6		0x8005

#define IS_OPAQUE_LSA(type) \
    ((type) == OSPF_OPAQUE_LINK_LSA  || \
     (type) == OSPF_OPAQUE_AREA_LSA  || \
     (type) == OSPF_OPAQUE_AS_LSA)
#define IS_OPAQUE_AREA_LSA(type) ((type) == OSPF_OPAQUE_AREA_LSA)

struct opaque_tlv_header
{
	u_int16_t	type;			/* TE_TLV_XXX (see below) */
	u_int16_t	length;			/* Value portion only, in octets */
};

struct opaque_tlv_manuid
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
	u_char value;
};

struct opaque_tlv_devid
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
  	u_char value;
};

struct opaque_tlv_devmac
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
	u_char value[6];
};

struct opaque_tlv_neid
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
	struct in_addr value;
};

struct opaque_tlv_neipv4
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
	struct in_addr value;
};

struct opaque_tlv_neipv6
{
	struct opaque_tlv_header	header;		/* Value length is 4 octets. */
	struct in6_addr value;
};

#define OPAQUE_TLV_HDR_SIZE (sizeof (struct opaque_tlv_header))

#define OPAQUE_TLV_BODY_SIZE(tlvh)	ntohs((tlvh)->length)


#define OPAQUE_TLV_SIZE(tlvh) (OPAQUE_TLV_HDR_SIZE + OPAQUE_TLV_BODY_SIZE(tlvh))
...
#define OPAQUE_TLV_HDR_TOP(lsah) \
	(struct opaque_tlv_header *)((char *)(lsah) + OSPF_LSA_HEADER_SIZE)

#define OPAQUE_TLV_HDR_NEXT(tlvh, offset) \
	(struct opaque_tlv_header *)((char *)(tlvh) + OPAQUE_TLV_SIZE(tlvh) + offset)
...
/*用于OSPF DCN 网元信息自动上报*/
#define OPAQUE_TYPE_NMS_AUTOREPORT			202
...
extern struct ospf_lsa *ospf_opaque_lsa_refresh (struct ospf_lsa *lsa,struct ospf*);
