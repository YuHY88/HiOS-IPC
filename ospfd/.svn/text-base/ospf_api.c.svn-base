...
api_opaque_lsa_print (struct lsa_header *data)
{
    ...
    zlog_debug (OSPF_DBG_OTHER, "apiserver_lsa_print: opaquelen=%d\n", opaquelen);
    for (i = 0; i < opaquelen; i++)
    {
        zlog_debug (OSPF_DBG_OTHER, "0x%x ", olsa->mydata[i]);
    }
    zlog_debug (OSPF_DBG_OTHER, "\n");
}
...
msg_new (u_char msgtype, void *msgbody, u_int32_t seqnum, u_int16_t msglen)
{
	...
	do
  	{	
		new_msg = XCALLOC (MTYPE_OSPF_API_MSG, sizeof (struct msg));
	...
	if (new_msg == NULL)
    {
        ...
        return (struct msg *)NULL;
    }
	memset(new_msg, 0, sizeof(struct msg));

    new_msg->hdr.version = OSPF_API_VERSION;
    new_msg->hdr.msgtype = msgtype;
    new_msg->hdr.msglen = htons (msglen);
    new_msg->hdr.msgseq = htonl (seqnum);

    new_msg->s = stream_new (msglen);
	if(new_msg->s == NULL)
	{
		zlog_warn("%s[%d]  new_msg->s NULL",__func__,__LINE__);
		return NULL;
	}
    assert (new_msg->s);
    stream_put (new_msg->s, msgbody, msglen);
...
msg_dup (struct msg *msg)
{
	...
	if(msg == NULL)
	{
		zlog_warn("%s[%d]  msg is NULL",__func__,__LINE__);
		return NULL;
	}
    assert (msg);

    new_msg = msg_new (msg->hdr.msgtype, STREAM_DATA (msg->s),
...
msg_print (struct msg *msg)
{
    if (!msg)
    {
        zlog_debug (OSPF_DBG_OTHER, "msg_print msg=NULL!\n");
        return;
    }
#ifdef ORIGINAL_CODING
    zlog_debug
    (OSPF_DBG_OTHER, "msg=%p msgtype=%d msglen=%d msgseq=%d streamdata=%p streamsize=%lu\n",
    ...
    zlog_debug
    (OSPF_DBG_OTHER, "API-msg [%s]: type(%d),len(%d),seq(%lu),data(%p),size(%zd)",
...
msg_fifo_new ()
{
	struct msg_fifo *msg_fifo = NULL;

    //return XCALLOC (MTYPE_OSPF_API_FIFO, sizeof (struct msg_fifo));
    do
  	{	
		msg_fifo = XCALLOC (MTYPE_OSPF_API_FIFO, sizeof (struct msg_fifo));;
...
msg_write (int fd, struct msg *msg)
{
	...
	if(msg == NULL || msg->s == NULL)
	{
		zlog_warn("%s[%d]  msg or msg->s is NULL",__func__,__LINE__);
		return -1;
	}
                           ...
                           u_char is_self_originated, struct lsa_header *data)
{
	...
	if(data == NULL)
	{
		zlog_warn("%s[%d]  data is NULL",__func__,__LINE__);
		return NULL;
	}
