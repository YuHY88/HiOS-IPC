...
/* Check LSA is related to external info. used in refesh the lsa with the same ei*/
...
ospf_external_info_check (struct ospf *ospf, struct ospf_lsa *lsa)
{
    ...
    nh_al.family = AF_INET;
    nh_al.u.prefix4 = al->e[0].fwd_addr;
    nh_al.prefixlen = IPV4_MAX_BITLEN;
                    ...
                    if (rn)
                    {
                        ...
                        for(ei = rn->info ; ei != NULL ; ei = ei->next)
                        {
                            if (ospf_is_type_redistributed (ospf,ei->type,ei->instance))
                            {
                                ...
                                {
                                    nh_ei.family = AF_INET;
                                    nh_ei.u.prefix4 = ei->nexthop;
                                    nh_ei.prefixlen = IPV4_MAX_BITLEN;
                                    if(lsa->data->type == OSPF_AS_EXTERNAL_LSA)
                                    {
                                        if(prefix_match (&nh_ei, &nh_al))
                                        {
                                            return ei;
                                        }
                                    }
...
ospf_process_self_originated_lsa (struct ospf *ospf,
                                  struct ospf_lsa *new_lsa, struct ospf_lsa *current, struct ospf_area *area)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "LSA[Type%d:%s]: Process self-originated LSA seq 0x%x",
                    new_lsa->data->type, inet_ntoa (new_lsa->data->id), ntohl(new_lsa->data->ls_seqnum));
    ...
    switch (new_lsa->data->type)
    {
    case OSPF_ROUTER_LSA:
        /* Originate a new instance and schedule flooding */
        if (area->router_lsa_self)
        {
            area->router_lsa_self->data->ls_seqnum = new_lsa->data->ls_seqnum;
        }
    ...
    case OSPF_OPAQUE_LINK_LSA:
        ...
        for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, node, oi))
        {
            ...
            if (oi && IPV4_ADDR_SAME (&oi->address->u.prefix4, &new_lsa->data->id))
            {
                /* If the interface is no more a broadcast type or we are no more
                    the DR, we flush the LSA otherwise -- create the new instance and
                    schedule flooding. changed by tai Bug #40800*/
                if (oi == NULL||
                        oi->area != area ||
                ...
                {
                    ospf_schedule_lsa_flush_area (area, new_lsa);
                    return;
                }
                if (new_lsa->data->type == OSPF_OPAQUE_LINK_LSA)
                {
                    ospf_opaque_lsa_refresh (new_lsa,ospf);
                    return;
                }
                if (oi->network_lsa_self)
                {
                    oi->network_lsa_self->data->ls_seqnum = new_lsa->data->ls_seqnum;
                }
    ...
    case OSPF_AS_EXTERNAL_LSA :
        /*recive self but lsdb is null flush the lsa*/
        ...
        {
            ospf_translated_nssa_refresh (ospf, NULL, new_lsa);
            return;
        }
        else if(current == NULL || ei ==  NULL || (current != NULL && (current->data != NULL) && IS_LSA_MAXAGE(current)))
        {
            if(!CHECK_FLAG (new_lsa->flags, OSPF_LSA_LOCAL_XLT))
            {
                ospf_lsa_flush_as(ospf, new_lsa);
            }
            return;
        }
        else
        {
            ospf_lsa_refresh(ospf, new_lsa);
        }
        break;
    case OSPF_AS_NSSA_LSA:
        if(!IS_AREA_NSSA(new_lsa->area))
        {
            ospf_lsa_flush_as (ospf, new_lsa);
        }
        else
        {
            ...
            if (ei)
            {
			   	/*first refresh Type-7 LSA*/
                //ospf_install_flood_nssa(ospf, new_lsa, ei);
				ospf_nssa_lsa_refresh (area->ospf, new_lsa, ei, LSA_REFRESH_FORCE);

                /*  ABR should  compare Type-7 LSA priory */
                if(IS_OSPF_ABR(ospf))
                {
                    lsa_best = ospf_find_nssa_lsa_highestprio(ospf, &ei->p.prefix);
                    if (lsa_best && IS_LSA_SELF(lsa_best))
                    {
                        /* ABR refresh 2 times Type-7 LSA --- this need to optimize*/
                        ospf_external_lsa_refresh (ospf, new_lsa, ei, LSA_REFRESH_FORCE);
            ...
            else
            {
                ...
                {
                    if (new_lsa->data->id.s_addr == OSPF_DEFAULT_DESTINATION)
                    {
                        if(IS_OSPF_ABR(ospf))
                        {
                            ospf_as_nssa_default_lsa_refresh(ospf, new_lsa);
                        }
                        else
                        {
                            ospf_lsa_flush_as (ospf, new_lsa);
    ...
    case OSPF_OPAQUE_AS_LSA:
        ospf_opaque_lsa_refresh (new_lsa,ospf); /* Reconsideration may needed. *//* XXX */
        break;
    default:
        break;
    }
...
ospf_flood (struct ospf *ospf, struct ospf_neighbor *nbr,
            struct ospf_lsa *current, struct ospf_lsa *new_lsa)
{
    struct ospf_interface *oi = NULL;
    int lsa_ack_flag;
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "LSA[Flooding]: start, NBR %s (%s), cur(%p), New-LSA[%s]",
    ...
    if (current != NULL)      /* -- endo. */
    {
        ...
        {
            if (IS_DEBUG_OSPF_EVENT)
                zlog_debug (OSPF_DBG_EVENT, "LSA[Flooding]: Got a self-originated LSA, "
                            "while local one is initial instance.");
            ; /* Accept this LSA for quick LSDB resynchronization. */
        }
        else if (tv_cmp (tv_sub (time_get_recent_relative_time (), current->tv_recv),
                         msec2tv (ospf->min_ls_arrival)) < 0)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "LSA[Flooding]: LSA is received recently.");
            }
            return -1;
        }
    }
    /* Flood the new LSA out some subset of the router's interfaces.
       ...
       interface. */
    lsa_ack_flag = ospf_flood_through (ospf, nbr, new_lsa);
    ...
    SET_FLAG (new_lsa->flags, OSPF_LSA_RECEIVED);
    ospf_lsa_is_self_originated (ospf, new_lsa); /* Let it set the flag */
    ...
    if (! (new_lsa = ospf_lsa_install (ospf, nbr->oi, new_lsa)))
    {
    ...
    if (lsa_ack_flag)
    {
        ospf_flood_delayed_lsa_ack (nbr, new_lsa);
    }
    /* If this new LSA indicates that it was originated by the
       ...
       the routing domain. */
    if (ospf_lsa_is_self_originated (ospf, new_lsa))
    {
        ospf_process_self_originated_lsa (ospf, new_lsa, current, oi->area);
    }
    else
                              ...
                              struct ospf_lsa *lsa)
{
    ...
    struct ospf_neighbor *onbr = NULL;
    int retx_flag;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_flood_through_interface(): "
    ...
    for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
    {
        ...
        onbr = rn->info;
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_flood_through_interface(): considering nbr %s (%s)",
        ...
        if (onbr->state < NSM_Full)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_flood_through_interface(): nbr adj is not Full");
            }
        ...
        if (IS_OPAQUE_LSA (lsa->data->type))
        {
            if (! CHECK_FLAG (onbr->options, OSPF_OPTION_O))
            {
                ...
                {
                    zlog_debug (OSPF_DBG_LSA_FLOODING, "Skip this neighbor: Not Opaque-capable.");
                }
                continue;
            }
        }
        ...
        if (inbr)
        {
            ...
            */
            if (IPV4_ADDR_SAME (&inbr->router_id, &onbr->router_id))
            {
                if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                {
                    zlog_debug (OSPF_DBG_LSA_FLOODING, "Skip this neighbor: inbr == onbr");
                }
                continue;
            }
        }
        else
        {
            ...
            if (IPV4_ADDR_SAME (&lsa->data->adv_router, &onbr->router_id))
            {
                if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                {
                    zlog_debug (OSPF_DBG_LSA_FLOODING, "Skip this neighbor: lsah->adv_router == onbr");
                }
                continue;
            }
        }
    ...
    if (inbr && (inbr->oi == oi))
    {
        ...
        if (NBR_IS_DR (inbr) || NBR_IS_BDR (inbr))
        {
            if (IS_DEBUG_OSPF_NSSA)
                zlog_debug (OSPF_DBG_NSSA, "ospf_flood_through_interface(): "
                         ...
                         struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
    ...
    for (ALL_LIST_ELEMENTS (area->oiflist, node, nnode, oi))
    {
        ...
        if ((lsa->data->type == OSPF_OPAQUE_LINK_LSA) && (lsa->oi != oi))
        {
             ...
             */
            if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug (OSPF_DBG_LSA_FLOODING, "Type-9 Opaque-LSA: lsa->oi(%p) != oi(%p)",
                            (void *)lsa->oi, (void *)oi);
            continue;
        }
                       ...
                       struct ospf_lsa *lsa)
{
    ...
    if (CHECK_FLAG (lsa->flags, OSPF_LSA_LOCAL_XLT)) /* Translated from 7  */
        if (IS_DEBUG_OSPF_NSSA)
        {
            zlog_debug (OSPF_DBG_NSSA, "Flood/AS: NSSA TRANSLATED LSA");
        }
                    ...
                    struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
    ...
    case OSPF_AS_NSSA_LSA:
        ...
        if (IS_DEBUG_OSPF_NSSA)
        {
            zlog_debug (OSPF_DBG_NSSA, "ospf_flood_through: LOCAL NSSA FLOOD of Type-7.");
        }
        break;
    default:
        break;
...
ospf_ls_request_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
     ...
     */
    if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
        zlog_debug (OSPF_DBG_LSA_FLOODING, "RqstL(%lu)++, NBR(%s), LSA[%s]",
...
ospf_ls_request_new (struct lsa_header *lsah)
{
    ...
    new_lsa->data = ospf_lsa_data_new (OSPF_LSA_HEADER_SIZE);
	if(new_lsa->data == NULL)
	{
		zlog_err("%s %d Can't creat ospf_lsa_data_new : malloc failed",__func__,__LINE__);
		return NULL;
	}
    memcpy (new_lsa->data, lsah, OSPF_LSA_HEADER_SIZE);
...
ospf_ls_retransmit_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
    ...
    if (ospf_lsa_more_recent (old, lsa) < 0)
    {
        ...
        */
        if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug (OSPF_DBG_LSA_FLOODING, "RXmtL(%lu)++, NBR(%s), LSA[%s]",
...
ospf_lsa_flush_area (struct ospf_lsa *lsa, struct ospf_area *area)
{
    ...
    lsa->tv_recv = time_get_recent_relative_time ();
...
ospf_lsa_flush (struct ospf *ospf, struct ospf_lsa *lsa)
{
    ...
    case OSPF_OPAQUE_AS_LSA:
        ...
        break;
    default:
        zlog_debug (OSPF_DBG_EVENT, "%s: Unknown LSA type %u", __func__, lsa->data->type);
        break;
    }
}
