...
#include "lib/hash.h"
#include "lib/sockunion.h"      /* for inet_aton() */
...
update_linkparams(struct mpls_te_link *lp)
{
    ...
    if (IS_PARAM_SET(ifp->link_params, LP_RMT_AS))
    {
        ...
        {
            if (IS_DEBUG_OSPF_TE)
                zlog_debug(OSPF_DBG_TE, "OSPF MPLS-TE Update IF: Switch from Standard LSA to INTER-AS for %s[%d/%d]",
    ...
    {
        if (IS_DEBUG_OSPF_TE)
            zlog_debug(OSPF_DBG_TE, "OSPF MPLS-TE Update IF: Switch from INTER-AS LSA to Standard for %s[%d/%d]",
                       ifp->name, lp->flags, lp->type);
...
initialize_linkparams(struct mpls_te_link *lp)
{
    ...
    if (IS_DEBUG_OSPF_TE)
        zlog_debug(OSPF_DBG_TE, "MPLS-TE(initialize_linkparams) Initialize Link Parameters for interface %s",
                   ifp->name);
...
{
    struct mpls_te_link *new_mpls;
    int rc = -1;

    if (IS_DEBUG_OSPF_TE)
        zlog_debug(OSPF_DBG_TE, "MPLS-TE(ospf_mpls_te_new_if) Add new %s interface %s to MPLS-TE list",
	...
	do
	{		
		new_mpls = XCALLOC(MTYPE_OSPF_MPLS_TE, sizeof(struct mpls_te_link));
    ...
    if (new_mpls == NULL)
    {
        zlog_err("ospf_mpls_te_new_if: XMALLOC: %s", safe_strerror(errno));
        goto out;
    }
	memset(new_mpls, 0, sizeof(struct mpls_te_link));

    new_mpls->instance = get_mpls_te_instance_value();
    new_mpls->ifp = ifp;
    ...
    new_mpls->type = STD_TE | FLOOD_AREA;
    new_mpls->flags = LPFLG_LSA_INACTIVE;

    /* Initialize Link Parameters from Interface */
    initialize_linkparams(new_mpls);

    /* Set TE Parameters from Interface */
    update_linkparams(new_mpls);

    /* Add Link Parameters structure to the list */
    listnode_add(OspfMplsTE.iflist, new_mpls);

    if (IS_DEBUG_OSPF_TE)
        zlog_debug(OSPF_DBG_TE, "OSPF MPLS-TE New IF: Add new LP context for %s[%d/%d]",
                   ifp->name, new_mpls->flags, new_mpls->type);
...
ospf_mpls_te_update_if(struct interface *ifp)
{
    ...
    if (IS_DEBUG_OSPF_TE)
        zlog_debug(OSPF_DBG_TE, "OSPF MPLS-TE: Update LSA parameters for interface %s [%s]",
...
ospf_mpls_te_lsa_new(struct ospf_area *area, struct mpls_te_link *lp)
{
    ...
    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Create an Opaque-LSA/MPLS-TE instance",
    ...
    if ((new_lsa->data = ospf_lsa_data_new(length)) == NULL)
    {
        zlog_warn("ospf_mpls_te_lsa_new: ospf_lsa_data_new() ?");
        ospf_lsa_unlock(&new_lsa);
        new_lsa = NULL;
        stream_free(s);
        goto out;
    }
	memset(new_lsa->data, 0 ,length);
...
ospf_mpls_te_lsa_originate1(struct ospf_area *area, struct mpls_te_link *lp)
{
    ...
    if ((new_lsa = ospf_mpls_te_lsa_new(area, lp)) == NULL)
    {
    ...
    if (ospf_lsa_install(area->ospf, NULL/*oi*/, new_lsa) == NULL)
    {
    ...
    ospf_flood_through_area(area, NULL/*nbr*/, new_lsa);

    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
    {
        char area_id[INET_ADDRSTRLEN];
        strcpy(area_id, inet_ntoa(area->area_id));
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Originate Opaque-LSA/MPLS-TE: Area(%s), Link(%s)",
                   new_lsa->data->type, inet_ntoa(new_lsa->data->id), area_id, lp->ifp->name);
...
ospf_mpls_te_lsa_originate_area(void *arg)
{
    ...
    {
        zlog_debug(OSPF_DBG_TE, "ospf_mpls_te_lsa_originate_area: MPLS-TE is disabled now.");
    ...
    for (ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp))
    {
        ...
        if (IS_DEBUG_OSPF_TE)
            zlog_debug(OSPF_DBG_TE, "MPLS-TE(ospf_mpls_te_lsa_originate_area) Let's finally reoriginate the LSA %d through the Area %s for Link %s",
...
ospf_mpls_te_lsa_originate2(struct ospf *top, struct mpls_te_link *lp)
{
    ...
    if ((new_lsa = ospf_mpls_te_lsa_new(NULL, lp)) == NULL)
    {
    ...
    /* Install this LSA into LSDB. */
    if (ospf_lsa_install(top, NULL /*oi */ , new_lsa) == NULL)
    {
    ...
    ospf_flood_through_as(top, NULL /*nbr */ , new_lsa);

    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
    {
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Originate Opaque-LSA/MPLS-TE Inter-AS",
...
ospf_mpls_te_lsa_originate_as(void *arg)
{
    ...
    {
        zlog_debug
        (OSPF_DBG_TE, "ospf_mpls_te_lsa_originate_as: MPLS-TE Inter-AS is disabled for now.");
        rc = 0;                   /* This is not an error case. */
        goto out;
    }
    ...
    for (ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp))
    {
        ...
        if (IS_DEBUG_OSPF_TE)
            zlog_debug(OSPF_DBG_TE, "MPLS-TE(ospf_mpls_te_lsa_originate_as) Let's finally re-originate the Inter-AS LSA %d through the %s for Link %s",
...
ospf_mpls_te_lsa_refresh(struct ospf_lsa *lsa)
{
    ...
    if (OspfMplsTE.status == disabled)
    {
        ...
        */
        zlog_debug(OSPF_DBG_TE, "ospf_mpls_te_lsa_refresh: MPLS-TE is disabled now.");
    ...
    if (IS_FLOOD_AS(lp->type))
        ospf_flood_through_as(top, NULL, new_lsa);
    else
        ospf_flood_through_area(area, NULL/*nbr*/, new_lsa);
    ...
    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
    {
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Refresh Opaque-LSA/MPLS-TE",
                   new_lsa->data->type, inet_ntoa(new_lsa->data->id));
        ospf_lsa_header_dump(new_lsa->data);
    }
...
show_vty_router_addr(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Router-Address: %s", inet_ntoa(top->value));
...
show_vty_link_header(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Link: %u octets of data", ntohs(top->header.length));
...
show_vty_link_subtlv_link_type(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Link-Type: %s (%u)", cp, top->link_type.value);
...
show_vty_link_subtlv_link_id(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Link-ID: %s", inet_ntoa(top->value));
                                  ...
                                  struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Local Interface IP Address(es): %d", n);
    ...
    for (i = 0; i < n; i++)
    {
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_TE, "      #%d: %s", i, inet_ntoa(top->value[i]));
    }
                                  ...
                                  struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug (OSPF_DBG_TE, "    Remote Interface IP Address(es): %d", n);
    }
    for (i = 0; i < n; i++)
    {
        if (vty != NULL)
...
show_vty_link_subtlv_te_metric(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug(OSPF_DBG_TE, "    Traffic Engineering Metric: %u",
...
show_vty_link_subtlv_max_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug(OSPF_DBG_TE, "    Maximum Bandwidth: %g (Bytes/sec)", fval);
    }
    return TLV_SIZE(tlvh);
}
...
show_vty_link_subtlv_max_rsv_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug (OSPF_DBG_TE, "    Maximum Reservable Bandwidth: %g (Bytes/sec)", fval);
    }
    return TLV_SIZE (tlvh);
}
...
show_vty_link_subtlv_unrsv_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Unreserved Bandwidth per Class Type in Byte/s:");
    ...
    for (i = 0; i < MAX_CLASS_TYPE; i += 2)
    {
        ...
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_TE, "      [%d]: %g (Bytes/sec),\t[%d]: %g (Bytes/sec)",
                       i, fval1, i + 1, fval2);
    }
...
show_vty_link_subtlv_rsc_clsclr(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Resource Class/Color: 0x%x",
                   (u_int32_t) ntohl(top->value));
...
show_vty_link_subtlv_lrrid(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug(OSPF_DBG_TE, "    Local  TE Router ID: %s", inet_ntoa(top->local));
        zlog_debug(OSPF_DBG_TE, "    Remote TE Router ID: %s", inet_ntoa(top->remote));
    }
...
show_vty_link_subtlv_llri(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    {
        zlog_debug(OSPF_DBG_TE, "    Link Local  ID: %d", (u_int32_t) ntohl(top->local));
        zlog_debug(OSPF_DBG_TE, "    Link Remote ID: %d", (u_int32_t) ntohl(top->remote));
    }
...
show_vty_link_subtlv_rip(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Inter-AS TE Remote ASBR IP address: %s",
                   inet_ntoa(top->value));
...
show_vty_link_subtlv_ras(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Inter-AS TE Remote AS number: %u", ntohl(top->value));
...
show_vty_link_subtlv_av_delay(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    %s Average Link Delay: %d (micro-sec)",
...
show_vty_link_subtlv_mm_delay(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    %s Min/Max Link Delay: %d/%d (micro-sec)",
...
show_vty_link_subtlv_delay_var(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Delay Variation: %d (micro-sec)", jitter);
...
show_vty_link_subtlv_pkt_loss(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    %s Link Loss: %g (%%)", anomalous ? "Anomalous" : "Normal",
                   fval);
...
show_vty_link_subtlv_res_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Unidirectional Residual Bandwidth: %g (Bytes/sec)",
                   fval);
...
show_vty_link_subtlv_ava_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Unidirectional Available Bandwidth: %g (Bytes/sec)",
                   fval);
...
show_vty_link_subtlv_use_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Unidirectional Utilized Bandwidth: %g (Bytes/sec)",
                   fval);
...
show_vty_unknown_tlv(struct vty *vty, struct te_tlv_header *tlvh)
{
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_TE, "    Unknown TLV: [type(0x%x), length(0x%x)]",
      ...
      "Enable the MPLS-TE functionality\n")
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "MPLS-TE: OFF -> ON");
      ...
      "Disable the MPLS-TE functionality\n")
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "MPLS-TE: ON -> OFF");
      ...
      "MPLS-TE router address in IPv4 address format\n")
{
    ...
    {
        vty_error_out(vty, "Please specify Router-Addr by A.B.C.D%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
                  ...
                  const char *area_id)
{
    ...
    if (OspfMplsTE.status == enabled)
    {
        ...
        {
            vty_error_out(vty, "Unknown mode. Please choose between as or area%s",
                    VTY_NEWLINE);
            return CMD_WARNING;
        }

        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug(OSPF_DBG_EVENT, "MPLS-TE: Inter-AS enable with %s flooding support",
                       mode2text[mode]);

        /* Register new callbacks regarding the flooding scope (AS or Area) */
        if (ospf_mpls_te_register(mode) < 0)
        {
            vty_error_out(vty, "Internal error: Unable to register Inter-AS functions%s",
                    VTY_NEWLINE);
            return CMD_WARNING;
        }
        ...
        {
            vty_error_out(vty, "Please change Inter-AS support to disable first before going to mode %s%s",
      ...
      "Disable MPLS-TE Inter-AS support\n")
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "MPLS-TE: Inter-AS support OFF");
...
{
    install_element(CONFIG_NODE, &show_ip_ospf_mpls_te_router_cmd, CMD_LOCAL);
    install_element(CONFIG_NODE, &show_ip_ospf_mpls_te_link_cmd, CMD_LOCAL);

    install_element(OSPF_NODE, &ospf_mpls_te_on_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &no_ospf_mpls_te_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &ospf_mpls_te_router_addr_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &ospf_mpls_te_inter_as_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &ospf_mpls_te_inter_as_area_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &no_ospf_mpls_te_inter_as_cmd, CMD_SYNC);
