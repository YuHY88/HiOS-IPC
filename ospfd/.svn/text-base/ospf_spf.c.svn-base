...
{
	struct vertex_nexthop *v_nexthop = NULL;
	do
	{		
		v_nexthop = XCALLOC (MTYPE_OSPF_NEXTHOP, sizeof (struct vertex_nexthop));	
...
{
    struct vertex_parent *new_vertex;

    new_vertex = XMALLOC (MTYPE_OSPF_VERTEX_PARENT, sizeof (struct vertex_parent));
    ...
    new_vertex->parent = v;
    new_vertex->backlink = backlink;
    new_vertex->nexthop = hop;
    return new_vertex;
}
...
ospf_vertex_new (struct ospf_lsa *lsa)
{
	...
	do
	{		
		new_vertex = XCALLOC (MTYPE_OSPF_VERTEX, sizeof (struct vertex));
	...
	if(new_vertex == NULL)
    {
        zlog_err("Can't creat vertex : malloc failed");
        return (struct vertex *)NULL;
    }
	memset(new_vertex, 0, sizeof(struct vertex));

    new_vertex->flags = 0;
    new_vertex->stat = &(lsa->stat);
    new_vertex->type = lsa->data->type;
    new_vertex->id = lsa->data->id;
    new_vertex->lsa = lsa->data;
    new_vertex->children = list_new ();
    new_vertex->parents = list_new ();
    new_vertex->parents->del = vertex_parent_free;

    listnode_add (&vertex_list, new_vertex);

    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "%s: Created %s vertex %s", __func__,
                    new_vertex->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
                    inet_ntoa (new_vertex->lsa->id));
    return new_vertex;
}
...
ospf_vertex_free (void *data)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "%s: Free %s vertex %s", __func__,
                 ...
                 int print_parents, int print_children)
{
    ...
    zlog_debug(OSPF_DBG_EVENT, "%s %s vertex %s  distance %u flags %u",
               msg,
               v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
               inet_ntoa(v->lsa->id),
               v->distance,
    ...
    if (print_parents)
    {
        ...
        for (ALL_LIST_ELEMENTS_RO (v->parents, node, vp))
        {
            ...
            if (vp)
            {
                zlog_debug (OSPF_DBG_EVENT, "parent %s backlink %d nexthop %s  interface %s",
                     ...
                     unsigned int distance)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        ...
        zlog_debug (OSPF_DBG_EVENT, "%s: Adding %s as parent of %s",
                    __func__,
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "%s: distance %d better than %d, flushing existing parents",
    ...
    for (ALL_LIST_ELEMENTS_RO(w->parents, node, wp))
    {
        if (memcmp(newhop, wp->nexthop, sizeof(*newhop)) == 0)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "%s: ... nexthop already on parent list, skipping add", __func__);
            }
            return;
        }
    }
                          ...
                          unsigned int distance, int lsa_pos)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_nexthop_calculation(): Start");
        ospf_vertex_dump("V (parent):", v, 1, 1);
        ospf_vertex_dump("W (dest)  :", w, 1, 1);
        zlog_debug (OSPF_DBG_EVENT, "V->W distance: %d", distance);
    }
    if (v == area->spf)
    {
        ...
        */
        ...
        if (!oi)
        {
            zlog_debug(OSPF_DBG_EVENT, "%s: OI not found in LSA: lsa_pos:%d link_id:%s link_data:%s",
                       __func__, lsa_pos,
        ...
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug(OSPF_DBG_EVENT, "%s: considering link:%s "
            ...
            */
            ...
            if (l->m[0].type == LSA_LINK_TYPE_POINTOPOINT)
            {
                ...
                if (added)
                {
					...
					if(nh == NULL)
					{
						zlog_err("%s %d Can't creat vertex_nexthop_new : malloc failed",__func__,__LINE__);
						return 0;
					}
					memset(nh, 0, sizeof(struct vertex_nexthop));
                    ...
                    zlog_debug(OSPF_DBG_EVENT, "%s: could not determine nexthop for link %s",
            ...
            else if (l->m[0].type == LSA_LINK_TYPE_VIRTUALLINK)
            {
                 ...
                 */
                /*bug#52370: fix by zzl 20180328 
                 *Solution: 使用了新的查找函数 ospf_vl_lookup_for_clc_nexthop()替换了ospf_vl_lookup()
                 */
                vl_data = ospf_vl_lookup_for_culc_nexthop(area->ospf, l->link_id, l->link_data);
                if (vl_data
                        && CHECK_FLAG (vl_data->flags, OSPF_VL_FLAG_APPROVED))
                {
					...
					zlog_debug(OSPF_DBG_EVENT, "vlink nexthop oi = %s", IF_NAME(nh->oi));
					zlog_debug(OSPF_DBG_EVENT, "vlink nexthop router = %s", inet_ntoa(nh->router));	
                    ospf_spf_add_parent (v, w, nh, distance);
					zlog_debug(OSPF_DBG_EVENT, "v(%s)---->", inet_ntoa(v->id));
					zlog_debug(OSPF_DBG_EVENT, "w(%s) distance = %d", inet_ntoa(w->id), distance);
					
                    return 1;
                }
                else
                    zlog_debug(OSPF_DBG_EVENT, "ospf_nexthop_calculation(): "
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "%s: Intervening routers, adding parent(s)", __func__);
    }
    for (ALL_LIST_ELEMENTS (v->parents, node, nnode, vp))
    {
        added = 1;
               ...
               struct pqueue * candidate)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "%s: Next vertex of %s vertex %s",
                    __func__,
                    v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
                    inet_ntoa(v->lsa->id));
    p = ((u_char *) v->lsa) + OSPF_LSA_HEADER_SIZE + 4;
    ...
    while (p < lim)
    {
        ...
        if (v->lsa->type == OSPF_ROUTER_LSA)
        {
            ...
            */
            ...
            case LSA_LINK_TYPE_VIRTUALLINK:
                ...
                {
                    if (IS_DEBUG_OSPF_EVENT)
                        zlog_debug (OSPF_DBG_EVENT, "looking up LSA through VL: %s",
                                    inet_ntoa (l->link_id));
                }
                ...
                if (w_lsa)
                {
                    if (IS_DEBUG_OSPF_EVENT)
                    {
                        zlog_debug (OSPF_DBG_EVENT, "found Router LSA %s", inet_ntoa (l->link_id));
                    }
                }
                break;
            case LSA_LINK_TYPE_TRANSIT:
                if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug (OSPF_DBG_EVENT, "Looking up Network LSA, ID: %s",
                                inet_ntoa (l->link_id));
                ...
                if (w_lsa)
                    if (IS_DEBUG_OSPF_EVENT)
                    {
                        zlog_debug (OSPF_DBG_EVENT, "found the LSA");
                    }
                break;
            default:
        ...
        else
        {
            ...
            if (w_lsa)
            {
                if (IS_DEBUG_OSPF_EVENT)
                {
                    zlog_debug (OSPF_DBG_EVENT, "found Router LSA %s", inet_ntoa (w_lsa->data->id));
                }
            }
        }
        ...
        if (w_lsa == NULL)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "No LSA found");
            }
            continue;
        }
        if (IS_LSA_MAXAGE (w_lsa))
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "LSA is MaxAge");
            }
            continue;
        }
        if (ospf_lsa_has_link (w_lsa->data, v->lsa) < 0 )
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "The LSA doesn't have a link back");
            }
            continue;
        }
        ...
        if (w_lsa->stat == LSA_SPF_IN_SPFTREE)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "The LSA is already in SPF");
            }
            continue;
        }
        ...
        if (w_lsa->stat == LSA_SPF_NOT_EXPLORED)
        {
            ...
            {
                zlog_debug (OSPF_DBG_EVENT, "Nexthop Calc failed");
            }
        }
...
ospf_spf_dump (struct vertex *v, int i)
{
    ...
    if (v->type == OSPF_VERTEX_ROUTER)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "SPF Result: %d [R] %s", i, inet_ntoa (v->lsa->id));
        }
    }
    else
    {
        ...
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "SPF Result: %d [N] %s/%d", i, inet_ntoa (v->lsa->id),
                        ip_masklen (lsa->mask));
    }
    if (IS_DEBUG_OSPF_EVENT)
    {
        ...
        {
            zlog_debug (OSPF_DBG_EVENT, " nexthop %p %s %s",
                        (void *)parent->nexthop,
                        ...
                        int parent_is_root)
{
    ...
    struct vertex *child;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_process_stub():processing stubs for area %s",
    ...
    if (v->type == OSPF_VERTEX_ROUTER)
    {
        ...
        int lsa_pos = 0;
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_process_stubs():processing router LSA, id: %s",
                        inet_ntoa (v->lsa->id));
        rlsa = (struct router_lsa *) v->lsa;
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_process_stubs(): we have %d links to process",
                        ntohs (rlsa->links));
        p = ((u_char *) v->lsa) + OSPF_LSA_HEADER_SIZE + 4;
        lim = ((u_char *) v->lsa) + ntohs (v->lsa->length);
        while (p < lim)
...
ospf_rtrs_free (struct route_table *rtrs)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Route: Router Routing Table free");
    }
                    ...
                    struct route_table *new_rtrs)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_spf_calculate: Start");
        zlog_debug (OSPF_DBG_EVENT, "ospf_spf_calculate: running Dijkstra for area %s",
                    inet_ntoa (area->area_id));
    }
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_spf_calculate: "
    ...
    *(v->stat) = LSA_SPF_IN_SPFTREE;
    ...
    time_get_time (TIME_CLK_MONOTONIC, &area->ospf->ts_spf);
    area->ts_spf = area->ospf->ts_spf;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "ospf_spf_calculate: Stop. %ld vertices",
...
static int
ospf_spf_calculate_timer (void *thread)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "SPF: Timer (SPF calculation expire)");
    }
    ospf->t_spf_calc = 0;
    time_get_time (TIME_CLK_MONOTONIC, &spf_start_time);
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "SPF Processing Time(usecs): %ld", total_spf_time);
        zlog_debug (OSPF_DBG_EVENT, "\t    SPF Time: %ld", spf_time);
        zlog_debug (OSPF_DBG_EVENT, "\t   InterArea: %ld", ia_time);
        zlog_debug (OSPF_DBG_EVENT, "\t       Prune: %ld", prune_time);
        zlog_debug (OSPF_DBG_EVENT, "\tRouteInstall: %ld", rt_time);
        if (IS_OSPF_ABR (ospf))
            zlog_debug (OSPF_DBG_EVENT, "\t         ABR: %ld (%d areas)",
                       abr_time, areas_processed);
        zlog_debug (OSPF_DBG_EVENT, "Reason(s) for SPF: %s", rbuf);
    }
    ospf_clear_spf_reason_flags ();
    return 0;
}
...
ospf_spf_calculate_schedule (struct ospf *ospf, ospf_spf_reason_t reason)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "SPF: calculation timer scheduled");
    }
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "SPF: calculation timer is already scheduled: %p",
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "SPF: calculation timer delay = %ld", delay);
    }
    zlog_debug (OSPF_DBG_EVENT, "SPF: Scheduled in %ld msec", delay);
    /*ospf->t_spf_calc =
        thread_add_timer_msec (master_ospf, ospf_spf_calculate_timer, ospf, delay);*/
    ospf->t_spf_calc =
        high_pre_timer_add ((char *)"ospf_spf_timer", LIB_TIMER_TYPE_NOLOOP, ospf_spf_calculate_timer, ospf, (10+delay));
}
