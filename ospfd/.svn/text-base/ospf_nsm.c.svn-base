...
//void nsm_clear_adj (struct ospf_neighbor *);

/* OSPF NSM Timer functions. */
int ospf_inactivity_timer (void *thread)
{
    ...
    if (IS_DEBUG_OSPF (nsm, NSM_TIMERS))
    {
		...
		zlog_debug (OSPF_DBG_NSM_TIMERS, "NSM[%s:%s]: Timer (Inactivity timer expire)",
              IF_NAME (nbr->oi), inet_ntoa (nbr->router_id));
    }
...
void
ospf_inactive_timer_restart(struct ospf_neighbor *nbr)
{
...
int ospf_db_desc_timer (void *thread)
{
    struct ospf_neighbor *nbr;
    nbr = (struct ospf_neighbor *)(thread);
    if(nbr == NULL)
    ...
    if (IS_DEBUG_OSPF (nsm, NSM_TIMERS))
    {
		...
		zlog_debug (OSPF_DBG_NSM_TIMERS, "NSM[%s:%s]: Timer (DD Retransmit timer expire)",
              IF_NAME (nbr->oi), inet_ntoa (nbr->src));
    }
	
	if(nbr->last_send == NULL)
    {
        zlog_warn("%s[%d] nbr->last_send is NULL",__func__,__LINE__);
        return 0;
    }
 ...
 */
void nsm_timer_set (struct ospf_neighbor *nbr)
{
    switch (nbr->state)
    {
    case NSM_Deleted:
...
/* OSPF NSM functions. */
int nsm_packet_received (struct ospf_neighbor *nbr)
{
    /* Start or Restart Inactivity Timer. */
    OSPF_NSM_TIMER_OFF (nbr->t_inactivity);
    OSPF_NSM_TIMER_ON (nbr->t_inactivity, ospf_inactivity_timer,
...
int nsm_start (struct ospf_neighbor *nbr)
{
    if (nbr->nbr_nbma)
    {
        OSPF_POLL_TIMER_OFF (nbr->nbr_nbma->t_poll);
...
int nsm_twoway_received (struct ospf_neighbor *nbr)
{
...
int ospf_db_summary_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
    ...
    /* Translated Type-7 LSA should flood delete by tai*/
...
int nsm_negotiation_done (struct ospf_neighbor *nbr)
{
...
int nsm_exchange_done (struct ospf_neighbor *nbr)
{
...
int nsm_adj_ok (struct ospf_neighbor *nbr)
{
 ...
 * to <= ExStart.
 */
void nsm_clear_adj (struct ospf_neighbor *nbr)
{
...
int nsm_kill_nbr (struct ospf_neighbor *nbr)
{
    /* killing nbr_self is invalid */
    if (nbr == nbr->oi->nbr_self)
    {
    	zlog_err("%s %d %s\n",__func__,__LINE__,inet_ntoa(nbr->address.u.prefix4));
    ...
    if (nbr->oi->type == OSPF_IFTYPE_NBMA && nbr->nbr_nbma != NULL)
    {
        ...
        if (IS_DEBUG_OSPF (nsm, NSM_EVENTS))
            zlog_debug (OSPF_DBG_NSM_EVENTS, "NSM[%s:%s]: Down (PollIntervalTimer scheduled)",
...
void nsm_notice_state_change (struct ospf_neighbor *nbr, int next_state, int event)
{
    /* Logging change of status. */
    
    zlog_debug (OSPF_DBG_NSM_EVENTS, "NSM[%s:%s]: State change %s -> %s (%s)",
                IF_NAME (nbr->oi), inet_ntoa (nbr->router_id),
                LOOKUP (ospf_nsm_state_msg, nbr->state),
                LOOKUP (ospf_nsm_state_msg, next_state),
                ospf_nsm_event_str [event]);
    
    if(nbr == NULL || nbr->oi == NULL || nbr->oi->ospf == NULL)
    {
        return;
    }
    ...
    {
        nbr->ts_last_progress = time_get_recent_relative_time ();
    }
    else /* regression in NSM */
    {
        nbr->ts_last_regress = time_get_recent_relative_time ();
        nbr->last_regress_str = ospf_nsm_event_str [event];
    }
}

void nsm_change_state (struct ospf_neighbor *nbr, int state)
{
    struct ospf_interface *oi = nbr->oi;
	...
	ospf_bfd_update_session (nbr, old_state);//ospf bfd
            ...
            (old_state == NSM_Full && state != NSM_Full))
    {
        ...
        zlog_debug (OSPF_DBG_NSM_EVENTS, "nsm_change_state(%s, %s -> %s): "
    ...
    if (state == NSM_ExStart)
    {
        if (nbr->dd_seqnum == 0)
        {
            nbr->dd_seqnum = time_get_real_time (NULL);
        }
        else
        {
            nbr->dd_seqnum++;
...
ospf_nsm_event (struct thread *thread)
{
    ...
    assert(nbr);
    if (IS_DEBUG_OSPF (nsm, NSM_EVENTS))
        zlog_debug (OSPF_DBG_NSM_EVENTS, "NSM[%s:%s]: %s (%s)", IF_NAME (nbr->oi),
                    inet_ntoa (nbr->router_id),
                    LOOKUP (ospf_nsm_state_msg, nbr->state),
                    ospf_nsm_event_str [event]);
    ...
    {
        ospf_schedule_asbr_task(nbr->oi->ospf);
        ospf_schedule_abr_task(nbr->oi->ospf);
        ospf_nbr_delete (nbr);
		nbr = NULL;
    }
    return 0;
