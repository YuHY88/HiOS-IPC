...
ospf_nbr_new (struct ospf_interface *oi)
{
	...
	if(nbr == NULL)
    {
        zlog_err("Can't creat ospf_neighbor : malloc failed");
        return (struct ospf_neighbor *)NULL;
    }
...
ospf_nbr_free (struct ospf_neighbor *nbr)
{
    ...
    thread_cancel_event (master_ospf, nbr);
	thread_cancel_event_normal (master_ospf, nbr);
...
ospf_nbr_delete (struct ospf_neighbor *nbr)
{
    ...
    if (rn)
    {
        ...
        if (rn->info)
        {
            ...
            zlog_debug (OSPF_DBG_NSM_EVENTS, "Can't find neighbor %s in the interface %s",
...
ospf_nbr_self_reset (struct ospf_interface *oi)
{
    ...
    /* Initialize neighbor list. case switch network type corrupted*/
    route_table_finish (oi->nbrs);
    oi->nbrs = route_table_init ();
    /*create nbr_self*/
    oi->type = IF_DEF_PARAMS (oi->ifp)->type;
...
ospf_nbr_self_reset(struct ospf_interface *oi)
{
    ...
    oi->nbr_self = ospf_nbr_new(oi);
	if(oi->type != OSPF_IFTYPE_VIRTUALLINK)//add by zzl
	{
    	oi->type = IF_DEF_PARAMS(oi->ifp)->type;
	}
    ospf_nbr_add_self(oi);
...
ospf_renegotiate_optional_capabilities (struct ospf *top)
{
    ...
    for (ALL_LIST_ELEMENTS_RO (top->oiflist, node, oi))
    {
        ...
        for (rn = route_top (nbrs); rn; rn = route_next (rn))
        {
            ...
            {
                zlog_debug (OSPF_DBG_EVENT, "Renegotiate optional capabilities with neighbor(%s)", inet_ntoa (nbr->router_id));
            }
              ...
              struct prefix *p)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "NSM[%s:%s]: start", IF_NAME (nbr->oi),
              ...
              struct ip iph, struct prefix *p)
{
    ...
    //add by zhangyun start for fixing the bug BugID=39469
    if ((nbr = ospf_nbr_lookup_by_routerid (oi->nbrs, &ospfh->router_id)) != NULL)
    {
        if (IPV4_ADDR_CMP (&nbr->src, &(iph.ip_src)))
        {
            nbr->ef_flags = 1;
            if (oi->type == OSPF_IFTYPE_VIRTUALLINK ||
                    oi->type == OSPF_IFTYPE_POINTOPOINT)
