...
ospf_route_new ()
{
	...
	do
	{		
		new_route = XCALLOC (MTYPE_OSPF_ROUTE, sizeof (struct ospf_route));
	...
	}while(new_route == NULL);
	if(new_route == NULL)
    {
        zlog_err("Can't creat ospf_route : malloc failed");
        return (struct ospf_route *)NULL;
    }
	memset(new_route, 0, sizeof(struct ospf_route));

    new_route->ctime = time_get_real_time (NULL);
    new_route->mtime = new_route->ctime;
    new_route->paths = list_new ();
    new_route->paths->del = (void (*) (void *))ospf_path_free;
...
ospf_path_new ()
{
    struct ospf_path *new_path = NULL;

	do
	{		
		new_path = XCALLOC (MTYPE_OSPF_PATH, sizeof (struct ospf_path));	
		if (new_path == NULL)
		{
			zlog_err("%s():%d: XCALLOC failed!", __FUNCTION__, __LINE__);
			sleep(1);
    ...
    {
        zlog_err("Can't creat ospf_route : malloc failed");
        return (struct ospf_path *)NULL;
    }
	memset(new_path, 0, sizeof(struct ospf_path));
...
ospf_path_dup (struct ospf_path *path)
{
    ...
    new_path = ospf_path_new ();
    memcpy (new_path, path, sizeof (struct ospf_path));
...
ospf_route_exist_new_table (struct route_table *rt, struct prefix_ipv4 *prefix)
{
	...
	{
		zlog_warn("%s[%d] rt is NULL or prefix is NULL",__func__,__LINE__);
		return 0;
	}
                       ...
                       struct ospf_route *newor, struct ospf *ospf)
{
    ...
    if (or->type == newor->type && or->cost == newor->cost)
    {
        if (or->type == OSPF_DESTINATION_NETWORK)
        {
            ...
            {
                ospf_zebra_delete (prefix, or, ospf);
                return 0;
            }
            /* Check each path. */
...
ospf_route_delete_uniq (struct route_table *rt, struct route_table *cmprt,struct ospf* ospf)
{
	...
	if(cmprt == NULL)
	{
		zlog_warn("%s[%d] cmprt is NULL",__func__,__LINE__);
		return ;
	}
    for (rn = route_top (rt); rn; rn = route_next (rn))
    {
        ...
        if ((or = rn->info) != NULL)
        {
            if (or->type == OSPF_DESTINATION_NETWORK)
            {
                if (! ospf_route_match_same (ospf->old_table,
                                             (struct prefix_ipv4 *)&rn->p, or,ospf))
                {
                       ...
                       struct ospf_area *area)
{
    ...
    struct router_lsa *lsa = NULL;
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_router: Start");
    }
    lsa = (struct router_lsa *) v->lsa;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_router: LS ID: %s",
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_router: "
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_router: talking about %s/%d",
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_router: Stop");
    }
}
                        ...
                        struct ospf_area *area)
{
    ...
    p.prefix.s_addr = (v->id.s_addr) & (lsa->mask.s_addr);
                     ...
                     int parent_is_root, int lsa_pos)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): Start");
    }
    lsa = (struct router_lsa *) v->lsa;
    p.family = AF_INET;
    ...
    apply_mask_ipv4 (&p);
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): processing route to %s/%d",
                    inet_ntoa (p.prefix), p.prefixlen);
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): calculated cost is %d + %d = %d",
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "%s: ignoring host route %s/32 to self.",
    ...
    if (rn->info)
    {
        ...
        cur_or = rn->info;
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): "
        ...
        if (cost > cur_or->cost)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): old route is better, exit");
            }
            return;
        }
        ...
        if (cost == cur_or->cost)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): routes are equal, merge");
            }
        ...
        if (cost < cur_or->cost)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): new route is better, set it");
            }
            cur_or->cost = cost;
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): installing new route");
    }
    or = ospf_route_new ();
    or->id = v->id;
    or->u.std.area_id = area->area_id;
    ...
    if (v != area->spf)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): this network is on remote router");
        }
        ospf_route_copy_nexthops_from_vertex (or, v);
    }
    else
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): this network is on this router");
        }
        if ((oi = ospf_if_lookup_by_lsa_pos (area, lsa_pos)))
        {
            if (IS_DEBUG_OSPF_EVENT)
                zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): the interface is %s",
                            IF_NAME (oi));
            path = ospf_path_new ();
            //struct connected *ifc = get_ospf_out_if_address(oi->ifp->ifindex);
			//if(ifc != NULL)
            //	path->nexthop = ifc->address->u.prefix4;/* in order to put the nexthop to outif*/
            if(oi->address != NULL)
            	path->nexthop = oi->address->u.prefix4;
            path->ifindex = oi->ifp->ifindex;
            path->area_id.s_addr = oi->area->area_id.s_addr;
        ...
        else
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "ospf_intra_add_stub(): where's the interface ?");
            }
        }
    }
    rn->info = or;
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug(OSPF_DBG_EVENT, "ospf_intra_add_stub(): Stop");
    }
}
...
ospf_route_table_dump (struct route_table *rt)
{
    ...
    struct ospf_path *path;
    zlog_debug (OSPF_DBG_EVENT, "========== OSPF routing table ==========");
    for (rn = route_top (rt); rn; rn = route_next (rn))
    {
        if ((or = rn->info) != NULL)
        {
            if (or->type == OSPF_DESTINATION_NETWORK)
            {
                zlog_debug (OSPF_DBG_EVENT, "N %s/%d\t%s\t%s\t%d",
                ...
                {
                    zlog_debug (OSPF_DBG_EVENT, "  -> %s", inet_ntoa (path->nexthop));
                }
            }
            else
                zlog_debug (OSPF_DBG_EVENT, "R %s\t%s\t%s\t%d",
                            inet_ntop (AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ),
                            inet_ntop (AF_INET, &or->u.std.area_id, buf2,
                                       BUFSIZ),
                            ospf_path_type_str[or->path_type],
                ...
                struct ospf_route *r2)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Route[Compare]: Path types are the same.");
    }
    ...
    case OSPF_PATH_INTER_AREA:
        if (r1->cost == r2->cost)
        {
            return r2->u.std.area_id.s_addr - r1->u.std.area_id.s_addr;
        }
        else
        {
            break;
                                      ...
                                      struct vertex *v)
{
	...
	if(to == NULL || to->paths == NULL)
	{
		zlog_warn("%s[%d] to == NULL || to->paths == NULL",__func__,__LINE__);
		return ;
	}
    assert (to->paths);
    ...
    for (ALL_LIST_ELEMENTS_RO (v->parents, node, vp))
    {
        ...
        if (nexthop->oi != NULL)
        {
            ...
            {
                path = ospf_path_new ();
                if(nexthop->router.s_addr)
                {
                    path->nexthop = nexthop->router;
                }
                else/* in order to put the nexthop to outif*/
                {
                    //struct connected *ifc = NULL;
                    //ifc =get_ospf_out_if_address(nexthop->oi->ifp->ifindex);
                    //path->nexthop = ifc->address->u.prefix4;
                    if(nexthop->oi->address != NULL)
            			path->nexthop = nexthop->oi->address->u.prefix4;
                }
                path->ifindex = nexthop->oi->ifp->ifindex;
                path->area_id.s_addr = nexthop->oi->area->area_id.s_addr;
                ...
                struct ospf_route *new_or, struct ospf_route *over)
{
    ...
    if (rn->info)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_route_add(): something's wrong !");
        }
        route_unlock_node (rn);
        return;
    }
...
ospf_prune_unreachable_networks (struct route_table *rt)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Pruning unreachable networks");
    }
    for (rn = route_top (rt); rn; rn = next)
    {
        ...
        if (rn->info != NULL)
        {
            ...
            {
                if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug (OSPF_DBG_EVENT, "Pruning route to %s/%d",
...
ospf_prune_unreachable_routers (struct route_table *rtrs)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Pruning unreachable routers");
    }
    for (rn = route_top (rtrs); rn; rn = next)
    {
        ...
        for (ALL_LIST_ELEMENTS (paths, node, nnode, or))
        {
            if (listcount (or->paths) == 0)
            {
                if (IS_DEBUG_OSPF_EVENT)
                {
                    zlog_debug (OSPF_DBG_EVENT, "Pruning route to rtr %s",
                                inet_ntoa (rn->p.u.prefix4));
                    zlog_debug (OSPF_DBG_EVENT, "               via area %s",
        ...
        if (listcount (paths) == 0)
        {
            if (IS_DEBUG_OSPF_EVENT)
            {
                zlog_debug (OSPF_DBG_EVENT, "Pruning router node %s", inet_ntoa (rn->p.u.prefix4));
            }
            list_delete (paths);
			paths = NULL;
            rn->info = NULL;
                        ...
                        struct prefix_ipv4 *p)
{
    ...
    if (rn == NULL)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_add_discard_route(): router installation error");
        }
        return 0;
    }
    if (rn->info) /* If the route to the same destination is found */
    {
        ...
        {
            if (IS_DEBUG_OSPF_EVENT)
                zlog_debug (OSPF_DBG_EVENT, "ospf_add_discard_route(): "
    ...
    rn->info = new_or;
    //ospf_zebra_add_discard (area->ospf,p);
    return 1;
}
...
ospf_delete_discard_route (struct ospf* ospf,struct route_table *rt, struct prefix_ipv4 *p)
{
    ...
    struct ospf_route *or;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "ospf_delete_discard_route(): "
    ...
    if (rn == NULL)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug(OSPF_DBG_EVENT, "ospf_delete_discard_route(): no route found");
        }
        return;
    }
    or = rn->info;
...
void
ospf_route_delete_by_area(struct ospf_area *area, struct route_table *rt)
{
    struct route_node *rn;
    struct ospf_route *or;
    //struct ospf_path *path;
    for (rn = route_top (rt); rn; rn = route_next (rn))
    {
        if ((or = rn->info) != NULL)
        {
            ...
            case OSPF_PATH_INTER_AREA:
                break;
            case OSPF_PATH_INTRA_AREA:
                if(area->area_id.s_addr == or->u.std.area_id.s_addr)
                {
                    struct prefix_ipv4 *p = (struct prefix_ipv4 *)(&rn->p);
                    ospf_zebra_delete(p, or, area->ospf);
                }
                break;
            default:
                break;
