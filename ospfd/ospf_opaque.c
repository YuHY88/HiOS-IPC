...
{
    struct list *funclist;
    struct ospf_opaque_functab *new_opa = NULL;
    int rc = -1;
	...
	do
	{		
		new_opa = XCALLOC (MTYPE_OSPF_OPAQUE_FUNCTAB,
...
/*	
    if ((new_opa = XCALLOC (MTYPE_OSPF_OPAQUE_FUNCTAB,
                        sizeof (struct ospf_opaque_functab))) == NULL)
    {
        zlog_err("ospf_register_opaque_functab: XMALLOC: %s",
                   safe_strerror (errno));
        goto out;
    }
*/	
	memset(new_opa, 0, sizeof(struct ospf_opaque_functab));
    new_opa->opaque_type    = opaque_type;
    new_opa->oipt           = NULL;
    new_opa->new_if_hook    = new_if_hook;
    new_opa->del_if_hook    = del_if_hook;
    new_opa->ism_change_hook     = ism_change_hook;
    new_opa->nsm_change_hook     = nsm_change_hook;
    new_opa->config_write_router = config_write_router;
    new_opa->config_write_if     = config_write_if;
    new_opa->config_write_debug  = config_write_debug;
    new_opa->show_opaque_info    = show_opaque_info;
    new_opa->lsa_originator = lsa_originator;
    new_opa->lsa_refresher  = lsa_refresher;
    new_opa->new_lsa_hook   = new_lsa_hook;
    new_opa->del_lsa_hook   = del_lsa_hook;
    listnode_add (funclist, new_opa);
    rc = 0;
out:
    return rc;
}
...
static struct opaque_info_per_id *register_opaque_lsa (struct ospf_lsa *new_opa);


static struct opaque_info_per_type *
register_opaque_info_per_type (struct ospf_opaque_functab *functab,
                               struct ospf_lsa *new_opa)
{
	...
	do
	{		
		oipt = XCALLOC (MTYPE_OPAQUE_INFO_PER_TYPE, sizeof (struct opaque_info_per_type));
...
/*
    ...
    {
        zlog_err ("register_opaque_info_per_type: XMALLOC: %s", safe_strerror (errno));
        goto out;
    }
 */
	memset(oipt, 0, sizeof(struct opaque_info_per_type));

    switch (new_opa->data->type)
    {
    case OSPF_OPAQUE_LINK_LSA:
        oipt->owner = new_opa->oi;
        listnode_add (new_opa->oi->opaque_lsa_self, oipt);
        break;
    case OSPF_OPAQUE_AREA_LSA:
        oipt->owner = new_opa->area;
        listnode_add (new_opa->area->opaque_lsa_self, oipt);
        break;
    case OSPF_OPAQUE_AS_LSA:
        //top = ospf_lookup ();
        top = ospf_lookup_id(ospf_id);
		if(top == NULL)
		{
        	zlog_warn ("%s %d ospf is NULL %d\n", __func__,__LINE__,ospf_id);
			goto out;
		}
			
        if (new_opa->area != NULL && (top = new_opa->area->ospf) == NULL)
        {
        ...
        break;
    default:
        zlog_warn ("register_opaque_info_per_type: Unexpected LSA-type(%u)", new_opa->data->type);
        free_opaque_info_per_type ((void *) oipt);
        oipt = NULL;
        goto out; /* This case may not exist. */
    }

    oipt->lsa_type = new_opa->data->type;
    oipt->opaque_type = GET_OPAQUE_TYPE (ntohl (new_opa->data->id.s_addr));
    oipt->status = PROC_NORMAL;
    oipt->t_opaque_lsa_self = 0;
                             ...
                             struct ospf_lsa *new_lsa)
{
	...
	do
	{		
		oipi = XCALLOC (MTYPE_OPAQUE_INFO_PER_TYPE,
...
/*
    ...
    {
        zlog_err ("register_opaque_info_per_id: XMALLOC: %s", safe_strerror (errno));
        goto out;
    }
*/	
	memset(oipi, 0, sizeof(struct opaque_info_per_id));
    oipi->opaque_id = GET_OPAQUE_ID (ntohl (new_lsa->data->id.s_addr));
    oipi->t_opaque_lsa_self = 0;
    oipi->opqctl_type = oipt;
    oipi->lsa = ospf_lsa_lock (new_lsa);
...
static struct opaque_info_per_id *
register_opaque_lsa (struct ospf_lsa *new_lsa)
{
    struct ospf_opaque_functab *functab;
    struct opaque_info_per_type *oipt;
    struct opaque_info_per_id *oipi = NULL;

    if ((functab = ospf_opaque_functab_lookup (new_lsa)) == NULL)
    {
        goto out;
    }

    if ((oipt = lookup_opaque_info_by_type (new_lsa)) == NULL
            &&  (oipt = register_opaque_info_per_type (functab, new_lsa)) == NULL)
    {
        goto out;
    }

    if ((oipi = register_opaque_info_per_id (oipt, new_lsa)) == NULL)
    {
        goto out;
    }
       ...
       "Opaque LSA\n")
{
    ...
    if (!CHECK_FLAG (ospf->config, OSPF_OPAQUE_CAPABLE))
    {
        ...
        {
            zlog_debug (OSPF_DBG_EVENT, "Opaque capability: OFF -> ON");
        }
       ...
       "Opaque LSA\n")
{
    ...
    if (CHECK_FLAG (ospf->config, OSPF_OPAQUE_CAPABLE))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Opaque capability: ON -> OFF");
        }
...
{
    install_element (OSPF_NODE, &capability_opaque_cmd, CMD_SYNC);
    install_element (OSPF_NODE, &no_capability_opaque_cmd, CMD_SYNC);

	install_element (OSPF_NODE, &capability_opaque_h3c_cmd, CMD_SYNC);
    install_element (OSPF_NODE, &no_capability_opaque_h3c_cmd, CMD_SYNC);
    return;
}
...
ospf_opaque_nsm_change (struct ospf_neighbor *nbr, int old_state)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug(OSPF_DBG_EVENT, "%s[%d]: opaque nsm change start!\n", __FUNCTION__, __LINE__);
    }
    ...
    if (old_state != NSM_Full && nbr->state == NSM_Full)
    {
        if (CHECK_FLAG (nbr->options, OSPF_OPTION_O))
        {
            if (! CHECK_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT))
            {
                if (IS_DEBUG_OSPF_EVENT)
                {
                    zlog_debug (OSPF_DBG_EVENT, "Opaque-LSA: Now get operational!");
                }
                SET_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT);
            }
			if((temp_lsa = ospf_lsdb_lookup_by_id(nbr->oi->area->lsdb, OSPF_OPAQUE_AREA_LSA, dcn_opaque_id, nbr->oi->ospf->router_id)))
			{
				//printf("%s()--%d\n",__FUNCTION__, __LINE__);
				//ospf_opaque_lsa_refresh_schedule (temp_lsa);
			}
			else
			{
				//printf("%s()--%d\n",__FUNCTION__, __LINE__);
	...
	if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug(OSPF_DBG_EVENT, "%s[%d]: opaque nsm change end!\n", __FUNCTION__, __LINE__);
    }
out:
    return;
}
...
ospf_opaque_config_write_router (struct vty *vty, struct ospf *ospf)
{
    ...
    if (CHECK_FLAG (ospf->config, OSPF_OPAQUE_CAPABLE))
    {
        vty_out (vty, " capability opaque-lsa%s", VTY_NEWLINE);
    }
    funclist = ospf_opaque_wildcard_funclist;
...
show_opaque_info_detail (struct vty *vty, struct ospf_lsa *lsa)
{
    ...
    {
        zlog_debug (OSPF_DBG_DETAIL, "    Opaque-Type %u (%s)", opaque_type,
                    ospf_opaque_type_name (opaque_type));
        zlog_debug (OSPF_DBG_DETAIL, "    Opaque-ID   0x%x", opaque_id);
        zlog_debug (OSPF_DBG_DETAIL, "    Opaque-Info: %u octets of data%s",
                    ntohs (lsah->length) - OSPF_LSA_HEADER_SIZE,
                    VALID_OPAQUE_INFO_LEN(lsah) ? "" : "(Invalid length?)");
    }
...
static int ospf_opaque_type9_lsa_originate (void *t);
static int ospf_opaque_type10_lsa_originate (void *t);
static int ospf_opaque_type11_lsa_originate (void *t);
...
ospf_opaque_lsa_originate_schedule (struct ospf_interface *oi, int *delay0)
{
    ...
    if (! CHECK_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_opaque_lsa_originate_schedule: Not operational.");
        }
            ...
            &&    oi->t_opaque_lsa_self == 0)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Schedule Type-9 Opaque-LSA origination in %d ms later.", delay);
        }
        /*oi->t_opaque_lsa_self =
            thread_add_timer_msec (master_ospf, ospf_opaque_type9_lsa_originate, oi, delay);*/

		oi->t_opaque_lsa_self =
            high_pre_timer_add ((char *)"ospf_oi_opaque_lsa_timer", LIB_TIMER_TYPE_NOLOOP,\
            					ospf_opaque_type9_lsa_originate, oi, delay);
            ...
            &&    area->t_opaque_lsa_self == 0)
    {
        ...
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Schedule Type-10 Opaque-LSA origination in %d ms later.", delay);
        }
        /*area->t_opaque_lsa_self =
            thread_add_timer_msec (master_ospf, ospf_opaque_type10_lsa_originate,
                                   area, delay);*/
		area->t_opaque_lsa_self =
            high_pre_timer_add ((char *)"ospf_area_opaque_lsa_timer", LIB_TIMER_TYPE_NOLOOP,\
            					ospf_opaque_type10_lsa_originate, area, delay);
            ...
            &&    top->t_opaque_lsa_self == 0)
    {
        ...
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Schedule Type-11 Opaque-LSA origination in %d ms later.", delay);
        }
        /*top->t_opaque_lsa_self =
            thread_add_timer_msec (master_ospf, ospf_opaque_type11_lsa_originate,
                                   top, delay);*/
		top->t_opaque_lsa_self =
            high_pre_timer_add ((char *)"ospf_opaque_timer", LIB_TIMER_TYPE_NOLOOP,\
            			ospf_opaque_type11_lsa_originate, top, delay);
        delay += top->min_ls_interval;
    }
    /*
            ...
            &&  ! list_isempty (oi->opaque_lsa_self))
    {
        for (ALL_LIST_ELEMENTS (oi->opaque_lsa_self, node, nnode, oipt))
        {
             ...
             * not being empty.
             */
            if (oipt->t_opaque_lsa_self != 0 /* Waiting for a thread call. */
...
static int
ospf_opaque_type9_lsa_originate (void *t)
{
    struct ospf_interface *oi;
    int rc;
    oi = (struct ospf_interface *)(t);
    if(oi == NULL)
    {
    ...
    oi->t_opaque_lsa_self = 0;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type9-LSA]: Originate Opaque-LSAs for OI %s",
                    IF_NAME (oi));
...
static int
ospf_opaque_type10_lsa_originate (void *t)
{
    struct ospf_area *area;
    int rc;
    area = (struct ospf_area *)(t);
    if(area == NULL)
    {
        zlog_warn("%s[%d] get thread arg is NULL",__func__,__LINE__);
        return 0;
    ...
    area->t_opaque_lsa_self = 0;
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type10-LSA]: Originate Opaque-LSAs for Area %s",
...
static int
ospf_opaque_type11_lsa_originate (void *t)
{
    struct ospf *top;
    int rc;
    top = (struct ospf *)(t);
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type11-LSA]: Originate AS-External Opaque-LSAs");
    }
...
ospf_opaque_lsa_install (struct ospf_lsa *lsa, int rt_recalc)
{
    ...
    if (! IS_LSA_SELF (lsa))
    {
        new_lsa = lsa; /* Don't touch this LSA. */
        goto out;
    }

    if (IS_DEBUG_OSPF (lsa, LSA_INSTALL))
    {
        zlog_debug (OSPF_DBG_LSA, "Install Type-%u Opaque-LSA: [opaque-type=%u, opaque-id=%x]", \
...
struct ospf_lsa *
ospf_opaque_lsa_refresh (struct ospf_lsa *lsa, struct ospf* ospf)
{
    struct ospf_opaque_functab *functab;
    struct ospf_lsa *new_lsa = NULL;
            ...
            || functab->lsa_refresher == NULL)
    {
        ...
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "LSA[Type%d:%s]: Flush stray Opaque-LSA", lsa->data->type, inet_ntoa (lsa->data->id));
        }
    ...
    else
    {
    	if((om->u0_flag)
			...
			&& (OPAQUE_TYPE_NMS_AUTOREPORT == GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr))))
    	{
			new_lsa = ospf_opaque_type10_lsa_refresh_for_u0(lsa);
		}
		else
		{
        	new_lsa = (* functab->lsa_refresher)(lsa);
		}
    }
...
#define OSPF_OPAQUE_TIMER_ON(T,F,L,V) \
      if (!(T)) \
        (T) = thread_add_timer_msec (master_ospf, (F), (L), (V))
#endif

/*Change by zzl 20180911 for IPC optimization*/
#define OSPF_OPAQUE_TIMER_ON(T,F,L,V) \
		if (!(T)) \
			(T) = high_pre_timer_add ((char *)"ospf_opaque_timer", LIB_TIMER_TYPE_NOLOOP, (F), (L), (V));


static struct ospf_lsa *pseudo_lsa (struct ospf_interface *oi, struct ospf_area *area, u_char lsa_type, u_char opaque_type);
static int ospf_opaque_type9_lsa_reoriginate_timer (void *t);
static int ospf_opaque_type10_lsa_reoriginate_timer (void *t);
static int ospf_opaque_type11_lsa_reoriginate_timer (void *t);
static int ospf_opaque_lsa_refresh_timer (void *t);
                                      ...
                                      u_char lsa_type, u_char opaque_type)
{
    ...
    if (!CHECK_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "ospf_opaque_lsa_reoriginate_schedule: Not operational.");
        }
        goto out;                 /* This is not an error. */
    }
    ...
    if (oipt->t_opaque_lsa_self != 0)
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "Type-%u Opaque-LSA has already scheduled to"
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "Schedule Type-%u Opaque-LSA to RE-ORIGINATE in %d"
                    " ms later: [opaque-type=%u]",
                    lsa_type, delay,
                    GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)));
    OSPF_OPAQUE_TIMER_ON (oipt->t_opaque_lsa_self, func, oipt, (delay+10));
out:
    return;
}
...
static int
ospf_opaque_type9_lsa_reoriginate_timer (void *t)
{
    struct opaque_info_per_type *oipt;
    struct ospf_opaque_functab *functab;
    struct ospf *top;
    struct ospf_interface *oi;
    int rc = -1;
    oipt = (struct opaque_info_per_type *)(t);
    if(oipt == NULL)
    {
        zlog_warn("%s[%d] get thread arg is NULL",__func__,__LINE__);
        return 0;
            ...
            ||    ospf_nbr_count_opaque_capable (oi) == 0)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Suspend re-origination of Type-9 Opaque-LSAs (opaque-type=%u) for a while...", oipt->opaque_type);
        }
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type9-LSA]: Re-originate Opaque-LSAs (opaque-type=%u) for OI (%s)", oipt->opaque_type, IF_NAME (oi));
    }
...
static int
ospf_opaque_type10_lsa_reoriginate_timer (void *t)
{
    struct opaque_info_per_type *oipt;
    struct ospf_opaque_functab *functab;
    struct listnode *node, *nnode;
    ...
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug (OSPF_DBG_EVENT, "Suspend re-origination of Type-10 Opaque-LSAs"
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type10-LSA]: Re-originate Opaque-LSAs"
...
static int
ospf_opaque_type11_lsa_reoriginate_timer (void *t)
{
    struct opaque_info_per_type *oipt;
    struct ospf_opaque_functab *functab;
    struct ospf *top;
    ...
    if (! CHECK_FLAG (top->config, OSPF_OPAQUE_CAPABLE))
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Suspend re-origination of Type-11 Opaque-LSAs (opaque-type=%u) for a while...", oipt->opaque_type);
        }
        oipt->status = PROC_SUSPEND;
        rc = 0;
        goto out;
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Timer[Type11-LSA]: Re-originate Opaque-LSAs (opaque-type=%u).", oipt->opaque_type);
    }
...
ospf_opaque_lsa_refresh_schedule (struct ospf_lsa *lsa0)
{
    ...
    if (oipi->t_opaque_lsa_self != 0)
    {
        if (IS_DEBUG_OSPF_EVENT)
        {
            zlog_debug (OSPF_DBG_EVENT, "Type-%u Opaque-LSA has already scheduled to REFRESH: [opaque-type=%u, opaque-id=%x]", lsa->data->type, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
        }
        goto out;
    }
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Schedule Type-%u Opaque-LSA to REFRESH in %d sec later: [opaque-type=%u, opaque-id=%x]", lsa->data->type, delay, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
    }
...
static int
ospf_opaque_lsa_refresh_timer (void *t)
{
    struct opaque_info_per_id *oipi;
    struct ospf_opaque_functab *functab;
    struct ospf_lsa *lsa;
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Timer[Opaque-LSA]: (Opaque-LSA Refresh expire)");
    }
    oipi = (struct opaque_info_per_id *)(t);
    if(oipi == NULL)
    {
        zlog_warn("%s[%d] get thread arg is NULL",__func__,__LINE__);
        return 0;
...
ospf_opaque_lsa_flush_schedule (struct ospf_lsa *lsa0)
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
    {
        zlog_debug (OSPF_DBG_EVENT, "Schedule Type-%u Opaque-LSA to FLUSH: [opaque-type=%u, opaque-id=%x]", lsa->data->type, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
    }
