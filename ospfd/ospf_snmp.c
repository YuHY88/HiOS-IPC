                  ...
                  struct in_addr *router_id, int router_id_next)
{
    ...
    {
        zlog_debug(OSPF_DBG_OTHER, "Strange request with LSA type %d\n", i);
        return NULL;
    }
...
{
	ospf_snmp_if *ospf_snmp_if = NULL;
	do
	{		
		ospf_snmp_if = return XCALLOC (0, sizeof (struct ospf_snmp_if));	

		if (ospf_snmp_if == NULL)
		{
			zlog_err("%s():%d: XCALLOC failed!", __FUNCTION__, __LINE__);
			sleep(1);
		}
	}while(ospf_snmp_if == NULL);
	return ospf_snmp_if;
}
...
ospf_snmp_if_update (struct interface *ifp)
{
	...
	if(osif == NULL)
	{
		zlog_err("Can't creat ospf_snmp_if_new : malloc failed");
        return ;
	}
    ...
    osif->ifp = ifp;
    listnode_add_next (ospf_snmp_iflist, pn, osif);
}
