...
#define OSPF_LSA_LOCAL_XLT    0x20    /* Translated Type-7 LSAs */
#define OSPF_LSA_PREMATURE_AGE    0x40
#define OSPF_LSA_IN_MAXAGE    0x80

	u_char opaque_lsa_flag;				/*Mark whether the NE described by the lsa is online. 0X1 mark the NE online*/
#define OPAQUE_LSA_FOR_DCN		0x01	/*Default value = 0X1.*/
...
#define LSA_SPF_IN_SPFTREE    -2
    ...
    /* For 7->5 originata lsa */
	struct in_addr adv_type7;
};
...
#define IS_LSA_TRANIST(L)          (CHECK_FLAG ((L)->flags, OSPF_LSA_LOCAL_XLT))
...
#define OSPF_LSA_UPDATE_TIMER_ON(T,F) \
      if (!(T)) \
        (T) = thread_add_timer (master_ospf, (F), 0, 2)
*/
...
extern int
ospf_external_lsa_check_ei_and_lsa(struct ospf * , 
                        struct external_info * , struct ospf_lsa *);
...
extern void ospf_external_summary_lsa_flush (struct ospf *, u_char, struct prefix_ipv4 *,
        ifindex_t /* , struct in_addr nexthop */);


extern int ospf_translated_external_lsa_flush_byarea (struct ospf *,
        struct ospf_area *);
...
extern int ospf_external_lsa_originate_timer (void *);
extern void ospf_external_lsa_fw_update(struct ospf* ospf);
extern void ospf_nssa_lsa_fw_update(struct ospf_area *);
extern int ospf_default_originate_timer (void *);
...
extern void ospf_external_self_originated_lsas_now(struct ospf *ospf);
extern void ospf_area_self_originated_lsas_now(struct ospf_area *area);
...
extern int ospf_lsa_maxage_walker (void *);
extern struct ospf_lsa *ospf_lsa_refresh (struct ospf *, struct ospf_lsa *);

extern void ospf_external_lsa_refresh_default (struct ospf *);

extern void ospf_external_lsa_refresh_type (struct ospf *, u_char, int, u_int8_t);
extern struct ospf_lsa *ospf_external_lsa_refresh_only(struct ospf *,
        struct ospf_lsa *,
...
extern struct in_addr ospf_lsa_unique_id (struct ospf *, struct ospf_lsdb *, u_char,
        struct prefix_ipv4 *, u_int32_t *, struct ospf_area *);
...
extern int ospf_lsa_refresh_walker (void *);
...
extern struct ospf_lsa *ospf_as_nssa_defaults_lsa_originate (struct ospf *);
extern struct ospf_lsa *ospf_as_nssa_lsa_originate (struct ospf *);
extern void ospf_area_as_nssa_lsa_flush (struct ospf_area *);

extern int metric_type (struct ospf *, u_char, u_int8_t);
extern int metric_value (struct ospf *, u_char, u_int8_t);
extern void ospf_install_flood_nssa (struct ospf *, struct ospf_lsa *, struct external_info *);
extern void ospf_nssa_lsa_orignate_only(struct ospf *ospf, struct external_info *ei);
struct ospf_lsa *ospf_nssa_lsa_refresh (struct ospf *, struct ospf_lsa *, struct external_info *, int );
void ospf_nssa_lsa_refresh_all(struct ospf *);
void ospf_as_nssa_defaults_lsa_flush(struct ospf *);
...
//extern void ospf_remove_lsa_now(struct ospf *);

extern struct ospf_lsa* ospf_as_nssa_default_lsa_refresh (struct ospf *, struct ospf_lsa *);
extern struct ospf_lsa *ospf_find_nssa_lsa_highestprio (struct ospf *, struct in_addr *);
extern struct ospf_lsa *ospf_external_lsa_new (struct ospf *, struct external_info *, struct in_addr *);

extern void ospf_dcn_ne_online_lookup(struct ospf_area *, struct list *, struct router_lsa *);

extern int ospf_opaque_type10_init (void);
...
struct ospf_lsa * ospf_summary_lsa_new (struct ospf_area *area, struct prefix *p, u_int32_t metric, struct in_addr id);
struct ospf_lsa *ospf_summary_lsa_refresh (struct ospf *ospf, struct ospf_lsa *lsa);
void ospf_summary_asbr_lsa_body_set (struct stream *s, struct prefix *p, u_int32_t metric);
struct ospf_lsa *ospf_summary_asbr_lsa_new (struct ospf_area *area, struct prefix *p, u_int32_t metric, struct in_addr id);
...
struct ospf_lsa *ospf_network_lsa_install (struct ospf *ospf,
                          ...
                          int rt_recalc);
struct ospf_lsa *ospf_summary_lsa_install (struct ospf *ospf, struct ospf_lsa *new_lsa, int rt_recalc);
struct ospf_lsa *ospf_external_lsa_install (struct ospf *ospf, struct ospf_lsa *new_lsa, int rt_recalc);

extern int ospf_opaque_type10_lsa_generate_for_u0 (void *arg, struct u0_device_info *u0_device);
extern struct ospf_lsa *ospf_opaque_type10_lsa_refresh_for_u0 (struct ospf_lsa *lsa);

extern struct u0_device_info_local *lookup_u0_device_by_adv_router(struct in_addr adv_router);
extern struct ospf_lsa *ospf_router_lsa_refresh_for_u0 (struct ospf_lsa *lsa);
extern struct ospf_lsa *ospf_router_lsa_originate_for_u0 (struct ospf_area *area, struct u0_device_info *u0_device);
