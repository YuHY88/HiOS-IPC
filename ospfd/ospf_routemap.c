...
static int
ospf_route_match_delete (struct vty *vty, struct route_map_index *index_r,
                         const char *command, const char *arg)
{
    int ret;
    ret = route_map_delete_match (index_r, command, arg);
    if (ret)
    {
        switch (ret)
        {
        case RMAP_RULE_MISSING:
            vty_error_out (vty, "Can't find rule.%s", VTY_NEWLINE);
            return CMD_WARNING;
        case RMAP_COMPILE_ERROR:
            vty_error_out (vty, "Argument is malformed.%s", VTY_NEWLINE);
            return CMD_WARNING;
		default:
            vty_error_out (vty, "ret:%d is error.%s", ret, VTY_NEWLINE);
			return CMD_WARNING;
        }
    }
    return CMD_SUCCESS;
...
static int
ospf_route_match_add (struct vty *vty, struct route_map_index *index_r,
                      const char *command, const char *arg)
{
    int ret;
    ret = route_map_add_match (index_r, command, arg);
    if (ret)
    {
        switch (ret)
        {
...
static int
ospf_route_set_add (struct vty *vty, struct route_map_index *index_r,
                    const char *command, const char *arg)
{
    int ret;
    ret = route_map_add_set (index_r, command, arg);
    if (ret)
    {
        switch (ret)
        {
...
/* Delete rip route map rule. */
static int
ospf_route_set_delete (struct vty *vty, struct route_map_index *index_r,
                       const char *command, const char *arg)
{
    int ret;
    ret = route_map_delete_set (index_r, command, arg);
    if (ret)
    {
        switch (ret)
        {
...
route_set_metric_compile (const char *arg)
{
	...
	if(metric == NULL)
    {
        zlog_err("Can't creat metric : malloc failed");
        return NULL;
    }
	
	memset(metric, 0, sizeof(u_int32_t));
    ret = atoi (arg);
    if (ret >= 0)
    {
...
route_set_metric_type_compile (const char *arg)
{
	...
	}while(metric_type == NULL);
	if(metric_type == NULL)
    {
        zlog_err("Can't creat metric_type : malloc failed");
        return NULL;
    }
	
	memset(metric_type, 0, sizeof(u_int32_t));
