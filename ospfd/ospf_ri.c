...
ospf_router_info_register(u_int8_t scope)
{
    ...
    zlog_debug(OSPF_DBG_OTHER, "Register Router Information with scope %s(%d)",
...
set_pce_domain(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce)
{
    ...
    struct ri_pce_subtlv_domain *new_pce = NULL;
	...
	do
	{		
		new_pce = XCALLOC(MTYPE_OSPF_PCE_PARAMS,
	...
	if(new_pce == NULL)
    {
        zlog_err("Can't creat ri_pce_subtlv_domain : malloc failed");
        return ;
    }
	memset(new_pce, 0, sizeof(struct ri_pce_subtlv_domain));

    new_pce->header.type = htons(RI_PCE_SUBTLV_DOMAIN);
    new_pce->header.length = htons(PCE_ADDRESS_LENGTH_IPV4);
    new_pce->type = htons(type);
    new_pce->value = htonl(domain);

    /* Add new domain to the list */
    listnode_add(pce->pce_domain, new_pce);
...
set_pce_neighbor(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce)
{
    ...
    struct ri_pce_subtlv_neighbor *new_pce;
	...
	memset(new_pce, 0, sizeof(struct ri_pce_subtlv_neighbor));

    new_pce->header.type = htons(RI_PCE_SUBTLV_NEIGHBOR);
    new_pce->header.length = htons(PCE_ADDRESS_LENGTH_IPV4);
    new_pce->type = htons(type);
    new_pce->value = htonl(domain);

    /* Add new domain to the list */
    listnode_add(pce->pce_neighbor, new_pce);
...
initialize_params(struct ospf_router_info *ori)
{
    ...
    top = ospf_lookup();
    zlog_debug(OSPF_DBG_OTHER, "RI-> Initialize Router Info for %s scope within area %s",
...
ospf_router_info_lsa_new()
{
    ...
    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
        zlog_debug
        (OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Create an Opaque-LSA/ROUTER INFORMATION instance",
...
ospf_router_info_lsa_originate1(void *arg)
{
    ...
    if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA)
    {
        ...
        {
            zlog_debug(OSPF_DBG_LSA_GENERATE, "RI -> This is not the Router Information Area. Stop processing");
            goto out;
        }
    ...
    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
    {
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Originate Opaque-LSA/ROUTER INFORMATION",
...
ospf_router_info_lsa_originate(void *arg)
{
    ...
    {
        zlog_debug(OSPF_DBG_LSA_GENERATE, "ospf_router_info_lsa_originate: ROUTER INFORMATION is disabled now.");
...
ospf_router_info_lsa_refresh(struct ospf_lsa *lsa)
{
    ...
    if (OspfRI.status == disabled)
    {
         ...
         */
        zlog_debug(OSPF_DBG_LSA_GENERATE, "ospf_router_info_lsa_refresh: ROUTER INFORMATION is disabled now.");
    ...
    if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
    {
        zlog_debug(OSPF_DBG_LSA_GENERATE, "LSA[Type%d:%s]: Refresh Opaque-LSA/ROUTER INFORMATION",
                   new->data->type, inet_ntoa(new->data->id));
        ospf_lsa_header_dump(new->data);
    }
...
ospf_router_info_lsa_schedule(opcode_t opcode)
{
    ...
    zlog_debug(OSPF_DBG_LSA_GENERATE, "RI-> LSA schedule %s%s%s",
...
show_vty_router_cap(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_OTHER, "    Router Capabilities: 0x%x", ntohl(top->value));
...
show_vty_pce_subtlv_address(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (ntohs(top->address.type) == PCE_ADDRESS_TYPE_IPV4)
    {
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE Address: %s", inet_ntoa(top->address.value));
    }
    else
    {
        ...
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE Address: 0x%x",
...
show_vty_pce_subtlv_path_scope(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_OTHER, "    PCE Path Scope: 0x%x", ntohl(top->value));
...
show_vty_pce_subtlv_domain(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (ntohs(top->type) == PCE_DOMAIN_TYPE_AREA)
    {
        ...
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE domain Area: %s", inet_ntoa(tmp));
    }
    else
    {
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE domain AS: %d", ntohl(top->value));
    }
...
show_vty_pce_subtlv_neighbor(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (ntohs(top->type) == PCE_DOMAIN_TYPE_AREA)
    {
        ...
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE neighbor Area: %s", inet_ntoa(tmp));
    }
    else
    {
        if (vty != NULL)
            ...
            zlog_debug(OSPF_DBG_OTHER, "    PCE neighbor AS: %d", ntohl(top->value));
    }
...
show_vty_pce_subtlv_cap_flag(struct vty *vty, struct ri_tlv_header *tlvh)
{
    ...
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_OTHER, "    PCE Capabilities Flag: 0x%x", ntohl(top->value));
...
show_vty_unknown_tlv(struct vty *vty, struct ri_tlv_header *tlvh)
{
    if (vty != NULL)
        ...
        zlog_debug(OSPF_DBG_OTHER, "    Unknown TLV: [type(0x%x), length(0x%x)]",
      ...
      "OSPF area ID in IP format")
{
    ...
    if (argc == 1)
    {
        ...
        {
            vty_error_out(vty, "Please specify Router Info Area by A.B.C.D%s",
                    VTY_NEWLINE);
            return CMD_WARNING;
        }
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "RI-> Router Information (%s flooding): OFF -> ON",
                   OspfRI.scope == OSPF_OPAQUE_AREA_LSA ? "Area" : "AS");
    ...
    if (OspfRI.flags & RIFLG_LSA_ENGAGED)
    {
        zlog_debug(OSPF_DBG_EVENT, "RI-> Initial origination following configuration");
      ...
      "Disable the Router Information functionality\n")
{
    ...
    if (IS_DEBUG_OSPF_EVENT)
        zlog_debug(OSPF_DBG_EVENT, "RI-> Router Information: ON -> OFF");
      ...
      "PCE address in IPv4 address format\n")
{
    ...
    {
        vty_error_out(vty, "Please specify PCE Address by A.B.C.D%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "32-bit Hexadecimal value\n")
{
    ...
    {
        vty_error_out(vty, "pce_path_scope: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "AS number in decimal <0-65535>\n")
{
    ...
    {
        vty_error_out(vty, "pce_domain: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "AS number in decimal <0-65535>\n")
{
    ...
    if (sscanf(argv[0], "%d", &as) != 1)
    {
        vty_error_out(vty, "no_pce_domain: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "AS number in decimal <0-65535>\n")
{
    ...
    if (sscanf(argv[0], "%d", &as) != 1)
    {
        vty_error_out(vty, "pce_neighbor: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "AS number in decimal <0-65535>\n")
{
    ...
    if (sscanf(argv[0], "%d", &as) != 1)
    {
        vty_error_out(vty, "no_pce_neighbor: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "32-bit Hexadecimal value\n")
{
    ...
    {
        vty_error_out(vty, "pce_cap_flag: fscanf: %s%s", safe_strerror(errno),
                VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      "Router Information\n")
{
    ...
    if (OspfRI.status == enabled)
    {
        vty_error_out(vty, "--- Router Information parameters ---%s", VTY_NEWLINE);
...
{
    install_element(CONFIG_NODE, &show_ip_ospf_router_info_cmd, CMD_LOCAL);
    install_element(CONFIG_NODE, &show_ip_ospf_router_info_pce_cmd, CMD_LOCAL);

    install_element(OSPF_NODE, &router_info_area_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &router_info_as_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &no_router_info_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &pce_address_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &pce_path_scope_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &pce_domain_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &no_pce_domain_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &pce_neighbor_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &no_pce_neighbor_cmd, CMD_SYNC);
    install_element(OSPF_NODE, &pce_cap_flag_cmd, CMD_SYNC);
