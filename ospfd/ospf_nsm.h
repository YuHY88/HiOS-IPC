...
#define OSPF_NSM_TIMER_ON(T,F,V)                                              \
      do {                                                                    \
        if (!(T))                                                             \
          (T) = thread_add_timer (master_ospf, (F), nbr, (V));                     \
      } while (0)
...
/* Macro for OSPF NSM timer turn on. (Change by zzl 20180910 for IPC optimization)*/
#define OSPF_NSM_TIMER_ON(T,F,V)                                   		\
	do {																\
		if (!(T))														\
			(T) = high_pre_timer_add ((char *)"ospf_nsm_timer", LIB_TIMER_TYPE_NOLOOP, (F), nbr, (V)*1000);\
	} while (0)


#define OSPF_NSM_TIMER_ON_LOOP(T,F,V)                                   		\
	do {																\
		if (!(T))														\
			(T) = high_pre_timer_add ((char *)"ospf_nsm_timer", LIB_TIMER_TYPE_LOOP, (F), nbr, (V)*1000);\
	} while (0)
...
#define OSPF_NSM_EVENT_SCHEDULE(N,E)                                          \
      thread_add_event (master_ospf, ospf_nsm_event, (N), (E))

/* Macro for OSPF NSM execute event. */
#define OSPF_NSM_EVENT_EXECUTE(N,E)                                           \
      thread_execute (master_ospf, ospf_nsm_event, (N), (E))

/* Prototypes. */
extern void ospf_inactive_timer_restart(struct ospf_neighbor *);
