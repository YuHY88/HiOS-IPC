...
struct rip_offset_list *rip_offset_list_new(void);
...
struct rip_offset_list *rip_offset_list_new(void)
{
	struct rip_offset_list *rip_offset_list_temp = NULL;
	
	do
	{
    	rip_offset_list_temp = XCALLOC(MTYPE_RIP_OFFSET_LIST, sizeof(struct rip_offset_list));
		if (rip_offset_list_temp == NULL)
		{
			zlog_err("%s():%d: XCALLOC failed!", __FUNCTION__, __LINE__);
			sleep(1);
		}
	}while(rip_offset_list_temp == NULL);

	memset(rip_offset_list_temp, 0, sizeof(struct rip_offset_list));

	return rip_offset_list_temp;
}
...
rip_offset_init()
{
    ...
    install_element(RIP_NODE, &rip_offset_list_cmd, CMD_SYNC);
    install_element(RIP_NODE, &rip_offset_list_ifname_cmd, CMD_SYNC);
    install_element(RIP_NODE, &no_rip_offset_list_cmd, CMD_SYNC);
    install_element(RIP_NODE, &no_rip_offset_list_ifname_cmd, CMD_SYNC);
}
