...
struct ripng_hop_node
{
    struct in6_addr nexhop;
    struct listnode *node;

};

extern struct stats ripng_stats;
 ...
 */
void
ripng_rte_send(struct list *ripng_rte_list, struct connected *ifc,
               struct sockaddr_in6 *to, struct ripng_instance *ripng)
{
    ...
    rtemax = (min(mtu, RIPNG_MAX_PACKET_SIZE) -
              ...
              sizeof(struct rte_ipv6)) / sizeof(struct rte_ipv6);
    ...
    for (ALL_LIST_ELEMENTS(ripng_rte_list, node, nnode, data))
    {
        ...
        if (!IPV6_ADDR_SAME(&last_nexthop, NEXTHOP_OUT_PTR(data)))
        {
            ...
            if (num == (rtemax - 1))
            {
                ret = ripng_send_packet((u_char *) STREAM_DATA(s), stream_get_endp(s),
                                        to, ifc);

                if (ret == 0 && IS_RIPNG_DEBUG_PACKET)
                {
                    ripng_stats.tx_routes += num;
