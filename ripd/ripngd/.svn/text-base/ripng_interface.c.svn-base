...
ripng_multicast_join(struct interface *ifp)
{
    ...
    if (if_is_up(ifp) && if_is_multicast(ifp))
    {
        ...
        if (ret < 0 && save_errno == EADDRINUSE)
        {
             ...
             */
            zlog_debug(RIPNG_DBG_EVENT, "ripng join on %s EADDRINUSE (ignoring)\n", ifp->name);
            return 0;       /* not an error */
        }

        if (ret < 0)
            zlog_debug(RIPNG_DBG_EVENT, "can't setsockopt IPV6_JOIN_GROUP: %s",
                      safe_strerror(save_errno));

        //if (IS_RIPNG_DEBUG_EVENT)
            zlog_debug(RIPNG_DBG_EVENT, "RIPng %s join to all-rip-routers multicast group", ifp->name);
...
ripng_multicast_leave(struct interface *ifp)
{
    ...
    if (if_is_up(ifp) && if_is_multicast(ifp))
    {
        ...
        if (ret < 0)
            zlog_debug(RIPNG_DBG_EVENT, "can't setsockopt IPV6_LEAVE_GROUP: %s\n", safe_strerror(errno));

        //if (IS_RIPNG_DEBUG_EVENT)
            zlog_debug(RIPNG_DBG_EVENT, "RIPng %s leave from all-rip-routers multicast group",
                       ifp->name);
...
ripng_if_down(struct interface *ifp)
{
    ...
    {
        //if (IS_RIPNG_DEBUG_EVENT)
            zlog_debug(RIPNG_DBG_EVENT, "turn off %s", ifp->name);
                   ...
                   vrf_id_t vrf_id)
{
    ...
    //if (IS_RIPNG_DEBUG_ZEBRA)
        zlog_debug(RIPNG_DBG_ZEBRA, "interface up %s index %d flags %llx metric %d mtu %d",
                     ...
                     zebra_size_t length, vrf_id_t vrf_id)
{
    ...
    //if (IS_RIPNG_DEBUG_ZEBRA)
        zlog_debug(RIPNG_DBG_ZEBRA, "interface down %s index %d flags %#llx metric %d mtu %d",
                    ...
                    vrf_id_t vrf_id)
{
    ...
    //if (IS_RIPNG_DEBUG_ZEBRA)
        zlog_debug(RIPNG_DBG_ZEBRA, "RIPng interface add %s index %d flags %#llx metric %d mtu %d",
                   ifp->name, ifp->ifindex, (unsigned long long) ifp->flags,
                   ifp->metric, ifp->mtu6);
                       ...
                       zebra_size_t length, vrf_id_t vrf_id)
{
    ...
    zlog_debug(RIPNG_DBG_ZEBRA, "interface delete %s index %d flags %#llx metric %d mtu %d",
              ifp->name, ifp->ifindex, (unsigned long long) ifp->flags,
              ifp->metric, ifp->mtu6);
...
ripng_interface_clean(void)
{
    ...
    for (ALL_LIST_ELEMENTS(iflist, node, nnode, ifp))
    {
        ...
        if (ri->t_wakeup)
        {
            ...
            //ri->t_wakeup = NULL;
			high_pre_timer_delete (ri->t_wakeup);
			ri->t_wakeup = 0;
        }
    }
}
                            ...
                            zebra_size_t length, vrf_id_t vrf_id)
{
    ...
    if (p->family == AF_INET6)
    {
        ...
        //if (IS_RIPNG_DEBUG_ZEBRA)
            zlog_debug(RIPNG_DBG_ZEBRA, "RIPng connected address %s/%d add",
                               ...
                               zebra_size_t length, vrf_id_t vrf_id)
{
    ...
    if (ifc)
    {
        ...
        {
            //if (IS_RIPNG_DEBUG_ZEBRA)
                zlog_debug(RIPNG_DBG_ZEBRA, "RIPng connected address %s/%d delete",
...
ripng_enable_apply(struct interface *ifp)
{
            ...
            (ri->enable_network || ri->enable_interface))
    {
        ri->enable_network = 0;
        ri->enable_interface = 0;
        zlog_debug(RIPNG_DBG_EVENT, "Interface %s does not have any link-local address",
                  ifp->name);
    }
    ...
    {
        //if (IS_RIPNG_DEBUG_EVENT)
            zlog_debug(RIPNG_DBG_EVENT,"RIPng turn on %s", ifp->name);
		...
		if (!ri->t_wakeup)
		{
			ri->t_wakeup = high_pre_timer_add ((char *)"ripng_wakeup_timer", \
					LIB_TIMER_TYPE_NOLOOP, ripng_interface_wakeup, ifp, 1*1000);
		}
...
ripng_if_init()
{
    ...
    install_element(RIPNG_NODE, &ripng_network_cmd, CMD_SYNC);
    install_element(RIPNG_NODE, &no_ripng_network_cmd, CMD_SYNC);
    install_element(RIPNG_NODE, &ripng_passive_interface_cmd, CMD_SYNC);
    install_element(RIPNG_NODE, &no_ripng_passive_interface_cmd, CMD_SYNC);

    install_element(INTERFACE_NODE, &ipv6_ripng_split_horizon_cmd, CMD_SYNC);
    install_element(INTERFACE_NODE, &ipv6_ripng_split_horizon_poisoned_reverse_cmd, CMD_SYNC);
    install_element(INTERFACE_NODE, &no_ipv6_ripng_split_horizon_cmd, CMD_SYNC);
    install_element(INTERFACE_NODE, &no_ipv6_ripng_split_horizon_poisoned_reverse_cmd, CMD_SYNC);
}
