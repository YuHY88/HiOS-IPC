...
bgp_info_mpath_cmp (void *val1, void *val2)
{
    ...
    if (!compare)
    {
        compare = sockunion_cmp (&bi1->peer->su, &bi2->peer->su);
    }
    ...
    while (mp_node || cur_mpath)
    {
        ...
        if (mp_node && (listgetdata (mp_node) == cur_mpath))
        {
            ...
            else
            {
                mpath_changed = 1;
                if (debug)
                    zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s remove mpath nexthop %s peer %s", pfx_buf,
                                inet_ntop (AF_INET, &cur_mpath->attr->nexthop,
                                           nh_buf[0], sizeof (nh_buf[0])),
                                sockunion2str (&cur_mpath->peer->su,
        ...
        else
        {
                    ...
                    bgp_info_nexthop_cmp (prev_mpath, new_mpath))
            {
                ...
                mpath_count++;
                if (debug)
                    zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s add mpath nexthop %s peer %s", pfx_buf,
                                inet_ntop (AF_INET, &new_mpath->attr->nexthop,
                                           nh_buf[0], sizeof (nh_buf[0])),
                                sockunion2str (&new_mpath->peer->su,
                                               nh_buf[1], sizeof (nh_buf[1])));
            }
            mp_node = mp_next_node;
        }
...
bgp_mp_dmed_deselect (struct bgp_info *dmed_best)
{
	...
	if(NULL != bgp_info_mpath_first (dmed_best))
	{
		return ;
	}
}
