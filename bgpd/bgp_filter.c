...
static int
as_list_dup_check (struct as_list *aslist, struct as_filter *new_asfilter)
{
    struct as_filter *asfilter;
	
    for (asfilter = aslist->head; asfilter; asfilter = asfilter->next)
    {
        if (asfilter->type == new_asfilter->type
        && strcmp (asfilter->reg_str, new_asfilter->reg_str) == 0)
        {
            return 1;
        }
    }
...
{
    install_element (CONFIG_NODE, &ip_as_path_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_as_path_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_as_path_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_as_path_access_list_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_as_path_access_list_all_cmd, CMD_SYNC);
}
void
bgp_filter_reset (void)
{
	...
	if(as_list_master.num.head || as_list_master.num.tail \
		|| as_list_master.str.head || as_list_master.str.tail)
	{
		return ;
	}
