                 ...
                 const char *direct)
{
    ...
    if (bgp_flag_check (peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%%NOTIFICATION: %s neighbor %s %d/%d (%s%s) %d bytes %s",
       ...
       "BGP Finite State Machine\n")
{
    ...
    vty_out (vty, "Success to debug bgp fsm!%s", VTY_NEWLINE);
    return CMD_SUCCESS;
}
       ...
       "BGP events\n")
{
    ...
    vty_out (vty, "Success to debug bgp events!%s", VTY_NEWLINE);
    return CMD_SUCCESS;
}
       ...
       "BGP packet\n")
{
    ...
    else
    {
        ...
        TERM_DEBUG_ON (as4, AS4_SEGMENT);
        TERM_DEBUG_ON (filter, FILTER);
        vty_out (vty, "BGP packet debugging is on%s", VTY_NEWLINE);
    }
    vty_out (vty, "Success to debug bgp packet!%s", VTY_NEWLINE);
    return CMD_SUCCESS;
}

DEFUN (no_debug_bgp_packet,
       ...
       "BGP packet\n")
{
    ...
    else
    {
        ...
        TERM_DEBUG_OFF (as4, AS4_SEGMENT);
        TERM_DEBUG_OFF (filter, FILTER);
        vty_out (vty, "BGP packet debugging is off%s", VTY_NEWLINE);
    }
    return CMD_SUCCESS;
}
       ...
       "Debugging functions\n")
{
    ...
    if (BGP_DEBUG (normal, NORMAL) && BGP_DEBUG (keepalive, KEEPALIVE) && BGP_DEBUG (update, UPDATE_IN) && BGP_DEBUG (update, UPDATE_OUT))
    {
        vty_out (vty, "  BGP packet debugging is on%s", VTY_NEWLINE);
    }
       ...
       "Bgp module timer\n")
{
    ...
    else vty_error_out(vty, "Show bgp module timer infor error. %s", VTY_NEWLINE);

    return CMD_SUCCESS;
}

DEFUN (bgp_conf_debug_fun,
	   bgp_conf_debug_cmd,
      ...
      )
{
    ...
    if(strcmp(argv[1],"other") == 0)
        type = BGP_DEBUG_TYPE_OTHER;
    else if(strcmp(argv[1],"as4") == 0)
        type = BGP_DEBUG_TYPE_AS4;
    else if(strcmp(argv[1],"fsm") == 0)
        type = BGP_DEBUG_TYPE_FSM;
    else if(strcmp(argv[1],"events") == 0)
        type = BGP_DEBUG_TYPE_EVENTS;
    else if(strcmp(argv[1],"packet") == 0)
        type = BGP_DEBUG_TYPE_PACKET;
	else if(strcmp(argv[1],"keepalive") == 0)
        type = BGP_DEBUG_TYPE_KEEPALIVE;
	else if(strcmp(argv[1],"update") == 0)
        type = BGP_DEBUG_TYPE_UPDATE;
	else if(strcmp(argv[1],"normal") == 0)
        type = BGP_DEBUG_TYPE_NORMAL;
	else if(strcmp(argv[1],"zebra") == 0)
        type = BGP_DEBUG_TYPE_ZEBRA;
    else type = BGP_DEBUG_TYPE_ALL;
...
bgp_debug_init (void)
{
    ...
    install_element (BGP_NODE, &show_debugging_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_debugging_bgp_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_debugging_bgp_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_debugging_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_as4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_as4_segment_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_fsm_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_events_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_keepalive_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_update_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_update_direct_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_normal_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_zebra_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_as4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_as4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_as4_segment_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_as4_segment_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_fsm_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_fsm_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_events_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_events_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_keepalive_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_keepalive_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_update_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_update_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_normal_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_normal_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_zebra_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_zebra_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &undebug_bgp_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &debug_bgp_packet_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &no_debug_bgp_packet_cmd, CMD_LOCAL);

	install_element (CONFIG_NODE, &bgp_conf_debug_cmd, CMD_LOCAL);
	install_element (CONFIG_NODE, &bgp_show_timerinfo_cmd, CMD_LOCAL);
}
