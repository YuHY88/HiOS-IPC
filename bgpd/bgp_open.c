...
bgp_afi_safi_valid_indices (afi_t afi, safi_t *safi)
{
    ...
    zlog_debug (BGP_DEBUG_TYPE_OTHER,"unknown afi/safi (%u/%u)", afi, *safi);
    return 0;
}
...
bgp_capability_mp (struct peer *peer, struct capability_header *hdr)
{
	...
	//{
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s OPEN has MP_EXT CAP for afi/safi: %u/%u",
                                ...
                                u_char type, u_char mode)
{
    //if (BGP_DEBUG (normal, NORMAL))
	//{
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Addr-family %d/%d has ORF type/mode %d/%d not supported",
...
bgp_capability_orf_entry (struct peer *peer, struct capability_header *hdr)
{
    ...
    //if (BGP_DEBUG (normal, NORMAL))
	//{
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s ORF Cap entry for afi/safi: %u/%u",
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Addr-family %d/%d not supported."
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s ORF Capability entry length error,"
                   " Cap length %u, num %u",
    ...
    for (i = 0 ; i < entry.num ; i++)
    {
        ...
        //if (BGP_DEBUG (normal, NORMAL))
    	//{
            zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s OPEN has %s ORF capability"
...
bgp_capability_restart (struct peer *peer, struct capability_header *caphdr)
{
    ...
    //if (BGP_DEBUG (normal, NORMAL))
    //{
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s OPEN has Graceful Restart capability", peer->host);
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Peer has%srestarted. Restart Time : %d",
                    peer->host,
    ...
    while (stream_get_getp (s) + 4 <= end)
    {
        ...
        if (!bgp_afi_safi_valid_indices (afi, &safi))
        {
            //if (BGP_DEBUG (normal, NORMAL))
        	//{
                zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Addr-family %d/%d(afi/safi) not supported."
        ...
        else if (!peer->afc[afi][safi])
        {
            //if (BGP_DEBUG (normal, NORMAL))
        	//{
                zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Addr-family %d/%d(afi/safi) not enabled."
                            " Ignore the Graceful Restart capability",
                            peer->host, afi, safi);
        	//}
        }
        else
        {
            //if (BGP_DEBUG (normal, NORMAL))
        	//{
                zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Address family %s is%spreserved", peer->host,
...
bgp_capability_as4 (struct peer *peer, struct capability_header *hdr)
{
	...
	//{
        zlog_debug (BGP_DEBUG_TYPE_AS4,"%s [AS4] about to set cap PEER_CAP_AS4_RCV, got as4 %u",
                    peer->host, as4);
	//}
                      ...
                      u_char **error)
{
    ...
    while (stream_get_getp (s) < end)
    {
        ...
        {
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Capability length error (< header)", peer->host);
            bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSPECIFIC);
        ...
        {
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Capability length error (< length)", peer->host);
            bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSPECIFIC);
        ...
        //if (BGP_DEBUG (normal, NORMAL))
    	//{
            zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s OPEN has %s capability (%u), length %u",
                        peer->host,
        ...
        switch (caphdr.code)
        {
	        ...
	        case CAPABILITY_CODE_DYNAMIC:
        	{
	            ...
	            {
	                zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s %s Capability length error: got %u,"
	                           " expected at least %u",
	                           peer->host,
	                           LOOKUP (capcode_str, caphdr.code),
	                           caphdr.length,
        ...
        switch (caphdr.code)
        {
	        ...
	        default:
        	{
	            if (caphdr.code > 128)
	            {
	                ...
	                zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Vendor specific capability %d",
        ...
        if (stream_get_getp(s) != (start + caphdr.length))
        {
            if (stream_get_getp(s) > (start + caphdr.length))
        	{
                zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Cap-parser for %s read past cap-length, %u!",
...
peek_for_as4_capability (struct peer *peer, u_char length)
{
    ...
    //if (BGP_DEBUG (as4, AS4))
	//{
        zlog_debug (BGP_DEBUG_TYPE_AS4,"%s [AS4] rcv OPEN w/ OPTION parameter len: %u,"
                   " peeking for as4",
                   peer->host, length);
	//}
        ...
        if (opt_type == BGP_OPEN_OPT_CAP)
        {
            ...
            while (stream_get_getp (s) < capd_end)
            {
                ...
                if (hdr.code == CAPABILITY_CODE_AS4)
                {
                    //if (BGP_DEBUG (as4, AS4))
                    //{
                        zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] found AS4 capability, about to parse");
                    //}
...
bgp_open_option_parse (struct peer *peer, u_char length, int *mp_capability)
{
    ...
    //if (BGP_DEBUG (normal, NORMAL))
	//{
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s rcv OPEN w/ OPTION parameter len: %u",
                    peer->host, length);
	//}
    ...
    while (stream_get_getp(s) < end)
    {
        ...
        {
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s Option length error", peer->host);
            bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                             BGP_NOTIFY_OPEN_UNSPECIFIC);
            return -1;
        }
        ...
        //if (BGP_DEBUG (normal, NORMAL))
    	//{
            zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s rcvd OPEN w/ optional parameter type %u (%s) len %u",
                        peer->host, opt_type,
