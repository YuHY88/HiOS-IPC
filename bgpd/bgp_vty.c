...
peer_and_group_lookup_vty (struct vty *vty, const char *peer_str)
{
    ...
    vty_error_out (vty, "The neighbor session does not exist.%s", VTY_NEWLINE);
...
bgp_vty_return (struct vty *vty, int ret)
{
    ...
    switch (ret)
    {
		...
		case BGP_ERR_REMOVE_PRIVATE_AS:
		    ...
		    break;
		case BGP_ERR_ALLOWED_ONLY_FOR_EBGP:
		    str = "The type of the neighbor is not EBGP.";
		    break;
		case BGP_ERR_CANNOT_HAVE_LOCAL_AS_SAME_AS:
		    str = "The local AS number cannot be the same as the BGP AS number.";
		    break;
		case BGP_ERR_TCPSIG_FAILED:
		    str = "while applying TCP-Sig to session(s)";
		    break;
		case BGP_ERR_NO_EBGP_MULTIHOP_WITH_TTLHACK:
		    str = "ebgp-multihop and ttl-security cannot be configured together";
		    break;
		case BGP_ERR_PEER_AS_MATCH:
		    str = "The neighbor already exists";
		    break;
		case BGP_ERR_CANNOT_HAVE_LOCAL_AS_SAME_AS_REMOTE_AS:
		    str = "The local AS number cannot be the same as the remote AS number.";
		    break;
		case BGP_ERR_ALLOWED_ONLY_FOR_IBGP:
		    str = "The type of the neighbor is not IBGP.";
		    break;
		default:
			break;
    }
       ...
       AS_STR)
{
    ...
    switch (ret)
    {
	    case BGP_ERR_MULTIPLE_INSTANCE_NOT_SET:
	        vty_out (vty, "Please specify 'bgp multiple-instance' first%s", VTY_NEWLINE);
	        return CMD_WARNING;
	    case BGP_ERR_AS_MISMATCH:
	        vty_error_out (vty, "BGP is already running; AS is %u.%s", as, VTY_NEWLINE);
	        return CMD_WARNING;
    ...
    {   
        pmesg = ipc_sync_send_n2(NULL, 0, 1, MODULE_ID_ROUTE,
                                            MODULE_ID_BGP, IPC_TYPE_BGP, BGP_GET_ROUTERID, IPC_OPCODE_GET, 0, 2000);
        if(NULL == pmesg)
        {
           	ret = bgp_add_routerId_from_connect_table(bgp);
    		if(ret < 0)
    		{
    			vty_error_out (vty, "The router-id is not configured.%s", VTY_NEWLINE);	
    		}
	    }
        else{
            router_id = ( struct in_addr *)pmesg->msg_data;
		    bgp_router_id_set (bgp, router_id);
		}
		
		mem_share_free(pmesg, MODULE_ID_BGP);
    }
#endif
    ...
    vty->node = BGP_NODE;
    vty->index = bgp;
	bgp_sendmsg_to_tcp(BGP_ENABLE_MSG,NULL);
    return CMD_SUCCESS;
}

ALIAS (router_bgp,
       router_bgp_view_cmd,
       "bgp as " CMD_AS_RANGE " view WORD",
       BGP_STR
       "Autonomous System\n"
       AS_STR
       "BGP view\n"
       ...
       AS_STR)
{
    ...
    if (! bgp)
    {
        ...
        if(bgp)
        {
            vty_error_out (vty, "BGP AS number mismatch. Local AS is %u.%s", bgp->as,VTY_NEWLINE);
        }
        else
        {
            vty_error_out (vty, "The BGP protocol has not been started.%s", VTY_NEWLINE);
        }
        return CMD_WARNING;
    }
    bgp_delete (bgp);
	bgp_sendmsg_to_tcp(BGP_DISABLE_MSG,NULL);
    return CMD_SUCCESS;
}

ALIAS (no_router_bgp,
       no_router_bgp_view_cmd,
       "no bgp as " CMD_AS_RANGE " view WORD",
       NO_STR
       BGP_STR
       "Autonomous System\n"
       AS_STR
       ...
       "Manually configured router identifier\n")
{
    ...
    if (! ret)
    {
        vty_error_out (vty, "Malformed bgp router identifier%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if(id.s_addr == 0 || id.s_addr == 0xFFFFFFFF)
    {
       ...
       "Route-Reflector Cluster-id in IP address format\n")
{
    ...
    if (! ret)
    {
        vty_error_out (vty, "Malformed bgp cluster identifier%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...

{
    ...
    VTY_GET_INTEGER_RANGE ("keepalive", keepalive, argv[0], 0, 3600);
    VTY_GET_INTEGER_RANGE ("holdtime", holdtime, argv[1], 0, 65535);
	
    if(keepalive == 0 && holdtime)
    {
        vty_error_out (vty, "The KeepaliveTime cannot be zero when HoldTime is non-zero.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    {
        vty_error_out (vty, "The HoldTime must be either 0 or greater than or equal to 3.%s",
                 VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if(keepalive*3 > holdtime)
    {
        vty_error_out (vty, "The HoldTime should be at least three times of the KeepaliveTime.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if ((keepalive == bgp->default_keepalive) && (holdtime == bgp->default_holdtime))
    {
        return CMD_SUCCESS;
    }
       ...
       "Value of Hold timer(seconds)\n")
{
    ...
    if ((keepalive != bgp->default_keepalive)|| (holdtime != bgp->default_holdtime))
    {
        vty_out (vty, "%% The configuration to be deleted does not exist%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...
{
    struct bgp *bgp;
    u_int32_t conenct_retry = 0;
	
    bgp = vty->index;
	
    VTY_GET_INTEGER_RANGE ("conenctretry", conenct_retry, argv[0], 1, 3600);
	
    bgp_conenct_timers_set (bgp, conenct_retry);
	
    return CMD_SUCCESS;
}

DEFUN (no_connect_timers,
...
{	
    ...
    if(bgp->default_local_pref == local_pref)
    {
        return CMD_SUCCESS;
    }
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi ++)
	{
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi ++)
		{
			bgp_local_pref_update(bgp, afi, safi);
		}
	}
...
{	
    ...
    if(BGP_DEFAULT_LOCAL_PREF == bgp->default_local_pref)
    {
        return CMD_SUCCESS;
    }
...
{ 
    ...
    VTY_GET_INTEGER ("med value", med, argv[0]);
                    ...
                    const char *as_str, afi_t afi, safi_t safi)
{
    ...
    switch (ret)
    {
	    ...
	    case BGP_ERR_PEER_GROUP_PEER_TYPE_DIFFERENT:
	        ...
	        return CMD_WARNING;
	    case BGP_ERR_PEER_AS_MATCH:
	        vty_error_out (vty, "The neighbor already exists in AS %u.%s", as, VTY_NEWLINE);
	        return CMD_WARNING;
		default:
			break;
    }
       ...
       "AS number used as local AS\n")
{
    ...
    VTY_GET_INTEGER_RANGE ("LOCAL_AS", as, argv[1], 1, BGP_AS4_MAX);
...
DEFUN (neighbor_source,
       neighbor_source_cmd,
       NEIGHBOR_CMD2 "source A.B.C.D",
       NEIGHBOR_STR
       NEIGHBOR_ADDR_STR2
       "source\n"
       "source A.B.C.D\n")
{
    ...
    str2sockunion (argv[1], &peer->su_local);
	
    bgp_nexthop_set (&peer->su_local, &peer->su, &peer->nexthop, peer);
	
    return CMD_SUCCESS;
}

DEFUN (no_neighbor_source,
       no_neighbor_source_cmd,
       NO_NEIGHBOR_CMD2 "source A.B.C.D",
       NO_STR
       NEIGHBOR_STR
       NEIGHBOR_ADDR_STR2
       "source\n"
       "source A.B.C.D\n")
{
    ...
    memset(&peer->su_local,0x0,sizeof(union sockunion));
       ...
       "peer-group name\n")
{
    ...
    if (ret < 0)
    {
        vty_error_out (vty, "Malformed address: %s%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    {
        vty_warning_out (vty, "Local interface is already configured with this address.%s",
                 VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "BGP attribute is propagated unchanged to this neighbor\n")
{
    return peer_af_flag_set_vty (vty, argv[0], bgp_node_afi (vty), bgp_node_safi (vty),
                                 (PEER_FLAG_AS_PATH_UNCHANGED | PEER_FLAG_NEXTHOP_UNCHANGED | PEER_FLAG_MED_UNCHANGED));
}
...
DEFUN (no_neighbor_attr_unchanged,
       ...
       "BGP attribute is propagated unchanged to this neighbor\n")
{
    return peer_af_flag_unset_vty (vty, argv[0], bgp_node_afi (vty),bgp_node_safi (vty),
                                   (PEER_FLAG_AS_PATH_UNCHANGED | PEER_FLAG_NEXTHOP_UNCHANGED | PEER_FLAG_MED_UNCHANGED));
}
                             ...
                             const char *restart_str)
{
    ...
    VTY_GET_INTEGER_RANGE ("maximum number", max, num_str, 1, 200000);
       ...
       "Specify an IPv4 peer address\n")
{
    ...
    bgp = vty->index;
    memset(&peer_ip,0,sizeof(struct in_addr));
    ...
    ret = str2sockunion (argv[0], &su);
    if (ret < 0 || su.sa.sa_family == AF_INET6) 
    {
    ...
    if (peer_address_self_check (&su))
    {
        vty_warning_out (vty, "Local interface is already configured with this address.%s", VTY_NEWLINE);
    }
	
    ret = inet_aton (argv[0], &peer_ip);
    if (! ret)
    {
        vty_error_out (vty, "The neighbor address is invalid.%s",  VTY_NEWLINE);
        return CMD_WARNING;
    ...
    ret = inet_valid_ipv4(ntohl(peer_ip.s_addr));
    if (! ret)
    {
        vty_error_out(vty, "The neighbor address is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    ...
    if(bgp->peer_str)
    {
        XFREE (MTYPE_BGP_PEER_STR, bgp->peer_str);
        bgp->peer_str = NULL;
    }
    ...
    bgp->peer_str = XSTRDUP (MTYPE_BGP_PEER_STR, buf);
	
    return CMD_SUCCESS;
}

DEFUN (no_bgp_neighbor,
       no_bgp_neighbor_cmd,
       ...
       AS_STR)
{
    ...
    return peer_remote_as_vty (vty, bgp->peer_str, argv[0],  bgp_node_afi (vty), SAFI_UNICAST);
}

DEFUN (neighbor_enable,
       neighbor_enable_cmd,
       ...
       "Disable neighbor\n")
{
    ...
    bgp = vty->index;
    peer = peer_and_group_lookup_vty (vty, bgp->peer_str);
    if (! peer)
    {
        return CMD_WARNING;
    }
	
    if((peer->su_local.sa.sa_family == AF_INET && peer->su_local.sin.sin_addr.s_addr == 0)
       	|| (peer->su_local.sa.sa_family == AF_INET6 && (ipv6_is_zero((struct ipv6_addr *)&peer->su_local.sin6.sin6_addr))))
    {
        vty_error_out (vty, "Configure source address first. %s", VTY_NEWLINE);
        return CMD_WARNING;
    }

    if(peer->afc[AFI_IP][SAFI_MPLS_VPN] == 0 && peer->ifindex_tnl)
    {
        vty_error_out (vty, "Configure ipv4-family vpnv4. %s", VTY_NEWLINE);
        return CMD_WARNING;
    }

    if(!strncmp(argv[0], "e", 1) && !CHECK_FLAG(peer->flags,PEER_FLAG_ENABLE))
    {
        SET_FLAG(peer->flags,PEER_FLAG_ENABLE);
        bgp_timer_set (peer);
        peer_flag_unset_vty (vty, bgp->peer_str, PEER_FLAG_PASSIVE);
    }
    else if(strncmp(argv[0], "d", 1) == 0 && CHECK_FLAG(peer->flags,PEER_FLAG_ENABLE))
    {
        UNSET_FLAG(peer->flags,PEER_FLAG_ENABLE);
        peer_flag_set_vty (vty, bgp->peer_str, PEER_FLAG_PASSIVE);
    }
	
    return CMD_SUCCESS;
}

DEFUN (description_name,
       ...
       "Up to 80 characters describing this neighbor\n")
{
    ...
    bgp = vty->index;
    if(strlen(argv[0]) > 80 || strlen(argv[0]) < 1)
    {
        vty_error_out(vty,"Please input no more than 80 characters%s",VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "The password\n")
{
    ...
    ret = peer_password_set (peer, argv[0]);
	
    return bgp_vty_return (vty, ret);
}

DEFUN (no_bgp_auth_md5_password,
       ...
       "source A.B.C.D\n")
{
    ...
    ret = str2sockunion (argv[0], &sulocal);
    if (ret < 0 || sulocal.sa.sa_family == AF_INET6)
    {
		vty_error_out (vty, "The %s is invalid.%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if (! inet_valid_ipv4(ntohl(sulocal.sin.sin_addr.s_addr)))
    {
        vty_error_out(vty, "The %s is invalid.%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if(sockunion_same (&peer->su, &sulocal))
    {
        vty_error_out(vty, "The source address and the neighbor address are the same.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if(peer->su_local.sin.sin_addr.s_addr && sockunion_same (&peer->su_local, &sulocal))
    {
		vty_warning_out(vty, "The %s already exists.%s", argv[0], VTY_NEWLINE);
        return CMD_SUCCESS;
    }
    ...
    memset(&peer->su_local,0,sizeof(union sockunion));
    memcpy(&peer->su_local,&sulocal,sizeof(union sockunion));
    bgp_nexthop_set (&peer->su_local, &peer->su, &peer->nexthop, peer);
...
DEFUN (no_source_address,
       no_source_address_cmd,
       "no source A.B.C.D",
       NO_STR
       "source address\n"
       "source A.B.C.D\n")
{
    ...
    if(sockunion_same (&peer->su_local, &su))
    {
        memset(&peer->su_local,0x0,sizeof(union sockunion));
    }
    else
    {
		vty_error_out (vty, "The %s does not exists.%s", argv[0], VTY_NEWLINE);
    }
	
    return CMD_SUCCESS;
}

DEFUN (ebgp_multihop_ttl,
       ...
       "maximum hop count\n")
{
    ...
    return peer_ebgp_multihop_set_vty (vty, bgp->peer_str, argv[0]);
}

DEFUN (no_ebgp_multihop_ttl,
       ...
       "Allow EBGP neighbors not on directly connected networks\n")
{
    ...
    return peer_ebgp_multihop_unset_vty (vty, bgp->peer_str);
}
       ...
       "AS number used as local AS\n")
{
    ...
    VTY_GET_INTEGER_RANGE ("LOCAL_AS", as, argv[0], 1, BGP_AS4_MAX);
...
DEFUN (route_redistribute_default_route,
       ...
       "Advertise default route to this neighbor\n")
{
    ...
    return peer_default_originate_set_vty (vty, bgp->peer_str, bgp_node_afi (vty),bgp_node_safi (vty), NULL, 1);
}

DEFUN (no_route_redistribute_default_route,
       ...
       "Advertise default route to this neighbor\n")
{
    ...
    return peer_default_originate_set_vty (vty, bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), NULL, 0);
}

static int
peer_redistribute_ibgp_set_vty (struct vty *vty, const char *peer_str,
                                afi_t afi, safi_t safi, int set)
{
    int ret;
    struct peer *peer;
    ...
    ret = peer_redistribute_ibgp_route (peer, afi, safi, set);
	
    return bgp_vty_return (vty, ret);
}

DEFUN (route_redistribute_ibgp_route,
       ...
       "Advertise ibgp route to this neighbor\n")
{
    ...
    return peer_redistribute_ibgp_set_vty (vty, bgp->peer_str, bgp_node_afi (vty),bgp_node_safi (vty), 1);
}

DEFUN (no_route_redistribute_ibgp_route,
       ...
       "Advertise ibgp route to this neighbor\n")
{
    ...
    return peer_redistribute_ibgp_set_vty (vty, bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), 0);
}
       ...
       "maximum no. of prefix limit\n")
{
    ...
    return peer_maximum_prefix_set_vty (vty, bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), argv[0], NULL, 0, NULL);
}

DEFUN (no_route_limit,
       no_route_limit_cmd,
       "no route limit",
       NO_STR
       "route\n"
       "Maximum number of prefix accept from this peer\n")
{
    ...
    return peer_maximum_prefix_unset_vty (vty, bgp->peer_str, bgp_node_afi (vty),bgp_node_safi (vty));
}

DEFUN (route_weight_bgp,
       ...
       "default weight\n")
{
    ...
    return peer_weight_set_vty (vty, bgp->peer_str, argv[0]);
}

DEFUN (no_route_weight_bgp,
       ...
       "Set default weight for routes from this neighbor\n")
{
    ...
    return peer_weight_unset_vty (vty, bgp->peer_str);
}

DEFUN (route_next_hop_change_local,
       ...
       "local\n")
{
    ...
    ret = peer_af_flag_set_vty (vty,bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), flags);
    if ( ret == CMD_SUCCESS && unset )
    {
        ret = peer_af_flag_unset_vty (vty, bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), unset);
    }
	
    return ret;
}

DEFUN (no_route_next_hop_change_local,
       ...
       "local\n")
{
    ...
    return peer_af_flag_unset_vty (vty, bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), PEER_FLAG_NEXTHOP_SELF|PEER_FLAG_NEXTHOP_SELF_ALL);
}

DEFUN (as_path_exclude_private_as,
       ...
       "Remove private AS number from outbound updates\n")
{
    ...
    return peer_af_flag_set_vty (vty,bgp->peer_str, bgp_node_afi (vty), bgp_node_safi (vty), PEER_FLAG_REMOVE_PRIVATE_AS);
}

DEFUN (no_as_path_exclude_private_as,
       ...
       "Remove private AS number from outbound updates\n")
{
    ...
    return peer_af_flag_unset_vty (vty, bgp->peer_str, bgp_node_afi (vty),   bgp_node_safi (vty), PEER_FLAG_REMOVE_PRIVATE_AS);
}

DEFUN (advertise_interval,
       ...
       "Time in seconds\n")
{
    ...
    return peer_advertise_interval_vty (vty, bgp->peer_str, argv[0], 1);
}

DEFUN (no_advertise_interval,
       ...
       "Minimum interval between sending BGP routing updates\n")
{
    ...
    return peer_advertise_interval_vty (vty, bgp->peer_str, NULL, 0);
}

DEFUN (as_path_allow_as_loop,
       ...
       "Number of occurances of AS number\n")
{
    ...
    VTY_GET_INTEGER_RANGE ("AS number", allow_num, argv[0], 1, 10);
...
DEFUN (no_as_path_allow_as_loop,
       ...
       "Specify VPNv4 address family\n")
{
	...
	ret = peer_activate(peer,AFI_IP,SAFI_MPLS_VPN);
		
    return bgp_vty_return (vty, ret);
}

DEFUN (no_ipv4_family_vpnv4,
       ...
       "Specify VPNv4 address family\n")
{
	...
	ret = peer_deactivate(peer,AFI_IP,SAFI_MPLS_VPN);
...
DEFUN (ipv4_family_vpn_instance,
       ...
       "Number of the L3VPN instance\n")
{
	...
	VTY_GET_INTEGER_RANGE ("vrf_id", id, argv[0], BGP_VRF_MIN, BGP_VRF_MAX);

	if(vty->config_read_flag != 1)
	{		
		if(bgp_vrf_redist_info[id] == NULL)
		{
	        vty_error_out(vty, "The VPN-Instance %d does not exist.%s",id, VTY_NEWLINE);
	        return CMD_WARNING;
		}

		if(bgp_vrf_redist_info[id] && bgp_vrf_redist_info[id]->prd.prefixlen != 64)
		{
			vty_error_out(vty, "The route-distinguisher is not configured%s", VTY_NEWLINE);
	        return CMD_WARNING;
		}
...
DEFUN (no_ipv4_family_vpn_instance,
       ...
       "Number of the L3VPN instance\n")
{
	...
	if(peer->vrf_id == 0)
	{
		vty_error_out(vty, "The neighbor is not bound with the VPN instance.%s", VTY_NEWLINE);
        return CMD_WARNING;
	}
	
	if(peer->vrf_id != id)
	{
		vty_error_out(vty, "The neighbor is not bound with the VPN instance %d.%s",id, VTY_NEWLINE);
        return CMD_WARNING;
	}
       ...
       CLI_INTERFACE_TUNNEL_VHELP_STR)
{
	...
	if(CHECK_FLAG(peer->flags,PEER_FLAG_ENABLE))
    {
		if(peer->afc[AFI_IP][SAFI_MPLS_VPN] == 0)
		{
			vty_error_out (vty, "Configure ipv4-family vpnv4. %s", VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	peer->ifindex_tnl = ifindex;
	
	return CMD_SUCCESS;
}

DEFUN (no_binding_tunnel,
	  ...
	  CLI_INTERFACE_TUNNEL_STR)
{
   ...
   if(peer->ifindex_tnl == 0)
   {
	   vty_error_out(vty, "The neighbor is not bound with tunnel.%s", VTY_NEWLINE);
	   return CMD_SUCCESS;
   }
       ...
       "Specify an IPv6 peer address\n")
{
    ...
    ret = str2sockunion (argv[0], &su);
    if (ret < 0 || su.sa.sa_family == AF_INET)
    {
        vty_error_out (vty, "The neighbor address is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
	if(ipv6_is_zero((struct ipv6_addr *)&su.sin6.sin6_addr) || ipv6_is_loopback((struct ipv6_addr *)&su.sin6.sin6_addr))
    {
		vty_error_out (vty, "The neighbor address is invalid.%s",  VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "source X:X::X:X\n")
{
    ...
    ret = str2sockunion (argv[0], &sulocal);
    if (ret < 0 || sulocal.sa.sa_family == AF_INET)
    {
		vty_error_out (vty, "The %s is invalid.%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }

	if(ipv6_is_zero((struct ipv6_addr *)&sulocal.sin6.sin6_addr)|| ipv6_is_loopback((struct ipv6_addr *)&sulocal.sin6.sin6_addr))
    {
		vty_error_out (vty, "The %s is invalid.%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    if(sockunion_same (&peer->su, &sulocal))
    {
        vty_error_out(vty, "vty_error_outThe source address and the neighbor address are the same.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if(peer->su_local.sa.sa_family == AF_INET6 && sockunion_same (&peer->su_local, &sulocal))
    {
		vty_warning_out(vty, "The %s already exists.%s", argv[0], VTY_NEWLINE);
        return CMD_SUCCESS;
    }
...
DEFUN (no_source_address_ipv6,
       no_source_address_ipv6_cmd,
       "no source X:X::X:X",
       NO_STR
       "source address\n"
       "source X:X::X:X\n")
{
    ...
    {
        memset(&peer->su_local,0x0,sizeof(union sockunion));
		vty_warning_out (vty, "The neighbor does not have source IP.%s", VTY_NEWLINE);
    }
    else
    {
		vty_error_out (vty, "The %s does not exists.%s", argv[0], VTY_NEWLINE);
           ...
           enum clear_sort sort,enum bgp_clear_type stype, const char *arg)
{
    ...
    if (sort == clear_peer)
    {
        ...
        if (ret < 0)
        {
            vty_error_out (vty, "Malformed address: %s%s", arg, VTY_NEWLINE);
            return CMD_WARNING;
        }
       ...
       "Global BGP memory statistics\n")
{
    ...
    count = mtype_stats_alloc (MTYPE_BGP_INFO);
    ...
    if ((count = mtype_stats_alloc (MTYPE_BGP_ROUTE)))
...
bgp_show_summary (struct vty *vty, struct bgp *bgp, int afi, int safi)
{
    ...
    static char header[] = "Neighbor        V         AS MsgRcvd MsgSent OutQ Up/Down         State   PfxRcd";
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if(!CHECK_FLAG(peer->flags,PEER_FLAG_ENABLE))
        {
		...
		if((afi == AFI_IP && peer->su.sin.sin_family == AF_INET) ||(afi == AFI_IP6 && peer->su.sin6.sin6_family == AF_INET6))
	         peer_count ++;
    }
    ...
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        ...
        if (peer->afc[afi][safi])
        {
            if (!count)
            {
                ...
                vty_out (vty,
                         "%sBGP local router ID : %s%s",
                         VTY_NEWLINE,inet_ntoa (bgp->router_id), VTY_NEWLINE);
                vty_out (vty,"Local AS number : %u%s", bgp->as, VTY_NEWLINE);
				
                /* Peer related usage */
                //ents = listcount (bgp->peer);
                vty_out (vty, "Total number of neighbors : %ld%s",
                         peer_count, VTY_NEWLINE);
			...
			if(peer->vrf_id)
			{
				vty_out (vty, " %8ld", peer->pcount[afi][SAFI_MPLS_VPN]);
			}
			else if(peer->afc[afi][SAFI_MPLS_VPN])
			{
				if(peer->afc[afi][safi])
				{
					vty_out (vty, " %8ld", (peer->pcount[afi][SAFI_MPLS_VPN] + peer->pcount[afi][safi]));
				}
				else
				{
					vty_out (vty, " %8ld", peer->pcount[afi][SAFI_MPLS_VPN]);	
...
DEFUN (show_bgp_neighbors_summary,
       show_bgp_neighbors_summary_cmd,
       "show bgp neighbors",
       SHOW_STR
       BGP_STR
...
bgp_show_peer_afi (struct vty *vty, struct peer *p, afi_t afi, safi_t safi)
{
    ...
    vty_out (vty, " For address family: %s%s", p->vrf_id ? afi_safi_print (afi, SAFI_MPLS_VPN) : afi_safi_print (afi, safi),
             VTY_NEWLINE);
    ...
    vty_out (vty, "  %ld accepted prefixes%s",  p->vrf_id ? p->pcount[afi][SAFI_MPLS_VPN]: p->pcount[afi][safi], VTY_NEWLINE);
...
bgp_show_peer (struct vty *vty, struct peer *p)
{
    ...
    bgp = p->bgp;
    if(!CHECK_FLAG(p->flags,PEER_FLAG_ENABLE))
    {
        return;
    }
    ...
    if (p->status == Established)
    {
        ...
        vty_out (vty, ", hold time is %d seconds, keepalive interval is %d seconds%s",
                 p->v_holdtime, p->v_keepalive, VTY_NEWLINE);
        if (CHECK_FLAG (p->config, PEER_CONFIG_TIMER))
        {
            vty_out (vty, "  Configured hold time is %d seconds", p->holdtime);
    ...
    {
        vty_out (vty, "  Configured retry time is %d seconds%s", p->connect,VTY_NEWLINE);
    }
            ...
            || p->afc_recv[AFI_IP][SAFI_MPLS_VPN])
        {
            ...
            if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV) || CHECK_FLAG (p->cap, PEER_CAP_RESTART_ADV))
            {
                ...
                if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV))
                {
                    ...
                    vty_out (vty, "      Address families by neighbor:%s        ", VTY_NEWLINE);
                    for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
                        for (safi = SAFI_UNICAST ; safi < SAFI_MAX ; safi++)
    ...
    if (CHECK_FLAG (p->config, PEER_CONFIG_WEIGHT))
        vty_out (vty, "  Weight value %d%s", p->weight,
                 VTY_NEWLINE);
    ...
    if (CHECK_FLAG (p->sflags, PEER_STATUS_PREFIX_OVERFLOW))
    {
        ...
        if (p->t_pmax_restart)
            vty_out (vty, "  Reduce the no. of prefix from %s, %s",
                     p->host,  VTY_NEWLINE);
        else
    ...
    vty_out (vty, "  Local host: %s   Local port: %d  Remote port: %d%s",sockunion2str (&p->su_local, buf1, SU_ADDRSTRLEN),p->send_sport,p->send_dport, VTY_NEWLINE);

	if(p->su.sin.sin_family == AF_INET6)
	{
                   ...
                   enum show_type type, union sockunion *su)
{
    ...
    {
        vty_info_out (vty, "The neighbor session does not exist.%s", VTY_NEWLINE);
    }
    return CMD_SUCCESS;
}
       ...
       "Path information\n")
{
    ...
    unsigned long count = 0, total_count = 0;
    ...
    vty_out(vty, "%sTotal Number of Routes: %lu%s",VTY_NEWLINE,total_count, VTY_NEWLINE);
    vty_out(vty, "Total Number of Paths: %lu%s", count, VTY_NEWLINE);
    vty_out(vty,"%s",VTY_NEWLINE);
	
    if(count)
    {
        vty_out(vty, "Address      Refcount     MED          Path/Origin%s", VTY_NEWLINE);
        attr_show_all (vty);
    }
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
    ...
    if((type != ROUTE_PROTO_STATIC) && (type != ROUTE_PROTO_CONNECT))
    {
        vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    bgp = vty->index;
    if(bgp->redist[afi][type] && bgp->redist_metric_flag[afi][type])
    {
        bgp_redistribute_unset (vty->index, afi, type,0);
    }
    else if(bgp->redist[afi][type])
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    return bgp_redistribute_set (vty->index, afi, type, 0, 0);
}

DEFUN (bgp_redistribute_roi_ipv4,
       ...
       QUAGGA_IP_REDIST_HELP_STR_ROI_BGPD)
{
	...
	afi = bgp_node_afi(vty);
    type = proto_redistnum (afi, argv[0]);   
    if (afi == AFI_IP6 || ((type != ROUTE_PROTO_ISIS) && (type != ROUTE_PROTO_OSPF) && (type != ROUTE_PROTO_RIP)))
    {
        vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    VTY_GET_INTEGER_RANGE ("instance", instance, argv[1], 1, 255);
	
    type_redis = type - ROUTE_PROTO_OSPF;
    bgp = vty->index;
    if(bgp->redist_instance[type_redis][instance].redist_instance_flag && bgp->redist_instance[type_redis][instance].redist_instance_metric_flag)
    {
        bgp_redistribute_unset (vty->index, afi, type,instance);
    }
    else if (bgp->redist_instance[type_redis][instance].redist_instance_flag)
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    return bgp_redistribute_set (vty->index, afi, type, instance, metric);
}

DEFUN (bgp_redistribute_roi_ipv4_metric,
       ...
       "Default metric\n")
{
    ...
    && bgp->redist_instance[type_redis][instance].redist_instance_metric_flag
    && metric == bgp->redist_instance[type_redis][instance].redist_instance_metric)
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "Pointer to route-map entries\n")
{
    ...
    if (type < 0 || type == ROUTE_PROTO_IBGP || type == ROUTE_PROTO_EBGP)
    {
        vty_out (vty, "%% Invalid route type%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    bgp_redistribute_unset (vty->index, AFI_IP, type,0);
	
    bgp_redistribute_rmap_set (vty->index, AFI_IP, type, argv[1]);
	
    return bgp_redistribute_set (vty->index, AFI_IP, type, 0, 0);
}
       ...
       "Default metric\n")
{
    ...
    bgp = vty->index;
    if(bgp->redist[afi][type] && bgp->redist_metric_flag[afi][type] && bgp->redist_metric[afi][type] == metric)
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    else if(bgp->redist[AFI_IP][type])
    {
        bgp_redistribute_unset (vty->index, afi, type,0);
    }
	
    bgp_redistribute_metric_set (vty->index, afi, type, metric);
...
DEFUN (bgp_redistribute_ipv4_rmap_metric,
       ...
       "redistribute " QUAGGA_IP_REDIST_STR_BGPD " route-map WORD metric <1-4294967295>",
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP_REDIST_HELP_STR_BGPD
       "Route map reference\n"
       "Pointer to route-map entries\n"
...
DEFUN (bgp_redistribute_ipv4_metric_rmap,
       ...
       "redistribute " QUAGGA_IP_REDIST_STR_BGPD " metric <1-4294967295> route-map WORD",
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
       "Default metric\n"
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
    ...
    bgp = vty->index;
    if(bgp->redist[afi][type] == 0)
    {
        vty_error_out (vty, "The policy does not exist.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    return bgp_redistribute_unset (vty->index, afi, type,0);
}

DEFUN (no_bgp_redistribute_roi_ipv4,
       ...
       QUAGGA_IP_REDIST_HELP_STR_ROI_BGPD)
{
    ...
    bgp = vty->index;
    if(bgp->redist_instance[type_redis][instance].redist_instance_flag == 0)
    {
        vty_error_out (vty, "The policy does not exist.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    return bgp_redistribute_unset (vty->index, afi, type,instance);
}
...
DEFUN (no_bgp_redistribute_ipv4_metric,
       ...
       "no redistribute " QUAGGA_IP_REDIST_STR_BGPD " metric <1-4294967295>",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
...
DEFUN (no_bgp_redistribute_ipv4_rmap_metric,
       ...
       "no redistribute " QUAGGA_IP_REDIST_STR_BGPD " route-map WORD metric <1-4294967295>",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP_REDIST_HELP_STR_BGPD
       "Route map reference\n"
...
ALIAS (no_bgp_redistribute_ipv4_rmap_metric,
       ...
       "no redistribute " QUAGGA_IP_REDIST_STR_BGPD " metric <1-4294967295> route-map WORD",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
...
DEFUN (bgp_redistribute_roi_ipv6,
       ...
       QUAGGA_IP6_REDIST_HELP_STR_ROI_BGPD)
{
	...
	afi = bgp_node_afi(vty);
    type = proto_redistnum (afi, argv[0]);
    if ( afi == AFI_IP || ((type != ROUTE_PROTO_ISIS6) && (type != ROUTE_PROTO_OSPF6) && (type != ROUTE_PROTO_RIPNG)))
    {
        vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...
DEFUN (bgp_redistribute_roi_ipv6_metric,
...
DEFUN (no_bgp_redistribute_roi_ipv6,
       ...
       QUAGGA_IP6_REDIST_HELP_STR_BGPD)
{
    ...
    return bgp_redistribute_set (vty->index, AFI_IP6, type, 0 ,0);
}
...
DEFUN (bgp_redistribute_ipv6_metric,
       ...
       "redistribute " QUAGGA_IP6_REDIST_STR_BGPD " metric <1-4294967295>",
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
       "Default metric\n")
...
DEFUN (bgp_redistribute_ipv6_rmap_metric,
       ...
       "redistribute " QUAGGA_IP6_REDIST_STR_BGPD " route-map WORD metric <1-4294967295>",
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Route map reference\n"
       "Pointer to route-map entries\n"
...
DEFUN (bgp_redistribute_ipv6_metric_rmap,
       ...
       "redistribute " QUAGGA_IP6_REDIST_STR_BGPD " metric <1-4294967295> route-map WORD",
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
       "Default metric\n"
...
DEFUN (no_bgp_redistribute_ipv6_metric,
       ...
       "no redistribute " QUAGGA_IP6_REDIST_STR_BGPD " metric <1-4294967295>",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
...
DEFUN (no_bgp_redistribute_ipv6_rmap_metric,
       ...
       "no redistribute " QUAGGA_IP6_REDIST_STR_BGPD " route-map WORD metric <1-4294967295>",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Route map reference\n"
...
ALIAS (no_bgp_redistribute_ipv6_rmap_metric,
       ...
       "no redistribute " QUAGGA_IP6_REDIST_STR_BGPD " metric <1-4294967295> route-map WORD",
       NO_STR
       "Redistribute information from another routing protocol\n"
       QUAGGA_IP6_REDIST_HELP_STR_BGPD
       "Metric for redistributed routes\n"
...
void
bgp_config_write_redistribute (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi)
{
    ...
    for (i = 0; i < ROUTE_PROTO_MAX; i++)
    {
        ...
        if(ROUTE_PROTO_OSPF == i || ROUTE_PROTO_ISIS == i || ROUTE_PROTO_RIP == i)
        {
            ...
            for(instance = 0; instance < MAX_INSTANCE_ID ; instance ++)
            {
                if(bgp->redist_instance[type_redis][instance].redist_instance_flag)
                {
                    vty_out (vty, " redistribute %s %d", zebra_route_string(i),instance);
        ...
        else if (bgp->redist[afi][i] && ((i == ROUTE_PROTO_STATIC) || (i == ROUTE_PROTO_CONNECT)))
        {
...
DEFUN (address_family_vpn_instance,
       ...
       "Number of the L3VPN instance\n")
{
   ...
   // char *pprompt = NULL;
    ...
    if (NULL != pprompt)
    {
        snprintf(pprompt, VTY_BUFSIZ, "%%s(config-bgp-af-vpn-instance-%s)#", argv[0]);
    }
	#endif
	return ret;
}
...
DEFUN (bgp_redistribute_vrf_ipv4,
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
    ...
    type = proto_redistnum (AFI_IP, argv[0]);
    if (type != ROUTE_PROTO_CONNECT && type != ROUTE_PROTO_STATIC)
    {
		vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    bgp = vty->index;
	id = (uint32_t)vty->index_sub;	

	if(ROUTE_PROTO_CONNECT == type)
	{
		i = 1;
	}
	...
	if(bgp_vrf_redist_info[id]->redist[i] && bgp_vrf_redist_info[id]->redist_metric_flag[i])
    {
    	bgp_vrf_redist_info[id]->redist[i] = 0;
    	bgp_vrf_redist_info[id]->redist_metric_flag[i] = 0;
    	bgp_vrf_redist_info[id]->redist_metric[i] = 0;
        bgp_redistribute_route_delete_type (bgp, AFI_IP, type, 0, id);
    }
    else if(bgp_vrf_redist_info[id]->redist[i])
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
	bgp_vrf_redist_info[id]->redist[i] = 1;
		
    bgp_redistribute_route_add_type(bgp, AFI_IP, type, 0, id);
	
	return CMD_SUCCESS;
}


DEFUN (bgp_redistribute_vrf_ipv4_metric,
       ...
       "Default metric\n")
{
    ...
    if(bgp_vrf_redist_info[id]->redist[i] && bgp_vrf_redist_info[id]->redist_metric_flag[i] && bgp_vrf_redist_info[id]->redist_metric[i] == metric)
    {
        vty_error_out (vty, "The policy already exists.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	...
	bgp_vrf_redist_info[id]->redist[i] = 1;
	bgp_vrf_redist_info[id]->redist_metric_flag[i] = 1;
	bgp_vrf_redist_info[id]->redist_metric[i] = metric;
...
DEFUN (no_bgp_redistribute_vrf_ipv4,
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
    ...
    if(bgp_vrf_redist_info[id]->redist[i]== 0)
    {
        vty_error_out (vty, "The policy does not exist.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...
DEFUN (bgp_redistribute_vrf_roi_ipv4,
...
{  
	...
	{
		ret = bgp_redist_vrf_ospf_add(id,instance,metric_flag,metric);
	}
	else if(ROUTE_PROTO_RIP == type)
	{
		ret = bgp_redist_vrf_rip_add(id,instance,metric_flag,metric);
	}
	else if(ROUTE_PROTO_ISIS == type)
	{	
		ret = bgp_redist_vrf_isis_add(id,instance,metric_flag,metric);
	}
	else
	{
		vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
	...
	{	
		vty_error_out (vty, "memory xcalloc error.%s", VTY_NEWLINE);
		return ret;
	}
		
    bgp_redistribute_route_add_type(bgp, AFI_IP, type, instance, id);

	return CMD_SUCCESS;
}

DEFUN (bgp_redistribute_vrf_roi_ipv4_metric,
...
DEFUN (no_bgp_redistribute_vrf_roi_ipv4,
       ...
       QUAGGA_IP_REDIST_HELP_STR_ROI_BGPD)
{
	...
	if(ROUTE_PROTO_OSPF == type)
	{
		ret = bgp_redist_vrf_ospf_delete(id,instance);
	}
	else if(ROUTE_PROTO_RIP == type)
	{
		ret = bgp_redist_vrf_rip_delete(id,instance);
	}
	else if(ROUTE_PROTO_ISIS == type)
	{
		ret = bgp_redist_vrf_isis_delete(id,instance);
	}
	else
	{
		vty_error_out (vty, "The route type is invalid.%s", VTY_NEWLINE);
	...
	bgp_redistribute_route_delete_type(bgp,AFI_IP, type, instance, id);
...
static void
bgp_vty_neighbor_init (void)
{
    /*neighbor node commands*/
    install_element (BGP_NODE, &bgp_neighbor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_neighbor_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &remote_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &neighbor_enable_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &description_name_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_description_name_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &bgp_auth_md5_password_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_bgp_auth_md5_password_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &source_address_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_source_address_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &ebgp_multihop_ttl_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_ebgp_multihop_ttl_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &local_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_local_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &route_redistribute_default_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_route_redistribute_default_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &route_redistribute_ibgp_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_route_redistribute_ibgp_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &route_limit_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_route_limit_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &route_weight_bgp_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_route_weight_bgp_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &route_next_hop_change_local_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_route_next_hop_change_local_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &as_path_exclude_private_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_as_path_exclude_private_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &advertise_interval_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_advertise_interval_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &as_path_allow_as_loop_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_as_path_allow_as_loop_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &ipv4_family_vpnv4_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_ipv4_family_vpnv4_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &ipv4_family_vpn_instance_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_ipv4_family_vpn_instance_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &binding_tunnel_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE, &no_binding_tunnel_cmd, CMD_SYNC);
}

static void
bgp_vty_neighbor_ipv6_init (void)
{
    install_element (BGP_NODE, &bgp_neighbor_ipv6_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_neighbor_ipv6_cmd, CMD_SYNC);
	
    install_element (NEIGHBOR_NODE_IPV6, &remote_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &neighbor_enable_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &description_name_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_description_name_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &bgp_auth_md5_password_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_bgp_auth_md5_password_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &source_address_ipv6_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_source_address_ipv6_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &ebgp_multihop_ttl_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_ebgp_multihop_ttl_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &local_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_local_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &route_redistribute_default_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_route_redistribute_default_route_cmd, CMD_SYNC);
    //install_element (NEIGHBOR_NODE_IPV6, &route_redistribute_ibgp_route_cmd, CMD_SYNC);
    //install_element (NEIGHBOR_NODE_IPV6, &no_route_redistribute_ibgp_route_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &route_limit_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_route_limit_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &route_weight_bgp_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_route_weight_bgp_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &route_next_hop_change_local_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_route_next_hop_change_local_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &as_path_exclude_private_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_as_path_exclude_private_as_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &advertise_interval_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_advertise_interval_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &as_path_allow_as_loop_cmd, CMD_SYNC);
    install_element (NEIGHBOR_NODE_IPV6, &no_as_path_allow_as_loop_cmd, CMD_SYNC);
}
...
bgp_vty_init (void)
{
    ...
    install_node (&bgpd_node, bgp_config_write);
    ...
    install_default (NEIGHBOR_NODE);
    install_default (NEIGHBOR_NODE_IPV6);
    install_default (BGP_IPV6_NODE);
	install_default (BGP_VPN_INSTANCE_NODE);
	install_default (BGP_VPNV4_NODE);
	  
    /* "bgp multiple-instance" commands. */
    install_element (CONFIG_NODE, &bgp_multiple_instance_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_bgp_multiple_instance_cmd, CMD_SYNC);
	
    /* "bgp config-type" commands. */
    install_element (CONFIG_NODE, &bgp_config_type_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_bgp_config_type_cmd, CMD_SYNC);
	
    /* Dummy commands (Currently not supported) */
    install_element (BGP_NODE, &no_synchronization_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_auto_summary_cmd, CMD_SYNC);
	
    /* "router bgp" commands. */
    install_element (CONFIG_NODE, &router_bgp_cmd, CMD_SYNC);
	
    /* "no router bgp" commands. */
    install_element (CONFIG_NODE, &no_router_bgp_cmd, CMD_SYNC);

	
    install_element (BGP_NODE, &address_family_vpnv4_cmd, CMD_SYNC);
	install_element (BGP_NODE, &address_family_vpn_instance_cmd, CMD_SYNC);
    install_element (BGP_NODE, &address_family_ipv6_unicast_cmd, CMD_SYNC);
	
    /* "bgp router-id" commands. */
    install_element (BGP_NODE, &bgp_router_id_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_router_id_cmd, CMD_SYNC);
	
    /* "bgp cluster-id" commands. */
    install_element (BGP_NODE, &bgp_cluster_id_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_cluster_id32_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_cluster_id_cmd, CMD_SYNC);
	
    /* "bgp confederation" commands. */
    install_element (BGP_NODE, &bgp_confederation_identifier_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_confederation_identifier_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_confederation_identifier_arg_cmd, CMD_SYNC);
	
    /* "bgp confederation peers" commands. */
    install_element (BGP_NODE, &bgp_confederation_peers_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_confederation_peers_cmd, CMD_SYNC);
	
    /* "maximum-paths" commands. */
    install_element (BGP_NODE, &bgp_maxpaths_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_maxpaths_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_maxpaths_ibgp_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_maxpaths_ibgp_cmd, CMD_SYNC);
	
    /* "timers bgp" commands. */
    install_element (BGP_NODE, &bgp_timers_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_timers_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_timers_arg_cmd, CMD_SYNC);
    install_element (BGP_NODE, &conenct_timers_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_conenct_timers_cmd, CMD_SYNC);
	
    /* "bgp client-to-client reflection" commands */
    install_element (BGP_NODE, &no_bgp_client_to_client_reflection_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_client_to_client_reflection_cmd, CMD_SYNC);
	
    /* "bgp always-compare-med" commands */
    install_element (BGP_NODE, &bgp_always_compare_med_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_always_compare_med_cmd, CMD_SYNC);
	
    /* "bgp deterministic-med" commands */
    install_element (BGP_NODE, &bgp_deterministic_med_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_deterministic_med_cmd, CMD_SYNC);
	
    /* "bgp graceful-restart" commands */
    install_element (BGP_NODE, &bgp_graceful_restart_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_graceful_restart_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_graceful_restart_stalepath_time_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_graceful_restart_stalepath_time_cmd, CMD_SYNC);
	
    /* "bgp fast-external-failover" commands */
    install_element (BGP_NODE, &bgp_fast_external_failover_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_fast_external_failover_cmd, CMD_SYNC);
	
    /* "bgp enforce-first-as" commands */
    install_element (BGP_NODE, &bgp_enforce_first_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_enforce_first_as_cmd, CMD_SYNC);
	
    /* "bgp bestpath compare-routerid" commands */
    install_element (BGP_NODE, &bgp_bestpath_compare_router_id_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_compare_router_id_cmd, CMD_SYNC);
	
    /* "bgp bestpath as-path ignore" commands */
    install_element (BGP_NODE, &bgp_bestpath_aspath_ignore_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_aspath_ignore_cmd, CMD_SYNC);
	
    /* "bgp bestpath as-path confed" commands */
    install_element (BGP_NODE, &bgp_bestpath_aspath_confed_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_aspath_confed_cmd, CMD_SYNC);
	
    /* "bgp bestpath as-path multipath-relax" commands */
    install_element (BGP_NODE, &bgp_bestpath_aspath_multipath_relax_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_aspath_multipath_relax_cmd, CMD_SYNC);
	
    /* "bgp log-neighbor-changes" commands */
    install_element (BGP_NODE, &bgp_log_neighbor_changes_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_log_neighbor_changes_cmd, CMD_SYNC);
	
    /* "bgp bestpath med" commands */
    install_element (BGP_NODE, &bgp_bestpath_med_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_bestpath_med2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_bestpath_med3_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_med_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_med2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_bestpath_med3_cmd, CMD_SYNC);
	
    /* "no bgp default ipv4-unicast" commands. */
    install_element (BGP_NODE, &no_bgp_default_ipv4_unicast_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_default_ipv4_unicast_cmd, CMD_SYNC);
	
    /* "bgp network import-check" commands. */
    install_element (BGP_NODE, &bgp_network_import_check_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_import_check_cmd, CMD_SYNC);
	
    /* "bgp default local-preference" commands. */
    install_element (BGP_NODE, &bgp_default_local_preference_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_default_local_preference_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_default_local_preference_val_cmd, CMD_SYNC);
	
    /* "bgp default med" commands. */
    install_element (BGP_NODE, &med_default_value_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_med_default_value_cmd, CMD_SYNC);
	
    /* "neighbor remote-as" commands. */
    install_element (BGP_NODE, &neighbor_remote_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_cmd, CMD_SYNC);
	
    /* "neighbor peer-group" commands. */
    install_element (BGP_NODE, &neighbor_peer_group_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_peer_group_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_peer_group_remote_as_cmd, CMD_SYNC);
	
    /* "neighbor local-as" commands. */
    install_element (BGP_NODE, &neighbor_local_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_local_as_no_prepend_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_local_as_no_prepend_replace_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_local_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_local_as_val_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_local_as_val2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_local_as_val3_cmd, CMD_SYNC);
	
    /* "neighbor password" commands. */
    install_element (BGP_NODE, &neighbor_password_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_password_cmd, CMD_SYNC);
	
    /* "neighbor activate" commands. */
    install_element (BGP_NODE, &neighbor_activate_cmd, CMD_SYNC);
	
    /* "no neighbor activate" commands. */
    install_element (BGP_NODE, &no_neighbor_activate_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_source_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_source_cmd, CMD_SYNC);
	
    /* "neighbor peer-group set" commands. */
    install_element (BGP_NODE, &neighbor_set_peer_group_cmd, CMD_SYNC);
	
    /* "no neighbor peer-group unset" commands. */
    install_element (BGP_NODE, &no_neighbor_set_peer_group_cmd, CMD_SYNC);
	
    /* "neighbor softreconfiguration inbound" commands.*/
    install_element (BGP_NODE, &neighbor_soft_reconfiguration_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_soft_reconfiguration_cmd, CMD_SYNC);
	
    /* "neighbor attribute-unchanged" commands.  */
    install_element (BGP_NODE, &neighbor_attr_unchanged_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged1_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged3_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged4_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged5_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged6_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged7_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged8_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged9_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_attr_unchanged10_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged1_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged3_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged4_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged5_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged6_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged7_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged8_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged9_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_attr_unchanged10_cmd, CMD_SYNC);
	
    /* "transparent-as" and "transparent-nexthop" for old version
       compatibility.  */
    install_element (BGP_NODE, &neighbor_transparent_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_transparent_nexthop_cmd, CMD_SYNC);
	
    /* "neighbor next-hop-self" commands. */
    install_element (BGP_NODE, &neighbor_nexthop_self_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_nexthop_self_cmd, CMD_SYNC);
	
    /* "neighbor remove-private-AS" commands. */
    install_element (BGP_NODE, &neighbor_remove_private_as_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_remove_private_as_cmd, CMD_SYNC);
	
    /* "neighbor send-community" commands.*/
    install_element (BGP_NODE, &neighbor_send_community_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_send_community_type_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_send_community_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_send_community_type_cmd, CMD_SYNC);
	
    /* "neighbor route-reflector" commands.*/
    install_element (BGP_NODE, &neighbor_route_reflector_client_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_route_reflector_client_cmd, CMD_SYNC);
	
    /* "neighbor route-server" commands.*/
    install_element (BGP_NODE, &neighbor_route_server_client_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_route_server_client_cmd, CMD_SYNC);
	
    /* "neighbor passive" commands. */
    install_element (BGP_NODE, &neighbor_passive_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_passive_cmd, CMD_SYNC);
	
    /* "neighbor shutdown" commands. */
    install_element (BGP_NODE, &neighbor_shutdown_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_shutdown_cmd, CMD_SYNC);
	
    /* Deprecated "neighbor capability route-refresh" commands.*/
    install_element (BGP_NODE, &neighbor_capability_route_refresh_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_capability_route_refresh_cmd, CMD_SYNC);
	
    /* "neighbor capability orf prefix-list" commands.*/
    install_element (BGP_NODE, &neighbor_capability_orf_prefix_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_capability_orf_prefix_cmd, CMD_SYNC);
	
    /* "neighbor capability dynamic" commands.*/
    install_element (BGP_NODE, &neighbor_capability_dynamic_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_capability_dynamic_cmd, CMD_SYNC);
	
    /* "neighbor dont-capability-negotiate" commands. */
    install_element (BGP_NODE, &neighbor_dont_capability_negotiate_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_dont_capability_negotiate_cmd, CMD_SYNC);
	
    /* "neighbor ebgp-multihop" commands. */
    install_element (BGP_NODE, &neighbor_ebgp_multihop_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_ebgp_multihop_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_ebgp_multihop_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_ebgp_multihop_ttl_cmd, CMD_SYNC);
	
    /* "neighbor disable-connected-check" commands.  */
    install_element (BGP_NODE, &neighbor_disable_connected_check_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_disable_connected_check_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_enforce_multihop_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_enforce_multihop_cmd, CMD_SYNC);
	
    /* "neighbor description" commands. */
    install_element (BGP_NODE, &neighbor_description_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_description_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_description_val_cmd, CMD_SYNC);
	
    /* "neighbor update-source" commands. "*/
    install_element (BGP_NODE, &neighbor_update_source_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_update_source_cmd, CMD_SYNC);
	
    /* "neighbor default-originate" commands. */
    install_element (BGP_NODE, &neighbor_default_originate_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_default_originate_rmap_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_default_originate_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_default_originate_rmap_cmd, CMD_SYNC);
	
    /* "neighbor port" commands. */
    install_element (BGP_NODE, &neighbor_port_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_port_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_port_val_cmd, CMD_SYNC);
	
    /* "neighbor weight" commands. */
    install_element (BGP_NODE, &neighbor_weight_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_weight_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_weight_val_cmd, CMD_SYNC);
	
    /* "neighbor override-capability" commands. */
    install_element (BGP_NODE, &neighbor_override_capability_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_override_capability_cmd, CMD_SYNC);
	
    /* "neighbor strict-capability-match" commands. */
    install_element (BGP_NODE, &neighbor_strict_capability_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_strict_capability_cmd, CMD_SYNC);
	
    /* "neighbor timers" commands. */
    install_element (BGP_NODE, &neighbor_timers_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_timers_cmd, CMD_SYNC);
	
    /* "neighbor timers connect" commands. */
    install_element (BGP_NODE, &neighbor_timers_connect_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_timers_connect_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_timers_connect_val_cmd, CMD_SYNC);
	
    /* "neighbor advertisement-interval" commands. */
    install_element (BGP_NODE, &neighbor_advertise_interval_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_advertise_interval_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_advertise_interval_val_cmd, CMD_SYNC);
	
    /* "neighbor version" commands. */
    install_element (BGP_NODE, &neighbor_version_cmd, CMD_SYNC);
	
    /* "neighbor interface" commands. */
    install_element (BGP_NODE, &neighbor_interface_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_interface_cmd, CMD_SYNC);
	
    /* "neighbor distribute" commands. */
    install_element (BGP_NODE, &neighbor_distribute_list_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_distribute_list_cmd, CMD_SYNC);
	
    /* "neighbor allowas-in" */
    install_element (BGP_NODE, &neighbor_allowas_in_cmd, CMD_SYNC);
    install_element (BGP_NODE, &neighbor_allowas_in_arg_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_allowas_in_cmd, CMD_SYNC);
	
    /* "clear ip bgp commands" */
    install_element (CONFIG_NODE, &clear_ip_bgp_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_cmd, CMD_SYNC);
	
    /* "clear ip bgp neighbor soft in" */
    install_element (CONFIG_NODE, &clear_ip_bgp_all_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_vpnv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_vpnv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_vpnv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_vpnv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_vpnv4_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_vpnv4_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_encap_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_encap_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_encap_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_encap_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_encap_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_encap_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_all_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_in_prefix_filter_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_soft_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_in_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_in_prefix_filter_cmd, CMD_SYNC);
	
    /* "clear ip bgp neighbor soft out" */
    install_element (CONFIG_NODE, &clear_ip_bgp_all_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_vpnv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_vpnv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_vpnv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_vpnv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_vpnv4_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_vpnv4_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_encap_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_encap_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_encap_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_encap_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_encap_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_encap_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_all_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_soft_out_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_out_cmd, CMD_SYNC);
	
    /* "clear ip bgp neighbor soft" */
    install_element (CONFIG_NODE, &clear_ip_bgp_all_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_group_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_external_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_ipv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_vpnv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_vpnv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_vpnv4_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_all_encap_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_encap_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_as_encap_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_all_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_group_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_external_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_as_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_group_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_external_soft_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_as_soft_cmd, CMD_SYNC);
	
    /* "clear ip bgp neighbor rsclient" */
    install_element (CONFIG_NODE, &clear_ip_bgp_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_peer_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_instance_peer_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_instance_all_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_peer_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_instance_peer_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_peer_rsclient_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_bgp_ipv6_instance_peer_rsclient_cmd, CMD_SYNC);
	
    /* "show ip bgp summary" commands. */
    install_element (CONFIG_NODE, &show_bgp_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_summary_1w_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv4_safi_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv4_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_neighbors_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_safi_summary_cmd, CMD_LOCAL);
	
    /* "show ip bgp neighbors" commands. */
    install_element (CONFIG_NODE, &show_bgp_instance_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE_IPV6, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_neighbors_peer_cmd, CMD_LOCAL);
	
    /* "show ip bgp rsclient" commands. */
    install_element (CONFIG_NODE, &show_bgp_instance_ipv4_safi_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_instance_ipv6_safi_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rsclient_summary_cmd, CMD_LOCAL);
	
    /* "show ip bgp paths" commands. */
    install_element (CONFIG_NODE, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_paths_cmd, CMD_LOCAL);
	
    /* "show ip bgp community" commands. */
    install_element (CONFIG_NODE, &show_ip_bgp_community_info_cmd, CMD_LOCAL);
	
    /* "show ip bgp attribute-info" commands. */
    install_element (CONFIG_NODE, &show_ip_bgp_attr_info_cmd, CMD_LOCAL);
	
    /* "redistribute" commands.  */
    install_element (BGP_NODE, &bgp_redistribute_ipv4_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_ipv4_metric_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_ipv4_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_roi_ipv4_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_roi_ipv4_metric_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_roi_ipv4_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &bgp_redistribute_ipv4_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &bgp_redistribute_ipv4_metric_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &no_bgp_redistribute_ipv4_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &bgp_redistribute_roi_ipv6_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &bgp_redistribute_roi_ipv6_metric_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &no_bgp_redistribute_roi_ipv6_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_ipv4_rmap_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_ipv4_rmap_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_ipv4_metric_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_ipv4_rmap_metric_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_redistribute_ipv4_metric_rmap_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_ipv4_rmap_metric_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_redistribute_ipv4_metric_rmap_cmd, CMD_SYNC);

	/*redistribute vrf rip ospf isis commands*/
    install_element (BGP_VPN_INSTANCE_NODE, &bgp_redistribute_vrf_roi_ipv4_cmd, CMD_SYNC);
	install_element (BGP_VPN_INSTANCE_NODE, &bgp_redistribute_vrf_roi_ipv4_metric_cmd, CMD_SYNC);
	install_element (BGP_VPN_INSTANCE_NODE, &no_bgp_redistribute_vrf_roi_ipv4_cmd, CMD_SYNC);
	
	/*redistribute vrf static connect commands*/
	install_element (BGP_VPN_INSTANCE_NODE, &bgp_redistribute_vrf_ipv4_cmd, CMD_SYNC);
	install_element (BGP_VPN_INSTANCE_NODE, &bgp_redistribute_vrf_ipv4_metric_cmd, CMD_SYNC);
	install_element (BGP_VPN_INSTANCE_NODE, &no_bgp_redistribute_vrf_ipv4_cmd, CMD_SYNC);
		
    /* ttl_security commands */
    install_element (BGP_NODE, &neighbor_ttl_security_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_neighbor_ttl_security_cmd, CMD_SYNC);
	
    /* "show bgp memory" commands. */
    install_element (CONFIG_NODE, &show_bgp_memory_cmd, CMD_LOCAL);
	
    /* "show bgp views" commands. */
    install_element (CONFIG_NODE, &show_bgp_views_cmd, CMD_LOCAL);
	
    /* non afi/safi forms of commands */
    install_element (CONFIG_NODE, &show_ip_bgp_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_ipv4_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_all_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_rd_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_all_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_rd_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_all_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_rd_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_neighbors_peer_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv6_neighbors_cmd, CMD_LOCAL);	
    install_element (BGP_NODE, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv6_neighbors_peer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_instance_ipv4_rsclient_summary_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_paths_cmd, CMD_LOCAL);
	
    /* Community-list. */
    community_list_vty ();
    bgp_vty_neighbor_init();
	bgp_vty_neighbor_ipv6_init();
}
...
community_list_perror (struct vty *vty, int ret)
{
    switch (ret)
    {
	    ...
	    case COMMUNITY_LIST_ERR_MALFORMED_VAL:
	        vty_error_out (vty, "Malformed community-list value%s", VTY_NEWLINE);
	        break;
...
{
    /* Community-list.  */
    install_element (CONFIG_NODE, &ip_community_list_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_community_list_standard2_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_community_list_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_community_list_name_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_community_list_name_standard2_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_community_list_name_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_standard_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_expanded_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_name_standard_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_name_expanded_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_name_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_community_list_name_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_community_list_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_community_list_arg_cmd, CMD_SYNC);
	
    /* Extcommunity-list.  */
    install_element (CONFIG_NODE, &ip_extcommunity_list_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_extcommunity_list_standard2_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_extcommunity_list_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_extcommunity_list_name_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_extcommunity_list_name_standard2_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &ip_extcommunity_list_name_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_standard_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_expanded_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_name_standard_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_name_expanded_all_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_name_standard_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &no_ip_extcommunity_list_name_expanded_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_extcommunity_list_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_ip_extcommunity_list_arg_cmd, CMD_SYNC);
}
