...
static int bgp_dump_interval_func (void * para);
...
bgp_dump_open_file (struct bgp_dump *bgp_dump)
{
    ...
    if (ret == 0)
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"bgp_dump_open_file: strftime error");
        return NULL;
    }
    if (bgp_dump->fp)
    {
    ...
    if (bgp_dump->fp == NULL)
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"bgp_dump_open_file: %s: %s", realpath, strerror (errno));
        umask(oldumask);
        return NULL;
    }
    umask(oldumask);
...
bgp_dump_interval_add (struct bgp_dump *bgp_dump, int interval)
{
    ...
    if (interval > 0)
    {
        ...
        bgp_dump->t_interval = thread_add_timer (bm->master, bgp_dump_interval_func,
                               bgp_dump, interval);
#endif
        bgp_dump->t_interval =  high_pre_timer_add("bgp_dump_interval", LIB_TIMER_TYPE_NOLOOP, bgp_dump_interval_func, (void *)bgp_dump, interval * 1000);
    }
    else
    {
        /* One-off dump: execute immediately, don't affect any scheduled dumps */
        bgp_dump_interval_func((void *)bgp_dump);
...
static int
bgp_dump_interval_func (void * para)
{
    struct bgp_dump *bgp_dump = NULL;
	
    bgp_dump = (struct bgp_dump *) para;
    bgp_dump->t_interval = 0;
...
bgp_dump_unset (struct vty *vty, struct bgp_dump *bgp_dump)
{
    ...
    {
        high_pre_timer_delete(bgp_dump->t_interval);
        bgp_dump->t_interval = 0;
    }
    bgp_dump->interval = 0;
