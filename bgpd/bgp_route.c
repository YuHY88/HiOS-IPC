...
bgp_info_extra_new (void)
{
    ...
    extra = XCALLOC (MTYPE_BGP_ROUTE_EXTRA, sizeof (struct bgp_info_extra));
	if(!extra)
	{
		return NULL;
	}
...
bgp_info_new (void)
{
    return XCALLOC (MTYPE_BGP_INFO, sizeof (struct bgp_info));
}
...
bgp_pcount_adjust (struct bgp_node *rn, struct bgp_info *ri)
{
	...
	if(!rn || !(bgp_node_table (rn)) || !ri || !ri->peer || !ri->peer->bgp)
	{
		return ;
	}
        ...
        && CHECK_FLAG (ri->flags, BGP_INFO_COUNTED))
    {
        ...
        {
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: Asked to decrement 0 prefix count for peer %s",
                       __func__, ri->peer->host);
            zlog_backtrace (LOG_WARNING);
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: Please report to Quagga bugzilla", __func__);
        }
    }
...
static int
bgp_info_cmp (struct bgp *bgp, struct bgp_info *new_bi, struct bgp_info *exist_bi,
...
{        
     ...
     */
    if (new_bi->sub_type == BGP_ROUTE_AGGREGATE)
    {
        return -1;
    }
	
    if (exist_bi->sub_type == BGP_ROUTE_AGGREGATE)
    {
        return 1;
    }
	
    if (new_bi->sub_type == BGP_ROUTE_STATIC)
    {
        return -1;
    }
	
    if (exist_bi->sub_type == BGP_ROUTE_STATIC)
    {
        return 1;
    }
	
    if (new_bi->sub_type == BGP_ROUTE_REDISTRIBUTE)
    {
        return -1;
    }
	
    if (exist_bi->sub_type == BGP_ROUTE_REDISTRIBUTE)
    {
        return 1;
    }
    ...
    if (bgp_flag_check (bgp, BGP_FLAG_ALWAYS_COMPARE_MED)
        ...
        || internal_as_route)
    {
        new_med = bgp_med_value (new_bi->attr, bgp);
        exist_med = bgp_med_value (exist_bi->attr, bgp);
    ...
    /* 7. Peer type check. */
    new_sort = new_bi->peer->sort;
    exist_sort = exist_bi->peer->sort;
    ...
    /* 8. IGP metric check. */
    newm = existm = ROUTE_COST;
	
    if (new_bi->extra)
    {
        newm = new_bi->extra->igpmetric;
    }
	
    if (exist_bi->extra)
    {
        existm = exist_bi->extra->igpmetric;
    }
    ...
    if (bgp_mpath_is_configured (bgp, afi, safi))
    {
        ...
        else if (new_bi->peer->sort == BGP_PEER_IBGP)
        {
            if (aspath_cmp (new_bi->attr->aspath, exist_bi->attr->aspath))
            {
                return 0;
            }
        }
        else if (new_bi->peer->as == exist_bi->peer->as)
        {
            return 0;
        }
    }
    ...
    {
#if 0
        if (CHECK_FLAG (new_bi->flags, BGP_INFO_SELECTED))
        {
            return -1;
        }
    ...
    {
        new_id.s_addr = new_bi->peer->remote_id.s_addr;
    }
    ...
    {
        exist_id.s_addr = exist_bi->peer->remote_id.s_addr;
    }
    ...
    if (newattr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST) && newattre)
    {
        new_cluster = newattre->cluster->length;
    }
	
    if (existattr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST) && existattre)
    {
     ...
     */
    if (CHECK_FLAG (exist_bi->flags, BGP_INFO_STALE))
    {
        return -1;
    }
	
    if (CHECK_FLAG (new_bi->flags, BGP_INFO_STALE))
    {
        return 1;
    }
	
    /* locally configured routes to advertise do not have su_remote */
    if (new_bi->peer->su.sin.sin_addr.s_addr == 0)
    {
        return 1;
    }
	
    if (exist_bi->peer->su.sin.sin_addr.s_addr == 0)
    {
        return -1;
    }
	
    ret = sockunion_cmp (&new_bi->peer->su, &exist_bi->peer->su);
    if (ret == 1)
    {
        return 1;
    }
                    ...
                    struct attr *attr, afi_t afi, safi_t safi)
{
    ...
    for(ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer_cnt))
    {
        if(!CHECK_FLAG(peer_cnt->flags,PEER_FLAG_ENABLE))
        {
            continue;
        }
		
		if( peer->vrf_id == 0 && peer->afc[afi][SAFI_MPLS_VPN] == 0)
		{
        	peer_count ++;
		}
    }
	
    if ((p->family == AF_INET && IPV4_NET127(ntohl(riattr->nexthop.s_addr)))
		|| (p->family == AF_INET6  && riattr->extra && ipv6_is_loopback((struct ipv6_addr *)&(riattr->extra->mp_nexthop_global))))
    {
        return 0;
    }
	
    if ( BGP_ROUTE_REDISTRIBUTE == ri->sub_type && (riattr->flag & ATTR_FLAG_BIT (BGP_ATTR_SEND_CONNECT)))
    {
        return 0;
    }
    ...
    if (from == peer)
    {
    	if(safi == SAFI_MPLS_VPN || peer->vrf_id || (safi == SAFI_UNICAST && peer_count == 1))
		{
        	return 0;
		}
    }
    ...
    {
        if (p->family == AF_INET && p->u.prefix4.s_addr == INADDR_ANY && p->prefixlen == 0)
        {
            return 0;
        }
    ...
    /* Route-Reflect check. */
    if (from->sort == BGP_PEER_IBGP && peer->sort == BGP_PEER_IBGP && from->vrf_id == peer->vrf_id)
    {
        reflect = 1;
    }
    else
	...
	/* Med applies to redistribute and aggragate*/
    if((peer->sort == BGP_PEER_EBGP) && ((BGP_ROUTE_REDISTRIBUTE == ri->sub_type)|| (BGP_ROUTE_AGGREGATE == ri->sub_type)))
    {
        if(bgp->default_med)
        {
        	if(BGP_ROUTE_AGGREGATE == ri->sub_type)				
				attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_MULTI_EXIT_DISC);
			
            attr->med = bgp->default_med;
        }
		else
		{
			if(BGP_ROUTE_AGGREGATE == ri->sub_type)	
    ...
    if((attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF)) && 
		((BGP_ROUTE_NORMAL != ri->sub_type)|| ((BGP_ROUTE_NORMAL == ri->sub_type) && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE))))
    {
        attr->local_pref = bgp->default_local_pref;
    }

	/*modify ibgp nexthop address*/
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE) && from != peer)
	{
        attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_SEND_IBGP);
	}
	else
	{
		attr->flag &= ~(ATTR_FLAG_BIT (BGP_ATTR_SEND_IBGP));
    ...
    if (peer && from && peer->sort == BGP_PEER_IBGP && from->sort == BGP_PEER_IBGP
...
#define NEXTHOP_IS_V6 (\
    ...
    if (transparent
        ...
        || (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED)
            && ((p->family == AF_INET && safi != SAFI_MPLS_VPN && attr->nexthop.s_addr)
            	|| (p->family == AF_INET && safi == SAFI_MPLS_VPN && attr->extra->mp_nexthop_global_in.s_addr)
                || (NEXTHOP_IS_V6 && ! ipv6_is_zero((struct ipv6_addr *)&(attr->extra->mp_nexthop_global))))))
    {
        /* NEXT-HOP Unchanged. */
    }
    else if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF)
             || (p->family == AF_INET && safi != SAFI_MPLS_VPN && attr->nexthop.s_addr == 0)
             || (p->family == AF_INET && safi == SAFI_MPLS_VPN && attr->extra->mp_nexthop_global_in.s_addr == 0)
             || (NEXTHOP_IS_V6 && ipv6_is_zero((struct ipv6_addr *)&(attr->extra->mp_nexthop_global)))
             || ((peer->sort == BGP_PEER_EBGP || ri->sub_type == BGP_ROUTE_STATIC || ri->sub_type == BGP_ROUTE_REDISTRIBUTE)
                 && bgp_multiaccess_check_v4 (attr->nexthop, peer->host) == 0))
    {
        ...
        if (p->family == AF_INET)
        {
            if(safi == SAFI_MPLS_VPN)
        	{
				...
				uint32_t inlabel = 0 , in_tag = 0;

				inlabel = decode_label(bgp_info_extra_get(ri)->in_tag);
				if(bgp_vrf_redist_info[ri->vrf_id] && bgp_vrf_redist_info[ri->vrf_id]->label_apply == LABEL_APPLY_PER_VPN)
				{
					if(inlabel && bgp_vrf_redist_info[ri->vrf_id]->inlabel && inlabel != bgp_vrf_redist_info[ri->vrf_id]->inlabel)
					{
						bgp_com_label_free(ri->vrf_id,inlabel);
						bgp_zebra_announce (p, ri, bgp, safi, IPC_OPCODE_DELETE, 1);
						inlabel = 0;
					}
					
					if(inlabel == 0 || inlabel != bgp_vrf_redist_info[ri->vrf_id]->inlabel)
					{
						label_decode(ri->extra->in_tag,bgp_vrf_redist_info[ri->vrf_id]->inlabel);
						SET_FLAG(ri->extra->in_tag[2],BGP_INFO_EXTRA_TAG_BOTTOM_BIT);
						label_flag = 1;
					}	
				}
				
				if(bgp_vrf_redist_info[ri->vrf_id] && bgp_vrf_redist_info[ri->vrf_id]->label_apply == LABEL_APPLY_PER_ROUTE)
				{		
					if(inlabel && inlabel == bgp_vrf_redist_info[ri->vrf_id]->inlabel && !CHECK_FLAG(ri->flags,BGP_INFO_DELETE))
					{
						bgp_zebra_announce (p, ri, bgp, safi, IPC_OPCODE_DELETE, 1);
						SET_FLAG(ri->flags,BGP_INFO_DELETE);
						inlabel = 0;
					}
					
					if(inlabel == 0)
					{
						in_tag = bgp_com_label_new(ri->vrf_id);
						if( in_tag == 0)
						{                                   
	                        bgp_attr_flush (attr);
							zlog_err("%s[%d] ",__func__,__LINE__);
	                        return 0;
	                    }
						label_decode(ri->extra->in_tag,in_tag);
						SET_FLAG(ri->extra->in_tag[2],BGP_INFO_EXTRA_TAG_BOTTOM_BIT);
						label_flag = 1;
					}
				}
        	}
            else if (safi == SAFI_UNICAST && peer->vrf_id)
            {
    ...
    if(p->family == AF_INET && safi != SAFI_MPLS_VPN && attr->nexthop.s_addr && attr->nexthop.s_addr == peer->su.sin.sin_addr.s_addr)
    {
        memcpy (&attr->nexthop, &peer->nexthop.v4, IPV4_MAX_BYTELEN);
    }
	
    if(p->family == AF_INET && safi == SAFI_MPLS_VPN && attr->extra && attr->extra->mp_nexthop_global_in.s_addr && attr->extra->mp_nexthop_global_in.s_addr == peer->su.sin.sin_addr.s_addr)
    {
		memcpy (&attr->extra->mp_nexthop_global_in, &peer->nexthop.v4, IPV4_MAX_BYTELEN);
    }
    ...
    {		
		...
		if(bgp_vrf_redist_info[vrf_id] && bgp_vrf_redist_info[vrf_id]->rt_export)
		{
			SET_FLAG(attr->flag, ATTR_FLAG_BIT (BGP_ATTR_EXT_COMMUNITIES));
			attr->extra->ecommunity = bgp_vrf_redist_info[vrf_id]->rt_export;
		}
		else if(bgp_vrf_redist_info[vrf_id] && bgp_vrf_redist_info[vrf_id]->rt_export == NULL)
		{
	...
	if(label_flag)
	{
		bgp_zebra_announce (p, ri, bgp, safi, IPC_OPCODE_ADD, 1);
		ri->label_apply = bgp_vrf_redist_info[ri->vrf_id]->label_apply;
	}
	
	 if (peer->sort == BGP_PEER_EBGP
        && (! CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED) || attr->aspath->segments == NULL)
        && (! CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)))
    {
        if (CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION))
        {

            attr->local_as = bgp->confed_id;
        }
        else
        {
            if (peer->change_local_as)
            {
                attr->local_as = peer->change_local_as;
            }
            else
            {
                attr->local_as = peer->local_as;
            }
        }
    }
                             ...
                             struct prefix *p, struct attr *attr, afi_t afi, safi_t safi)
{
    ...
    if ((p->family == AF_INET && attr->nexthop.s_addr == 0)
        || (p->family == AF_INET6 && IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global)))
    {
        /* Set IPv4 nexthop. */
        if (p->family == AF_INET)
        {
...
{
    struct bgp_info *old_bi;
    struct bgp_info *new_bi;
};
                    ...
                    afi_t afi, safi_t safi)
{
    ...
    result->old_bi = result->new_bi = NULL;
    if (rn->info == NULL)
    {
        char buf[PREFIX_STRLEN];
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: Called for route_node %s with no routing entries!",
                   __func__,
    ...
    result->old_bi = old_select;
    result->new_bi = new_select;
	
    return;
}

int bgp_process_announce_selected (struct peer *peer, struct bgp_info *selected,
                               struct bgp_node *rn, afi_t afi, safi_t safi)
{
    ...
    if((p->family == AF_INET  && IPV4_NET127(ntohl(p->u.prefix4.s_addr)))
		||(p->family == AF_INET6  && ipv6_is_loopback((struct ipv6_addr *)&(p->u.prefix6))))
    {
        return 0;
    }
	...
	//{
    	zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s %s[%d] %s/%d peer->afc_nego[%d][%d] %d vrf_id %d",
               peer->host,__FUNCTION__,__LINE__,
               inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
               p->prefixlen,afi,safi,peer->afc_nego[afi][safi],peer->vrf_id);
	//}
    ...
    switch (bgp_node_table (rn)->type)
    {
		...
		default:
			{
				zlog_err("%s[%d] type %d ", __func__, __LINE__, bgp_node_table (rn)->type);
			}
			break;
    }
    bgp_attr_flush (&attr);
...
bgp_process_rsclient (struct work_queue *wq, void *data)
{
    ...
    new_select = old_and_new.new_bi;
    old_select = old_and_new.old_bi;
...
{	
	...
	{
		if(CHECK_FLAG(ri->flags,BGP_INFO_TARGET))
		{
			UNSET_FLAG(ri->flags,BGP_INFO_TARGET);
			flag = 1;
		}
	}
	...
	{	    
		struct bgp_node *prn ;
		struct bgp_vrf_config *bgp_vrf;
   	  
		prn = rn->prn;
		bgp_vrf = bgp_vrf_get_by_prd((struct prefix_rd * )&prn->p);
		if(bgp_vrf)
		{
			vrf_id = bgp_vrf->vpn_id;
		}

		/*in tag is not null or out tag is not null*/
		if(old_select && old_select->extra 
			&& (! BGP_TAG_IS_EMPTY(old_select->extra->tag) || ! BGP_TAG_IS_EMPTY(old_select->extra->in_tag)))
		{
			if( old_select->attr && old_select->attr->extra && old_select->attr->extra->ecommunity)
			{
				for (int i = BGP_VRF_MIN ; i < BGP_VRF_MAX + 1; i ++)
				{
					if(bgp_vrf_redist_info[i] == NULL)
					{
						continue;
					}
					
					if(bgp_vrf_redist_info[i]->vpn_id == 0)
					{
						continue;
					}
					
					if(!bgp_vrf_redist_info[i]->rt_import)
					{
						continue;
					}
					
					if(ecommunity_match(bgp_vrf_redist_info[i]->rt_import,old_select->attr->extra->ecommunity))
					{
						old_select->vrf_id_to = i;
						bgp_zebra_announce (p, old_select, bgp, safi,IPC_OPCODE_DELETE, 1);
					}
				}
			}
		...
		/*in tag is not null or out tag is not null*/
		if(new_select && new_select->extra 
			&& (! BGP_TAG_IS_EMPTY(new_select->extra->tag) || ! BGP_TAG_IS_EMPTY(new_select->extra->in_tag)))
		{
			if(new_select->attr && ((new_select->attr->extra && new_select->attr->extra->ecommunity) || flag ))
			{
				for (int i = BGP_VRF_MIN ; i < BGP_VRF_MAX + 1; i ++)
				{
					if(bgp_vrf_redist_info[i] == NULL)
					...
					if(bgp_vrf_redist_info[i]->rt_import || flag)
					{
						if((bgp_vrf_redist_info[i]->rt_import && ecommunity_match(bgp_vrf_redist_info[i]->rt_import,new_select->attr->extra->ecommunity))
							|| ( flag &&  BGP_TAG_IS_EMPTY(new_select->extra->tag) && ! BGP_TAG_IS_EMPTY(new_select->extra->in_tag)))
						{
							new_select->vrf_id_to = i;
							bgp_zebra_announce (p, new_select, bgp, safi,IPC_OPCODE_ADD, 1);
						}
					}
	    ...
	    {			
	    	if(!peer->vrf_id)
    		{
    			if(vrf_id && new_select && new_select->extra && (! BGP_TAG_IS_EMPTY(new_select->extra->tag)))
				{
					new_select->vrf_id_to = vrf_id;
				}
				
				if(peer->afc[afi][safi] && peer->afc_recv[afi][safi])
				{
	        		bgp_process_announce_selected (peer, new_select, rn, afi, safi);
				}
				
				if(new_select && CHECK_FLAG(new_select->flags,BGP_INFO_DELETE))
				{
					UNSET_FLAG(new_select->flags,BGP_INFO_DELETE);
				}
    		}
			else
			{	
				if((old_select && old_select->vrf_id == peer->vrf_id)
  				   ||(new_select && new_select->vrf_id == peer->vrf_id))
				{
					bgp_process_announce_selected (peer, new_select, rn, afi, SAFI_UNICAST);
				}
				else
				{
		            if (old_select && bgp_vrf_redist_info[peer->vrf_id] && ecommunity_match(bgp_vrf_redist_info[peer->vrf_id]->rt_import, old_select->attr->extra->ecommunity))
					{						
		                bgp_process_announce_selected (peer, NULL, rn, afi, SAFI_UNICAST);
		            }
		            
		            if (new_select && bgp_vrf_redist_info[peer->vrf_id] && ecommunity_match(bgp_vrf_redist_info[peer->vrf_id]->rt_import, new_select->attr->extra->ecommunity))
					{
		                bgp_process_announce_selected (peer, new_select, rn, afi, SAFI_UNICAST);
		            }
				}
		...
		if(old_select && new_select == NULL)
		{
			if(old_select->extra && !BGP_TAG_IS_EMPTY(old_select->extra->in_tag))
			{
				if(old_select->label_apply == LABEL_APPLY_PER_ROUTE)
				{
					inlabel = decode_label(old_select->extra->in_tag);
					bgp_com_label_free(old_select->vrf_id, inlabel);
				}
    ...
    if ((safi == SAFI_UNICAST || safi == SAFI_MULTICAST 
		|| (safi == SAFI_MPLS_VPN && ((old_select && old_select->vrf_id && old_select->extra && BGP_TAG_IS_EMPTY(old_select->extra->tag) && BGP_TAG_IS_EMPTY(old_select->extra->in_tag ) )
								   || (new_select && new_select->vrf_id && new_select->extra && BGP_TAG_IS_EMPTY(new_select->extra->tag) && BGP_TAG_IS_EMPTY(new_select->extra->in_tag)))) )
		&& (! bgp->name && ! bgp_option_check (BGP_OPT_NO_FIB)))
    {
        if (old_select
            && old_select->type == ROUTE_PROTO_EBGP
            && withdraw
            && ( BGP_ROUTE_NORMAL == old_select->sub_type || BGP_ROUTE_AGGREGATE == old_select->sub_type))
        {
            	bgp_zebra_announce (p, old_select, bgp, safi,IPC_OPCODE_DELETE, 0);
        }
		
        if (new_select
            && new_select->type == ROUTE_PROTO_EBGP
            && (BGP_ROUTE_NORMAL == new_select->sub_type || BGP_ROUTE_AGGREGATE == new_select->sub_type))
        {
            	bgp_zebra_announce (p, new_select, bgp, safi,IPC_OPCODE_ADD, 0);
        }
    }
...
static int
bgp_maximum_prefix_restart_timer (void *arg)
{
    struct peer *peer = NULL;
	
    peer = (struct peer *)(arg);
    peer->t_pmax_restart = 0;
	
    //if (BGP_DEBUG (events, EVENTS))
	//{
        zlog_debug (BGP_DEBUG_TYPE_EVENTS,"%s Maximum-prefix restart timer expired, restore peering",
                    peer->host);
	//}
                             ...
                             safi_t safi, int always)
{
	...
	if(peer->vrf_id || peer->afc[afi][SAFI_MPLS_VPN])
	{
		safi_new = SAFI_MPLS_VPN;
	}
	
    if (peer->pcount[afi][safi_new] > peer->pmax[afi][safi])
    {
        ...
        zlog (peer->log, LOG_INFO,
              ...
              "limit %ld", afi_safi_print (afi, safi_new), peer->host,
              peer->pcount[afi][safi_new], peer->pmax[afi][safi]);
        ...
        if (peer->pmax_restart[afi][safi])
        {
            ...
            //if (BGP_DEBUG (events, EVENTS))
                zlog_debug (BGP_DEBUG_TYPE_EVENTS,"%s Maximum-prefix restart timer started for %d secs",
                            peer->host, peer->v_pmax_restart);

         //peer->t_pmax_restart = high_pre_timer_add("bgp_maximum_prefix_restart", LIB_TIMER_TYPE_NOLOOP, bgp_maximum_prefix_restart_timer, (void*)peer, (peer->v_pmax_restart) * 1000);
    ...
    if (peer->pcount[afi][safi_new] > (peer->pmax[afi][safi] * peer->pmax_threshold[afi][safi] / 100))
    {
        ...
        zlog (peer->log, LOG_INFO,
              ...
              afi_safi_print (afi, safi_new), peer->host, peer->pcount[afi][safi_new],
              peer->pmax[afi][safi]);
                     ...
                     int sub_type, struct prefix_rd *prd, u_char *tag)
{
    ...
    new_bi = bgp_info_new ();
    new_bi->type = type;
    new_bi->sub_type = sub_type;
    new_bi->peer = peer;
    new_bi->attr = attr_new;
    new_bi->uptime = bgp_clock ();
	
    /* Update MPLS tag. */
    if (safi == SAFI_MPLS_VPN)
    {
        memcpy ((bgp_info_extra_get (new_bi))->tag, tag, 3);
    }
	
    bgp_info_set_flag (rn, new_bi, BGP_INFO_VALID);
	
    /* Register new BGP information. */
    bgp_info_add (rn, new_bi);
                 ...
                 struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
	...
	if(peer->vrf_id)
	{
    	...
    	rn = bgp_afi_node_get (bgp->rib[afi][SAFI_MPLS_VPN], afi, SAFI_MPLS_VPN, p, &bgp_vrf_redist_info[peer->vrf_id]->prd);
	}
	else
	{
		if(AFI_IP == afi && SAFI_MPLS_VPN == safi)
		{
			vrf_id = bgp_vrf_get_rd(prd);
			if(vrf_id)
			{
				rn = bgp_afi_node_get (bgp->rib[afi][safi], afi, safi, p, &bgp_vrf_redist_info[vrf_id]->prd);
			}
			else
			{
				...
				if(attr->extra 
					...
					&& ((vrf_id = bgp_match_ecommunity(attr->extra->ecommunity)))
					&& bgp_vrf_redist_info[vrf_id] 
					&& bgp_vrf_redist_info[vrf_id]->prd.prefixlen == 64)
				{
					rn = bgp_afi_node_get (bgp->rib[afi][safi], afi, safi, p, &bgp_vrf_redist_info[vrf_id]->prd);
					memcpy(&bgp_vrf_redist_info[vrf_id]->prd_src, prd, sizeof(struct prefix_rd));
				}
				else
				{
	...
	if(AFI_IP == afi && SAFI_MPLS_VPN == safi)
	{
		if(!(attr->extra && attr->extra->ecommunity && ((vrf_id = bgp_match_ecommunity(attr->extra->ecommunity)))
			&& bgp_vrf_redist_info[vrf_id] && bgp_vrf_redist_info[vrf_id]->prd.prefixlen == 64))
		{
    ...
    if (afi == AFI_IP && safi == SAFI_UNICAST)
    {
        ...
        if (peer->sort == BGP_PEER_EBGP && peer->ttl == 1
            && ! bgp_nexthop_onlink (afi, &new_attr, peer->vrf_id)
            && ! CHECK_FLAG (peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)
            && ! CHECK_FLAG (peer->flags, PEER_FLAG_LOOPBACK))
        {
	...
	if(peer->vrf_id)
	{
		bgp_attr_extra_get(&new_attr);
		if(bgp_vrf_redist_info[peer->vrf_id] && bgp_vrf_redist_info[peer->vrf_id]->rt_export)
		{
	    	new_attr.extra->ecommunity = bgp_vrf_redist_info[peer->vrf_id]->rt_export;
			SET_FLAG(new_attr.flag, ATTR_FLAG_BIT (BGP_ATTR_EXT_COMMUNITIES));
		}
	}
    ...
    if (ri)
    {
        ...
        if (!CHECK_FLAG (ri->flags, BGP_INFO_REMOVED)
            && attrhash_cmp (ri->attr, attr_new)
            && (tag == NULL || memcmp((bgp_info_extra_get (ri))->tag,tag,3) == 0))
        {
            bgp_info_unset_flag (rn, ri, BGP_INFO_ATTR_CHANGED);
            ...
            {
                if (BGP_DEBUG (update, UPDATE_IN))
                    zlog (peer->log, LOG_DEBUG, "%s %s[%d] safi %d rcvd %s/%d nexthop 0x%x",
                          peer->host,__FUNCTION__,__LINE__, safi,
                          inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
                          p->prefixlen,
                          ntohl(attr_new->nexthop.s_addr));
        ...
        if (BGP_DEBUG (update, UPDATE_IN))
    	{
            zlog (peer->log, LOG_DEBUG, "%s %s[%d] safi %d rcvd %s/%d",
                  peer->host,__FUNCTION__,__LINE__, safi,
                  inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
                  p->prefixlen);
    	}
		...
		if(SAFI_MPLS_VPN == safi || peer->vrf_id)
		{
			if(CHECK_FLAG(ri->flags, BGP_INFO_SELECTED))
			{
        		bgp_zebra_announce (p, ri, bgp, safi,IPC_OPCODE_DELETE, 1);
		...
		{
        	bgp_zebra_announce (p, ri, bgp, safi,IPC_OPCODE_DELETE, 0);
		}
		...
		{			
			uint32_t inlabel = 0;
			label_decode((bgp_info_extra_get (ri))->in_tag, inlabel);
		}
        ...
        memset (&q, 0, sizeof (struct prefix));
        memset (&d, 0, sizeof (struct prefix));
		if(afi == AFI_IP)
		{
		    q.family = AF_INET;
		    q.prefixlen = IPV4_MAX_BITLEN;
		    q.u.prefix4 = ri->attr->nexthop;
		...
		else if(afi == AFI_IP6)
		{
			...
			{
				memcpy(&q.u.prefix6, &ri->attr->extra->mp_nexthop_global, IPV6_MAX_BYTELEN);
			}
        ...
        if ((afi == AFI_IP || afi == AFI_IP6)
            ...
            && (peer->sort == BGP_PEER_IBGP
                ...
                || CHECK_FLAG (peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)))
        {
            if (bgp_nexthop_lookup (afi, peer, ri, p, NULL))
            {
                bgp_info_set_flag (rn, ri, BGP_INFO_VALID);
            }
            else
		...
		if(((p->family == AF_INET && p->u.prefix4.s_addr) 
		     || (p->family == AF_INET6 && (p->u.prefix6.s6_addr32[0] | p->u.prefix6.s6_addr32[1] | p->u.prefix6.s6_addr32[2] | p->u.prefix6.s6_addr32[3])))
		       	&& prefix_match(p,&q) 
		       	&& ((d.family == AF_INET && d.u.prefix4.s_addr) 
		       	   || (d.family == AF_INET6 && (d.u.prefix6.s6_addr32[0] | d.u.prefix6.s6_addr32[1] | d.u.prefix6.s6_addr32[2] | d.u.prefix6.s6_addr32[3]))))
		{
			if(prefix_match(&d,p) || (CHECK_FLAG(peer->flags, PEER_FLAG_LOOPBACK) 
				                      && ((p->family == AF_INET && p->prefixlen == IPV4_MAX_PREFIXLEN) || (p->family == AF_INET6 && p->prefixlen == IPV6_MAX_PREFIXLEN))))
			{
	        	bgp_info_unset_flag (rn, ri, BGP_INFO_VALID);
			}
		}
    ...
    if (BGP_DEBUG (update, UPDATE_IN))
    {
        zlog (peer->log, LOG_DEBUG, "%s %s[%d] safi %d rcvd vrf %d %s/%d",
              peer->host,__FUNCTION__,__LINE__,safi,peer->vrf_id,
              inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
              p->prefixlen);
    }
    ...
    new_bi->attr = attr_new;
    new_bi->uptime = bgp_clock ();
	new_bi->vrf_id = peer->vrf_id;
	new_bi->vrf_id_to = vrf_id;
	...
	else if(afi == AFI_IP6)
	{
		...
		if(attr_new->extra)
		{
			memcpy(&q.u.prefix6, &attr_new->extra->mp_nexthop_global, IPV6_MAX_BYTELEN);
		}
	...
	{	
		uint32_t inlabel = 0;
		label_decode((bgp_info_extra_get (new_bi))->in_tag, inlabel);
	}
    ...
    if ((afi == AFI_IP || afi == AFI_IP6)
        ...
        && (peer->sort == BGP_PEER_IBGP
            ...
            || CHECK_FLAG (peer->flags, PEER_FLAG_LOOPBACK)))
    {
        if (bgp_nexthop_lookup (afi, peer, new_bi, p, NULL))
        {
            bgp_info_set_flag (rn, new_bi, BGP_INFO_VALID);
        }
        else
        {
            bgp_info_unset_flag (rn, new_bi, BGP_INFO_VALID);
        }
    }
    else
    {
    ...
    if (!CHECK_FLAG (new_bi->flags, BGP_INFO_COUNTED))
    {
        struct bgp_table *table;
        if(rn && bgp_node_table (rn))
        {
            table = bgp_node_table (rn);
            SET_FLAG (new_bi->flags, BGP_INFO_COUNTED);
            new_bi->peer->pcount[table->afi][table->safi]++;
        }
    }
	
    /* Increment prefix */
    bgp_aggregate_increment (bgp, p, new_bi, afi, safi);
...
filtered:
    ...
    if (ri)
    {
    	if(peer->vrf_id)
		{
			bgp_rib_remove (rn, ri, peer, afi, SAFI_MPLS_VPN);
		}
		else
		{
        	bgp_rib_remove (rn, ri, peer, afi, safi);
              ...
              struct prefix_rd *prd, u_char *tag)
{
    ...
    if (BGP_DEBUG (update, UPDATE_IN))
	{
        zlog (peer->log, LOG_DEBUG, "%s safi %d vrf %d rcvd UPDATE about %s/%d -- withdrawn",
              peer->host, safi, peer->vrf_id ? peer->vrf_id : vrf_id,
              inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
              p->prefixlen);
	}
    ...
    if (ri && ! CHECK_FLAG (ri->flags, BGP_INFO_HISTORY))
    {
    	if(peer->vrf_id)
		{
			bgp_rib_withdraw (rn, ri, peer, afi, SAFI_MPLS_VPN, prd);
		}
		else
		{
        	bgp_rib_withdraw (rn, ri, peer, afi, safi, prd);
...
/**************************************************************
1 update_attr: mofify local_pref, then update default originate
2 default originate 优先级高于network 0.0.0.0
***************************************************************/
void
bgp_default_originate (struct peer *peer, afi_t afi, safi_t safi, int withdraw,int update_attr)
{
    struct bgp *bgp;
    struct attr attr;
    ...
    struct bgp_node *bn = NULL;
    struct bgp_info *ri;
    struct bgp_info *bi = NULL;
    ...
    bn = bgp_node_lookup (bgp->rib[afi][safi], &q);
    if(bn)
    {
        if((bi = bn->info))
        {
    ...
    //if(BGP_DEBUG (update, UPDATE_OUT))
    //{
        zlog_debug(BGP_DEBUG_TYPE_UPDATE,"%s[%d] af_sflags 0x%x withdraw %d update %d update_attr %d",__FUNCTION__,__LINE__,
                   peer->af_sflags[afi][safi],withdraw,update,update_attr);
    //}
    ...
    if(bgp->default_med && peer->sort == BGP_PEER_EBGP)
    {
        attr.med = bgp->default_med;
    }
    else
    ...
    {
        if (update_attr || ! CHECK_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_DEFAULT_ORIGINATE))
        {
                    ...
                    struct bgp_table *table, int rsclient)
{
    ...
    {
        bgp_default_originate (peer, afi, safi, 0, 1);
    }
    ...
    {
        if(IPV4_NET127(ntohl(rn->p.u.prefix4.s_addr)))
        {
            continue;
        }
        ...
        for (ri = rn->info; ri; ri = ri->next)
    	{
            if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED) && (ri->peer != peer || peer_count > 1))
            {
                if ( (rsclient) ?
...
static void
bgp_announce_table_vrf (struct peer *peer, afi_t afi, safi_t safi)
{
    struct bgp_node *rn;
    struct bgp_info *ri;
    struct attr attr;
    struct attr_extra extra;
	struct bgp_node *prn;
	struct bgp_table *table_vrf = NULL;
	...
	memset(&attr,0x0,sizeof(struct attr));
    memset(&extra, 0, sizeof(extra));
	...
	prn = bgp_node_get (peer->bgp->rib[afi][SAFI_MPLS_VPN], (struct prefix *) &(bgp_vrf_redist_info[peer->vrf_id]->prd));
	table_vrf = prn->info;
	if(NULL == table_vrf)
	{
		bgp_unlock_node(prn);
	...
	{
		attr.extra = &extra;
		for (rn = bgp_table_top (table_vrf); rn; rn = bgp_route_next(rn))
		{
			for (ri = rn->info; ri; ri = ri->next)
			{
				if(IPV4_NET127(ntohl(rn->p.u.prefix4.s_addr)))
				...
				{
					if (bgp_announce_check (ri, peer, &rn->p, &attr, afi, SAFI_UNICAST))
					{
						bgp_adj_out_set (rn, peer, &rn->p, &attr, afi, SAFI_UNICAST, ri);
					}
					else
					{
						bgp_adj_out_unset (rn, peer, &rn->p, afi, SAFI_UNICAST);
					}
				}
			}
		}
    ...
    memset(&extra, 0, sizeof(extra));
	attr.extra = &extra;
	for (prn = bgp_table_top (peer->bgp->rib[afi][SAFI_MPLS_VPN]); prn; prn = bgp_route_next(prn))
	{
		if ((table = (prn->info)) != NULL && table != table_vrf)
		{
			for (rn = bgp_table_top (table); rn; rn = bgp_route_next(rn))
			{
				for (ri = rn->info; ri; ri = ri->next)
				{       
					...
					if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED) 
						&& ri->peer != peer 
					    && ecommunity_match(bgp_vrf_redist_info[peer->vrf_id]->rt_import, ri->attr->extra->ecommunity))
					{
						if (bgp_announce_check (ri, peer, &rn->p, &attr, afi, SAFI_UNICAST))
						{
							bgp_adj_out_set (rn, peer, &rn->p, &attr, afi, SAFI_UNICAST, ri);
...
bgp_announce_route (struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if(peer->vrf_id)
	{
		bgp_announce_table_vrf(peer,afi, safi);
		return;
	}
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
	{
        for (ain = rn->adj_in; ain; ain = ain->next)
        {
            ...
            bgp_update_rsclient (rsclient, afi, safi, ain->attr, ain->peer,
                                 &rn->p, ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, prd, tag);
        }
	}
}
        ...
        for (ain = rn->adj_in; ain; ain = ain->next)
        {
            if (ain->peer == peer)
            {
                ...
                ret = bgp_update (peer, &rn->p, ain->attr, afi, safi,
                                  ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL,
                                  prd, tag, 1);
                if (ret < 0)
                {
                    bgp_unlock_node (rn);
...
bgp_drain_workqueue_immediate (struct work_queue *wq)
{
    ...
    while (wq->items->count)
    {
        if (wq->queue_id)
        {
            high_pre_timer_delete(wq->queue_id);
            wq->queue_id = 0;
        }
...
static void
bgp_cleanup_table(struct bgp_table *table, struct bgp *bgp, safi_t safi)
{
    struct bgp_node *rn;
    struct bgp_info *ri;
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
	{
        for (ri = rn->info; ri; ri = next)
        {
            next = ri->next;
            if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED)
                && ri->type == ROUTE_PROTO_EBGP
                && (ri->sub_type == BGP_ROUTE_NORMAL
                    || ri->sub_type == BGP_ROUTE_AGGREGATE ))
            {
                bgp_zebra_announce (&rn->p, ri, bgp, safi,IPC_OPCODE_DELETE, 0);
            }
        }
	}
}
...
bgp_cleanup_routes (void)
{
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        ...
        {
            struct bgp_node *rn;
            bgp_cleanup_table(bgp->rib[afi][SAFI_UNICAST],bgp, SAFI_UNICAST);
                    ...
                    rn = bgp_route_next (rn))
            {
                ...
                {
                	table = rn->info;
                    bgp_cleanup_table(table , bgp, SAFI_MPLS_VPN);
                    ...
                    rn = bgp_route_next (rn))
            {
                ...
                {
                	table = rn->info;
                    bgp_cleanup_table(table, bgp, SAFI_ENCAP);
                    bgp_table_finish (&table);
                    rn->info = NULL;
                    bgp_unlock_node(rn);
                }
    ...
    for (; pnt < lim; pnt += psize)
    {
        ...
        if (attr)
            ret = bgp_update (peer, &p, attr, packet->afi, packet->safi,
                              ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, NULL, NULL, 0);
        else
            ret = bgp_withdraw (peer, &p, attr, packet->afi, packet->safi,
                                ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, NULL, NULL);
...
bgp_static_new (void)
{
    return XCALLOC (MTYPE_BGP_ROUTE, sizeof (struct bgp_static));
}
...
bgp_static_free (struct bgp_static *bgp_static)
{
    ...
    XFREE (MTYPE_BGP_ROUTE, bgp_static);
}

static struct bgp_network *
bgp_network_new (void)
{
    return XCALLOC (MTYPE_BGP_NETWORK, sizeof (struct bgp_network));
}

static void
bgp_network_free (struct bgp_network *bgp_network)
{
    XFREE (MTYPE_BGP_NETWORK, bgp_network);
}
                            ...
                            afi_t afi, safi_t safi)
{
    ...
    attr.nexthop = bgp_static->nexthop;
    ...
    for (ri = rn->info; ri; ri = ri->next)
        if (ri->peer == bgp->peer_self && ri->type == ROUTE_PROTO_EBGP
                && ri->sub_type == BGP_ROUTE_STATIC)
            break;
    ...
    /* Make new BGP info. */
    new_bi = bgp_info_new ();
    new_bi->type = ROUTE_PROTO_EBGP;
    new_bi->sub_type = BGP_ROUTE_STATIC;
    new_bi->peer = bgp->peer_self;
    SET_FLAG (new_bi->flags, BGP_INFO_VALID);
    new_bi->attr = attr_new;
    new_bi->uptime = bgp_clock ();
                        ...
                        struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
	...
	if(afi == AFI_IP6)
	{
		...
		IPV6_ADDR_COPY(&extra->mp_nexthop_global,&bgp_static->nexthop_v6);
                        ...
                        struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
    ...
    SET_FLAG (new_bi->flags, BGP_INFO_VALID);
    new_bi->uptime = bgp_clock ();
    new_bi->extra = bgp_info_extra_new();
    memcpy (new_bi->extra->tag, bgp_static->tag, 3);
                ...
                afi_t afi, safi_t safi, const char *rmap, int backdoor)
{
    ...
    struct prefix p;
    struct bgp_static *bgp_static;
    struct bgp_network *bgp_network;
    struct bgp_node *rn;
    ...
    {
        vty_error_out (vty, "Malformed prefix (link-local address)%s",
                 VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    if (! ret)
    {
        vty_error_out(vty, "The route cannot be imported into the BGP routing-table.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    rn = bgp_node_get (bgp->network[afi][safi], &p);
	
    if(rn->info)
    {
        bgp_network = rn->info;
		
        if (bgp_network->valid && bgp_network->backdoor != backdoor)
        {
            need_update = 1;
        }
		
        bgp_network->backdoor = backdoor;
        bgp_unlock_node (rn);
    }
    else
    {
        bgp_network = bgp_network_new();
        bgp_network->valid = 0;
        bgp_network->backdoor = backdoor;
        rn->info = bgp_network;
    }
    ...
    {
        bgp_static = rn->info;
        if(ROUTE_PROTO_IBGP == bgp_static->type || ROUTE_PROTO_EBGP == bgp_static->type)
        {
            bgp_unlock_node (rn);
            return CMD_SUCCESS;
        }
    ...
    {
        vty_info_out(vty, "The network does not exist.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
                  ...
                  afi_t afi, safi_t safi)
{
    ...
    rn = bgp_node_lookup (bgp->network[afi][safi], &p);
    if (! rn)
    {
        return CMD_WARNING;
    }
    ...
    bgp_network = rn->info;
    backdoor = bgp_network->backdoor;
    bgp_network_free (bgp_network);
    rn->info = NULL;
    bgp_unlock_node (rn);
    bgp_unlock_node (rn);
...
bgp_static_delete (struct bgp *bgp)
{
    ...
    for (afi = AFI_IP; afi < AFI_MAX; afi++)
	{
        for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
    	{
            for (rn = bgp_table_top (bgp->network[afi][safi]); rn; rn = bgp_route_next (rn))
        	{
                if (rn->info != NULL)
                {
                    if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP))
                     ...
                     const char *rmap_str)
{
    ...
    if (! ret)
    {
        vty_error_out (vty, "Malformed rd%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    if (! ret)
    {
        vty_error_out (vty, "Malformed tag%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...
static void
bgp_aggregate_route (struct bgp *bgp, struct prefix *p, struct prefix *q, struct bgp_info *rinew,
                     ...
                     struct bgp_aggregate *aggregate)
{
    ...
    struct bgp_info *new_bi;
    struct bgp_info *bi = NULL;
    struct bgp_info *next = NULL;
    struct bgp_info *new_ri = NULL;
    ...
    for (rn = bgp_node_get (table, p); rn; rn = bgp_route_next_until (rn, top))
	{
        if (rn->p.prefixlen > p->prefixlen)
        {
            ...
            new_select = NULL;
            count = 0;
            for(new_ri = rn->info; (new_ri != NULL) && (next = new_ri->next, 1); new_ri = next)
            {
                if(BGP_INFO_HOLDDOWN (new_ri))
                {
                    continue;
                }
                if(prefix_same(&rn->p,q) && del && del == new_ri)
                {
                    continue;
                }
                count ++;
                if(bgp_info_cmp (bgp, new_ri, new_select, afi, safi) == -1)
                {
                    new_select = new_ri;
                }
            }
            if(rinew && prefix_same(&rn->p,q) && rn->info == NULL)
            {
                first = 1;
            }
            for (ri = rn->info; ri; ri = ri->next)
            {
                ...
                if(count > 0 && new_select && new_select != ri)
                {
                    continue;
                }
                if(rinew && prefix_same(&rn->p,q) && bgp_info_cmp (bgp, rinew, ri, afi, safi) == -1)
                {
                    bi = rinew;
                }
                else
                ...
                if (! IPV4_ADDR_SAME (&bi->attr->nexthop, &nexthop)
                        || bi->attr->med != med)
                {
                    if (aspath)
                    {
                ...
                if (bi->sub_type != BGP_ROUTE_AGGREGATE)
                {
                    if (aggregate->summary_only)
                    {
                        (bgp_info_extra_get (bi))->suppress++;
                        bgp_info_set_flag (rn, bi, BGP_INFO_ATTR_CHANGED);
                        match++;
                    }
                    aggregate->count++;
                    if(origin < bi->attr->origin)
                    {
                        origin = bi->attr->origin;
                    }
                    if (aggregate->as_set)
                    {
                        if (aspath)
                        {
                            asmerge = aspath_aggregate (aspath, bi->attr->aspath);
                            aspath_free (aspath);
                            aspath = asmerge;
                        }
                        else
                        {
                            aspath = aspath_dup (bi->attr->aspath);
                        }
                        if (bi->attr->community)
                        {
                            if (community)
                            {
                            ...
                            {
                                community = community_dup (bi->attr->community);
                            }
                        }
                    }
                }
    ...
    if (aggregate->count > 0)
    {
        ...
        new_bi = bgp_info_new ();
        new_bi->type = ROUTE_PROTO_EBGP;
        new_bi->sub_type = BGP_ROUTE_AGGREGATE;
        new_bi->peer = bgp->peer_self;
        SET_FLAG (new_bi->flags, BGP_INFO_VALID);
        new_bi->attr = bgp_attr_aggregate_intern (bgp, origin, aspath, community, aggregate->as_set);
        new_bi->uptime = bgp_clock ();
        bgp_info_add (rn, new_bi);
        bgp_unlock_node (rn);
        bgp_process (bgp, rn, afi, safi);
                         ...
                         struct bgp_info *ri, afi_t afi, safi_t safi)
{
    ...
    for (rn = child; rn; rn = bgp_node_parent_nolock (rn))
	{
        if ((aggregate = rn->info) != NULL && rn->p.prefixlen < p->prefixlen)
        {
            ...
            bgp_aggregate_route (bgp, &rn->p, p,ri, afi, safi, NULL, aggregate);
        }
	}
                         ...
                         struct bgp_info *del, afi_t afi, safi_t safi)
{
    ...
    for (rn = child; rn; rn = bgp_node_parent_nolock (rn))
	{
        ...
        {
            bgp_aggregate_delete (bgp, &rn->p, afi, safi, aggregate);
            bgp_aggregate_route (bgp, &rn->p, p, NULL, afi, safi, del, aggregate);
        }
	}
    bgp_unlock_node (child);
}
                     ...
                     afi_t afi, safi_t safi)
{
	...
	if(p.family == AF_INET)
	{
	    ...
	    if(!ret)
	    {
	        vty_error_out (vty, "The IP address or mask is invalid.%s", VTY_NEWLINE);
	        return CMD_WARNING;
	    }
	}
                   ...
                   u_char summary_only, u_char as_set)
{
    ...
    apply_mask (&p);
    if((p.family == AF_INET && p.u.prefix4.s_addr == 0) || (p.family == AF_INET6 && p.prefixlen == 0))
    {
        vty_error_out (vty, "The default route cannot be generated for the current command.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }

	if(p.family == AF_INET6)
	{
		if(ipv6_is_zero((struct ipv6_addr *)&(p.u.prefix6))
			|| ipv6_is_loopback((struct ipv6_addr *)&(p.u.prefix6))
			|| ipv6_is_multicast((struct ipv6_addr *)&(p.u.prefix6))
			|| ipv6_is_linklocal((struct ipv6_addr *)&(p.u.prefix6))
			|| ipv6_is_sitelocal((struct ipv6_addr *)&(p.u.prefix6)))
	    {
	        vty_error_out (vty, "The IP address or mask is invalid.%s", VTY_NEWLINE);
	        return CMD_WARNING;
	    }
    ...
    if (rn->info)
    {
        ...
        if (ret)
        {
            vty_error_out (vty, "deleting aggregate.%s", VTY_NEWLINE);
            bgp_unlock_node (rn);
            return CMD_WARNING;
        }
    }
...
DEFUN (aggregate_address_summary_only,
       ...
       "summary A.B.C.D/M summary-only",
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
...
DEFUN (aggregate_address_mask_summary_only,
       ...
       "summary A.B.C.D A.B.C.D summary-only",
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
       "Filter more specific routes from updates\n")
...
DEFUN (aggregate_address_mask_as_set,
       ...
       "summary A.B.C.D A.B.C.D as-set",
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
       "Generate AS set path information\n")
...
ALIAS (aggregate_address_as_set_summary,
       ...
       "summary A.B.C.D/M summary-only as-set",
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Filter more specific routes from updates\n"
       "Generate AS set path information\n")
...
DEFUN (aggregate_address_mask_as_set_summary,
       ...
       "summary A.B.C.D A.B.C.D as-set summary-only",
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
       "Generate AS set path information\n"
...
ALIAS (aggregate_address_mask_as_set_summary,
       ...
       "summary A.B.C.D A.B.C.D summary-only as-set",
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
       "Filter more specific routes from updates\n"
...
ALIAS (no_aggregate_address,
       ...
       "no summary A.B.C.D/M summary-only",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Filter more specific routes from updates\n")
...
ALIAS (no_aggregate_address,
       ...
       "no summary A.B.C.D/M as-set summary-only",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Generate AS set path information\n"
...
ALIAS (no_aggregate_address,
       ...
       "no summary A.B.C.D/M summary-only as-set",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Filter more specific routes from updates\n"
...
ALIAS (no_aggregate_address_mask,
       ...
       "no summary A.B.C.D A.B.C.D summary-only",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
...
ALIAS (no_aggregate_address_mask,
       ...
       "no summary A.B.C.D A.B.C.D as-set",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
...
ALIAS (no_aggregate_address_mask,
       ...
       "no summary A.B.C.D A.B.C.D as-set summary-only",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
...
ALIAS (no_aggregate_address_mask,
       ...
       "no summary A.B.C.D A.B.C.D summary-only as-set",
       NO_STR
       "Configure BGP aggregate entries\n"
       "Aggregate address\n"
       "Aggregate mask\n"
...
DEFUN (ipv6_aggregate_address_summary_only,
       ...
       "summary X:X::X:X/M summary-only",
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Filter more specific routes from updates\n")
{
...
ALIAS (ipv6_aggregate_address_as_set_summary,
       ipv6_aggregate_address_summary_as_set_cmd,
       "summary X:X::X:X/M summary-only as-set",
       "Configure BGP aggregate entries\n"
       "Aggregate prefix\n"
       "Filter more specific routes from updates\n"
       "Generate AS set path information\n")
...
bgp_redistribute_add (struct prefix *p, const struct in_addr *nexthop,
                      ...
                      u_int32_t metric, u_char type, u_int32_t ifindex,uint8_t instance)
{
    struct bgp *bgp;
    struct listnode *node, *nnode;
    struct bgp_info *new_bi;
    ...
    struct attr *new_attr;
    afi_t afi;
    int ret ,type_redis = 0;
    struct ifm_l3  l3 = {0};
    u_char flag = 0 ;
	
    if((p->family == AF_INET && prefix_ipv4_any ((struct prefix_ipv4 *) p)) || (p->family == AF_INET6 && p->prefixlen == 0))
    {
        return;
    }
    ...
    if(ROUTE_PROTO_CONNECT == type && ((IPV4_MAX_BITLEN == p->prefixlen && p->family == AF_INET)||(IPV6_MAX_BITLEN == p->prefixlen && p->family == AF_INET6)))
    {
        ret = ifm_get_l3if(ifindex, MODULE_ID_BGP, &l3);
        if((ret == 0) && ((IPV4_MAX_BITLEN != l3.ipv4[0].prefixlen && p->family == AF_INET) || (IPV6_MAX_BITLEN != l3.ipv6[0].prefixlen && p->family == AF_INET6)))
        {
            attr.flag |= ATTR_FLAG_BIT (BGP_ATTR_SEND_CONNECT);
        }
    }
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        ...
        if(ROUTE_PROTO_OSPF == type || ROUTE_PROTO_ISIS == type || ROUTE_PROTO_RIP == type || ROUTE_PROTO_RIPNG == type ||ROUTE_PROTO_OSPF6 == type ||ROUTE_PROTO_ISIS6 == type )
        {
        ...
        if (flag)
        {
            ...
            memset (&attr_new, 0, sizeof (struct attr));
            memset (&extra_new, 0, sizeof (struct attr_extra));
			...
			if(ROUTE_PROTO_OSPF == type || ROUTE_PROTO_ISIS == type || ROUTE_PROTO_RIP == type || ROUTE_PROTO_RIPNG == type ||ROUTE_PROTO_OSPF6 == type ||ROUTE_PROTO_ISIS6 == type )
            {
                ...
                {
                    attr_new.med = bgp->redist_instance[type_redis][instance].redist_instance_metric;
                }
            }
            else if (bgp->redist_metric_flag[afi][type])
            {
            ...
            new_bi = bgp_info_new ();
            new_bi->type = type;
            new_bi->sub_type = BGP_ROUTE_REDISTRIBUTE;
            new_bi->instance = instance;
            new_bi->peer = bgp->peer_self;
            SET_FLAG (new_bi->flags, BGP_INFO_VALID);
            new_bi->attr = new_attr;
            new_bi->uptime = bgp_clock ();
			
            bgp_aggregate_increment (bgp, p, new_bi, afi, SAFI_UNICAST);
            bgp_info_add (bn, new_bi);
            bgp_unlock_node (bn);
            bgp_process (bgp, bn, afi, SAFI_UNICAST);
        }
    }
...
void
bgp_redistribute_delete (struct prefix *p, u_char type,uint8_t instance)
{
    afi_t afi;
    struct bgp *bgp;
    struct listnode *node, *nnode;
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        ...
        if (flag)
        {
            rn = bgp_node_lookup (bgp->rib[afi][SAFI_UNICAST], p);
			if(rn)
			{
	            for (ri = rn->info; ri; ri = ri->next)
	        	{
...
void
bgp_redistribute_vrf_add (struct prefix *p, const struct in_addr *nexthop, u_int32_t metric, u_char type, u_int32_t ifindex,uint8_t instance,u_int16_t vrf_id)
{
    int   ret = 0;
    afi_t afi = AFI_IP;
    ...
    struct ifm_l3  l3 = {0};
    u_char flag = 0 ;
	u_int32_t redist_metric = 0 ;
	
	if(vrf_id == 0 || bgp_vrf_redist_info[vrf_id] == NULL)
		return;
    ...
    attr.med = metric;
    attr.flag |= ATTR_FLAG_BIT (BGP_ATTR_MULTI_EXIT_DISC);
    if(ROUTE_PROTO_CONNECT == type && IPV4_MAX_BITLEN == p->prefixlen)
    {
        ret = ifm_get_l3if(ifindex, MODULE_ID_BGP, &l3);
        if((ret == 0) && IPV4_MAX_BITLEN != l3.ipv4[0].prefixlen)
        {
            attr.flag |= ATTR_FLAG_BIT (BGP_ATTR_SEND_CONNECT);
        }
    }
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
		...
		/*static and connect*/
       	if(ROUTE_PROTO_STATIC == type && bgp_vrf_redist_info[vrf_id]->redist[0])
   		{
			if(bgp_vrf_redist_info[vrf_id]->redist_metric_flag[0])
				redist_metric = bgp_vrf_redist_info[vrf_id]->redist_metric[0];
			
   			flag = 1;
   		}
       	else if(ROUTE_PROTO_CONNECT == type && bgp_vrf_redist_info[vrf_id]->redist[1])
   		{			
   			if(bgp_vrf_redist_info[vrf_id]->redist_metric_flag[1])
				redist_metric = bgp_vrf_redist_info[vrf_id]->redist_metric[1];
			
   			flag = 1;
   		}
		else if(bgp_redistribute_vrf_lookup_by_instance(vrf_id,instance,type))
		{
			flag = 1;
		}
        ...
        if (flag)
        {
			...
			if(redist_metric)
			{
                attr_new.med = redist_metric;
            }
			
			bn = bgp_afi_node_get (bgp->rib[afi][safi], afi, safi, p, &bgp_vrf_redist_info[vrf_id]->prd); 
...
void
bgp_redistribute_vrf_delete (struct prefix *p, struct bgp *bgp, afi_t afi, int type, uint8_t instance, u_int16_t vrf_id)
{
    int withdraw = 1;
    struct bgp_node *rn;
    struct bgp_info *ri;

	if(bgp_vrf_redist_info[vrf_id] == NULL)
	{
		return;
	}
	...
	rn = bgp_afi_node_get (bgp->rib[afi][SAFI_MPLS_VPN], afi, SAFI_MPLS_VPN, p, &bgp_vrf_redist_info[vrf_id]->prd);
	if(rn)
	{
		for (ri = rn->info; ri; ri = ri->next)
		{
		...
		if (ri)
		{
			bgp_aggregate_decrement (bgp, p, ri, afi, SAFI_MPLS_VPN);
			bgp_info_delete (rn, ri);
			bgp_process (bgp, rn, afi, SAFI_MPLS_VPN);
		}
...
int bgp_fsm_change_base_interface(struct prefix *p, int flag , uint16_t vrf_id)
{
    struct bgp *bgp;
    struct peer *peer;
    struct listnode *node, *nnode;
    ...
    if((p->family == AF_INET && p->prefixlen != IPV4_MAX_BITLEN) || (p->family == AF_INET6 && p->prefixlen != IPV6_MAX_BITLEN))
    {
        return 0;
    }
    ...
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if(p->family != peer->su.sa.sa_family)
        {
            continue;
        }
		
        if(((p->family == AF_INET && p->u.prefix4.s_addr == peer->su_local.sin.sin_addr.s_addr)
			|| (p->family == AF_INET6 && IPV6_ADDR_SAME(&p->u.prefix6,&peer->su_local.sin6.sin6_addr)))
		    && peer->vrf_id == vrf_id)
        {       
	        //if(BGP_DEBUG (events, EVENTS))
	        //{
				memset(buf, 0x0, SU_ADDRSTRLEN);
	            zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] su_local %s flag %d", __FUNCTION__, __LINE__,sockunion2str(&peer->su_local, buf, SU_ADDRSTRLEN),flag);
	        //}
...
int bgp_fsm_change_base_route(struct prefix *p, uint16_t vrf_id)
{
    struct bgp *bgp;
    struct peer *peer;
    struct listnode *node, *nnode;
    ...
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if(!peer->su.sin.sin_addr.s_addr)
        {
            continue;
        }
        ...
        if((p->u.prefix4.s_addr == (peer->su.sin.sin_addr.s_addr & mask.s_addr)) && peer->vrf_id == vrf_id)
        {       
	        //if(BGP_DEBUG (events, EVENTS))
	        //{
	            zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] %s su %s status %s", __FUNCTION__, __LINE__,peer->host,inet_ntoa(peer->su.sin.sin_addr),LOOKUP (bgp_status_msg, peer->status));
	        //}
...
void bgp_network_add(struct bgp *bgp, struct bgp_static *bgp_static, struct prefix *p, afi_t afi, safi_t safi )
{
    ...
    {	
    	//if(BGP_DEBUG (events, EVENTS))
		//{			
			memset(buf, 0x0, IPV6_ADDR_STRLEN);
			prefix2str(p, buf, IPV6_ADDR_STRLEN);
        	zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] afi %d safi %d p %s", __FUNCTION__, __LINE__, afi, safi, buf);
		//}
...
void bgp_network_delete(struct bgp *bgp, struct prefix *p, afi_t afi, safi_t safi )
{
    char buf[IPV6_ADDR_STRLEN];
    struct bgp_node *rn;
...
void
bgp_route_add (struct prefix *p, struct zapi_route *api)
{
    int ecmp = 0;
    afi_t afi = AFI_IP;
    ...
    struct bgp_node *bn;
    struct listnode *node, *nnode;
    struct bgp_static *bi = NULL;
    struct bgp_static *ci = NULL;
    struct bgp_static *pi = NULL;
    char buf[2][IPV6_ADDR_STRLEN];
	
    for(ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        afi = family2afi (p->family);
        bn = bgp_afi_node_get (bgp->route[afi][safi], afi, safi, p, NULL);
        for(bi = bn->info ; bi; bi = bi->next)
        {
			if(api->type == bi->type 
               && api->instance == bi->instance
               && api->metric == bi->igpmetric
               && api->ifindex == bi->ifindex
               &&((afi == AFI_IP && api->nexthop.s_addr == bi->nexthop.s_addr)
                  || (afi == AFI_IP6 && IPV6_ADDR_SAME(&api->nexthop_v6,&bi->nexthop_v6)))
               && api->vrf_id == bi->vrf_id)
            {
                bgp_unlock_node (bn);
                return;
            }
        ...
        bi = bgp_static_new ();
        bi->igpmetric = api->metric;
        bi->ifindex = api->ifindex;
        bi->type = api->type;
        bi->instance = api->instance;
        bi->distance = api->distance;
        bi->nhp_type = api->nhp_type;
		bi->vrf_id = api->vrf_id;
		if(afi == AFI_IP6)
		{
			IPV6_ADDR_COPY(&bi->nexthop_v6,&api->nexthop_v6);
		}
		else
		{
        	bi->nexthop.s_addr = api->nexthop.s_addr;
		}
		
        for(pi = NULL,ci = bn->info; ci; pi = ci, ci = ci->next)
        {			
            /*sort by distance metric*/
            if(bi->distance < ci->distance)
            {
                break;
            }
        ...
        /*consider ecmp*/
        if(pi && pi->distance == bi->distance && pi->igpmetric == bi->igpmetric && pi->vrf_id == bi->vrf_id)
        {
            if((pi->type != bi->type)||((pi->type == bi->type) && (pi->instance != bi->instance)))
            {
                ecmp = 1;
            }
        }
        ...
        //{
            ...
            zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] IPv4 add %s vrf %d %s nexthop %s metric %d ifindex 0x%x instance %d ecmp %d ",__FUNCTION__,__LINE__,
                       zebra_route_string(bi->type),bi->vrf_id, buf[0],
                       (p->family == AF_INET6) ? inet_ntop(AF_INET6, &bi->nexthop_v6, buf[1], sizeof(buf[1])):inet_ntop(AF_INET, &bi->nexthop, buf[1], sizeof(buf[1])),
                       bi->igpmetric,bi->ifindex,bi->instance,ecmp);
            if(pi)
                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] %s vrf %d nexthop %s metric %d ifindex 0x%x instance %d ",__FUNCTION__,__LINE__,
                           zebra_route_string(pi->type),pi->vrf_id,
                           (p->family == AF_INET6) ? inet_ntop(AF_INET6, &pi->nexthop_v6, buf[1], sizeof(buf[1])):inet_ntop(AF_INET, &pi->nexthop, buf[1], sizeof(buf[1])),
                           pi->igpmetric,pi->ifindex,pi->instance);
            if(ci)
                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] %s vrf %d nexthop %s metric %d ifindex 0x%x instance %d ",__FUNCTION__,__LINE__,
                           zebra_route_string(ci->type),ci->vrf_id,
                           (p->family == AF_INET6) ? inet_ntop(AF_INET6, &ci->nexthop_v6, buf[1], sizeof(buf[1])):inet_ntop(AF_INET, &ci->nexthop, buf[1], sizeof(buf[1])),
                           ci->igpmetric,ci->ifindex,ci->instance);
       // }
		
        if(ROUTE_PROTO_IBGP != api->type && ROUTE_PROTO_EBGP != api->type)
        {
			if(api->vrf_id)
			{
				if(afi == AFI_IP)
				{
					bgp_redistribute_vrf_add(p, &api->nexthop, api->metric, api->type, api->ifindex,api->instance,api->vrf_id);
				}
			}
			else
			{
	            if(ci && ci->vrf_id == bi->vrf_id && ecmp == 0 )
	            {
	                bgp_network_delete(bgp,p,afi,safi);
	                bgp_redistribute_delete (p, ci->type, ci->instance);
	            }
	            if((ci && (ci->vrf_id == bi->vrf_id)) || (!pi && !ci ) || (pi && pi->vrf_id != bi->vrf_id))
	            {
	                bgp_network_add(bgp,bi,p,afi,safi);

					if(afi == AFI_IP6)
					{
						bgp_redistribute_add (p, NULL, NULL, api->metric, api->type, api->ifindex,api->instance);
					}
					else
					{
						bgp_redistribute_add (p, &api->nexthop, NULL, api->metric, api->type, api->ifindex,api->instance);
					}
	            }
			}
		}
	
        if(ROUTE_PROTO_CONNECT == api->type)
        {
            bgp_fsm_change_base_interface(p, 1, api->vrf_id);
        }
		
		if(afi == AFI_IP  && (ROUTE_PROTO_CONNECT != api->type ||  IPV4_MAX_PREFIXLEN !=  p->prefixlen))
		{
        	bgp_fsm_change_base_route(p, api->vrf_id);
		}
    }
    return;
}
...
void
bgp_route_delete(struct prefix *p, struct zapi_route * api)
{
    int update = 0;
    afi_t afi = AFI_IP;
    safi_t safi = SAFI_UNICAST;
    ...
    struct bgp_static *bu = NULL;
    char buf[2][INET_ADDRSTRLEN];
	
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        afi = family2afi (p->family);
        bn = bgp_node_lookup (bgp->route[afi][safi], p);
        if(bn)
        {
            for(bi = bn->info; bi; bi = bi->next)
            {
                if(bi->type == api->type
                   && ((afi == AFI_IP && bi->nexthop.s_addr == api->nexthop.s_addr)
                       || (afi == AFI_IP6 && IPV6_ADDR_SAME(&bi->nexthop_v6,&api->nexthop_v6)))
                   && bi->instance == api->instance
                   && bi->distance == api->distance 
                   && bi->ifindex == api->ifindex 
                   && bi->igpmetric == api->metric
            ...
            /*not ecmp*/
            if(bi->next && (bi->next->distance != bi->distance || bi->next->igpmetric != bi->igpmetric))
            {
                update = 1;
                bu = bi->next;
            }
            /*ecmp*/
            else if(bi->next && bi->next->distance == bi->distance && bi->next->igpmetric == bi->igpmetric)
            {
                bu = bi->next;
                update = 1;
            }
            ...
            /*not ecmp*/
            if(bi->prev && (bi->prev->distance != bi->distance || bi->prev->igpmetric != bi->igpmetric))
            {
                update = 1;
                bu = bi->prev;
            }
            /*ecmp*/
            else if(bi->prev && bi->prev->distance == bi->distance && bi->prev->igpmetric == bi->igpmetric)
            {
                bu = bi->prev;
                update = 1;
            }
            ...
            //{
				prefix2str(p, buf[0], IPV6_ADDR_STRLEN);
                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] %s IPv4 addr %s nexthop %s instance %d update %d vrf %d",__FUNCTION__,__LINE__,
                           zebra_route_string(api->type),buf[0],
                           (afi == AFI_IP6)?inet_ntop(AF_INET6, &bi->nexthop_v6, buf[1], sizeof(buf[1])):inet_ntop(AF_INET, &bi->nexthop, buf[1], sizeof(buf[1])),
                           api->instance,update,api->vrf_id);
                if(bu)
                    zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] %s nexthop %s metric %d ifindex 0x%x instance %d vrf %d",__FUNCTION__,__LINE__,
                               zebra_route_string(bu->type),
                               (afi == AFI_IP6)?inet_ntop(AF_INET6, &bu->nexthop_v6, buf[1], sizeof(buf[1])):inet_ntop(AF_INET, &bu->nexthop, buf[1], sizeof(buf[1])),
                               bu->igpmetric,bu->ifindex,bu->instance,bi->vrf_id);
            //}
            ...
            if(ROUTE_PROTO_IBGP != api->type && ROUTE_PROTO_EBGP != api->type)
            {
				...
				if(api->vrf_id)
				{
					if(afi == AFI_IP)
					{
						bgp_redistribute_vrf_delete (p, bgp, afi, api->type,api->instance,api->vrf_id);
					}
				}
				else
				{
                	bgp_redistribute_delete (p, api->type,api->instance);
				}
				
                if(bu)
                {
                    bgp_network_add(bgp,bu,p,afi,safi);
                    if(update)
                    {
						if(afi == AFI_IP6)
						{
                        	bgp_redistribute_add (p, NULL, NULL, bu->igpmetric, bu->type, bu->ifindex,bu->instance);
						}
						else
						{						
							bgp_redistribute_add (p, &bu->nexthop, NULL, bu->igpmetric, bu->type, bu->ifindex,bu->instance);
						}
                    }
                }
            }
            ...
            if(ROUTE_PROTO_CONNECT == api->type)
            {
                bgp_fsm_change_base_interface(p, 0, api->vrf_id);
            }
...
void
bgp_route_delete_all ( struct bgp_table *table)
{
    struct bgp_node *bn = NULL;
    struct bgp_static *bi = NULL;
    struct bgp_static *next = NULL;
...
void bgp_redistribute_route_add_type (struct bgp *bgp, afi_t afi, u_char type,uint8_t instance,uint16_t vrf_id)
{
    struct bgp_node *rn;
    struct bgp_table *table;
    struct bgp_static *bgp_static;
	
    table = bgp->route[afi][SAFI_UNICAST];
    if(!table)
    {
        return;
    }
	...
	//if(BGP_DEBUG (events, EVENTS))
    //{
        zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d]  %s afi %d vrf %d instance %d ", __func__,__LINE__, zebra_route_string(type), afi, vrf_id, instance);
    //}
	
    for(rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
    {
        if((rn->p.family == AF_INET && rn->p.u.prefix4.s_addr == INADDR_ANY && rn->p.prefixlen == 0)
			|| (rn->p.family == AF_INET6 && rn->p.prefixlen == 0))
        {
            continue;
        }
        ...
        {
            if(ROUTE_PROTO_CONNECT == bgp_static->type && ROUTE_PROTO_CONNECT != type)
            {
                break;
            }
			
            if((bgp_static->type == type) && (bgp_static->instance == instance) && (vrf_id == bgp_static->vrf_id))
            {
            	if(vrf_id)
            	{
                	bgp_redistribute_vrf_add(&rn->p, &bgp_static->nexthop, bgp_static->igpmetric, type, bgp_static->ifindex,instance,vrf_id);
            	}
				else
				{
					if(rn->p.family == AF_INET6)
					{
                		bgp_redistribute_add(&rn->p, NULL, NULL, bgp_static->igpmetric, type, bgp_static->ifindex,instance);
					}
					else
					{
                		bgp_redistribute_add(&rn->p, &bgp_static->nexthop,NULL, bgp_static->igpmetric, type, bgp_static->ifindex,instance);
					}
				}
				break;
...
void
bgp_redistribute_route_delete_type (struct bgp *bgp, afi_t afi, int type ,uint8_t instance, uint16_t vrf_id)
{
	safi_t safi = SAFI_UNICAST;
    struct bgp_node *rn;
    struct bgp_info *ri;
	...
	{
		safi = SAFI_MPLS_VPN;
		prn = bgp_node_lookup(bgp->rib[afi][safi], (struct prefix *) &(bgp_vrf_redist_info[vrf_id]->prd));
		if( NULL == prn )
		{
			return ;
		}
...
route_vty_out_route (struct prefix *p, struct vty *vty)
{
    ...
    if (p->family == AF_INET)
    {
        ...
        if ((IN_CLASSC (destination) && p->prefixlen == 24)
            ...
            || (p->u.prefix4.s_addr && IN_CLASSA (destination) && p->prefixlen == 8)
    ...
    if (len < 1)
    {
        vty_out (vty, "%s%*s", VTY_NEWLINE, 22, " ");
    }
    else
    {
        vty_out (vty, "%*s", len, " ");
...
route_vty_short_status_out (struct vty *vty, struct bgp_info *binfo)
{
    ...
    else if (! CHECK_FLAG (binfo->flags, BGP_INFO_HISTORY) && CHECK_FLAG (binfo->flags, BGP_INFO_VALID))
    {
        vty_out (vty, "*");
    }
    else
    ...
    safi_t safi)
{
    ...
    if (attr)
    {
        ...
        if (p->family == AF_INET)
        {
            if((BGP_ROUTE_STATIC == binfo->sub_type)|| (BGP_ROUTE_REDISTRIBUTE == binfo->sub_type))
            {
                vty_out (vty, "%-17s","0.0.0.0");
            }
            else if(BGP_ROUTE_AGGREGATE == binfo->sub_type)
            {
                vty_out (vty, "%-17s","127.0.0.1");
            }
			else if (safi == SAFI_MPLS_VPN && attr->extra && attr->extra->mp_nexthop_global_in.s_addr)
			{
				vty_out (vty, "%-17s", inet_ntoa(attr->extra->mp_nexthop_global_in));
			}
            else
            {
                vty_out (vty, "%-17s", inet_ntoa (attr->nexthop));
            }
        }
        ...
        if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF))
        {
            vty_out (vty, "%-10u ", attr->local_pref);
        }
        else
        {
            vty_out (vty, "           ");
        }
        vty_out (vty, "%-10u ", (attr->extra ? attr->extra->weight : 0));
                   ...
                   struct bgp_info *binfo, int display, safi_t safi)
{
    ...
    if (attr)
    {
        if (p->family == AF_INET)
        {
			if((BGP_ROUTE_STATIC == binfo->sub_type)|| (BGP_ROUTE_REDISTRIBUTE == binfo->sub_type))
			{
				vty_out (vty, "%-16s","0.0.0.0");
			}
			else if(BGP_ROUTE_AGGREGATE == binfo->sub_type)
			{
				vty_out (vty, "%-16s","127.0.0.1");
			}
			else if (safi == SAFI_MPLS_VPN && attr->extra && attr->extra->mp_nexthop_global_in.s_addr)
			{
				vty_out (vty, "%-16s", inet_ntoa(attr->extra->mp_nexthop_global_in));
        ...
        else if (p->family == AF_INET6)
        {
            ...
            if (attr->extra->mp_nexthop_len == 16)
                ...
                vty_out (vty, "%s(%s)",
                         inet_ntop (AF_INET6, &attr->extra->mp_nexthop_global, buf, BUFSIZ),
                         inet_ntop (AF_INET6, &attr->extra->mp_nexthop_local, buf1, BUFSIZ));
        }
    }
    label = decode_label (binfo->extra->tag);
	label_in = decode_label (binfo->extra->in_tag);

	if(label_in == 0)
		vty_out (vty, "%s/", "NULL");
	else
    	vty_out (vty, "%d/", label_in);
                      ...
                      struct bgp_info *binfo, afi_t afi, safi_t safi)
{
    ...
    if (attr)
    {
        ...
        else
        {
            ...
            vty_out (vty, "    %s",
                     inet_ntop (AF_INET6, &attr->extra->mp_nexthop_global, buf, INET6_ADDRSTRLEN));
        }
        ...
        else if (binfo->type != ROUTE_PROTO_EBGP)
        {
            vty_out (vty, ", sourced");
        }
        else
...
#define BGP_SHOW_HEADER "     Network          Next Hop         Metric     LocPrf     Weight     Path/Ogn%s"
...
static unsigned long bgp_show_table_count (struct bgp_table *table,enum bgp_show_type type, void *output_arg)
{
    struct bgp_info *ri;
    struct bgp_node *rn;
        ...
        if(rn->info != NULL)
        {
            for (ri = rn->info; ri; ri = ri->next)
            {
                if(type == bgp_show_type_neighbor)
                {
                    ...
                    if(sockunion_same(&ri->peer->su, su))
                    {
                        total_count ++;
                    }
                }
                ...
                enum bgp_show_type type, void *output_arg)
{
    ...
    total_count  = bgp_show_table_count (table,  type,  output_arg);
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
	{
        if (rn->info != NULL)
        {
            ...
            for (ri = rn->info; ri; ri = ri->next)
            {
                ...
                {
                    union sockunion *su = output_arg;
                    if (ri->peer->su.sin.sin_addr.s_addr == 0 || ! sockunion_same(&ri->peer->su, su))
                    {
                        continue;
                    }
                }
                ...
                if (header)
                {
                    vty_out (vty, "%sBGP Local router ID is %s%s",VTY_NEWLINE, inet_ntoa (*router_id), VTY_NEWLINE);
                    vty_out (vty, BGP_SHOW_SCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);
                    vty_out (vty, BGP_SHOW_OCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);
                    vty_out (vty, "%sTotal Number of Routes: %ld %s", VTY_NEWLINE,total_count,VTY_NEWLINE);
          ...
          enum bgp_show_type type, void *output_arg)
{
    ...
    if (bgp == NULL)
    {
        vty_warning_out (vty, "No BGP process is configured%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "Network in the BGP routing table to display\n")
{
    ...
    if (! ret)
    {
        vty_error_out (vty, "Malformed Route Distinguisher%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
       ...
       "Network in the BGP routing table to display\n")
{
    ...
    {
        vty_error_out (vty, "Bad SAFI: %s%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
      ...
      )
{
    ...
    {
        vty_error_out (vty, "Bad AFI: %s%s", argv[0], VTY_NEWLINE);
        return CMD_WARNING;
    }
	
    if (bgp_parse_safi(argv[2], &safi))
    {
        vty_error_out (vty, "Bad SAFI: %s%s", argv[1], VTY_NEWLINE);
        return CMD_WARNING;
    }
...
DEFUN (show_ip_bgp_dampening_info,
       ...
       "show bgp route dampening parameters",
       SHOW_STR
       BGP_STR
       "BGP routing table\n"
       "Display detailed information about dampening\n"
                     ...
                     const char *ip_str)
{
    ...
    if (ret < 0)
    {
        vty_error_out (vty, "Malformed address: %s%s", ip_str, VTY_NEWLINE);
        return NULL;
    }

    ret = inet_valid_ipv4(ntohl(su.sin.sin_addr.s_addr));
    if (! ret)
    {
        vty_error_out(vty, "The neighbor address is invalid.%s", VTY_NEWLINE);
        return NULL;
    }
    ...
    if (! peer)
    {
        vty_info_out (vty, "The neighbor does not exist.%s", VTY_NEWLINE);
        return NULL;
    }
...
bgp_table_stats_walker (struct thread *t)
{
    ...
    switch (top->p.family)
    {
	    ...
	    case AF_INET6:
	        ...
	        break;
		default:
			zlog_err("%s[%d] family %d",__func__,__LINE__, top->p.family);
			return 0;
    }
...
/*Display routes learned from neighbor*/
DEFUN (show_bgp_ipv4_route_neighbor,
...
bgp_distance_apply (struct prefix *p, struct bgp_info *rinfo, struct bgp *bgp)
{
    ...
    peer = rinfo->peer;
    if ((peer->su.sa.sa_family != AF_INET  && peer->su.sa.sa_family != AF_INET6 ) && BGP_ROUTE_AGGREGATE != rinfo->sub_type)
    {
        return 0;
    }
	
    if(BGP_ROUTE_AGGREGATE == rinfo->sub_type)
    {
...
static void bgp_distance_update(struct bgp *bgp,afi_t afi, u_char is_ebgp , uchar is_ibgp, u_char is_local)
{
    struct bgp_info *ri;
    struct bgp_node *rn;
    struct bgp_node *rm;
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
    {
        if (rn->info != NULL)
        {
            for(ri = rn->info; ri; ri = ri->next)
            {
                if(!CHECK_FLAG (ri->flags, BGP_INFO_VALID) || !CHECK_FLAG (ri->flags, BGP_INFO_SELECTED))
                {
                    continue;
                }
                if((ROUTE_PROTO_EBGP == ri->type) && (BGP_ROUTE_REDISTRIBUTE == ri->sub_type || BGP_ROUTE_STATIC == ri->sub_type))
                {
                    continue;
                }
                if((is_local && BGP_ROUTE_AGGREGATE == ri->sub_type)
					|| (is_ebgp && BGP_PEER_EBGP == ri->peer->sort) 
					|| (is_ibgp && BGP_PEER_IBGP == ri->peer->sort))
                {
                    bgp_zebra_announce (&rn->p, ri, bgp, SAFI_UNICAST,IPC_OPCODE_UPDATE, 0);
                }
            }
        }
    }
			...
			if ((table = rn->info) != NULL)
	        {
				for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
	            {
					for(ri = rm->info; ri; ri = ri->next)
					{
		                ...
		                if((ROUTE_PROTO_EBGP == ri->type) && (BGP_ROUTE_REDISTRIBUTE == ri->sub_type))
		                {
		                    continue;
		                }
						
		                if((is_ebgp && BGP_PEER_EBGP == ri->peer->sort) ||(is_ibgp && BGP_PEER_IBGP == ri->peer->sort))
		                {
		                    bgp_zebra_announce (&rm->p, ri, bgp, SAFI_UNICAST,IPC_OPCODE_DELETE, 1);
		                }
					}
	            }
	        }
			...
			if ((table = rn->info) != NULL)
	        {
				for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
	            {
					for(ri = rm->info; ri; ri = ri->next)
					{
		                ...
		                if((is_ebgp && BGP_PEER_EBGP == ri->peer->sort) ||(is_ibgp && BGP_PEER_IBGP == ri->peer->sort))
		                {
		                    bgp_zebra_announce (&rm->p, ri, bgp, SAFI_UNICAST,IPC_OPCODE_ADD, 1);
		                }
					}
	            }
	        }
       ...
       "Distance for local routes\n")
{
    ...
    bgp->distance_ebgp= ROUTE_METRIC_EBGP;
    bgp->distance_ibgp = ROUTE_METRIC_IBGP;
    bgp->distance_local = ROUTE_METRIC_IBGP;
...
DEFUN (distance_ebgp_ibgp_local,
       ...
       "BGP distance\n")
{
    u_char distance = 0, is_ebgp = 0, is_ibgp = 0 ,is_local = 0;
    struct bgp *bgp;
	afi_t afi;
    ...
    {
        if(bgp->distance_ebgp != distance)
        {
            bgp->distance_ebgp = distance;
            is_ebgp = 1;
        }
    }
    else if (strncmp (argv[0], "i", 1) == 0)
    {
        if(bgp->distance_ibgp != distance)
        {
            bgp->distance_ibgp = distance;
            is_ibgp = 1;
        }
    }
    else if (strncmp (argv[0], "l", 1) == 0)
    {
        if(bgp->distance_local != distance)
        {
            bgp->distance_local = distance;
            is_local = 1;
        }
    }
    else
    {
        vty_out (vty, "%% Invalid %s%s",argv[0], VTY_NEWLINE);
    }
	
    if(is_ebgp || is_ibgp || is_local)
    {
    	for (afi = AFI_IP; afi < AFI_MAX; afi++)
		{
        	bgp_distance_update(bgp, afi, is_ebgp, is_ibgp, is_local);
		}
    }
...
DEFUN (no_distance_ebgp_ibgp_local,
       ...
       "BGP distance\n")
{
    u_char is_ebgp = 0, is_ibgp = 0, is_local = 0;
    struct bgp *bgp;
	afi_t afi;
    ...
    if (strncmp (argv[0], "e", 1) == 0)
    {
        if(bgp->distance_ebgp != ROUTE_METRIC_EBGP)
        {
            bgp->distance_ebgp = ROUTE_METRIC_EBGP;
            is_ebgp = 1;
        }
    ...
    {
        if(bgp->distance_ebgp != ROUTE_METRIC_IBGP)
        {
            bgp->distance_ibgp = ROUTE_METRIC_IBGP;
            is_ibgp = 1;
        }
       ...
       "Maximum duration to suppress a stable route\n")
{
    ...
    if (suppress <= reuse)
    {
        vty_error_out (vty, "The dampening suppression parameters are invalid. Note: Reuse Value should be less than Suppress Value. %s",
                 VTY_NEWLINE);
        return 0;
    }
    ...
    if (half >= max)
    {
        vty_error_out (vty, "The value for max duration to suppress should be larger than half-life.%s", VTY_NEWLINE);
        return 0;
    }
...
/* add by fenglihua */
void bgp_med_update(struct bgp *bgp, afi_t afi, safi_t safi)
{
	int update = 0;
    struct bgp_node *rn = NULL;
    struct bgp_node *bn = NULL;
    ...
    //if(BGP_DEBUG(events, EVENTS))
    //{
        zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d]  afi %d safi %d med %u",__FUNCTION__,__LINE__, afi , safi, bgp->default_med);
    //}
	
	if(safi != SAFI_MPLS_VPN)
	{
		for(bn = bgp_table_top (bgp->rib[afi][safi]); bn; bn = bgp_route_next (bn))
	    {
			update = 0;
	        for(bi = bn->info; bi; bi = bi->next)
	        {
	        	/* Med applies to redistribute and aggregate*/
	            if((BGP_ROUTE_REDISTRIBUTE != bi->sub_type) && (BGP_ROUTE_AGGREGATE != bi->sub_type))
	            {
	                continue;
	            }
	            ...
	            //{
	                ...
	                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] p %s/%d med %u flags 0x%x sub_type %d",__FUNCTION__,__LINE__,
	                          inet_ntop(bn->p.family, &bn->p.u.prefix, buf, INET_ADDRSTRLEN),bn->p.prefixlen,
	                           bi->attr->med, bi->flags,bi->sub_type);
	            //}
	        }
				...
				for(bn = bgp_table_top(table); bn; bn = bgp_route_next (bn))
			    {
			        ...
			        for(bi = bn->info; bi; bi = bi->next)
			        {
			            ...
			            //{
			                char buf[INET_ADDRSTRLEN];
			                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] p %s/%d med %u flags 0x%x sub_type %d",__FUNCTION__,__LINE__,
			                           inet_ntop(AF_INET, &bn->p.u.prefix4, buf, sizeof(buf)),bn->p.prefixlen,
			                           bi->attr->med, bi->flags,bi->sub_type);
			            //}
			        }
    ...
    /* add -- modify med update ebgp default originate*/
    for(ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        ...
        if(peer->sort != BGP_PEER_EBGP || peer->status != Established)
        {
            continue;
        }
...
void bgp_local_pref_update(struct bgp *bgp,afi_t afi, safi_t safi)
{
	int update = 0;
    struct bgp_node *bn = NULL;
    struct bgp_node *rn = NULL;
    ...
    //if(BGP_DEBUG(events, EVENTS))
    //{
        zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] afi %d safi %d local pref %u ",__FUNCTION__,__LINE__, afi, safi, bgp->default_local_pref);
    //}
	...
	if(safi != SAFI_MPLS_VPN)
	{
	    ...
	    {	
	        ...
	        {				    
	            ...
	            //{
	                char buf[INET_ADDRSTRLEN];
	                zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] p %s/%d flags 0x%x",__FUNCTION__,__LINE__,
	                           inet_ntop(AF_INET, &bn->p.u.prefix4, buf, sizeof(buf)),bn->p.prefixlen,bi->flags);
	            //}
	        }
    ...
    for(ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        ...
        if(peer->sort != BGP_PEER_IBGP || peer->status != Established)
        {
            continue;
        }
    ...
    for (rn = bgp_table_top (bgp->network[afi][safi]); rn; rn = bgp_route_next (rn))
	{
        if ((bgp_network = rn->info) != NULL)
        {
            ...
            if (bgp_option_check (BGP_OPT_CONFIG_CISCO) && afi == AFI_IP)
            {
                ...
                vty_out (vty, " redistribute network %s",
                         inet_ntop (p->family, &p->u.prefix, buf, SU_ADDRSTRLEN));
                if ((IN_CLASSC (destination) && p->prefixlen == 24)
                        || (IN_CLASSB (destination) && p->prefixlen == 16)
                        || (p->u.prefix4.s_addr && IN_CLASSA (destination) && p->prefixlen == 8)
    ...
    for (rn = bgp_table_top (bgp->aggregate[afi][safi]); rn; rn = bgp_route_next (rn))
	{
        if ((bgp_aggregate = rn->info) != NULL)
        {
            ...
            if (bgp_option_check (BGP_OPT_CONFIG_CISCO) && afi == AFI_IP)
            {
                struct in_addr netmask;
                masklen2ip (p->prefixlen, &netmask);
                vty_out (vty, " summary %s %s",
                         inet_ntop (p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
                         inet_ntoa (netmask));
            }
            else
            {
                vty_out (vty, " summary %s/%d",
                         inet_ntop (p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
                         p->prefixlen);
            }
...
void
bgp_config_write_ipv6_family (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi, int *write_ipv6)
{
    int i,type_redis = 0, instance = 0;
    struct bgp_node *rn;	
    struct bgp_aggregate *bgp_aggregate;
    char buf[SU_ADDRSTRLEN];
    struct prefix *p;
	...
	{
		if(bgp->redist[afi][i]  &&  (ROUTE_PROTO_STATIC == i || ROUTE_PROTO_CONNECT == i))
		{
			if(*write_ipv6 == 0)
			{
				vty_out (vty, " ipv6-family unicast%s", VTY_NEWLINE);
				*write_ipv6 = 1;
			}
        ...
        else if(ROUTE_PROTO_OSPF6 == i || ROUTE_PROTO_ISIS6 == i || ROUTE_PROTO_RIPNG == i)
		{
			type_redis = i - ROUTE_PROTO_OSPF;
			for(instance = 0; instance < MAX_INSTANCE_ID ; instance ++)
			{
			    if(bgp->redist_instance[type_redis][instance].redist_instance_flag)
			    {
			        ...
			        vty_out (vty, "  redistribute %s %d", (ROUTE_PROTO_OSPF6 == i) ? "ospfv3" : zebra_route_string(i),instance);
					
			        if (bgp->redist_instance[type_redis][instance].redist_instance_metric_flag)
			        {
			            vty_out (vty, " metric %u", bgp->redist_instance[type_redis][instance].redist_instance_metric);
			        }
...
{
    if (bgp->distance_ebgp  && (bgp->distance_ebgp != ROUTE_METRIC_EBGP))
    {
        vty_out (vty, " distance ebgp %d%s", bgp->distance_ebgp, VTY_NEWLINE);
    }
	
    if (bgp->distance_ibgp  && (bgp->distance_ibgp != ROUTE_METRIC_IBGP))
    {
        vty_out (vty, " distance ibgp %d%s", bgp->distance_ibgp, VTY_NEWLINE);
    }
	
    if (bgp->distance_local &&  (bgp->distance_local != ROUTE_METRIC_IBGP))
    {
        vty_out (vty, " distance local %d%s", bgp->distance_local, VTY_NEWLINE);
    }
...
bgp_route_init (void)
{
    ...
    /* IPv4 BGP commands. */
    install_element (BGP_NODE, &bgp_network_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_natural_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_natural_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_natural_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_natural_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_natural_route_map_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_natural_backdoor_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_mask_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_summary_only_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_mask_summary_only_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_mask_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_as_set_summary_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_mask_as_set_summary_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_summary_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &aggregate_address_mask_summary_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_summary_only_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_as_set_summary_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_summary_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_mask_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_mask_summary_only_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_mask_as_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_mask_as_set_summary_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_aggregate_address_mask_summary_as_set_cmd, CMD_SYNC);

    install_element (BGP_IPV6_NODE, &ipv6_aggregate_address_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &ipv6_aggregate_address_summary_only_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &ipv6_aggregate_address_as_set_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &ipv6_aggregate_address_as_set_summary_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &ipv6_aggregate_address_summary_as_set_cmd, CMD_SYNC);
    install_element (BGP_IPV6_NODE, &no_ipv6_aggregate_address_cmd, CMD_SYNC);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_rd_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_rd_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rd_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rd_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rd_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rd_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_afi_safi_view_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_afi_safi_neighbor_adv_recd_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_routes_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_ipv4_route_neighbor_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_route_neighbor_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_ipv4_route_neighbor_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_route_neighbor_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_address_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_flap_address_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_address_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_damp_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_damp_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv4_safi_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv4_safi_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv4_safi_rsclient_prefix_cmd, CMD_LOCAL);
	
    /* BGP dampening clear commands */
    install_element (CONFIG_NODE, &clear_ip_bgp_dampening_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_dampening_prefix_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_dampening_address_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &clear_ip_bgp_dampening_address_mask_cmd, CMD_SYNC);
	
    /* prefix count */
    install_element (CONFIG_NODE, &show_bgp_ipv4_safi_neighbor_prefix_counts_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_neighbor_prefix_counts_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_prefix_counts_cmd, CMD_LOCAL);
	
    /* Old config IPv6 BGP commands.  */
    install_element (BGP_NODE, &old_ipv6_bgp_network_cmd, CMD_SYNC);
    install_element (BGP_NODE, &old_no_ipv6_bgp_network_cmd, CMD_SYNC);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv4_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_safi_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_safi_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_ipv6_safi_rsclient_prefix_cmd, CMD_LOCAL);
	
    /* Statistics */
    install_element (CONFIG_NODE, &show_bgp_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_statistics_view_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &bgp_distance_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_distance_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_distance2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_distance_source_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_distance_source_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_distance_source_access_list_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_distance_source_access_list_cmd, CMD_SYNC);
    install_element (BGP_NODE, &distance_ebgp_ibgp_local_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_distance_ebgp_ibgp_local_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_damp_set_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_damp_set2_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_damp_set3_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_damp_unset_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_damp_unset2_cmd, CMD_SYNC);
	
    /* Deprecated AS-Pathlimit commands */
    install_element (BGP_NODE, &bgp_network_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_natural_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_backdoor_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_backdoor_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &bgp_network_mask_natural_backdoor_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_natural_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_backdoor_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_backdoor_ttl_cmd, CMD_SYNC);
    install_element (BGP_NODE, &no_bgp_network_mask_natural_backdoor_ttl_cmd, CMD_SYNC);
	
    /* old style commands */
    install_element (CONFIG_NODE, &show_ip_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_all_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_rd_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_all_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_rd_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_ip_bgp_dampening_params_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_dampened_paths_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_statistics_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_address_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_address_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_cidr_only_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_view_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_damp_flap_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_neighbor_prefix_counts_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_ipv4_neighbor_prefix_counts_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ip_bgp_vpnv4_neighbor_prefix_counts_cmd, CMD_LOCAL);
    install_element (BGP_NODE, &show_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE, &show_bgp_cmd, CMD_LOCAL);
    install_element (NEIGHBOR_NODE_IPV6, &show_bgp_cmd, CMD_LOCAL);
    install_element (BGP_IPV6_NODE, &show_bgp_cmd, CMD_LOCAL);
    install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_route_map_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_safi_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_received_prefix_filter_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_flap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_neighbor_damp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_rsclient_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_statistics_vpnv4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_statistics_view_vpnv4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_bgp_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_regexp_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_prefix_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_filter_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community_all_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_ipv6_mbgp_prefix_longer_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_bgp_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_mbgp_neighbor_advertised_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_bgp_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_mbgp_neighbor_received_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_bgp_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &ipv6_mbgp_neighbor_routes_cmd, CMD_LOCAL);
	
    /* old with name safi collision */
    install_element (CONFIG_NODE, &show_bgp_ipv6_community_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community2_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community3_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community4_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community2_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community3_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community4_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community_list_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_community_list_exact_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_rsclient_prefix_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_rsclient_route_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_view_rsclient_prefix_cmd, CMD_LOCAL);
}
