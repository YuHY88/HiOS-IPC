...
ecommunity_uniq_sort (struct ecommunity *ecom)
{
    ...
    for (i = 0; i < ecom->size; i++)
    {
        ...
        ecommunity_add_val (new_ecom, eval);
    }
...
ecommunity_dup (struct ecommunity *ecom)
{
    ...
    new_ecom = XCALLOC (MTYPE_ECOMMUNITY, sizeof (struct ecommunity));
    new_ecom->size = ecom->size;
    if (new_ecom->size)
    {
        new_ecom->val = XMALLOC (MTYPE_ECOMMUNITY_VAL, ecom->size * ECOMMUNITY_SIZE);
        memcpy (new_ecom->val, ecom->val, ecom->size * ECOMMUNITY_SIZE);
    }
    else
    {
        new_ecom->val = NULL;
                     ...
                     enum ecommunity_token *token)
{
	...
	memset(&ip, 0x0, sizeof(struct in_addr));
...
int
ecommunity_match (const struct ecommunity *ecom1, const struct ecommunity *ecom2)
{
    int i = 0;
    int j = 0;
	int r = 0;
    ...
    {
        r = memcmp (ecom1->val + i*ECOMMUNITY_SIZE, ecom2->val + j*ECOMMUNITY_SIZE, ECOMMUNITY_SIZE) ;
		if(r == 0)
		{
			return 1;
		}
...
int
ecommunity_include (const struct ecommunity *ecom1, const struct ecommunity *ecom2)
{	
	int i = 0;
	int j = 0;
	int r = 0;
	...
	for(j = 0 ; j < ecom2->size; j++)
	{	
		flag = 0;
		for(i = 0; i < ecom1->size; i++)
		{ 	
			r = memcmp(ecom1->val + i*ECOMMUNITY_SIZE, ecom2->val + j*ECOMMUNITY_SIZE, ECOMMUNITY_SIZE);			
			if(r == 0)
			{
...
struct ecommunity *
ecommunity_rt_add (struct ecommunity *ecom1,  struct ecommunity *ecom2)
{
	int j = 0;
	struct ecommunity_val eval;
	...
	for(j = 0; j < ecom2->size; j ++)
	{
		memcpy (eval.val, ecom2->val + j * ECOMMUNITY_SIZE , ECOMMUNITY_SIZE);
		ecommunity_add_val(ecom1, &eval);
	}
