...
community_uniq_sort (struct community *com)
{
    ...
    new_com = community_new ();;

    for(i = 0; i < com->size; i++)
    {
        val = community_val_get (com, i);
        if (! community_include (new_com, val))
        {
            community_add_val (new_com, val);
        }
    }

    qsort (new_com->val, new_com->size, sizeof (u_int32_t), community_compare);
...
community_parse (u_int32_t *pnt, u_short length)
{
    ...
    com = community_uniq_sort (&tmp);

    return community_intern (com);
}
...
community_dup (struct community *com)
{
    ...
    new_com = XCALLOC (MTYPE_COMMUNITY, sizeof (struct community));
    new_com->size = com->size;
    if (new_com->size)
    {
        new_com->val = XMALLOC (MTYPE_COMMUNITY_VAL, com->size * 4);
        memcpy (new_com->val, com->val, com->size * 4);
    }
    else
    {
        new_com->val = NULL;
