    ...
    struct bgp_tea_subtlv_encap_l2tpv3_over_ip  *st)
{
    ...
    struct bgp_attr_encap_subtlv *new_encap;
	
    /* sanity check */
    //assert(st->cookie_length <= sizeof(st->cookie));
    //assert(total <= 0xff);
	if((st->cookie_length > sizeof(st->cookie)) || (total > 0xff))
	{
		return NULL;
	}

    new_encap = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    ...
    new_encap->type = BGP_ENCAP_SUBTLV_TYPE_ENCAPSULATION;
    new_encap->length = total;
    p = new_encap->value;
    ...
    struct bgp_tea_subtlv_proto_type    *st)
{
    ...
    new_encap->type = BGP_ENCAP_SUBTLV_TYPE_PROTO_TYPE;
    new_encap->length = total;
    p = new_encap->value;
    ...
    struct bgp_tea_subtlv_color *st)
{
    ...
    new_encap->type = BGP_ENCAP_SUBTLV_TYPE_COLOR;
    new_encap->length = total;
    p = new_encap->value;
    ...
    struct bgp_tea_subtlv_ipsec_ta  *st)
{
    ...
    //assert(total <= 0xff);
	if((st->authenticator_length > sizeof(st->value)) || (total > 0xff))
	{
		return NULL;
	}
    ...
    new_encap->type = BGP_ENCAP_SUBTLV_TYPE_IPSEC_TA;
    new_encap->length = total;
    p = new_encap->value;
    ...
    struct bgp_tea_subtlv_remote_endpoint   *st)
{
    ...
    new_encap->type = BGP_ENCAP_SUBTLV_TYPE_REMOTE_ENDPOINT;
    new_encap->length = total;
    p = new_encap->value;
    ...
    else
    {
		...
		{
			XFREE(MTYPE_ENCAP_TLV, new_encap);
			new_encap = NULL;
			return NULL;
		}
...
#define ENC_SUBTLV(flag, function, field) do {\
    ...
    new_encap = function(&bet->field);\
    if (last) {\
        last->next = new_encap;\
    } else {\
        extra->encap_subtlvs = new_encap;\
    }\
    last = new_encap;\
    }\
} while (0)
    ...
    struct attr                 *attr)
{
	...
	if(!CHECK_FLAG(bet->valid_subtlvs, BGP_TEA_SUBTLV_ENCAP))
	{
		return ;
	}
    ...
    struct bgp_tea_subtlv_encap_l2tpv3_over_ip  *st)
{
    if (subtlv->length < 4)
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d is less than 4",
    ...
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d is greater than %d",
    ...
    struct bgp_tea_subtlv_encap_gre_key *st)
{
    if (subtlv->length != 4)
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d does not equal 4",
                   __func__, subtlv->length);
        return -1;
    }
    ...
    struct bgp_tea_subtlv_encap_pbb *st)
{
    ...
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d does not equal %d",
    ...
    struct bgp_tea_subtlv_proto_type    *st)
{
    if (subtlv->length != 2)
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d does not equal 2",
                   __func__, subtlv->length);
        return -1;
    }
    ...
    struct bgp_tea_subtlv_color     *st)
{
    if (subtlv->length != 8)
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d does not equal 8",
                   __func__, subtlv->length);
        return -1;
    }
    ...
    if ((subtlv->value[0] != 0x03) ||
            ...
            (subtlv->value[3] != 0))
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv value 1st 4 bytes are not 0x030b0000", __func__);
        return -1;
    }
    ...
    struct bgp_tea_subtlv_ipsec_ta  *st)
{
    ...
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, authenticator length %d exceeds storage maximum %d",
    ...
    struct bgp_tea_subtlv_remote_endpoint *st)
{
    ...
    {
        zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s, subtlv length %d does not equal 8 or 20",
                   __func__, subtlv->length);
        return -1;
    }
    ...
    for (st = stlv; st; st = st->next)
    {
        switch (st->type)
        {
	        ...
	        case BGP_ENCAP_SUBTLV_TYPE_REMOTE_ENDPOINT:
	            ...
	            break;
	        default:
	            zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s: unexpected subtlv type %d", __func__, st->type);
	            rc |= -1;
	            break;
        }
    }
