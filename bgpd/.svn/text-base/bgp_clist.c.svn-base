                       ...
                       const char *name, int master)
{
    size_t i;
    long number;
    struct community_list *new_list;
    ...
    new_list = community_list_new ();
	if(! new_list)
	{
		return NULL;
	}
	
    new_list->name = XSTRDUP (MTYPE_COMMUNITY_LIST_NAME, name);
    ...
    if (i == strlen (name))
    {
        new_list->sort = COMMUNITY_LIST_NUMBER;
    ...
    {
        new_list->sort = COMMUNITY_LIST_STRING;
    ...
    new_list->next = point;
    new_list->prev = point->prev;

    if (point->prev)
    {
        point->prev->next = new_list;
    }
    point->prev = new_list;
...
community_list_dup_check (struct community_list *list,
                          struct community_entry *new_entry)
{
    struct community_entry *entry;
	
    for (entry = list->head; entry; entry = entry->next)
    {
        if (entry->style != new_entry->style)
        {
            continue;
        }
		
        if (entry->direct != new_entry->direct)
        {
            continue;
        }
		
        if (entry->any != new_entry->any)
        {
            continue;
        }
        ...
        {
	        case COMMUNITY_LIST_STANDARD:
	            if (community_cmp (entry->u.com, new_entry->u.com))
	            {
	                return 1;
	            }
	            break;
	        case EXTCOMMUNITY_LIST_STANDARD:
	            if (ecommunity_cmp (entry->u.ecom, new_entry->u.ecom))
	            {
	                return 1;
	            }
	            break;
	        case COMMUNITY_LIST_EXPANDED:
	        case EXTCOMMUNITY_LIST_EXPANDED:
	            if (strcmp (entry->config, new_entry->config) == 0)
	            {
	                return 1;
	            }
	            break;
