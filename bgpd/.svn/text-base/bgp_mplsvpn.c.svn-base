...
void label_decode(u_char *tag, u_int32_t l)
{
    tag[0] = (u_char)(l >> 12);
    tag[1] = (u_char)(l >> 4);
    tag[2] = (u_char)(l << 4);
    ...
    for (; pnt < lim; pnt += psize)
    {
        ...
        if (attr)
    	{
            bgp_update (peer, &p, attr, packet->afi, SAFI_MPLS_VPN,
                        ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, &prd, tagpnt, 0);
    	}
        else
    	{
            bgp_withdraw (peer, &p, attr, packet->afi, SAFI_MPLS_VPN,
                          ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, &prd, tagpnt);
    	}
    }
...
int prefix_rd_same(struct prefix_rd *prd1, struct prefix_rd *prd2)
{
	if(NULL == prd1 || NULL == prd2)
	{
		return 0;
	}
	return (!memcmp(prd1->val, prd2->val, 8));
}
    ...
    int tags)
{
    ...
    int display = 0;
    char v4_header[] =  "     Network          Next Hop         Metric     LocPrf     Weight     Path%s";
    char v4_header_tag[] = "     Network          Next Hop        In/Out Label%s";
    ...
    {
        vty_error_out (vty, "Afi %d not supported%s", afi, VTY_NEWLINE);
        return CMD_WARNING;
    }
        ...
        if ((table = rn->info) != NULL)
        {
            ...
            for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
        	{
                ...
                for (ri = rm->info; ri; ri = ri->next)
                {
                    ...
                    if (header)
                    {
                        vty_out (vty, "%sBGP local router ID is %s%s",
						...
						vty_out (vty, "Total Number of Routes: %ld %s", total_count, VTY_NEWLINE);
                    ...
                    if (rd_header)
                    {       
						vty_out (vty, "%sVPN-Instance: %d %s", VTY_NEWLINE, ri->vrf_id ? ri->vrf_id : ri->vrf_id_to, VTY_NEWLINE);
                    	vty_out(vty, "%s",VTY_NEWLINE);
                    ...
                    {
                        route_vty_out (vty, &rm->p, ri, display, SAFI_MPLS_VPN);
                    }
					display ++;
                }
        	}
...
DEFUN (show_bgp_ipv4_vpn_instance,
...

{
   ...
   if(bgp_vrf_redist_info[id]->prd.prefixlen != 64)
   {
	   vty_error_out(vty, "The route-distinguisher is not configured%s", VTY_NEWLINE);
	   return CMD_WARNING;
   }

   return bgp_show_mpls_vpn (vty, AFI_IP, &bgp_vrf_redist_info[id]->prd, bgp_show_type_normal, NULL, 0);
}

DEFUN (show_bgp_ipv4_vpn_statistics ,
		...
		if ((table = rn->info) != NULL)
		{
			...
			for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
			{
				...
				{				
					if(header)
					{
	                    vty_out (vty, "%sBGP local router ID is %s%s", VTY_NEWLINE, inet_ntoa (bgp->router_id), VTY_NEWLINE);
						header = 0;
					}
					
					if(vrf_id == 0)
					{
						vrf_id = ri->vrf_id ? ri->vrf_id : ri->vrf_id_to;
					}
					total_count++;
				}
			}
			...
			if(total_count)
			{
				vty_out (vty, "VPN-Instance: %d   Total Number of Routes: %ld %s", vrf_id, total_count, VTY_NEWLINE);
			}
		}
	}
	return CMD_SUCCESS;
...
{
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);
	install_element (BGP_NODE, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);
	install_element (BGP_IPV6_NODE, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_vpn_cmd, CMD_LOCAL);

    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);
	install_element (BGP_NODE, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);
	install_element (BGP_IPV6_NODE, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_vpn_instance_cmd, CMD_LOCAL);

    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	install_element (BGP_NODE, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	install_element (BGP_IPV6_NODE, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_vpn_statistics_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_rd_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	install_element (BGP_NODE, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	install_element (NEIGHBOR_NODE_IPV6, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	install_element (BGP_IPV6_NODE, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	install_element (BGP_VPN_INSTANCE_NODE, &show_bgp_ipv4_vpn_tags_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_rd_tags_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_neighbor_routes_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_neighbor_advertised_routes_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_rd_neighbor_advertised_routes_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv4_vpn_rd_neighbor_routes_cmd, CMD_LOCAL);
	
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_rd_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_rd_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_neighbor_advertised_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_rd_neighbor_advertised_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_vpn_rd_neighbor_routes_cmd, CMD_LOCAL);
}
