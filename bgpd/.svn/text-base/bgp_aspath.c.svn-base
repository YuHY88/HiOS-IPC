...
assegment_new (u_char type, u_short length)
{
    ...
    seg = XCALLOC (MTYPE_AS_SEG, sizeof (struct assegment));
    ...
    if (length)
    {
        seg->as = assegment_data_new (length);
    }
...
assegment_dup (struct assegment *seg)
{
    ...
    seg1 = assegment_new (seg->type, seg->length);
    memcpy (seg1->as, seg->as, ASSEGMENT_DATA_SIZE (seg1->length, 1) );
...
assegment_dup_all (struct assegment *seg)
{
    ...
    while (seg)
    {
        if (head)
        {
            seg1->next = assegment_dup (seg);
            seg1 = seg1->next;
        }
        else
        {
            head = seg1 = assegment_dup (seg);
        }
        seg = seg->next;
    }
    return head;
...
aspath_intern (struct aspath *aspath)
{
    ...
    //assert (aspath->str);
	if(aspath->refcnt || !aspath->str)
	{
		zlog_debug(BGP_DEBUG_TYPE_OTHER, "%s[%d]: invalid aspath!\n", __FUNCTION__, __LINE__);
		return NULL;
	}
...
aspath_dup (struct aspath *aspath)
{
    ...
    if (aspath->segments)
    {
        as->segments = assegment_dup_all (aspath->segments);
    }
    ...
    as->str = XMALLOC (MTYPE_AS_STR, buflen);
    as->str_len = aspath->str_len;

    /* copy the string data */
    if (aspath->str_len > 0)
    {
        memcpy (as->str, aspath->str, buflen);
    }
    else
    {
        as->str[0] = '\0';
...
aspath_hash_alloc (void *arg)
{
    ...
    as = XMALLOC (MTYPE_AS_PATH, sizeof (struct aspath));
	if(!as)
	{
		return NULL;
	}
                  ...
                  struct assegment **result, int use32bit)
{
    ...
    //if (BGP_DEBUG (as4, AS4_SEGMENT))
        zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4SEG] Parse aspath segment: got total byte length %lu",
    ...
    while (bytes < length)
    {
        ...
        //if (BGP_DEBUG (as4, AS4_SEGMENT))
            zlog_debug (BGP_DEBUG_TYPE_AS4 ,"[AS4SEG] Parse aspath segment: got type %d, length %d",
        ...
        //if (BGP_DEBUG (as4, AS4_SEGMENT))
            zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4SEG] Parse aspath segment: Bytes now: %lu",
                        (unsigned long) bytes);
        prev = seg;
    }
...
aspath_parse (struct stream *s, size_t length, int use32bit)
{
	...
	if(NULL == find)
	{
		zlog_err("%s[%d]: get target aspath fail! there is a risk of memory leaks.\n", __FUNCTION__, __LINE__);
		return NULL;
	}
...
aspath_merge (struct aspath *as1, struct aspath *as2)
{
    ...
    last = seg = assegment_dup_all (as1->segments);
...
{
    struct assegment * srcseg;
	struct assegment * exclseg;
	struct assegment * lastseg;
...
aspath_reconcile_as4 ( struct aspath *aspath, struct aspath *as4path)
{
    int hops, cpasns = 0;
    struct assegment *seg = NULL;
	struct assegment *newseg = NULL;
	struct assegment *prevseg = NULL;
    struct aspath *newpath = NULL;
	struct aspath *mergedpath =NULL;
    ...
    if (hops < 0)
    {
        ...
        //{
            zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] Fewer hops in AS_PATH than NEW_AS_PATH");
        //}
    ...
    //if ( BGP_DEBUG(as4, AS4))
        zlog_debug(BGP_DEBUG_TYPE_AS4,"[AS4] got AS_PATH %s and AS4_PATH %s synthesizing now",
    ...
    while (seg && hops > 0)
    {
        switch (seg->type)
        {
	        ...
	        case AS_CONFED_SEQUENCE:
	            ...
	            {
	                //if (BGP_DEBUG (as4, AS4))
	                    zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] AS4PATHmangle: AS_CONFED_SEQUENCE falls"
	        ...
	        case AS_SEQUENCE:
				...
				break;
			default:
				zlog_err("%s[%d] type %d",__func__,__LINE__, seg->type);
				break;
        }
		
        //assert (cpasns <= seg->length);
		if(cpasns > seg->length)
		{
			zlog_debug(BGP_DEBUG_TYPE_OTHER, "%s[%d]: invalid length!\n", __FUNCTION__, __LINE__);
			return NULL;
		}
    ...
    //if ( BGP_DEBUG(as4, AS4))
        zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] result of synthesizing is %s",
                    mergedpath->str);
...
{
    struct assegment *seg = as->segments;
    struct assegment *seg2 = assegment_new (type, 0);
