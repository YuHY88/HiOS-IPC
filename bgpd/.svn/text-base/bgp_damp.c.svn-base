...
bgp_reuse_index (int penalty)
{
    ...
    index_s = damp->reuse_index[i] - damp->reuse_index[0];
	
    return (damp->reuse_offset + index_s) % damp->reuse_list_size;
}
...
bgp_reuse_list_add (struct bgp_damp_info *bdi)
{
    ...
    index_s = bdi->index = bgp_reuse_index (bdi->penalty);
	
    bdi->prev = NULL;
    bdi->next = damp->reuse_list[index_s];
	
    if (damp->reuse_list[index_s])
    {
        damp->reuse_list[index_s]->prev = bdi;
    }
	
    damp->reuse_list[index_s] = bdi;
}
...
bgp_reuse_timer (void * arg)
{
   ...
   // damp->reuse_timer =  high_pre_timer_add("bgp_reuse_timer", LIB_TIMER_TYPE_LOOP, bgp_reuse_timer, NULL, DELTA_REUSE * 1000 );
                   ...
                   afi_t afi, safi_t safi, int attr_change)
{
	...
	if((rn != bdi->rn) || (binfo != bdi->binfo))
	{
		zlog_err("%s[%d]: invalid process!!! There is a risk of memory leaks.\n", __FUNCTION__, __LINE__);
		return BGP_DAMP_NONE;
	}
                 ...
                 unsigned int reuse, unsigned int suppress, time_t max)
{
    ...
    SET_FLAG (bgp->af_flags[AFI_IP6][safi], BGP_CONFIG_DAMPENING);
...
bgp_damp_disable (struct bgp *bgp, afi_t afi, safi_t safi)
{
    ...
    if (damp->reuse_timer)
    {
        high_pre_timer_delete (damp->reuse_timer);
    }
    ...
    UNSET_FLAG (bgp->af_flags[AFI_IP6][safi], BGP_CONFIG_DAMPENING);
    return 0;
}
...
bgp_config_write_damp (struct vty *vty)
{
    ...
    {
        vty_out (vty, " dampening%s", VTY_NEWLINE);
    }
    else
        vty_out (vty, " dampening %lld %d %d %lld%s",
...
#define ONE_DAY_SECOND_DAMP (60*60*24)
#define ONE_WEEK_SECOND_DAMP (60*60*24*7)
    if (reuse_time == 0)
    {
        snprintf (buf, len, "00:00:00");
    }
    else if (reuse_time < ONE_DAY_SECOND_DAMP)
        snprintf (buf, len, "%02d:%02d:%02d",
                  tm->tm_hour, tm->tm_min, tm->tm_sec);
    else if (reuse_time < ONE_WEEK_SECOND_DAMP)
...
bgp_show_dampening_parameters (struct vty *vty, afi_t afi, safi_t safi)
{
    ...
    {
        vty_out (vty, "%s", VTY_NEWLINE);
        vty_out (vty, "Maximum Suppress Time(in  second) : %ld%s",
                 damp->max_suppress_time, VTY_NEWLINE);
        vty_out (vty, "Ceiling Value                     : %u%s",
                 damp->ceiling, VTY_NEWLINE);
        vty_out (vty, "Reuse Value                       : %d%s",
                 damp->reuse_limit, VTY_NEWLINE);
        vty_out (vty, "HalfLife Time(in  second)         : %ld%s",
                 damp->half_life, VTY_NEWLINE);
        vty_out (vty, "Suppress-Limit                    : %d%s",
