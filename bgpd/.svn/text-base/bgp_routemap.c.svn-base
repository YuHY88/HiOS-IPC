                      ...
                      route_map_object_t type, void *object)
{
    ...
    if (type == RMAP_BGP)
    {
        ...
        if (rins->peer_address)
        {
            ...
            {
                bgp_info->attr->nexthop.s_addr = sockunion2ip (&peer->su);
...
route_set_aspath_prepend (void *rule, struct prefix *prefix, route_map_object_t type, void *object)
{
    ...
    if (type == RMAP_BGP)
    {
        ...
        {
            new_aspath = aspath_dup (binfo->attr->aspath);
        }
        else
        {
            new_aspath = binfo->attr->aspath;
        }
		
        if ((uintptr_t)rule > 10)
        {
            aspath = rule;
            aspath_prepend (aspath, new_aspath);
        }
        else
        {
            as_t as = aspath_leftmost(new_aspath);
			
            if (!as)
            {
                as = binfo->peer->as;
            }
            new_aspath= aspath_add_seq_n (new_aspath, as, (uintptr_t) rule);
        }
        binfo->attr->aspath = new_aspath;
    }
    return RMAP_OKAY;
}
                     ...
                     route_map_object_t type, void *object)
{
    ...
    if (type == RMAP_BGP)
    {
        ...
        {
            new_com = community_dup (rcs->com);
        }
		
        /* will be interned by caller if required */
        attr->community = new_com;
        attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_COMMUNITIES);
    }
    return RMAP_OKAY;
}
                             ...
                             route_map_object_t type, void *object)
{
    ...
    if (type == RMAP_BGP)
    {
        ...
        if ((CHECK_FLAG (peer->rmap_type, PEER_RMAP_TYPE_IN) ||
                CHECK_FLAG (peer->rmap_type, PEER_RMAP_TYPE_IMPORT))
                && sockunion_family (&peer->su) == AF_INET6)
        {
            inet_pton (AF_INET6, sockunion2str (&peer->su,
                                                peer_addr_buf,
                                                INET6_ADDRSTRLEN),
                       &peer_address);
        }
