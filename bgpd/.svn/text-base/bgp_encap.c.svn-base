    ...
    struct bgp_nlri *packet)
{
    ...
    for (; pnt < lim; pnt += psize)
    {
        ...
        if (p.family == 0)
        {
            ...
            zlog_debug(BGP_DEBUG_TYPE_OTHER,"%s: bad afi %d, dropping incoming route", __func__, afi);
            continue;
        }
        ...
        if (rdtype == RD_TYPE_AS)
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"rd-as %u:%u prefix %s/%d", rd_as.as, rd_as.val,
                       inet_ntop (p.family, &p.u.prefix, buf, BUFSIZ),
                       p.prefixlen);
        else if (rdtype == RD_TYPE_IP)
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"rd-ip %s:%u prefix %s/%d", inet_ntoa (rd_ip.ip),
                       rd_ip.val,
                       inet_ntop (p.family, &p.u.prefix, buf, BUFSIZ),
                       p.prefixlen);
        else if (rdtype == RD_TYPE_AS4)
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"rd-as4 %u:%u prefix %s/%d", rd_as.as, rd_as.val,
                       inet_ntop (p.family, &p.u.prefix, buf, BUFSIZ),
                       p.prefixlen);
        else
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"rd unknown, default to 0:0 prefix %s/%d",
                       inet_ntop (p.family, &p.u.prefix, buf, BUFSIZ),
                       p.prefixlen);
        if (attr)
        {
            bgp_update (peer, &p, attr, afi, SAFI_ENCAP,
                        ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, &prd, NULL, 0);
        }
        else
        {
            bgp_withdraw (peer, &p, attr, afi, SAFI_ENCAP,
                          ROUTE_PROTO_EBGP, BGP_ROUTE_NORMAL, &prd, NULL);
        }
    }
...
bgp_encap_init (void)
{
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_rd_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_rd_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_rd_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_neighbor_advertised_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv4_encap_rd_neighbor_advertised_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_rd_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_rd_tags_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_rd_neighbor_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_neighbor_advertised_routes_cmd, CMD_LOCAL);
    install_element (CONFIG_NODE, &show_bgp_ipv6_encap_rd_neighbor_advertised_routes_cmd, CMD_LOCAL);
}
