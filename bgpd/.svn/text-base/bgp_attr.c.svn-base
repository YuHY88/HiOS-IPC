...
encap_tlv_dup(struct bgp_attr_encap_subtlv *orig)
{
    ...
    for (p = orig, tail = subtlv = NULL; p; p = p->next)
    {
        ...
        {
            tail = subtlv = XCALLOC(MTYPE_ENCAP_TLV, size);
        }
 ...
 */
void
bgp_attr_dup (struct attr *new_attr, struct attr *orig)
{
    struct attr_extra *extra = new_attr->extra;
    ...
    *new_attr = *orig;
    ...
    if (extra)
    {
        new_attr->extra = extra;
        memset(new_attr->extra, 0, sizeof(struct attr_extra));
        if (orig->extra)
        {
            *new_attr->extra = *orig->extra;
            if (orig->extra->encap_subtlvs)
            {
                new_attr->extra->encap_subtlvs = encap_tlv_dup(orig->extra->encap_subtlvs);
            }
        }
    }
    else if (orig->extra)
    {
        new_attr->extra = bgp_attr_extra_new();
        *new_attr->extra = *orig->extra;
        if (orig->extra->encap_subtlvs)
        {
            new_attr->extra->encap_subtlvs = encap_tlv_dup(orig->extra->encap_subtlvs);
...
attr_show_all_iterator (struct hash_backet *backet, struct vty *vty)
{
    ...
    vty_out (vty, "%p   %-12lu ", (void *)backet, attr->refcnt);
    ...
    if(attr->local_as)
    {
        vty_out (vty, "%u ", attr->local_as);
    }
	
    vty_out (vty, "%s", attr->aspath?attr->aspath->str:"");
	
    /* Print origin */
    vty_out (vty, "%s%s", bgp_origin_str[attr->origin],VTY_NEWLINE);
}
...
bgp_attr_default_intern (u_char origin)
{
    ...
    aspath_unintern (&new_attr->aspath);
    return new_attr;
}
                          ...
                          struct in_addr *as4_aggregator_addr)
{
    ...
    if (CHECK_FLAG (peer->cap, PEER_CAP_AS4_RCV))
    {
        ...
        //{
        	...
        	{
                zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] %s %s AS4_PATH",
                            peer->host, "AS4 capable peer, yet it sent");
        	}
			
            if (attr->flag & (ATTR_FLAG_BIT(BGP_ATTR_AS4_AGGREGATOR)))
        	{
                zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] %s %s AS4_AGGREGATOR",
                            peer->host, "AS4 capable peer, yet it sent");
        	}
        //}
        return BGP_ATTR_PARSE_PROCEED;
    ...
    if (attr->flag & (ATTR_FLAG_BIT (BGP_ATTR_AS4_AGGREGATOR) ) )
    {
        if (attr->flag & (ATTR_FLAG_BIT (BGP_ATTR_AGGREGATOR) ) )
        {
            ...
            if (attre->aggregator_as != BGP_AS_TRANS)
            {
                ...
                //if ( BGP_DEBUG(as4, AS4))
            	//{
                    zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] %s BGP not AS4 capable peer"
        ...
        else
        {
            ...
            //if ( BGP_DEBUG(as4, AS4))
        	//{
                zlog_debug (BGP_DEBUG_TYPE_AS4,"[AS4] %s BGP not AS4 capable peer send"
...
#define LEN_LEFT    (length - (stream_get_getp(s) - start))
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: %s sent invalid length, %lu",
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: %s, MP nexthop length, %u, goes past end of attribute",
	    ...
	    case 48:
	        ...
	        if (! IN6_IS_ADDR_LINKLOCAL (&attre->mp_nexthop_local))
	        {
	            ...
	            //if (BGP_DEBUG (update, UPDATE_IN))
	                zlog_debug (BGP_DEBUG_TYPE_UPDATE,"%s got two nexthop %s %s but second one is not a link-local nexthop", peer->host,
	        ...
	        break;
	    default:
	        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: (%s) Wrong multiprotocol next hop length: %d",
	                   __func__, peer->host, attre->mp_nexthop_len);
	        return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
    ...
    if (!LEN_LEFT)
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: (%s) Failed to read SNPA and NLRI(s)",
                   __func__, peer->host);
        return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
	
    {
    	...
    	{
            zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s sent non-zero value, %u, for defunct SNPA-length field",
                       peer->host, val);
    	}
    }
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"%s: (%s) Failed to read NLRI",
                   __func__, peer->host);
        return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
...
bgp_attr_unknown (struct bgp_attr_parser_args *args)
{
    ...
    //if (BGP_DEBUG (normal, NORMAL))
        zlog_debug (BGP_DEBUG_TYPE_NORMAL,"%s Unknown attribute is received (type %d, length %d)",
                      ...
                      struct peer *from, struct prefix_rd *prd, u_char *tag)
{
            ...
            safi ==  SAFI_UNICAST)   /* only write NH attr for unicast safi */
    {
		...
		if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE) && (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_SEND_IBGP)))
		{		
			stream_put_ipv4 (s, peer->su_local.sin.sin_addr.s_addr);
		}
        else
        {
            stream_put_ipv4 (s, attr->nexthop.s_addr);
