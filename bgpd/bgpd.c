...
struct bgp_vrf_config *bgp_vrf_redist_info[BGP_VRF_MAX + 1] = {NULL};
...
int
bgp_flag_unset (struct bgp *bgp, u_int16_t flag)
{
...
static void
bgp_config_unset (struct bgp *bgp, u_int16_t config)
{
...
time_t bgp_clock (void)
{
    ...
    time_get_time(TIME_CLK_MONOTONIC, &tv);
    return tv.tv_sec;
}
...
bgp_timers_set (struct bgp *bgp, u_int32_t keepalive, u_int32_t holdtime)
{
    ...
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        ret = peer_timers_set(peer,bgp->default_keepalive,bgp->default_holdtime);
        if (ret == 0 &&(BGP_IS_VALID_STATE_FOR_NOTIF(peer->status)))
        {
            bgp_notify_send (peer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_CONFIG_CHANGE);
        }
    }
...
peer_new (struct bgp *bgp)
{
    ...
    peer->ibuf_all = stream_new (BGP_MAX_PACKET_SIZE);
    peer->obuf = stream_fifo_new ();
	peer->hold_buf = stream_fifo_new ();
             ...
             as_t remote_as, afi_t afi, safi_t safi)
{
    ...
    if(BGP_DEFAULT_KEEPALIVE != bgp->default_keepalive ||  BGP_DEFAULT_HOLDTIME != bgp->default_holdtime )
    {
    ...
    if(BGP_DEFAULT_CONNECT_RETRY != bgp->default_connect)
    {
        SET_FLAG (peer->config, PEER_CONFIG_CONNECT);
        peer->connect = bgp->default_connect;
    }
	
    peer->v_connect = bgp->default_connect;
...
peer_delete (struct peer *peer)
{
	...
	if(peer->status == Deleted)
	{
		zlog_debug(BGP_DEBUG_TYPE_OTHER, "%s[%d]:The peer has deleted!\n", __FUNCTION__, __LINE__);
		return 0;	
	}
    ...
    if (peer->password)
    {
        ...
        if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
        {
			bgp_sendmsg_to_tcp(BGP_UNSET_PASSWD_MSG,peer);
        }
    }
	...
	if (peer->hold_buf)
    {
        stream_fifo_free (peer->hold_buf);
        peer->hold_buf = NULL;
    }
                             ...
                             afi_t afi, safi_t safi)
{
	...
	//bgp_sendmsg_to_tcp(BGP_SET_PASSWD_MSG,peer);
                   ...
                   struct peer_group *group, afi_t afi, safi_t safi)
{
    ...
    if (! peer_group_active (peer))
    {
		...
		if(NULL == listnode_lookup (group->peer, peer))
		{
			return BGP_ERR_PEER_GROUP_MEMBER_EXISTS;
		}
...
static int
bgp_startup_timer_expire (void * para)
{
    struct bgp *bgp;
...
bgp_create (as_t *as, const char *name)
{
    ...
    for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
        for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
        {
            ...
            bgp->network[afi][safi] = bgp_table_init (afi, safi);
    ...
    bgp->default_holdtime = BGP_DEFAULT_HOLDTIME;
    bgp->default_keepalive = BGP_DEFAULT_KEEPALIVE;
    bgp->default_connect = BGP_DEFAULT_CONNECT_RETRY;
    ...
    if(bgp->t_startup == 0) 
    {
        bgp->t_startup = high_pre_timer_add("bgp_start_timer",LIB_TIMER_TYPE_NOLOOP,bgp_startup_timer_expire, (void *)bgp,(bgp->restart_time)*1000);
	}

    bgp_redistribute_route_register();
	//bgp_redistribute_routev6_register();
...
bgp_delete (struct bgp *bgp)
{
    ...
    bgp_redistribute_route_unregister();
	//bgp_redistribute_routev6_unregister();
    ...
    for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
        ...
        {
            if((i == ROUTE_PROTO_IBGP) || ( i == ROUTE_PROTO_EBGP))
            {
                continue;
            }
            ...
            if(ROUTE_PROTO_OSPF == i || ROUTE_PROTO_ISIS == i || ROUTE_PROTO_RIP == i)
            {
                ...
                for(instance = 1; instance < MAX_INSTANCE_ID ; instance ++ )
                {
                    if(bgp->redist_instance[type_redis][instance].redist_instance_flag)
                    {
                        bgp_redistribute_unset (bgp, afi, i, instance);
                    }
                }
            }
            else
	...
	for(instance = BGP_VRF_MIN ; instance < BGP_VRF_MAX + 1 ; instance ++)
	{
		bgp_route_vrf_delete(bgp, instance);
	}
...
bgp_free (struct bgp *bgp)
{
    ...
    for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
        for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
        {
            ...
            {
            	bgp_route_delete_all(bgp->route[afi][safi]);
                bgp_table_finish (&bgp->route[afi][safi]);
            }
			
            if (bgp->network[afi][safi])
            {
                bgp_table_finish (&bgp->network[afi][safi]);
            }
...
/* add by fenglihua*/
void
bgp_calculate(struct bgp *bgp, afi_t afi, safi_t safi)
{
    struct bgp_node *rn;
    struct bgp_node *bn;
    struct bgp_info *bi;
    ...
    //if(BGP_DEBUG(events, EVENTS))
    //{
        zlog_debug ( BGP_DEBUG_TYPE_EVENTS,"%s[%d] bgp->flags 0x%x afi %d safi %d", __FUNCTION__,__LINE__,bgp->flags, afi, safi);
    //}
...
u_int16_t bgp_match_ecommunity(struct ecommunity *ecommunity )
{
	u_int16_t i = 0;	
	u_int16_t ret = 0;
	...
	for (i = BGP_VRF_MIN ;i < BGP_VRF_MAX + 1;i ++)
	{
		...
		ret = ecommunity_match(bgp_vrf_redist_info[i]->rt_import,ecommunity);
		if(ret)
		{
			return i;
		}
...
struct bgp_vrf_config *bgp_vrf_info_new(uint16_t vrf_id)
{
	struct bgp_vrf_config *bgp_vrf_config_new = NULL;
	
	bgp_vrf_config_new = (struct bgp_vrf_config *) XCALLOC (MTYPE_BGP_VRF,sizeof (struct bgp_vrf_config));

	if(NULL == bgp_vrf_config_new)
	{
		zlog_err("%s[%d], memory xcalloc error.", __func__, __LINE__);
		return NULL;
	}
	
	bgp_vrf_config_new->vpn_id = vrf_id;
	bgp_vrf_config_new->label_apply = LABEL_APPLY_PER_ROUTE;
	bgp_vrf_config_new->rt_export = NULL;
	bgp_vrf_config_new->rt_import = NULL;
	bgp_vrf_config_new->redist_ospf = list_new();
	bgp_vrf_config_new->redist_isis = list_new();
	bgp_vrf_config_new->redist_rip = list_new();
	
	return bgp_vrf_config_new;
}

struct bgp_vrf_config *bgp_vrf_info_get(uint16_t vrf_id)
{
	if(bgp_vrf_redist_info[vrf_id] == NULL)
	{
		bgp_vrf_redist_info[vrf_id] = bgp_vrf_info_new(vrf_id);
	}
	
	return bgp_vrf_redist_info[vrf_id];
}

void bgp_vrf_info_delete(uint16_t vrf_id)
{
    ...
    //if(BGP_DEBUG (events, EVENTS))
	//{
		zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] vrf %d ",__FUNCTION__,__LINE__, vrf_id);
	//}
	...
	if(bgp_vrf_redist_info[vrf_id]->redist_ospf)
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_ospf, node, next, redist))
		{
			if(redist->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_ospf,redist);
				XFREE(MTYPE_BGP_REDIST,redist);
				redist = NULL;
			}
		}
		list_free(bgp_vrf_redist_info[vrf_id]->redist_ospf);
	}
	
	if(bgp_vrf_redist_info[vrf_id]->redist_isis)
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_isis, node, next, redist))
		{
			if(redist->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_isis,redist);
				XFREE(MTYPE_BGP_REDIST,redist);
				redist = NULL;
			}
		}
		list_free(bgp_vrf_redist_info[vrf_id]->redist_isis);
	}
	
	if(bgp_vrf_redist_info[vrf_id]->redist_rip)
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_rip, node, next, redist))
		{
			if(redist->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_rip,redist);
				XFREE(MTYPE_BGP_REDIST,redist);
				redist = NULL;
			}
		}
		list_free(bgp_vrf_redist_info[vrf_id]->redist_rip);
	}	
	
	if(bgp_vrf_redist_info[vrf_id]->rt_import)
	{
		ecommunity_unintern(&bgp_vrf_redist_info[vrf_id]->rt_import);
		bgp_vrf_redist_info[vrf_id]->rt_import = NULL;
	}

	if(bgp_vrf_redist_info[vrf_id]->rt_export)
	{
		ecommunity_unintern(&bgp_vrf_redist_info[vrf_id]->rt_export);
		bgp_vrf_redist_info[vrf_id]->rt_export = NULL;
	}
	
    XFREE (MTYPE_BGP_VRF,bgp_vrf_redist_info[vrf_id]);
    bgp_vrf_redist_info[vrf_id] = NULL;

	return;

}

struct bgp_vrf_config *bgp_vrf_get_by_prd(struct prefix_rd *prd)
{
	int i;

	for(i = BGP_VRF_MIN; i < BGP_VRF_MAX + 1; i ++)
	{
		if(NULL == bgp_vrf_redist_info[i])
		{
			continue;
		}
				
		if( 64 != bgp_vrf_redist_info[i]->prd.prefixlen)
		{
			continue;
		}
		
		if(prefix_rd_same(&bgp_vrf_redist_info[i]->prd, prd))
		{
			return bgp_vrf_redist_info[i];
		}
	}
...
void bgp_vrf_manage_delete(uint16_t vrf_id)
{
	struct bgp *bgp;
	struct peer *peer;
    struct listnode *node, *next;

	if(vrf_id < BGP_VRF_MIN || vrf_id > BGP_VRF_MAX)
	{
		return;
	}
    ...
    //if(BGP_DEBUG (events, EVENTS))
	//{
		zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] vrf_id %d",__FUNCTION__,__LINE__,vrf_id);
	//}
	...
	{
		/*删除所有的路由*/
		bgp_route_vrf_delete(bgp,vrf_id);
		
		/*删除邻居*/
	    for (ALL_LIST_ELEMENTS (bgp->peer, node, next, peer))
	    {
	    	if(peer->vrf_id == vrf_id)
			{
	        	peer_delete (peer);
			}
	    }
	...
	bgp_vrf_info_delete(vrf_id);
	
	return ;
}

void bgp_vrf_manage_rd(struct l3vpn_entry *l3vpn_info)
{
	int ret = 0;
	uint16_t vrf_id = 0;
    struct prefix_rd prd;

	if(!(l3vpn_info && l3vpn_info->l3vpn.vrf_id && l3vpn_info->rd[0] != '\0' ))
	{
		return;
	}
	
	vrf_id = l3vpn_info->l3vpn.vrf_id;
	...
	ret = str2prefix_rd ((const char *)l3vpn_info->rd, &prd);
	if (! ret)
	{
		return;
	}
	...
	if(bgp_vrf_redist_info[vrf_id] == NULL)
	{
		bgp_vrf_info_get(vrf_id);
	}
	
	if(bgp_vrf_redist_info[vrf_id] && bgp_vrf_redist_info[vrf_id]->prd.prefixlen != 64)
	{
		memcpy(&bgp_vrf_redist_info[vrf_id]->prd,&prd,sizeof(struct prefix_rd ));
	}    
	
	return ;
}

uint16_t bgp_vrf_get_rd(struct prefix_rd *prd)
{
	uint16_t vrf_id = 0;
	...
	for(vrf_id = BGP_VRF_MIN ; vrf_id < BGP_VRF_MAX + 1; vrf_id ++)
	{
		if(bgp_vrf_redist_info[vrf_id] == NULL)
		{
			continue;
		...
		if(prefix_rd_same(prd,&bgp_vrf_redist_info[vrf_id]->prd_src))
		{
			return vrf_id;
		}
	}
...
void bgp_vrf_manage_import_add(uint32_t         vrf_id, char *target)
{
	struct ecommunity* rt_import = NULL;
	struct ecommunity* rt_tmp = NULL;
    ...
    //if(BGP_DEBUG (events, EVENTS))
	//{
		zlog_debug(BGP_DEBUG_TYPE_EVENTS,"%s[%d] vrf_id %d target %s ", __FUNCTION__, __LINE__, vrf_id, target);
	//}	

	rt_import = ecommunity_str2com((const char *)target, ECOMMUNITY_ROUTE_TARGET, 0);
	if(rt_import == NULL)
	{
		zlog_err("%s[%d] The import target '%s' is invalid ", __FUNCTION__, __LINE__, target);
		return;
	}
	
	if(bgp_vrf_redist_info[vrf_id]->rt_import == NULL)
	{
		bgp_vrf_redist_info[vrf_id]->rt_import = ecommunity_intern(rt_import);
	}
	else
	{
		if(bgp_vrf_redist_info[vrf_id]->rt_import)
		{
			rt_tmp = bgp_vrf_redist_info[vrf_id]->rt_import;
		}
		
		if(ecommunity_include(bgp_vrf_redist_info[vrf_id]->rt_import,rt_import))
		{
			ecommunity_free(&rt_import);
			return;
		}
		
		ecommunity_rt_add(rt_import, rt_tmp);
		ecommunity_unintern(&rt_tmp);
		bgp_vrf_redist_info[vrf_id]->rt_import = ecommunity_intern(rt_import);
	}
	
	bgp_vrf_redist_info[vrf_id]->import_num ++;
		
	bgp_vrf_refresh_import_add(vrf_id);
	
	return;
}

void bgp_vrf_manage_import_delete(uint32_t          vrf_id, char *target)
{
	int i = 0;
	int ret = 0;
	u_int8_t *p;
	...
	if(bgp_vrf_redist_info[vrf_id] == NULL || bgp_vrf_redist_info[vrf_id]->rt_import == NULL)
	{
		return;
	}
	...
	if(target == NULL || bgp_vrf_redist_info[vrf_id]->import_num == 1)
	{
		ecommunity_unintern(&bgp_vrf_redist_info[vrf_id]->rt_import);
		bgp_vrf_redist_info[vrf_id]->rt_import = NULL;
		bgp_vrf_redist_info[vrf_id]->import_num = 0;
	}
	else
	{
		rt_tmp = ecommunity_new ();

	    for (p = bgp_vrf_redist_info[vrf_id]->rt_import->val; i < bgp_vrf_redist_info[vrf_id]->rt_import->size; p += ECOMMUNITY_SIZE, i++)
	    {			
	        ret = memcmp (p, rt_import->val, ECOMMUNITY_SIZE);
	        if (ret)
	        {
				ecommunity_add_val (rt_tmp, (struct ecommunity_val *)p);
	        }
	    }

		if(rt_tmp)
		{
			for(i = 0 ; i < rt_tmp->size; i ++)
			{
				zlog_debug(BGP_DEBUG_TYPE_OTHER, "%s[%d] %d target  %d %d %d %d %d %d %d %d",__FUNCTION__,__LINE__,i,rt_tmp->val[i*8],
					rt_tmp->val[i*8+1],rt_tmp->val[i*8+2],
					rt_tmp->val[i*8+3],rt_tmp->val[i*8+4],
					rt_tmp->val[i*8+5],rt_tmp->val[i*8+6],
					rt_tmp->val[i*8+7]);
			}
		}
		...
		bgp_vrf_redist_info[vrf_id]->rt_import = rt_tmp;
		bgp_vrf_redist_info[vrf_id]->rt_import = ecommunity_intern(rt_tmp);
		
		bgp_vrf_redist_info[vrf_id]->import_num --;
	}
	
	bgp_vrf_refresh_import_delete(vrf_id, rt_import);
	
	return ;
}

void bgp_vrf_manage_export_add(uint32_t vrf_id, char *target)
{
	...
	rt_export = ecommunity_str2com((const char *)target, ECOMMUNITY_ROUTE_TARGET, 0);
	if(rt_export == NULL)
	{
		zlog_err("%s[%d] The export target '%s' is invalid ", __FUNCTION__, __LINE__, target);
		return;
	}
	
	if(bgp_vrf_redist_info[vrf_id]->rt_export == NULL)
	{
		bgp_vrf_redist_info[vrf_id]->rt_export = ecommunity_intern(rt_export);
	}
	else
	{
		if(bgp_vrf_redist_info[vrf_id]->rt_export)
		{
			rt_tmp = bgp_vrf_redist_info[vrf_id]->rt_export;
		}
		
		if(ecommunity_include(bgp_vrf_redist_info[vrf_id]->rt_export,rt_export))
		{
			ecommunity_free(&rt_export);
			return;
		}
		
		ecommunity_rt_add(rt_export, rt_tmp);
		ecommunity_unintern(&rt_tmp);
		bgp_vrf_redist_info[vrf_id]->rt_export = ecommunity_intern(rt_export);
	}
	
	bgp_vrf_redist_info[vrf_id]->export_num ++;
		
	bgp_vrf_refresh_export(vrf_id);
	
	return;

}

void bgp_vrf_manage_export_delete(uint32_t vrf_id, char *target)
{
	int i = 0;
	int ret = 0;
    u_int8_t *p;
	...
	if(bgp_vrf_redist_info[vrf_id] == NULL || bgp_vrf_redist_info[vrf_id]->rt_export == NULL)
	{
		return;
	}
	...
	if(target == NULL || bgp_vrf_redist_info[vrf_id]->export_num == 1)
	{
		ecommunity_unintern(&bgp_vrf_redist_info[vrf_id]->rt_export);
		bgp_vrf_redist_info[vrf_id]->rt_export = NULL;
		bgp_vrf_redist_info[vrf_id]->export_num = 0;
	}
	else
	{
		rt_tmp = ecommunity_new ();

	    for (p = bgp_vrf_redist_info[vrf_id]->rt_export->val; i < bgp_vrf_redist_info[vrf_id]->rt_export->size; p += ECOMMUNITY_SIZE, i++)
	    {
	        ret = memcmp (p, rt_export->val, ECOMMUNITY_SIZE);
	        if (ret)
	        {
				ecommunity_add_val (rt_tmp, (struct ecommunity_val *)p);
	        }
		...
		bgp_vrf_redist_info[vrf_id]->rt_export = rt_tmp;
		bgp_vrf_redist_info[vrf_id]->rt_export = ecommunity_intern(rt_tmp);
...
void bgp_vrf_manage_target_delete(uint32_t vrf_id)
{
	if(vrf_id < BGP_VRF_MIN || vrf_id > BGP_VRF_MAX)
	{
		return;
	...
	bgp_vrf_manage_import_delete(vrf_id, NULL);
	
	bgp_vrf_manage_export_delete(vrf_id, NULL);

	return;
}

void bgp_vrf_manage_apply_label(struct l3vpn_entry *l3vpn_info)
{
	int update = 0;
	uint32_t inlabel = 0;
	afi_t afi = AFI_IP;
	...
	if(vrf_id < BGP_VRF_MIN || vrf_id > BGP_VRF_MAX || bgp_vrf_redist_info[vrf_id] == NULL)	
	{
		return;
	}
	
	if(bgp_vrf_redist_info[vrf_id]->label_apply == l3vpn_info->label_apply)
	{
		return;
	}
	
	if(LABEL_APPLY_PER_VPN == l3vpn_info->label_apply)
	{
		bgp_vrf_redist_info[vrf_id]->label_apply = LABEL_APPLY_PER_VPN;
		bgp_vrf_redist_info[vrf_id]->inlabel = bgp_com_label_new(vrf_id);
		if(bgp_vrf_redist_info[vrf_id]->inlabel == 0)
		{
			zlog_err("%s[%d] vrf_id %d get label error ", __func__, __LINE__, vrf_id);
			return;
		}
	}
	else if(LABEL_APPLY_PER_ROUTE == l3vpn_info->label_apply)
	{
		bgp_vrf_redist_info[vrf_id]->label_apply = LABEL_APPLY_PER_ROUTE;
		bgp_com_label_free(vrf_id,bgp_vrf_redist_info[vrf_id]->inlabel);
	}
	else
	{
		return;
	...
	for(rn = bgp_table_top(bgp->rib[afi][SAFI_MPLS_VPN]); rn; rn = bgp_route_next (rn))
	{
        if ((table = rn->info) != NULL)
    	{
			for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
			{
			    ...
			    {
			        if (ri->extra && !BGP_TAG_IS_EMPTY(ri->extra->in_tag) && ri->vrf_id == vrf_id)
			        {
						...
						else
						{
							if(LABEL_APPLY_PER_VPN == l3vpn_info->label_apply)
							{
								...
								label_decode(ri->extra->in_tag,bgp_vrf_redist_info[vrf_id]->inlabel);
								SET_FLAG(ri->extra->in_tag[2],BGP_INFO_EXTRA_TAG_BOTTOM_BIT);
							}
							else
							{
								inlabel = bgp_com_label_new(ri->vrf_id);
								if( inlabel == 0)
								{                                   
									zlog_err("%s[%d] vrf_id %d ", __func__, __LINE__, ri->vrf_id);
			                        continue;
			                    }
								label_decode(ri->extra->in_tag,inlabel);
								SET_FLAG(ri->extra->in_tag[2],BGP_INFO_EXTRA_TAG_BOTTOM_BIT);
							}
						}
			        }
			    ...
			    if (update)
			    {
			        bgp_process (bgp, rm, afi, SAFI_MPLS_VPN);
			    }
			}
    	}
	}
...
void bgp_route_vrf_delete(struct bgp *bgp, uint16_t vrf_id)
{
	int flag = 0;
	afi_t afi = AFI_IP;
	...
	{		
		...
		{
			if (ri->vrf_id == vrf_id || ri->vrf_id_to == vrf_id)
			{
				flag = 1;
...
int 
bgp_redist_vrf_ospf_add(uint16_t vrf_id,uint8_t instance, uint8_t metric_flag,u_int32_t metric)
{	
	uint8_t flag = 1;
	struct bgp_redist *redist_ospf;
	struct bgp_redist *new_redist_ospf;
	struct listnode *node, *nnode;

	if(bgp_vrf_redist_info[vrf_id] == NULL || bgp_vrf_redist_info[vrf_id]->redist_ospf == NULL)
	{
		return 0;
	}
	
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_ospf, node, nnode, redist_ospf))
	{
		if(redist_ospf->instance == instance)
		{
			/*重复配置命令返回*/
			if( metric_flag == redist_ospf->metric_flag && metric == redist_ospf->metric )
			{
				return 0;
			}
			...
			if(metric_flag != redist_ospf->metric_flag || metric != redist_ospf->metric )
			{
				redist_ospf->metric_flag = metric_flag;
				if(metric_flag)
				{
					redist_ospf->metric = metric;
				}
				else
				{
					redist_ospf->metric = 0;
				}
			}
	...
	if(flag)
	{
		new_redist_ospf = XCALLOC (MTYPE_BGP_REDIST, sizeof (struct bgp_redist));

		if(NULL == new_redist_ospf)
		{
			return -1;
		}
		
		new_redist_ospf->instance = instance;
		if(metric_flag)
		{
			new_redist_ospf->metric_flag = 1;
			new_redist_ospf->metric = metric;
		}
	
		listnode_add_sort(bgp_vrf_redist_info[vrf_id]->redist_ospf, new_redist_ospf);
...
int 
bgp_redist_vrf_ospf_delete(uint16_t vrf_id,uint8_t instance)
{	
	int flag = 0;
	struct bgp_redist *redist_ospf;
	struct listnode *node, *nnode;
	...
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_ospf, node, nnode, redist_ospf))
	{
		if(redist_ospf->instance == instance)
		{				
			listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_ospf,redist_ospf);
			XFREE(MTYPE_BGP_REDIST,redist_ospf);
			redist_ospf = NULL;
			flag = 1;
		}
...
int 
bgp_redist_vrf_isis_add(uint16_t vrf_id,uint8_t instance, uint8_t metric_flag,u_int32_t metric)
{	
	uint8_t flag = 1;
	struct bgp_redist *redist_isis;
	struct bgp_redist *new_redist_isis;
	struct listnode *node, *nnode;

	if(bgp_vrf_redist_info[vrf_id] == NULL || bgp_vrf_redist_info[vrf_id]->redist_isis == NULL)
	{
		return 0;
	}
	
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_isis, node, nnode, redist_isis))
	{
		if(redist_isis->instance == instance)
		{
			/*重复配置命令返回*/
			if( metric_flag == redist_isis->metric_flag && metric == redist_isis->metric )
			{
				return 0;
			}
			...
			if(metric_flag != redist_isis->metric_flag || metric != redist_isis->metric )
			{
				redist_isis->metric_flag = metric_flag;
				if(metric_flag)
				{
					redist_isis->metric = metric;
				}
				else
				{
					redist_isis->metric = 0;
				}
			}
	...
	if(flag)
	{
		new_redist_isis = XCALLOC (MTYPE_BGP_REDIST, sizeof (struct bgp_redist));
		
		if(NULL == new_redist_isis)
		{
			return -1;
		}
		
		new_redist_isis->instance = instance;
		if(metric_flag)
		{
			new_redist_isis->metric_flag = 1;
			new_redist_isis->metric = metric;
		}
	
		listnode_add_sort(bgp_vrf_redist_info[vrf_id]->redist_isis, new_redist_isis);
...
int 
bgp_redist_vrf_isis_delete(uint16_t vrf_id,uint8_t instance)
{	
	int flag = 0;
	struct bgp_redist *redist_isis;
	struct listnode *node, *nnode;
	...
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_isis, node, nnode, redist_isis))
	{
		if(redist_isis->instance == instance)
		{				
			listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_isis,redist_isis);
			XFREE(MTYPE_BGP_REDIST,redist_isis);
			redist_isis = NULL;
			flag = 1;
		}
	}
...
int 
bgp_redist_vrf_rip_add(uint16_t vrf_id,uint8_t instance, uint8_t metric_flag,u_int32_t metric)
{	
	uint8_t flag = 1;
	struct bgp_redist *redist_rip;
	struct bgp_redist *new_redist_rip;
	struct listnode *node, *nnode;
		
	if(bgp_vrf_redist_info[vrf_id] == NULL || bgp_vrf_redist_info[vrf_id]->redist_rip == NULL)
	{
		return 0;
	}
	
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_rip, node, nnode, redist_rip))
	{
		if(redist_rip->instance == instance)
		{
			/*重复配置命令返回*/
			if( metric_flag == redist_rip->metric_flag && metric == redist_rip->metric )
			{
				return 0;
			}
			...
			if(metric_flag != redist_rip->metric_flag || metric != redist_rip->metric )
			{
				redist_rip->metric_flag = metric_flag;
				if(metric_flag)
				{
					redist_rip->metric = metric;
				}
				else
				{
					redist_rip->metric = 0;
	...
	if(flag)
	{
		new_redist_rip = XCALLOC (MTYPE_BGP_REDIST, sizeof (struct bgp_redist));
		
		if(NULL == new_redist_rip)
		{
			return -1;
		}
		
		new_redist_rip->instance = instance;
		if(metric_flag)
		{
			new_redist_rip->metric_flag = 1;
			new_redist_rip->metric = metric;
		}
	
		listnode_add_sort(bgp_vrf_redist_info[vrf_id]->redist_rip, new_redist_rip);
...
int 
bgp_redist_vrf_rip_delete(uint16_t vrf_id,uint8_t instance)
{	
	int flag = 0;
	struct bgp_redist *redist_rip;
	struct listnode *node, *nnode;
	...
	for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_rip, node, nnode, redist_rip))
	{
		if(redist_rip->instance == instance)
		{				
			listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_rip,redist_rip);
			XFREE(MTYPE_BGP_REDIST,redist_rip);
			redist_rip = NULL;
			flag = 1;
		}
...
int 
bgp_redistribute_vrf_lookup_by_instance(uint16_t vrf_id,uint8_t instance,uchar type )
{	
	struct bgp_redist *redist;
	struct listnode *node, *nnode;
	...
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_ospf, node, nnode, redist))
		{
			if(redist->instance == instance)
			{				
				return 1;
			}
	...
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_rip, node, nnode, redist))
		{
			if(redist->instance == instance)
			{				
				return 1;
	...
	{
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_isis, node, nnode, redist))
		{
			if(redist->instance == instance)
			{				
				return 1;
...
void bgp_redist_vrf_delete(struct bgp *bgp)
{
	uint16_t vrf_id = 0;
	struct bgp_redist *redist_route;
	struct listnode *node, *nnode;
	...
	for(vrf_id = BGP_VRF_MIN ; vrf_id < BGP_VRF_MAX + 1; vrf_id ++)
	{
		...
		bgp_vrf_redist_info[vrf_id]->redist[0] = 0;
		bgp_vrf_redist_info[vrf_id]->redist_metric_flag[0] = 0;
		bgp_vrf_redist_info[vrf_id]->redist_metric[0] = 0;

		bgp_vrf_redist_info[vrf_id]->redist[1] = 0;
		bgp_vrf_redist_info[vrf_id]->redist_metric_flag[1] = 0;
		bgp_vrf_redist_info[vrf_id]->redist_metric[1] = 0;

		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_ospf, node, nnode, redist_route))
		{
			if(redist_route->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_ospf,redist_route);
				XFREE(MTYPE_BGP_REDIST,redist_route);
				redist_route = NULL;
			}
		}
		
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_isis, node, nnode, redist_route))
		{
			if(redist_route->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_isis,redist_route);
				XFREE(MTYPE_BGP_REDIST,redist_route);
				redist_route = NULL;
			}
		...
		for (ALL_LIST_ELEMENTS (bgp_vrf_redist_info[vrf_id]->redist_rip, node, nnode, redist_route))
		{
			if(redist_route->instance)
			{				
				listnode_delete(bgp_vrf_redist_info[vrf_id]->redist_rip,redist_route);
				XFREE(MTYPE_BGP_REDIST,redist_route);
				redist_route = NULL;
			}
		}
...
void 
bgp_vrf_refresh_import_add(uint16_t vrf_id)
{
    struct bgp * bgp ;
    struct peer *peer;
    ...
    for( ALL_LIST_ELEMENTS_RO(bm->bgp,bgpnode,bgp) )
    {
		...
		{			
	        ...
	        {	
	        	if(peer->afc[AFI_IP][SAFI_MPLS_VPN] && peer->afc_recv[AFI_IP][SAFI_MPLS_VPN])
				{
	            	bgp_route_refresh_send (peer, AFI_IP, SAFI_MPLS_VPN, 0, 0, 0);
				}
	        }
		}
...
void bgp_vrf_refresh_import_delete(uint16_t vrf_id, struct ecommunity* rt_import)
{
	int flag = 0;
	struct bgp * bgp ;
	struct peer *peer;
	...
	for( ALL_LIST_ELEMENTS_RO(bm->bgp, bgpnode, bgp) )
	{
		for (bn = bgp_table_top (bgp->rib[AFI_IP][SAFI_MPLS_VPN]); bn; bn = bgp_route_next (bn))
		{
			if ((table = bn->info) != NULL)
			{
				for (rn = bgp_table_top(table); rn; rn = bgp_route_next (rn))
				{
					flag = 0;
					for (ri = rn->info; ri; ri = ri->next)
					{
						/*删除target之后，匹配不上target，删除本地路由，通告邻居删除*/
						if(CHECK_FLAG(ri->flags, BGP_INFO_VALID)
							&& (ri->vrf_id_to == vrf_id && ri->vrf_id != vrf_id)
							&& ri->attr 
							&& ri->attr->extra 
							&& ( (rt_import == NULL || ecommunity_match(ri->attr->extra->ecommunity, rt_import)) 
								&& !ecommunity_match(bgp_vrf_redist_info[vrf_id]->rt_import,ri->attr->extra->ecommunity)))
						{
							bgp_info_delete (rn, ri);
							
							if(CHECK_FLAG(ri->flags, BGP_INFO_SELECTED))
							{
								bgp_zebra_announce (&rn->p, ri, bgp, SAFI_MPLS_VPN, IPC_OPCODE_DELETE, 1);
							}
							
							SET_FLAG(ri->flags,BGP_INFO_TARGET);
					...
					if(flag)
					{
						...
						for (ALL_LIST_ELEMENTS (bgp->peer, node, next, peer))
						{
							...
							if(peer->vrf_id == vrf_id)
							{		
								bgp_process_announce_selected (peer, NULL, rn, AFI_IP, SAFI_UNICAST);
							}
						}
						
						/*删除本地路由*/
						bgp_process (bgp, rn, AFI_IP, SAFI_MPLS_VPN);	
					}
				}
			}
		}
...
void bgp_vrf_refresh_export(uint16_t vrf_id)
{	
	int flag = 0;
	afi_t afi = AFI_IP;
	struct bgp * bgp ;
	...
	for( ALL_LIST_ELEMENTS_RO(bm->bgp, bgpnode, bgp) )
	{
		...
		prn = bgp_node_lookup(table, (struct prefix *)&bgp_vrf_redist_info[vrf_id]->prd);
		if( NULL == prn )
		{
			continue;
		}
    ...
    if (flag == PEER_FLAG_SHUTDOWN)
    {
        if (CHECK_FLAG (peer->flags, flag))
        {
            ...
            if (peer->t_pmax_restart)
            {
                ...
                //if (BGP_DEBUG (events, EVENTS))
            	//{
                    zlog_debug (BGP_DEBUG_TYPE_EVENTS,"%s Maximum-prefix restart timer canceled", peer->host);
            	//}
            }
                     ...
                     int set)
{
    ...
    {
        return BGP_ERR_ALLOWED_ONLY_FOR_EBGP;
    }
...
peer_default_originate_unset (struct peer *peer, afi_t afi, safi_t safi)
{
    ...
    if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
        if (peer->status == Established && peer->afc_nego[afi][safi])
        {
            bgp_default_originate (peer, afi, safi, 1, 0);
        }
        return 0;
    }
...
int
peer_redistribute_ibgp_route (struct peer *peer, afi_t afi, safi_t safi,int set)
{
	int update = 0;
    struct bgp *bgp;
    struct bgp_node *rn;
    ...
    if(peer->sort != BGP_PEER_IBGP)
    {
        return BGP_ERR_ALLOWED_ONLY_FOR_IBGP;
    }
	...
	if(set)
    {
        ...
        {
            SET_FLAG (peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE);
			update = 1;
        }
        else
        {
    ...
    else
    {
        if(CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE))
        {
            UNSET_FLAG (peer->af_flags[afi][safi], PEER_FLAG_IBGP_ROUTE);
			update = 0;
        }
        else
        {
    ...
    if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
        if(peer->status == Established && peer->afc_nego[afi][safi])
        {
            for(rn = bgp_table_top(bgp->rib[afi][safi]); rn; rn = bgp_route_next (rn))
            {
                if(rn->p.family == AF_INET && rn->p.prefixlen == 0  && rn->p.u.prefix4.s_addr == INADDR_ANY)
                {
                    continue;
                }
                ...
                for(ri = rn->info ; ri; ri = ri->next)
                {
                	/* do not send redistribute or network and  do not send back route to sender.*/
                	if(! ri->peer || ri->peer == bgp->peer_self || ri->peer == peer)
						continue;
					
					/*send valid && selected && ibgp route*/
					if(! CHECK_FLAG(ri->flags, BGP_INFO_HISTORY)
						&& CHECK_FLAG(ri->flags, BGP_INFO_VALID) 
						&& CHECK_FLAG(ri->flags, BGP_INFO_SELECTED)
						&& (ri->sub_type == BGP_ROUTE_NORMAL)
						&& ((ri->peer->as) && (ri->peer->as == ri->peer->local_as)))
                    {
                        //if(BGP_DEBUG (events, EVENTS))
                        //{
                            char buf[INET_ADDRSTRLEN];
                            zlog_debug(BGP_DEBUG_TYPE_EVENTS, "%s[%d] %s/%d ",__FUNCTION__,__LINE__,
                                       inet_ntop(AF_INET, &rn->p.u.prefix, buf, sizeof(buf)),rn->p.prefixlen);
                        //}
						
						if(update)
						{
							bgp_process_announce_selected (peer, ri,rn, afi, safi);
						}
						else
						{
							bgp_process_announce_selected (peer, NULL,rn, afi, safi);
						}
...
{	
    ...
    if(CHECK_FLAG(peer->config, PEER_CONFIG_WEIGHT) && (weight == peer->weight))
    {
        return 0;
    }
...
int
bgp_conenct_timers_set (struct bgp *bgp, u_int32_t connect)
{
    struct peer *peer;
    struct listnode *node, *nnode;
	
    if(connect == bgp->default_connect)
    {
        return 0;
    }
	
    bgp->default_connect = connect;
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if (peer->su.sin.sin_addr.s_addr)
        {
            peer_timers_connect_set(peer,connect);
        }
    }
...
int
bgp_connect_timers_unset (struct bgp *bgp)
{
    struct peer *peer;
    struct listnode *node, *nnode;
...
static int is_ebgp_multihop_configured (struct peer *peer)
{
    ...
    {
        group = peer->group;
        if ((peer_sort(peer) != BGP_PEER_IBGP) && (group->conf->ttl != 1))
        {
            return 1;
        }
        for (ALL_LIST_ELEMENTS (group->peer, node, nnode, peer1))
        {
            if ((peer_sort (peer1) != BGP_PEER_IBGP) && (peer1->ttl != 1))
            {
                return 1;
            }
        }
...
peer_ttl_security_hops_set (struct peer *peer, int gtsm_hops)
{
    ...
    zlog_debug (BGP_DEBUG_TYPE_OTHER,"peer_ttl_security_hops_set: set gtsm_hops to %d for %s", gtsm_hops, peer->host);
    ...
    else
    {
        ...
        for (ALL_LIST_ELEMENTS (group->peer, node, nnode, peer))
        {
            ...
            else if (peer->status < Established)
            {
                //if (BGP_DEBUG (events, EVENTS))
                //{
                    zlog_debug (BGP_DEBUG_TYPE_EVENTS,"%s Min-ttl changed", peer->host);
                //}
				peer->event = BGP_Stop;
                BGP_EVENT_ADD (peer);
            }
...
peer_ttl_security_hops_unset (struct peer *peer)
{
    ...
    zlog_debug (BGP_DEBUG_TYPE_OTHER,"peer_ttl_security_hops_unset: set gtsm_hops to zero for %s", peer->host);
    ...
    if (! CHECK_FLAG (peer->flags, PEER_FLAG_SHUTDOWN))
    {
        if (CHECK_FLAG (peer->sflags, PEER_STATUS_PREFIX_OVERFLOW))
        {
            ...
            if (peer->t_pmax_restart)
            {
                BGP_TIMER_OFF (peer->t_pmax_restart);
                //if (BGP_DEBUG (events, EVENTS))
                    zlog_debug (BGP_DEBUG_TYPE_EVENTS,"%s Maximum-prefix restart timer canceled",
                                peer->host);
            }
...
peer_uptime (time_t uptime2, char *buf, size_t len)
{
    ...
    {
        zlog_debug (BGP_DEBUG_TYPE_OTHER,"peer_uptime (): buffer shortage %lu", (u_long)len);
                       ...
                       struct peer *peer, afi_t afi, safi_t safi)
{
    ...
    char buf[SU_ADDRSTRLEN];
    char *addr;
    char tnl_name[IFNET_NAMESIZE];
	
    addr = peer->host;
    vty_out (vty, " bgp neighbor ");
    ...
    vty_out (vty, "%s%s", addr, VTY_NEWLINE);
    ...
    if ((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST)
    {
        ...
        if (! peer_group_active (peer))
        {
            ...
            if (peer->as)
            {
                vty_out (vty, "  remote-as %u%s", peer->as, VTY_NEWLINE);
            }
        }
        else
        {
		...
		if(peer->su_local.sa.sa_family == AF_INET || peer->su_local.sa.sa_family == AF_INET6)
		{
        	vty_out (vty, "  source %s%s",  sockunion2str (&peer->su_local, buf, SU_ADDRSTRLEN), VTY_NEWLINE);
		}
		
		if(peer->vrf_id)
		{
            vty_out (vty, "  ipv4-family vpn-instance %d%s", peer->vrf_id , VTY_NEWLINE);
		}
		...
		{   
			if(peer->ifindex_tnl)
			{
				ifm_get_name_by_ifindex(peer->ifindex_tnl,tnl_name);
            	vty_out (vty, "  binding %s %s", tnl_name, VTY_NEWLINE);
			}
			
            vty_out (vty, "  ipv4-family vpnv4 %s", VTY_NEWLINE);
        ...
        if (peer->change_local_as)
            if (! peer_group_active (peer))
                vty_out (vty, "  ebgp local-as %u%s%s%s",
        ...
        if (peer->password)
            if (!peer_group_active (peer)
                ...
                vty_out (vty, "  auth md5 %s%s", peer->password,
                         VTY_NEWLINE);
        ...
        {
            if((peer_sort (peer) == BGP_PEER_IBGP && BGP_DEFAULT_IBGP_ROUTEADV != peer->v_routeadv)
                    ||(peer_sort (peer) == BGP_PEER_EBGP && BGP_DEFAULT_EBGP_ROUTEADV != peer->v_routeadv))
            {
                vty_out (vty, "  advertisement-interval %d%s",  peer->v_routeadv, VTY_NEWLINE);
            }
        }
		
        /* Default weight. */
        if (CHECK_FLAG (peer->config, PEER_CONFIG_WEIGHT) && (BGP_ATTR_DEFAULT_WEIGHT != peer->weight))
            if (! peer_group_active (peer) ||
                    g_peer->weight != peer->weight)
            {
                vty_out (vty, "  route weight %d%s", peer->weight,  VTY_NEWLINE);
            }
	...
	{
        vty_out (vty, "  route next-hop change local%s%s",
    ...
    {
        vty_out (vty, "  as-path exclude private-as%s", VTY_NEWLINE);
    }
    ...
    {
        vty_out (vty, "  route redistribute default-route");
        if (peer->default_rmap[afi][safi].name)
        {
    ...
    /* ibgp information */
    if (peer_af_flag_check (peer, afi, safi, PEER_FLAG_IBGP_ROUTE)
            && ! peer->af_group[afi][safi])
    {
        vty_out (vty, "  route redistribute ibgp");
        vty_out (vty, "%s", VTY_NEWLINE);
    }
    ...
    if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX))
	{
        ...
        {
            vty_out (vty, "  route limit %ld", peer->pmax[afi][safi]);
    ...
    if (peer_af_flag_check (peer, afi, safi, PEER_FLAG_ALLOWAS_IN))
	{
        ...
        {
            vty_out (vty, "  as-path allow as-loop %d%s", peer->allowas_in[afi][safi], VTY_NEWLINE);
        }
	}
    ...
    if (CHECK_FLAG (peer->flags, PEER_FLAG_ENABLE))
    {
        vty_out (vty, "  neighbor enable%s", VTY_NEWLINE);
    }
}
...
int bgp_redistribute_vrf_lookup( u_int16_t vrf_id)
{
	struct bgp_redist *redist;
	struct listnode *node, *nnode;
	...
	if(bgp_vrf_redist_info[vrf_id]->redist[0] || bgp_vrf_redist_info[vrf_id]->redist[1])
	{
		return 1;
	}
...
void
bgp_config_write_redistribute_vrf (struct vty *vty, safi_t safi)
{
	u_int16_t i = 0;
	struct bgp_redist *redist;
	struct listnode *node, *nnode;
    ...
    for (i = BGP_VRF_MIN; i < BGP_VRF_MAX + 1; i++)
    {
		...
		if(!bgp_redistribute_vrf_lookup(i))
			continue;
		
		vty_out (vty, " ipv4-family vpn-instance %d%s",i,VTY_NEWLINE);

		if(bgp_vrf_redist_info[i]->redist[0])
		{
			vty_out (vty, "  redistribute static");
			if(bgp_vrf_redist_info[i]->redist_metric_flag[0])
			{
				vty_out (vty, " metric %u", bgp_vrf_redist_info[i]->redist_metric[0]);
			}
			vty_out (vty, "%s", VTY_NEWLINE);
		}

		if(bgp_vrf_redist_info[i]->redist[1])
		{
			vty_out (vty, "  redistribute connected");
			if(bgp_vrf_redist_info[i]->redist_metric_flag[1])
			{
				vty_out (vty, " metric %u", bgp_vrf_redist_info[i]->redist_metric[1]);
			}
			vty_out (vty, "%s", VTY_NEWLINE);
		}
		...
		for(ALL_LIST_ELEMENTS (bgp_vrf_redist_info[i]->redist_ospf, node, nnode, redist))
		{
			if(redist->instance)
			{				
                vty_out (vty, "  redistribute ospf %d", redist->instance);
		...
		for(ALL_LIST_ELEMENTS (bgp_vrf_redist_info[i]->redist_isis, node, nnode, redist))
		{
			if(redist->instance)
			{				
                vty_out (vty, "  redistribute isis %d", redist->instance);
				if(redist->metric_flag)
				{
					vty_out (vty, " metric %u", redist->metric);
				}
		...
		for(ALL_LIST_ELEMENTS (bgp_vrf_redist_info[i]->redist_rip, node, nnode, redist))
		{
			if(redist->instance)
			{				
                vty_out (vty, "  redistribute rip %d", redist->instance);
				if(redist->metric_flag)
				{
					vty_out (vty, " metric %u", redist->metric);
				}
...
bgp_config_write (struct vty *vty)
{
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
        ...
        /* BGP router ID. */
        if (CHECK_FLAG (bgp->config, BGP_CONFIG_ROUTER_ID)&& bgp->router_id.s_addr)
            vty_out (vty, " router-id %s%s", inet_ntoa (bgp->router_id),
                     VTY_NEWLINE);
        ...
        if(bgp->default_med)
        {
            vty_out (vty, " med default %u%s", bgp->default_med, VTY_NEWLINE);
        }
		
        /* BGP configuration. */
        if (bgp_flag_check (bgp, BGP_FLAG_ALWAYS_COMPARE_MED))
        {
            vty_out (vty, " med compare different-as%s", VTY_NEWLINE);
        }
		
        /* BGP deterministic-med. */
        if (bgp_flag_check (bgp, BGP_FLAG_DETERMINISTIC_MED))
        {
            vty_out (vty, " med compare deterministic%s", VTY_NEWLINE);
        }
        ...
        {
            vty_out (vty, " local-preference %u%s", bgp->default_local_pref, VTY_NEWLINE);
        }
		
        /* BGP default retry time. */
        if (bgp->default_connect != BGP_DEFAULT_CONNECT_RETRY)
        {
            vty_out (vty, " retry-time %d%s", bgp->default_connect, VTY_NEWLINE);
        }
        ...
        {
            vty_out (vty, " as-path ignore%s", VTY_NEWLINE);
        }
        ...
        /* BGP redistribute vrf configuration. */
		bgp_config_write_redistribute_vrf(vty, SAFI_MPLS_VPN);
		
        /* BGP static route configuration. */
        bgp_config_write_network (vty, bgp, AFI_IP, SAFI_UNICAST, &write);

		bgp_config_write_ipv6_family (vty, bgp, AFI_IP6, SAFI_UNICAST, &write_ipv6);

        /* BGP timers configuration. */
        if (bgp->default_keepalive != BGP_DEFAULT_KEEPALIVE && bgp->default_holdtime != BGP_DEFAULT_HOLDTIME)
        {
            vty_out (vty, " keepalive %d hold-time %d%s", bgp->default_keepalive,  bgp->default_holdtime, VTY_NEWLINE);
        }
...
bgp_master_init (void)
{
    ...
    FIFO_INIT (&bm->routefifo);	
    FIFO_INIT (&bm->routefifo_vrf);		
}
...
bgp_terminate (void)
{
    ...
    struct routefifo_vrf *routefifo_vrf = NULL;
    ...
    {
        routefifo_entry = (struct routefifo *)FIFO_TOP(&bm->routefifo);
        if (routefifo_entry)
        {
    ...
    {
        routefifo_vrf = (struct routefifo_vrf *)FIFO_TOP(&bm->routefifo_vrf);
        if (routefifo_vrf)
        {
            FIFO_DEL(routefifo_vrf);
            XFREE (MTYPE_BGP_VRF_ROUTE_INFO, routefifo_vrf);
        }
    }
