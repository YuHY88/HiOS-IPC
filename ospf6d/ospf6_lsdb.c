...
ospf6_lsdb_create(void *data)
{
    ...
    if (lsdb == NULL)
    {
        zlog_err("%-15s[%d] Can't malloc lsdb", __FUNCTION__, __LINE__);
        return (struct ospf6_lsdb *)NULL;
    }
...
_lsdb_count_assert(struct ospf6_lsdb *lsdb)
{
    ...
    zlog_debug(OSPF6_DBG_OTHER, "PANIC !! lsdb[%p]->count = %d, real = %d",
               lsdb, lsdb->count, num);

    for (debug = ospf6_lsdb_head(lsdb); debug;
            debug = ospf6_lsdb_next(debug))
        zlog_debug(OSPF6_DBG_OTHER, "%p %p %s lsdb[%p]", debug->prev, debug->next, debug->name,
                   debug->lsdb);
                       ...
                       struct ospf6_lsdb *lsdb)
{
...

    {
        ...
        zlog_debug(OSPF6_DBG_OTHER, "lsdb_lookup_next: key: %s", buf);
    }
...
ospf6_lsdb_head(struct ospf6_lsdb *lsdb)
{
	...
	if(lsdb == NULL)
	{
		zlog_warn("%s %d lsdb is NULL",__func__,__LINE__);
		return NULL;
	}
...
ospf6_lsdb_maxage_remover(struct ospf6_lsdb *lsdb)
{
    ...
    for (lsa = ospf6_lsdb_head(lsdb); lsa; lsa = ospf6_lsdb_next(lsa))
    {
        ...
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Remove MaxAge %s", lsa->name);
        }
        ...
        if (CHECK_FLAG(lsa->flag, OSPF6_LSA_SEQWRAPPED))
        {
            ...
            /*thread_execute(master_ospf6, ospf6_lsa_refresh, lsa, 0);*/
			high_pre_timer_add ((char *)"ospf6_lsa_ref_timer", LIB_TIMER_TYPE_NOLOOP,\
						ospf6_lsa_refresh, (void *)lsa, 10);
        }
        else
        {
            ospf6_lsdb_remove(lsa, lsdb);
