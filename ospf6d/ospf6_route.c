...
ospf6_route_create(void)
{
	...
	if (route == NULL)
    {
        zlog_err("%-15s[%d] Can't malloc route", __FUNCTION__, __LINE__);
        return (struct ospf6_route *)NULL;
    }
	memset(route, 0, sizeof(struct ospf6_route));
    return route;
}
...
ospf6_route_cmp(struct ospf6_route *ra, struct ospf6_route *rb)
{
	...
	if(OSPF6_PATH_TYPE_INTER == ra->path.type)
	{
...
route_table_assert(struct ospf6_route_table *table)
{
    ...
    zlog_debug(OSPF6_DBG_OTHER, "table count = %d, real number = %d", table->count, num);
    zlog_debug(OSPF6_DBG_OTHER, "DUMP START");

    for (r = ospf6_route_head(table); r; r = ospf6_route_next(r))
    {
        prefix2str(&r->prefix, buf, sizeof(buf));
        zlog_debug(OSPF6_DBG_OTHER, "%p<-[%p]->%p : %s", r->prev, r, r->next, buf);
    }

    zlog_debug(OSPF6_DBG_OTHER, "DUMP END");
...
*Function: 	route add to specific table
*Parameter:	struct ospf6_route *route,
                struct ospf6_route_table *table , struct ospf6 *ospf6
*return: 	struct ospf6_route  
                ...
                struct ospf6_route_table *table , struct ospf6 *ospf6)
{
	char buf[64];
    struct timeval now;
	struct ospf6_route *old = NULL;
    struct ospf6_route *prev = NULL;
	struct ospf6_route *next = NULL;
	struct route_node *node = NULL;
	struct route_node *nextnode = NULL;
	struct route_node *prevnode = NULL;
    ...
    {
        zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: %s %s %d", ospf6_route_table_name(table),
                   (void *)table, (void *)route, buf,__func__,__LINE__);
    }
    else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
    {
        zlog_debug(OSPF6_DBG_ROUTE, "%s: route add: %s %s %d", ospf6_route_table_name(table), buf,__func__,__LINE__);
    }
    ...
    {	
		...
		/*originate from same path*/
        else if (ospf6_route_is_same_origin(current, route))
        {
            old = current;
        }
		/*route is Preferred route*/
        else if (ospf6_route_cmp(current, route) > 0)
        {
            next = current;
        }
		/*current is Preferred route,or they are ecmp route*/
        else
        {
            prev = current;
        }
    ...
    if (old)
    {
        ...
        {
            if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
                zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: needless update of %p",
                           ospf6_route_table_name(table),
                           (void *)table, (void *)route, (void *)old);
            else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
                zlog_debug(OSPF6_DBG_ROUTE, "%s: route add: needless update",
                           ospf6_route_table_name(table));
        ...
        if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
        {
            zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: update of %p",
                       ospf6_route_table_name(table),
                       (void *)table, (void *)route, (void *)old);
        }
        else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
        {
            zlog_debug(OSPF6_DBG_ROUTE, "%s: route add: update",
                       ospf6_route_table_name(table));
        }
    ...
    {
        if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
            zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: another path: prev %p, next %p",
                       ospf6_route_table_name(table),
                       (void *)table, (void *)route, (void *)prev, (void *)next);
        else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
            zlog_debug(OSPF6_DBG_ROUTE, "%s: route add: another path found",
                       ospf6_route_table_name(table));
		if(node->info)
		{
			route_unlock_node(node);
        ...
        if (node->info == next)
        {
        	/*Here,we need to send remove message to route in order to delete this route*/
        	if (table->hook_remove)
	        {
	            (*table->hook_remove)(next, ospf6);
	        }
            ...
            if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
                zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: replacing previous best: %p",
                          ospf6_route_table_name(table),
        ...
        {
			zlog_debug(OSPF6_DBG_ROUTE, "%s %d  OSPF6_ROUTE_BEST %d",__func__,__LINE__,route->path.cost);
		}
		else 
		{
			zlog_debug(OSPF6_DBG_ROUTE, "%s %d  OSPF6_ROUTE_BEST is not set %d",__func__,__LINE__,route->path.cost);	
		}
    ...
    if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
        zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route add %p: brand new route",
                   ospf6_route_table_name(table), (void *)table, (void *)route);
    else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
        zlog_debug(OSPF6_DBG_ROUTE, "%s: route add: brand new route",
                   ospf6_route_table_name(table));
                   ...
                   struct ospf6_route_table *table, struct ospf6 *ospf6)
{
    ...
    if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
    {
        zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route remove %p: %s",
                   ospf6_route_table_name(table),
                   (void *)table, (void *)route, buf);
    }
    else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
    {
        zlog_debug(OSPF6_DBG_ROUTE, "%s: route remove: %s", ospf6_route_table_name(table), buf);
    }
	...
	/* 使用route_node_lookup node不为空要将node 的lock值减一*/
	route_unlock_node(node);

    ospf6_route_unlock(route);
	/* 删除rn->info 的时候需要将rn的lock值置为0 */
	if(node->info == NULL)
		route_unlock_node (node);
...
ospf6_route_head(struct ospf6_route_table *table)
{
    ...
    if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
        zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route head: %p<-[%p]->%p",
...
ospf6_route_next(struct ospf6_route *route)
{
    ...
    if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
        zlog_debug(OSPF6_DBG_ROUTE, "%s %p: route next: %p<-[%p]->%p",
...
void
ospf6_route_remove_all(struct ospf6_route_table *table, struct ospf6* ospf6)
{
    struct ospf6_route *route = NULL;
	struct ospf6_route *route_next = NULL;

    for (route = ospf6_route_head(table); route;
            route = route_next)
    {
    	route_next = ospf6_route_next(route);
...
{
    struct ospf6_route_table *new_route = NULL;
	
    new_route = XCALLOC(MTYPE_OSPF6_ROUTE, sizeof(struct ospf6_route_table));
	if (new_route == NULL)
    {
        zlog_err("%-15s[%d] Can't malloc ospf6_route", __FUNCTION__, __LINE__);
        return (struct ospf6_route_table *)NULL;
    }
	memset(new_route, 0, sizeof(struct ospf6_route_table));
    new_route->table = route_table_init();
    new_route->scope_type = s;
    new_route->table_type = t;
    return new_route;
}

void
ospf6_route_table_delete(struct ospf6_route_table *table, struct ospf6 *ospf6)
{
...
ospf6_route_show(struct vty *vty, struct ospf6_route *route)
{
            ...
            ospf6_nexthop_is_set(&route->nexthop[i]); i++)
    {
		...
		if(ipv6_is_zero((struct ipv6_addr *)(&route->nexthop[i].address.s6_addr)))
			vty_out(vty, "        directly conneted,%6.*s %s", NAME_STRING_LEN, ifname, VNL);
		else
			vty_out(vty, "        %-25s %6.*s %s", nexthop, NAME_STRING_LEN, ifname, VNL);
...
ospf6_route_show_detail(struct vty *vty, struct ospf6_route *route)
{
            ...
            i < OSPF6_MULTI_PATH_LIMIT; i++)
    {
        ...
        vty_out(vty, "  %s %.*s%s", nexthop, NAME_STRING_LEN, ifname, VNL);
    }
                               ...
                               struct ospf6_route_table *table)
{
    ...
    vty_out(vty, "Number of OSPF6 routes: %d%s", number, VNL);
                             ...
                             struct ospf6_route_table *table)
{
	...
	vty_out(vty,"Codes :IA - Inter-Area, IE - Inter-Area, E1 - Type 1 External ,"\
			"E2 - Type 2 External, R - router, N - Network, "\
			"D - Discard, L - Linkstate,"\
			"A - AddressRange %s%s",VTY_NEWLINE, VTY_NEWLINE);
...
install_element_ospf6_debug_route(void)
{
    ...
    install_element(CONFIG_NODE, &no_debug_ospf6_route_cmd, CMD_SYNC);
}
