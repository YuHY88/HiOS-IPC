...
{
    "None", "Down", "Attempt", "Init", "2-Way", "ExStart", "ExChange",
    "Loading", "Full", NULL
};
...
ospf6_neighbor_create(u_int32_t router_id, struct ospf6_interface *oi)
{
    ...
    on->state_change = 0;
    time_get_time(TIME_CLK_MONOTONIC, &on->last_changed);
...
ospf6_neighbor_delete(struct ospf6_neighbor *on)
{
	...
	on->summary_list = NULL;
	on->request_list = NULL;
	on->retrans_list = NULL;
	on->dbdesc_list = NULL;
	on->lsupdate_list = NULL;
	on->lsack_list = NULL;
    OSPF6_TIMER_OFF(on->inactivity_timer);
    OSPF6_TIMER_OFF(on->thread_send_dbdesc);
    OSPF6_TIMER_OFF(on->thread_send_lsreq);
...
ospf6_neighbor_state_change(u_char next_state, struct ospf6_neighbor *on, int event)
{
    ...
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor state change %s: [%s]->[%s] (%s)", on->name,
...
hello_received(struct thread *thread)
{
    ...
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *HelloReceived*", on->name);
    }

    /* reset Inactivity Timer */
    OSPF6_TIMER_OFF(on->inactivity_timer);
    /*on->inactivity_timer = thread_add_timer(master_ospf6, inactivity_timer, on,
                                            on->ospf6_if->dead_interval);*/
	on->inactivity_timer = high_pre_timer_add ((char *)"ospf6_inact_timer", LIB_TIMER_TYPE_NOLOOP,\
						inactivity_timer, (void *)on, (on->ospf6_if->dead_interval)*1000);
...
twoway_received(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *2Way-Received*", on->name);
    }

    thread_add_event(master_ospf6, neighbor_change, on->ospf6_if, 0);
...
negotiation_done(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *NegotiationDone*", on->name);
    }
...
exchange_done(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *ExchangeDone*", on->name);
    }
...
ospf6_check_nbr_loading(struct ospf6_neighbor *on)
{
    ...
    */
    if(on == NULL)
    {
		zlog_warn("%s %d on is NULL",__func__,__LINE__);
		return ;
	}
...
loading_done(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *LoadingDone*", on->name);
    }
...
adj_ok(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *AdjOK?*", on->name);
    }
...
seqnumber_mismatch(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *SeqNumberMismatch*", on->name);
    }
...
bad_lsreq(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *BadLSReq*", on->name);
    }
...
oneway_received(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *1Way-Received*", on->name);
    }
...
inactivity_timer(void *thread)
{
    ...
    if (IS_OSPF6_DEBUG_NEIGHBOR(EVENT))
    {
        zlog_debug(OSPF6_DBG_NEIGHBOR, "Neighbor Event %s: *InactivityTimer*", on->name);
    }

    on->inactivity_timer = 0;
    ...
    //thread_add_event(master_ospf6, neighbor_change, on->ospf6_if, 0);
    thread_execute(master_ospf6, neighbor_change, on->ospf6_if, 0);
...
void 
ospf6_interface_name_simplify(char *name, char if_name[])
{
    char *p = NULL;
    if(!strncmp(name, "e", 1))
    {
...
ospf6_neighbor_show(struct vty *vty, struct ospf6_neighbor *on)
{
    ...
    {
        //s = on->inactivity_timer->u.sands.tv_sec - time_get_recent_relative_time().tv_sec;
		high_pre_timer_remain(on->inactivity_timer, (int*)&s);
        h = s / 3600;
        s -= h * 3600;
        m = s / 60;
        s -= m * 60;
    ...
    {
        snprintf(nstate, sizeof(nstate), "P2P");
    }
    else
    {
    ...
    vty_out (vty, "%-15s %3d %11s %6s/%-12s %11s %s[%s]%s",
             ...
             "I/F", "State", VNL);
    */
    sprintf(state, "%s/%s",ospf6_neighbor_state_str[on->state], nstate);
    ospf6_interface_name_simplify(on->ospf6_if->interface->name, if_name);
    vty_out(vty, "%-15s %3d %-11s %-15s %-11s %s[%s]%s",
            router_id, on->priority, deadtime,
            state, duration, if_name,
...
ospf6_neighbor_show_drchoice(struct vty *vty, struct ospf6_neighbor *on)
{
    ...
    */
    ...
    char if_name[IFNET_NAMESIZE] = "";
    ospf6_interface_name_simplify(on->ospf6_if->interface->name, if_name);
    vty_out(vty, "%-15s %-7s %-11s %-15s %-15s %s[%s]%s",
            router_id, ospf6_neighbor_state_str[on->state],
            duration, drouter, bdrouter, if_name,
...
ospf6_neighbor_show_detail(struct vty *vty, struct ospf6_neighbor *on)
{
    ...
    vty_out(vty, " Neighbor %s%s", on->name,
            VNL);
    vty_out(vty, "    Area %s via interface %s (ifindex 0x%0x)%s",
            on->ospf6_if->area->name,
            on->ospf6_if->interface->name,
            on->ospf6_if->interface->ifindex,
            VNL);
    vty_out(vty, "    His IfIndex: 0x%0x Link-local address: %s%s",
    ...
    if (on->thread_send_dbdesc)
    {
		...
		high_pre_timer_remain(on->thread_send_dbdesc, &remain_time);
    ...
    if (on->thread_send_lsreq)
    {
        ...
        high_pre_timer_remain(on->thread_send_lsreq, &remain_time);
		res.tv_sec = remain_time;
		res.tv_usec = 0;
    }
    ...
    if (on->thread_send_lsupdate)
    {
		...
		high_pre_timer_remain(on->thread_send_lsupdate, &remain_time);
		res.tv_sec = remain_time;
		res.tv_usec = 0;
    }
     ...
     )
{
    ...
    {
        vty_out(vty, "%-15s %3s %-11s %-15s %-11s %s[%s]%s",
                "Neighbor ID", "Pri", "DeadTime", "State", "Duration",
                "I/F", "State", VNL);
    }
     ...
     )
{
    ...
    if ((inet_pton(AF_INET, argv[1], &router_id)) != 1)
    {
        vty_out(vty, "Router-ID is not parsable: %s%s", argv[1],
                VNL);
        return CMD_SUCCESS;
    }
...
{
    install_element(CONFIG_NODE, &show_ipv6_ospf6_neighbor_cmd, CMD_LOCAL);
    install_element(OSPF6_NODE, &show_ipv6_ospf6_neighbor_cmd, CMD_LOCAL);
    install_element(CONFIG_NODE, &show_ipv6_ospf6_neighbor_detail_cmd, CMD_LOCAL);
    install_element(OSPF6_NODE, &show_ipv6_ospf6_neighbor_detail_cmd, CMD_LOCAL);
}
...
install_element_ospf6_debug_neighbor(void)
{
    ...
    install_element(CONFIG_NODE, &no_debug_ospf6_neighbor_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_neighbor_detail_cmd, CMD_SYNC);
}
#endif
