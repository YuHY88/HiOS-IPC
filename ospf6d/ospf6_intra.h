  ...
  do { \
    if (! (oa)->thread_router_lsa \
      ...
      (oa)->thread_router_lsa = \
        thread_add_event (master_ospf6, ospf6_router_lsa_originate, oa, 0); \
  } while (0)
  ...
  do { \
    if (! (oi)->thread_network_lsa \
      ...
      (oi)->thread_network_lsa = \
        thread_add_event (master_ospf6, ospf6_network_lsa_originate, oi, 0); \
  } while (0)
#define OSPF6_LINK_LSA_SCHEDULE(oi) \
  do { \
    if (! (oi)->thread_link_lsa \
        && ! CHECK_FLAG((oi)->flag, OSPF6_INTERFACE_DISABLE)) \
      (oi)->thread_link_lsa = \
        thread_add_event (master_ospf6, ospf6_link_lsa_originate, oi, 0); \
  } while (0)
  ...
  do { \
    if (! (oa)->thread_intra_prefix_lsa \
      ...
      (oa)->thread_intra_prefix_lsa = \
        thread_add_event (master_ospf6, ospf6_intra_prefix_lsa_originate_stub, \
                          oa, 0); \
  } while (0)
  ...
  do { \
    if (! (oi)->thread_intra_prefix_lsa \
      ...
      (oi)->thread_intra_prefix_lsa = \
        thread_add_event (master_ospf6, ospf6_intra_prefix_lsa_originate_transit, \
                          oi, 0); \
  } while (0)
...
#define OSPF6_NETWORK_LSA_EXECUTE(oi) \
  do { \
    ...
    thread_execute (master_ospf6, ospf6_network_lsa_originate, oi, 0); \
  } while (0)
#define OSPF6_INTRA_PREFIX_LSA_EXECUTE_TRANSIT(oi) \
  do { \
    THREAD_OFF ((oi)->thread_intra_prefix_lsa); \
    thread_execute (master_ospf6, ospf6_intra_prefix_lsa_originate_transit, oi, 0); \
  } while (0)


extern struct ospf6_lsa_handler router_handler;
extern struct ospf6_lsa_handler network_handler;
extern struct ospf6_lsa_handler link_handler;
extern struct ospf6_lsa_handler intra_prefix_handler;
