...
ospf6_router_lsa_originate(struct thread *thread)
{
    ...
    struct ospf6_neighbor *on = NULL, *drouter = NULL;
    struct ospf6_router_lsa *router_lsa = NULL;
    struct ospf6_router_lsdesc *lsdesc = NULL;
    u_int16_t type;
    u_int32_t router;
    int count;
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Originate Router-LSA for Area %s", oa->name);
    }
    ...
    if (ospf6_asbr_is_asbr(oa->ospf6))
    {
    ...
    for (ALL_LIST_ELEMENTS(oa->if_list, node, nnode, oi))
    {
                    ...
                    > oa->router_lsa_size_limit))
        {
                    ...
                    sizeof(struct ospf6_router_lsa))
            {
                if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER))
                {
                    zlog_debug(OSPF6_DBG_LSA_DEBUG, "Size limit setting for Router-LSA too short");
                }
...
ospf6_network_lsa_originate(struct thread *thread)
{
    ...
    struct ospf6_lsa_header *lsa_header = NULL;
    int count;
    struct ospf6_lsa *old = NULL, *lsa = NULL;
    struct ospf6_network_lsa *network_lsa = NULL;
    struct ospf6_network_lsdesc *lsdesc = NULL;
    struct ospf6_neighbor *on = NULL;
    struct ospf6_link_lsa *link_lsa = NULL;
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Originate Network-LSA for Interface %s", oi->interface->name);
    }
    ...
    if (count == 0)
    {
        if (IS_OSPF6_DEBUG_ORIGINATE(NETWORK))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Interface stub, ignore");
        }
...
ospf6_link_lsa_originate(struct thread *thread)
{
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Originate Link-LSA for Interface %s", oi->interface->name);
    }

    /* can't make Link-LSA if linklocal address not set */
    if (oi->linklocal_addr == NULL || (ospf6_interface_get_address_num(oi->interface) == 0))
    {
        if (IS_OSPF6_DEBUG_ORIGINATE(LINK))
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "No Linklocal address on %s, defer originating",
                       oi->interface->name);
...
ospf6_intra_prefix_lsa_originate_stub(struct thread *thread)
{
    ...
    if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Originate Intra-Area-Prefix-LSA for area %s's stub prefix",
                   oa->name);
    ...
    for (ALL_LIST_ELEMENTS_RO(oa->if_list, i, oi))
    {
        if (oi->state == OSPF6_INTERFACE_DOWN)
        {
            if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "  Interface %s is down, ignore", oi->interface->name);
            }
        ...
        {
            if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "  Interface %s is not stub, ignore",
                           oi->interface->name);
        ...
        if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "  Interface %s:", oi->interface->name);
        }
                ...
                route = ospf6_route_best_next(route))
        {
            if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
            {
                ...
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "    include %s", buf);
            }

            ospf6_route_add(ospf6_route_copy(route), route_advertise, oa->ospf6);
        }
    }
    ...
    if (prefix_num == 0)
    {
        if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Quit to Advertise Intra-Prefix: no route to advertise");
        }
...
ospf6_intra_prefix_lsa_originate_transit(struct thread *thread)
{
    ...
    struct ospf6_lsa_header *lsa_header = NULL;
    struct ospf6_lsa *old = NULL, *lsa = NULL;
    struct ospf6_intra_prefix_lsa *intra_prefix_lsa = NULL;
    struct ospf6_neighbor *on = NULL;
    struct ospf6_route *route = NULL;
    struct ospf6_prefix *op = NULL;
    struct listnode *i = NULL;
    int full_count = 0;
    unsigned short prefix_num = 0;
    struct ospf6_route_table *route_advertise = NULL;
    struct ospf6_link_lsa *link_lsa = NULL;
    char *start = NULL, *end = NULL, *current = NULL;
    u_int16_t type;
    char buf[BUFSIZ];
    oi = (struct ospf6_interface *) THREAD_ARG(thread);
    //assert(oi);
    ...
    if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Originate Intra-Area-Prefix-LSA for interface %s's prefix",
                   oi->interface->name);
    ...
    if (oi->state != OSPF6_INTERFACE_DR)
    {
        if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "  Interface is not DR");
        }
    ...
    if (full_count == 0)
    {
        if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "  Interface is stub");
        }
            ...
            lsa = ospf6_lsdb_type_next(type, lsa))
    {
        ...
        if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "  include prefix from %s", lsa->name);
        }
        ...
        if (lsa->header->adv_router != oi->area->ospf6->router_id)
        {
            ...
            if (on == NULL || on->state != OSPF6_NEIGHBOR_FULL)
            {
                if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
                {
                    zlog_debug(OSPF6_DBG_LSA_DEBUG, "    Neighbor not found or not Full, ignore");
                }
                ...
                current += OSPF6_PREFIX_SIZE(op))
        {
			...
			if(op == NULL)
			{
				zlog_warn("op is NULL %s %d",__func__,__LINE__);
				break;
			}
            ...
            ospf6_route_add(route, route_advertise, oi->ospf6);
            prefix_num--;
        }

        if (current != end && IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Trailing garbage in %s", lsa->name);
        }
    }
    ...
    ospf6_route_table_delete(route_advertise, oi->ospf6);
	route_advertise = NULL;
...
ospf6_intra_prefix_lsa_add(struct ospf6_lsa *lsa)
{
    ...
    struct ospf6_route *route = NULL, *ls_entry = NULL;
    int i, prefix_num;
    struct ospf6_prefix *op = NULL;
    char *start = NULL, *current, *end = NULL;
    char buf[64];
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "%s found", lsa->name);
    }
    ...
    {
        if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Unknown reference LS-type: %#hx",
    ...
    if (ls_entry == NULL)
    {
        if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
        {
            ...
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "LS entry does not exist: %s", buf);
        }
    ...
    for (current = start; current < end; current += OSPF6_PREFIX_SIZE(op))
    {
        ...
        if (CHECK_FLAG(op->prefix_options, OSPF6_PREFIX_OPTION_NU))
        {
            if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
            {
                ospf6_linkstate_prefix2str((struct prefix *)OSPF6_PREFIX_BODY(op),
                                           buf, sizeof(buf));
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "%s: Skipping Prefix %s has NU option set",
                           __func__, buf);
            }
        ...
        if (direct_connect)
        {
        	/* if lsa->header->id is 0,according to ifp,else according to lsa id*/
			if(lsa->header->id == 0)
			{
			...
			{
				route->nexthop[0].ifindex = htonl(lsa->header->id);
			}
			#if 0
            if (ifp)
            {
        ...
        {
            prefix2str(&route->prefix, buf, sizeof(buf));
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "  add %s", buf);
        }
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Trailing garbage ignored");
    }
}
...
ospf6_intra_prefix_lsa_remove(struct ospf6_lsa *lsa)
{
    ...
    if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "%s disappearing", lsa->name);
    }
    ...
    for (current = start; current < end; current += OSPF6_PREFIX_SIZE(op))
    {
                ...
                route = nroute)
        {
            ...
            {
                prefix2str(&route->prefix, buf, sizeof(buf));
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "remove %s", buf);
            }
...
ospf6_intra_route_calculation(struct ospf6_area *oa)
{
    ...
    if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Re-examin intra-routes for area %s", oa->name);
    }
    ...
    if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Re-examin intra-routes for area %s: Done", oa->name);
    }
}
...
ospf6_brouter_debug_print(struct ospf6_route *brouter)
{
    ...
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "Brouter: %s via area %s", brouter_name, area_name);
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "  memory: prev: %p this: %p next: %p parent rnode: %p",
              (void *)brouter->prev, (void *)brouter, (void *)brouter->next,
              (void *)brouter->rnode);
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "  type: %d prefix: %s installed: %s changed: %s",
              brouter->type, destination, installed, changed);
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "  lock: %d flags: %s%s%s%s", brouter->lock,
    ...
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "  path type: %s ls-origin %s id: %s adv-router %s",
              OSPF6_PATH_TYPE_NAME(brouter->path.type),
              ospf6_lstype_name(brouter->path.origin.type),
              id, adv_router);
    zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "  options: %s router-bits: %s metric-type: %d metric: %d/%d",
...
ospf6_intra_brouter_calculation(struct ospf6_area *oa)
{
    ...
    {
        zlog_debug(OSPF6_DBG_BROUTER, "border-router calculation for area %s", oa->name);
    }
            ...
            brouter = ospf6_route_next(brouter))
    {
        ...
        {
            zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "%p: mark as removing: area %s brouter %s",
            ...
            brouter = ospf6_route_next(brouter))
    {
        ...
        if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id) ||
                IS_OSPF6_DEBUG_ROUTE(MEMORY))
        {
            zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "%p: transfer: area %s brouter %s",
                      (void *)brouter, oa->name, brouter_name);
            ospf6_brouter_debug_print(brouter);
        }
    }
	...
	/* add to calculat type 4 border route */			
    for (brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter;
            brouter = ospf6_route_next(brouter))

    {
		u_int16_t type = htons(OSPF6_LSTYPE_INTER_ROUTER);
    	...
    	IS_OSPF6_DEBUG_ROUTE(MEMORY))
        {
			zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "ASBR border calculate %s %d  \n",__func__,__LINE__);
		}
		

        for (lsa = ospf6_lsdb_type_router_head(type, brouter_id, oa->lsdb); lsa;
                lsa = ospf6_lsdb_type_router_next(type, brouter_id, lsa))
        {
        	...
        	IS_OSPF6_DEBUG_ROUTE(MEMORY))
	        {
				zlog_debug(OSPF6_DBG_ROUTE|OSPF6_DBG_BROUTER, "%s %d  %s\n",__func__,__LINE__,lsa->name);
				ospf6_brouter_debug_print(brouter);
			}
        }
    }
            ...
            brouter = nbrouter)
    {
        ...
        if (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_REMOVE))
        {
            if (IS_OSPF6_DEBUG_BROUTER ||
                ...
                zlog_debug(OSPF6_DBG_BROUTER|OSPF6_DBG_BROUTER, "brouter %s disappears via area %s",
                          brouter_name, oa->name);

            ospf6_route_remove(brouter, oa->ospf6->brouter_table, oa->ospf6);
        }
                 ...
                 CHECK_FLAG(brouter->flag, OSPF6_ROUTE_CHANGE))
        {
            if (IS_OSPF6_DEBUG_BROUTER ||
                    IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id) ||
                    IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id))
                zlog_debug(OSPF6_DBG_BROUTER|OSPF6_DBG_BROUTER, "brouter %s appears via area %s",
                          brouter_name, oa->name);
        ...
        else
        {
            if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id) ||
                    IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id))
                zlog_debug(OSPF6_DBG_BROUTER, "brouter %s still exists via area %s",
                          brouter_name, oa->name);
        }
    ...
    if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id))
    {
        zlog_debug(OSPF6_DBG_BROUTER, "border-router calculation for area %s: done", oa->name);
    }
}
...
install_element_ospf6_debug_brouter(void)
{
    ...
    install_element(CONFIG_NODE, &debug_ospf6_brouter_area_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_brouter_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_brouter_router_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_brouter_area_cmd, CMD_SYNC);
}
#endif
