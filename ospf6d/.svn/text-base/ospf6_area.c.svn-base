...
ospf6_area_lsdb_hook_add(struct ospf6_lsa *lsa)
{
    switch (ntohs(lsa->header->type))
    {
        ...
        case OSPF6_LSTYPE_NETWORK:
            ...
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "Examin %s", lsa->name);
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "Schedule SPF Calculation for %s",
...
ospf6_area_lsdb_hook_remove(struct ospf6_lsa *lsa)
{
    switch (ntohs(lsa->header->type))
    {
        ...
        case OSPF6_LSTYPE_NETWORK:
            ...
            if (IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type))
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA disappearing: %s", lsa->name);
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "Schedule SPF Calculation for %s",
                           OSPF6_AREA(lsa->lsdb->data)->name);
            }
...
static void
ospf6_area_route_hook_add(struct ospf6_route *route , struct ospf6 *ospf6)
{
...
static void
ospf6_area_route_hook_remove(struct ospf6_route *route, struct ospf6 *ospf6)
{
...
ospf6_area_create(u_int32_t area_id, struct ospf6 *o)
{
	...
	if (oa == NULL)
    {
        zlog_err("Can't creat ospf6 area : malloc failed");
        return (struct ospf6_area *)NULL;
    }
	memset(oa, 0, sizeof(struct ospf6_area));
...
ospf6_area_delete(struct ospf6_area *oa)
{
	...
	oa->range_table = NULL;
	oa->summary_prefix = NULL;
	oa->summary_router = NULL;
...
ospf6_area_lookup(u_int32_t area_id, struct ospf6 *ospf6)
{
    ...
    if(ospf6 == NULL)
    {
    	zlog_err("%s %d OSPF6 is NULL",__func__,__LINE__);
		return NULL;
	}
...
{                                                          \
  ...
  if (inet_pton (AF_INET, str, &area_id) != 1)             \
    {                                                      \
      vty_error_out (vty, "Malformed Area-ID: %s%s", str, VNL);  \
      return CMD_SUCCESS;                                  \
    }                                                      \
  oa = ospf6_area_lookup (area_id, ospf6);                 \
  if (oa == NULL)                                          \
    {                                                      \
      vty_error_out (vty, "No such Area: %s%s", str, VNL);       \
      return CMD_SUCCESS;                                  \
    }                                                      \
}

#define OSPF6_CMD_AREA_GET(ospf6, str, oa)                        \
{                                                          \
	u_int32_t area_id = 0;                                   \
	if (inet_pton (AF_INET, str, &area_id) != 1)             \
    {                                                      \
	...
	{														\
		vty_error_out(vty, "Invalid Area-ID, Please input area <0-255>%s", VNL);	\
		return CMD_WARNING; 								\
	}														\
     ...
     )
{
    ...
    u_int32_t cost;
    OSPF6_CMD_AREA_GET(ospf6, argv[0], oa);
    //argc--;
    //argv++;
    ...
    {
        vty_error_out(vty, "Malformed argument: %s%s", argv[1], VNL);
        return CMD_WARNING;
    }
	...
	if(argv[3] != NULL)
	{
		...
		SET_FLAG(range->flag, OSPF6_ROUTE_SET);
		range->path.cost = cost; 
	}
	else
	{
		range->path.cost = 0; 
		UNSET_FLAG(range->flag, OSPF6_ROUTE_SET);
	}


	if (range->rnode == NULL)
	{
    	ospf6_route_add(range, oa->range_table, ospf6);
	}
	ospf6_spf_schedule(ospf6,1);
    return CMD_SUCCESS;
}
#if 0
ALIAS(area_range,
     ...
     )
{
    ...
    if (ret != 1 || prefix.family != AF_INET6)
    {
        vty_error_out(vty, "Malformed argument: %s%s", argv[0], VNL);
        return CMD_WARNING;
    }
    ...
    if (range == NULL)
    {
        vty_error_out(vty, "Range %s does not exists.%s", argv[0], VNL);
        return CMD_WARNING;
    }
    ...
    ospf6_route_remove(range, oa->range_table, ospf6);
    return CMD_SUCCESS;
}
...
ospf6_area_config_write(struct vty *vty, struct ospf6 *ospf6)
{
    ...
    for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa))
    {
                ...
                range = ospf6_route_next(range))
        {
			...
			if((!CHECK_FLAG(range->flag,OSPF6_ROUTE_DO_NOT_ADVERTISE)&&!CHECK_FLAG(range->flag,OSPF6_ROUTE_SET)))
            	vty_out(vty, " area %s abr-summary %s%s", oa->name, buf, VNL);
			else if((CHECK_FLAG(range->flag,OSPF6_ROUTE_DO_NOT_ADVERTISE)&&!CHECK_FLAG(range->flag,OSPF6_ROUTE_SET)))
				vty_out(vty, " area %s abr-summary %s not-advertise%s", oa->name, buf, VNL);
			else if((!CHECK_FLAG(range->flag,OSPF6_ROUTE_DO_NOT_ADVERTISE)&&CHECK_FLAG(range->flag,OSPF6_ROUTE_SET)))
				vty_out(vty, " area %s abr-summary %s cost %d%s", oa->name, buf, range->path.cost, VNL);
			else	
				vty_out(vty, " area %s abr-summary %s not-advertise cost %d%s", oa->name, buf, range->path.cost, VNL);
        }
      ...
      "Filter networks sent from this area\n")
{
    ...
    OSPF6_CMD_AREA_GET(ospf6, argv[0], area);
    argc--;
    argv++;
...
DEFUN(show_ipv6_ospf6_spf_tree,
      ...
      "show ipv6 ospf6 [<1-255>] spf tree",
      SHOW_STR
      IP6_STR
      OSPF6_STR
      OSPF6_INSTANCE
      ...
      "Show SPF tree\n")
{
    ...
    ospf6_linkstate_prefix(o->router_id, htonl(0), &prefix);
...
DEFUN(show_ipv6_ospf6_area_spf_tree,
      ...
      "show ipv6 ospf6 [<1-255>] area A.B.C.D spf tree",
      SHOW_STR
      IP6_STR
      OSPF6_STR
      OSPF6_INSTANCE
      ...
      "Show SPF tree\n")
{
    ...
    {
        vty_error_out(vty, "Malformed Area-ID: %s%s", argv[0], VNL);
        return CMD_WARNING;
    }
    ...
    if (oa == NULL)
    {
        vty_error_out(vty, "No such Area: %s%s", argv[0], VNL);
        return CMD_WARNING;
    }
    ...
    if (route == NULL)
    {
        vty_error_out(vty, "LS entry for root not found in area %s%s",
                oa->name, VNL);
        return CMD_WARNING;
    }
...
DEFUN(show_ipv6_ospf6_simulate_spf_tree_root,
      ...
      "show ipv6 ospf6 [<1-255>] simulate spf-tree A.B.C.D area A.B.C.D",
      SHOW_STR
      IP6_STR
      OSPF6_STR
      OSPF6_INSTANCE
      ...
      "Specify root's router-id to calculate another router's SPF tree\n")
{
    ...
    {
        vty_error_out(vty, "Malformed Area-ID: %s%s", argv[1], VNL);
        return CMD_SUCCESS;
    }
    ...
    if (oa == NULL)
    {
        vty_error_out(vty, "No such Area: %s%s", argv[1], VNL);
        return CMD_SUCCESS;
    }
    ...
    if (route == NULL)
    {
        ospf6_spf_table_finish(spf_table, o);
        ospf6_route_table_delete(spf_table, o);
		spf_table = NULL;
        return CMD_SUCCESS;
    }
...
{
    install_element(CONFIG_NODE, &show_ipv6_ospf6_spf_tree_cmd, CMD_LOCAL);
    install_element(CONFIG_NODE, &show_ipv6_ospf6_area_spf_tree_cmd, CMD_LOCAL);
    install_element(CONFIG_NODE, &show_ipv6_ospf6_simulate_spf_tree_root_cmd, CMD_LOCAL);
	install_element(OSPF6_NODE, &show_ipv6_ospf6_spf_tree_cmd, CMD_LOCAL);
    install_element(OSPF6_NODE, &show_ipv6_ospf6_area_spf_tree_cmd, CMD_LOCAL);
    install_element(OSPF6_NODE, &show_ipv6_ospf6_simulate_spf_tree_root_cmd, CMD_LOCAL);
	
    install_element(OSPF6_NODE, &area_range_cmd, CMD_SYNC);
    //install_element(OSPF6_NODE, &area_range_advertise_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_area_range_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &area_import_list_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_area_import_list_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &area_export_list_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_area_export_list_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &area_filter_list_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_area_filter_list_cmd, CMD_SYNC);
}
