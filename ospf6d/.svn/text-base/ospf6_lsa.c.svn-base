...
ospf6_lsa_age_set(struct ospf6_lsa *lsa)
{
    ...
    if (time_get_time(TIME_CLK_MONOTONIC, &now) < 0)
        zlog_warn("LSA: time_get_time failed, may fail LSA AGEs: %s",
                  safe_strerror(errno));
...
ospf6_lsa_premature_aging(struct ospf6_lsa *lsa)
{
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA: Premature aging: %s", lsa->name);
    }

    OSPF6_TIMER_OFF(lsa->expire);
    OSPF6_TIMER_OFF(lsa->refresh);
    /*
    ...
    lsa->header->age = htons(OSPF_LSA_MAXAGE);
    //thread_execute(master_ospf6, ospf6_lsa_expire, lsa, 0);
	high_pre_timer_add ((char *)"ospf6_lsa_expire_timer", LIB_TIMER_TYPE_NOLOOP,\
						ospf6_lsa_expire, (void *)lsa, 10);
}
...
ospf6_lsa_header_print_raw(struct ospf6_lsa_header *header)
{
    ...
    inet_ntop(AF_INET, &header->adv_router, adv_router,
              sizeof(adv_router));
    zlog_debug(OSPF6_DBG_MESSAGE_RECV, "    [%s Id:%s Adv:%s]",
               ospf6_lstype_name(header->type), id, adv_router);
    zlog_debug(OSPF6_DBG_MESSAGE_RECV, "    Age: %4hu SeqNum: %#08lx Cksum: %04hx Len: %d",
...
{
    vty_out(vty, "%-4s %-15s%-15s%-4s %-8s %-10s%s",
...
ospf6_lsa_show_summary(struct vty *vty, struct ospf6_lsa *lsa)
{
    ...
    {
        vty_out(vty, "%-4s %-15s%-15s%-4hu %-8lx %#06hx%s",
                ospf6_lstype_short_name(lsa->header->type),
                id, adv_router, ospf6_lsa_age_current(lsa),
                (u_long) ntohl(lsa->header->seqnum),
                ntohs(lsa->header->checksum), VNL);
    }
    else if (type != OSPF6_LSTYPE_UNKNOWN)
    {
        sprintf(tmpbuf, "%-4s %-15s%-15s%-4hu %-8lx %#06hx",
                ospf6_lstype_short_name(lsa->header->type),
                id, adv_router, ospf6_lsa_age_current(lsa),
                (u_long) ntohl(lsa->header->seqnum), ntohs(lsa->header->checksum));
		...
		vty_out(vty, "%s %s", tmpbuf, VNL);
		//vty_out(vty, "%s %d%s", tmpbuf, cnt, VNL);
    }
    else
    {
        vty_out(vty, "%-4s %-15s%-15s%-4hu %-8lx %#06hx%s ",
                ospf6_lstype_short_name(lsa->header->type),
                id, adv_router, ospf6_lsa_age_current(lsa),
                (u_long) ntohl(lsa->header->seqnum), ntohs(lsa->header->checksum), VNL);
    }
}
...
ospf6_lsa_show_internal(struct vty *vty, struct ospf6_lsa *lsa)
{
	...
	vty_out(vty, "Timer thread ID: Expire: 0x%x, Refresh: 0x%x %s",
            (unsigned int)lsa->expire, (unsigned int)lsa->refresh, VNL);
    vty_out(vty, "%s", VNL);
    return;
}
...
ospf6_lsa_copy(struct ospf6_lsa *lsa)
{
	...
	if(copy == NULL)
	{
		zlog_err("%s %d ospf lsa copy if NULL\n",__func__,__LINE__);
		return NULL;
	}
    assert(copy->lock == 0);
...
/* ospf6 lsa expiry */
int
ospf6_lsa_expire(void *thread)
{
    struct ospf6_lsa *lsa;
    lsa = (struct ospf6_lsa *)(thread);
    assert(lsa && lsa->header);
    ...
    if (IS_OSPF6_DEBUG_LSA_TYPE(lsa->header->type))
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA Expire:");
...
int
ospf6_lsa_refresh(void *thread)
{
    struct ospf6_lsa *old, *self, *new;
    struct ospf6_lsdb *lsdb_self;
    assert(thread);
    old = (struct ospf6_lsa *)(thread);
    ...
    if (self == NULL)
    {
        ...
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Refresh: could not find self LSA, flush %s", old->name);
        }
    ...
    new->lsdb = old->lsdb;
    /*new->refresh = thread_add_timer(master_ospf6, ospf6_lsa_refresh, new,
                                    OSPF_LS_REFRESH_TIME);*/
	new->refresh = high_pre_timer_add ((char *)"ospf6_lsa_ref_timer", LIB_TIMER_TYPE_NOLOOP,\
						ospf6_lsa_refresh, (void *)new, (OSPF_LS_REFRESH_TIME)*1000);
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA Refresh:");
...
install_element_ospf6_debug_lsa(void)
{
    ...
    install_element(CONFIG_NODE, &no_debug_ospf6_lsa_hex_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_lsa_hex_detail_cmd, CMD_SYNC);
}
#endif
