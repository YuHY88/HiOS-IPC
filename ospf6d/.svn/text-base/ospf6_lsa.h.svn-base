...
struct ospf6_lsa
{
	...
	TIMERID refresh;        /* For self-originated LSA */
...
#define CONTINUE_IF_ADDRESS_LINKLOCAL(debug,addr)      \
  if (IN6_IS_ADDR_LINKLOCAL (&(addr)->u.prefix6))      \
    {                                                  \
      ...
      if (debug)                                       \
        zlog_debug (OSPF6_DBG_INTERFACE, "Filter out Linklocal: %s", buf);  \
      continue;                                        \
    }
...
#define CONTINUE_IF_ADDRESS_UNSPECIFIED(debug,addr)    \
  if (IN6_IS_ADDR_UNSPECIFIED (&(addr)->u.prefix6))    \
    {                                                  \
      ...
      prefix2str (addr, buf, sizeof (buf));            \
      if (debug)                                       \
        zlog_debug (OSPF6_DBG_INTERFACE, "Filter out Unspecified: %s", buf);\
      continue;                                        \
    }
...
#define CONTINUE_IF_ADDRESS_LOOPBACK(debug,addr)       \
  if (IN6_IS_ADDR_LOOPBACK (&(addr)->u.prefix6))       \
    {                                                  \
      ...
      prefix2str (addr, buf, sizeof (buf));            \
      if (debug)                                       \
        zlog_debug (OSPF6_DBG_INTERFACE, "Filter out Loopback: %s", buf);   \
      continue;                                        \
    }
...
#define CONTINUE_IF_ADDRESS_V4COMPAT(debug,addr)       \
  if (IN6_IS_ADDR_V4COMPAT (&(addr)->u.prefix6))       \
    {                                                  \
      ...
      prefix2str (addr, buf, sizeof (buf));            \
      if (debug)                                       \
        zlog_debug (OSPF6_DBG_INTERFACE, "Filter out V4Compat: %s", buf);   \
      continue;                                        \
    }
...
#define CONTINUE_IF_ADDRESS_V4MAPPED(debug,addr)       \
  if (IN6_IS_ADDR_V4MAPPED (&(addr)->u.prefix6))       \
    {                                                  \
      ...
      prefix2str (addr, buf, sizeof (buf));            \
      if (debug)                                       \
        zlog_debug (OSPF6_DBG_INTERFACE, "Filter out V4Mapped: %s", buf);   \
      continue;                                        \
    }
...
extern int ospf6_lsa_expire(void *);
extern int ospf6_lsa_refresh(void *);
...
extern char *ospf6_lsa_handler_name(struct ospf6_lsa_handler *h);
