...
*Function: 	check ospf process if ABR,if ABR return 1,else return 0
*Parameter: struct ospf6 *o
*return: 	1: ABR
			0: not ABR
...
{    
	...
	if ((area_count > 1) && (bb_act_attached == 1))
    {
    	SET_FLAG(o->flag, OSPF_FLAG_ABR);
        return 1;
    }
	UNSET_FLAG(o->flag, OSPF_FLAG_ABR);
    return 0;
}
...
{
    struct ospf6_lsa *lsa = NULL, *old = NULL;
    struct ospf6_interface *oi = NULL;
    struct ospf6_route *summary = NULL, *range = NULL;
    struct ospf6_area *route_area = NULL;
    ...
    if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
        if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE(INTER_ROUTER))
        {
            ...
            inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)),
                      buf, sizeof(buf));
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "Originating summary in area %s for ABR %s",
                       area->name, buf);
        }
    ...
    else
    {
        if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE(INTER_PREFIX))
        {
            ...
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "Originating summary in area %s for %s",
                       area->name, buf);
        }
    ...
    if (!CHECK_FLAG(area->ospf6->flag, OSPF_FLAG_ABR))
	{
		if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The router is not ABR, purge previous LSA");
        }
    ...
    if (CHECK_FLAG(route->flag, OSPF6_ROUTE_REMOVE))
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The route has just removed, purge previous LSA");
        }
             ...
             ! CHECK_FLAG(route->path.router_bits, OSPF6_ROUTER_BIT_E)))
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "Route type is none of network, range nor ASBR, withdraw");
        }
            ...
            route->path.type == OSPF6_PATH_TYPE_EXTERNAL2)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "Path type is external, withdraw");
        }
    ...
    if (route->path.area_id == area->area_id)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The route is in the area itself, ignore");
        }
	...
	/* Bug #52184 adv router is self,don't originate type-4 lsa for self */
    if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
	    if (IPV4_ADDR_SAME (&ospf6_linkstate_prefix_adv_router(&route->prefix), &area->ospf6->router_id))
	    {
	        if (is_debug)
	        {
	            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The route adv is self, ignore");
	        }
    ...
    if (oi && oi->area && oi->area == area)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The route's nexthop is in the same area, ignore");
        }
    ...
    if (route->path.cost >= OSPF_LS_INFINITY)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The cost exceeds LSInfinity, withdraw");
        }
    ...
    if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
        ...
        if (! CHECK_FLAG(route->flag, OSPF6_ROUTE_BEST))
        {
            if (is_debug)
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "This is the secondary path to the ASBR, ignore");
            }
            ...
            route->path.type == OSPF6_PATH_TYPE_INTRA)
    {
                 ...
                 ! IS_AREA_TRANSIT(area)))
        {
            if (is_debug)
            {
                ...
                zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "Suppressed by range %s of area %s",
                           buf, route_area->name);
            }
    ...
    if (route->type == OSPF6_DEST_TYPE_RANGE)
    {
        ...
        if (CHECK_FLAG(route->flag, OSPF6_ROUTE_DO_NOT_ADVERTISE))
        {
            if (is_debug)
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "This is the range with DoNotAdvertise set. ignore");
            }
        ...
        if (! CHECK_FLAG(route->flag, OSPF6_ROUTE_ACTIVE_SUMMARY))
        {
            if (is_debug)
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "The range is not active. withdraw");
            }
        ...
        if (EXPORT_LIST(area))
                                  ...
                                  &route->prefix) == FILTER_DENY)
            {
                if (is_debug)
                {
                    inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)),
                              buf, sizeof(buf));
                    zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "prefix %s was denied by export list", buf);
                }
        ...
        if (PREFIX_LIST_OUT(area))
                                  ...
                                  &route->prefix) != PREFIX_PERMIT)
            {
                if (is_debug)
                {
                    inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)),
                              buf, sizeof(buf));
                    zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ABR, "prefix %s was denied by filter-list out", buf);
                }
    ...
    else
    {
        ...
        time_get_time(TIME_CLK_MONOTONIC, &summary->changed);
    }
...
ospf6_abr_range_update(struct ospf6_route *range, struct ospf6 *ospf6)
{
	...
	if(!CHECK_FLAG(range->flag,OSPF6_ROUTE_SET))
	{
	    if (range->path.cost != cost)
	    {
	        ...
	        //ospf6_abr_originate_summary(range, ospf6);
	    }
		else if(flag != 0)
		{
			SET_FLAG(range->flag, OSPF6_ROUTE_ACTIVE_SUMMARY);
...
ospf6_abr_examin_summary(struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
    ...
    if (lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX))
    {
        ...
        {
            is_debug++;
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Examin %s in area %s", lsa->name, oa->name);
        }
    ...
    if (cost == OSPF_LS_INFINITY)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "cost is LS_INFINITY, ignore");
        }
    ...
    if (OSPF6_LSA_IS_MAXAGE(lsa))
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA is MaxAge, ignore");
        }
    ...
    if (lsa->header->adv_router == oa->ospf6->router_id)
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "LSA is self-originated, ignore");
        }
    ...
    if (lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX))
    {
        ...
        if (range)
        {
            if (is_debug)
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "Prefix is equal to address range, ignore");
            }
                           ...
                           OSPF6_PREFIX_OPTION_LA))
        {
            if (is_debug)
            {
                zlog_debug(OSPF6_DBG_LSA_DEBUG, "Prefix has NU/LA bit set, ignore");
            }
            ...
            ! CHECK_FLAG(abr_entry->path.router_bits, OSPF6_ROUTER_BIT_B))
    {
        if (is_debug)
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "ABR router entry does not exist, ignore");
        }
    ...
    if (IMPORT_NAME(oa))
    {
        ...
        if (IMPORT_LIST(oa))
            if (access_list_apply(IMPORT_LIST(oa), &prefix) == FILTER_DENY)
            {
                if (is_debug)
                {
                    zlog_debug(OSPF6_DBG_LSA_DEBUG, "Prefix was denied by import-list");
                }
    ...
    if (PREFIX_NAME_IN(oa))
    {
        ...
        if (PREFIX_LIST_IN(oa))
            if (prefix_list_apply(PREFIX_LIST_IN(oa), &prefix) != PREFIX_PERMIT)
            {
                if (is_debug)
                {
                    zlog_debug(OSPF6_DBG_LSA_DEBUG, "Prefix was denied by prefix-list");
                }
    ...
    if (is_debug)
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Install route: %s", buf);
    }
...
void
ospf6_abr_examin_brouter(struct ospf6 *ospf6, u_int32_t router_id)
{
...
install_element_ospf6_debug_abr(void)
{
    ...
    install_element(CONFIG_NODE, &no_debug_ospf6_abr_cmd, CMD_SYNC);
}
#endif
