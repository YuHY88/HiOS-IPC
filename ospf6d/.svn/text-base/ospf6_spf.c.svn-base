...
ospf6_lsdesc_lsa(caddr_t lsdesc, struct ospf6_vertex *v)
{
    ...
    if (IS_OSPF6_DEBUG_SPF(PROCESS))
    {
        ...
        if (lsa)
        {
            zlog_debug(OSPF6_DBG_SPF, "  Link to: %s", lsa->name);
        }
        else
        {
            zlog_debug(OSPF6_DBG_SPF, "  Link to: [%s Id:%s Adv:%s] No LSA",
                      ...
                      caddr_t lsdesc, struct ospf6_vertex *v)
{
    ...
    if (IS_OSPF6_DEBUG_SPF(PROCESS))
    {
        zlog_debug(OSPF6_DBG_SPF, "  Backlink %s", (found ? "OK" : "FAIL"));
    }
                   ...
                   caddr_t lsdesc)
{
    ...
    if (oi == NULL)
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
        {
            zlog_debug(OSPF6_DBG_SPF, "Can't find interface in SPF: ifindex %d", ifindex);
        }
            ...
            lsa = ospf6_lsdb_type_router_next(type, adv_router, lsa))
    {
        ...
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
        {
            ...
            zlog_debug(OSPF6_DBG_SPF, "  nexthop %s from %s", buf, lsa->name);
        }
    ...
    {
        zlog_debug(OSPF6_DBG_SPF, "No nexthop for %s found", w->name);
    }
}

static int
ospf6_spf_install(struct ospf6 *ospf6, struct ospf6_vertex *v,
                  struct ospf6_route_table *result_table)
{
    struct ospf6_route *route = NULL;
    int i, j;
    struct ospf6_vertex *prev = NULL;

    if (IS_OSPF6_DEBUG_SPF(PROCESS))
        zlog_debug(OSPF6_DBG_SPF, "SPF install %s hops %d cost %d",
    ...
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
            zlog_debug(OSPF6_DBG_SPF, "  already installed with lower cost (%d), ignore",
                       route->path.cost);
    ...
    else if (route && route->path.cost == v->cost)
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
        {
            zlog_debug(OSPF6_DBG_SPF, "  another path found, merge");
        }
...
void
ospf6_spf_table_finish(struct ospf6_route_table *result_table, struct ospf6 *ospf6)
{
...
{
    struct pqueue *candidate_list = NULL;
    struct ospf6_vertex *root = NULL, *v = NULL, *w = NULL;
    int i;
    int size;
    caddr_t lsdesc;
    ...
    while (candidate_list->size)
    {
        ...
        if (ospf6_spf_install(oa->ospf6, v, result_table) < 0)
        {
            continue;
        }
                ...
                lsdesc + size <= OSPF6_LSA_END(v->lsa->header); lsdesc += size)
        {
            ...
            if (IS_OSPF6_DEBUG_SPF(PROCESS))
                zlog_debug(OSPF6_DBG_SPF, "  New candidate: %s hops %d cost %d",
...
ospf6_spf_log_database(struct ospf6_area *oa)
{
    ...
    zlog_debug(OSPF6_DBG_SPF, "%s", buffer);
}

static int
ospf6_spf_calculation_thread(void *t)
{
    ...
    char rbuf[32];
    ospf6 = (struct ospf6 *)(t);
    assert(ospf6);
    ospf6->t_spf_calc = 0;

    /* execute SPF calculation */
    time_get_time(TIME_CLK_MONOTONIC, &start);
    ...
    for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa))
    {
        ...
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
        {
            zlog_debug(OSPF6_DBG_SPF, "SPF calculation for Area %s", oa->name);
        }
    ...
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS))
            zlog_debug(OSPF6_DBG_SPF, "SPF calculation for Backbone area %s",
                       ospf6->backbone->name);
    ...
    time_get_time(TIME_CLK_MONOTONIC, &end);
    timersub(&end, &start, &runtime);
    ospf6->ts_spf_duration = runtime;
    ospf6_spf_reason_string(ospf6->spf_reason, rbuf, sizeof(rbuf));

    if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
        zlog_debug(OSPF6_DBG_SPF, "SPF runtime: %lld sec %lld usec",
                   (long long)runtime.tv_sec, (long long)runtime.tv_usec);

    zlog_debug(OSPF6_DBG_SPF, "SPF processing: # Areas: %d, SPF runtime: %lld sec %lld usec, "
              "Reason: %s\n", areas_processed,
              (long long)runtime.tv_sec, (long long)runtime.tv_usec,
              rbuf);
...
ospf6_spf_schedule(struct ospf6 *ospf6, unsigned int reason)
{
    ...
    if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
    {
        ...
        zlog_debug(OSPF6_DBG_SPF, "SPF: calculation timer scheduled (reason %s)", rbuf);
    }
    ...
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
            zlog_debug(OSPF6_DBG_SPF, "SPF: calculation timer is already scheduled: the Timer ID 0x%x",
                       ospf6->t_spf_calc);
    ...
    now = time_get_recent_relative_time();
    ...
    if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
    {
        zlog_debug(OSPF6_DBG_SPF, "SPF: calculation timer delay = %ld", delay+10);
    }

    zlog_debug(OSPF6_DBG_SPF, "SPF: Scheduled in %ld msec", delay+10);
    /*ospf6->t_spf_calc =
        thread_add_timer_msec(master_ospf6, ospf6_spf_calculation_thread, ospf6, delay);*/
	ospf6->t_spf_calc =
		high_pre_timer_add ((char *)"ospf6_spf_timer", LIB_TIMER_TYPE_NOLOOP,\
						ospf6_spf_calculation_thread, (void *)ospf6, (delay)+10);
}
                          ...
                          struct ospf6_vertex *v)
{
    ...
    len = strlen(prefix) + 4;
	next_prefix = (char *) XMALLOC(MTYPE_TMP, len);
...
config_write_ospf6_debug_spf(struct vty *vty)
{
    ...
    if (IS_OSPF6_DEBUG_SPF(DATABASE))
    {
        vty_out(vty, "debug ospf6 spf lsdb%s", VNL);
    }
...
install_element_ospf6_debug_spf(void)
{
    ...
    install_element(CONFIG_NODE, &debug_ospf6_spf_time_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &debug_ospf6_spf_database_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_spf_process_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_spf_time_cmd, CMD_SYNC);
    install_element(CONFIG_NODE, &no_debug_ospf6_spf_database_cmd, CMD_SYNC);
}
#endif
...
ospf6_spf_init(void)
{
    install_element(OSPF6_NODE, &ospf6_timers_throttle_spf_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_ospf6_timers_throttle_spf_cmd, CMD_SYNC);
}
