...
int
ospf6_redist_cmp(void *va, void *vb)
{
    struct ospf6_redist *ra = (struct ospf6_redist *) va;
    struct ospf6_redist *rb = (struct ospf6_redist *) vb;

    if (rb->source > ra->source)
    {
        return 1;
    }
    else if (rb->source == ra->source)
    {
        if (rb->instance > ra->instance)
        {
            return 1;
        }
        else if (rb->instance == ra->instance)
        {
            return 0;
        }
        else
...
struct ospf6_redist *
ospf6_redist_lookup(struct ospf6 *ospf6, int source, int instance)
{
    struct ospf6_redist *redist = NULL;
    struct listnode *node = NULL;

    for (ALL_LIST_ELEMENTS_RO(ospf6->redist, node, redist))
    {
        if (redist->source == source && redist->instance == instance)
        {
            return redist;
        }
    }
...
struct ospf6_redist *
ospf6_redist_create(struct ospf6 *o, int source, int instance, int type ,int metric)
{
    struct ospf6_redist *ospf6_redist = NULL;
    ospf6_redist = XCALLOC(MTYPE_OSPF6_REDIST, sizeof(struct ospf6_redist));

	if (ospf6_redist == NULL)
    {
        zlog_err("Can't creat ospf6 redist : malloc failed");
        return (struct ospf6_redist *)NULL;
    }
	memset(ospf6_redist, 0, sizeof(struct ospf6_redist));

    ospf6_redist->source = source;
    ospf6_redist->instance = instance;
    ospf6_redist->type = type;
	ospf6_redist->metric = metric;

    listnode_add_sort(o->redist, ospf6_redist);
    return ospf6_redist;

}

void
ospf6_redist_delete(struct ospf6 *o, struct ospf6_redist *redist)
{
    listnode_delete(o->redist, redist);
    /* free area */
    XFREE(MTYPE_OSPF6_REDIST, redist);
...
void
ospf6_as_external_lsa_originate(struct ospf6 *ospf6, struct ospf6_route *route)
{
    ...
    if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE(AS_EXTERNAL))
    {
        ...
        zlog_debug(OSPF6_DBG_LSA_DEBUG|OSPF6_DBG_ASBR, "Originate AS-External-LSA for %s", buf);
    }
...
void
ospf6_asbr_lsa_add(struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
    ...
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Calculate AS-External route for %s", lsa->name);
    }
    ...
    if (lsa->header->adv_router == ospf6->router_id)
    {
        if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Ignore self-originated AS-External-LSA");
        }
    ...
    if (OSPF6_ASBR_METRIC(external) == OSPF_LS_INFINITY)
    {
        if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Ignore LSA with LSInfinity Metric");
        }
    ...
    if (CHECK_FLAG(external->prefix.prefix_options, OSPF6_PREFIX_OPTION_NU))
    {
        if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
        {
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "Ignore LSA with NU bit set Metric");
        }
            ...
            ! CHECK_FLAG(asbr_entry->path.router_bits, OSPF6_ROUTER_BIT_E))
    {
        if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
        {
            ...
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "ASBR entry not found: %s", buf);
        }
    ...
    {
        prefix2str(&route->prefix, buf, sizeof(buf));
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "AS-External route add: %s", buf);
    }
...
void
ospf6_asbr_lsa_remove(struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
    struct ospf6_as_external_lsa *external;
    ...
    if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
    {
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "Withdraw AS-External route for %s", lsa->name);
    }
    ...
    if (route == NULL)
    {
        if (IS_OSPF6_DEBUG_EXAMIN(AS_EXTERNAL))
        {
            ...
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "AS-External route %s not found", buf);
        }
            ...
            route = nroute)
    {
        ...
        {
            prefix2str(&route->prefix, buf, sizeof(buf));
            zlog_debug(OSPF6_DBG_LSA_DEBUG, "AS-External route remove: %s", buf);
        }

        ospf6_route_remove(route, ospf6->route_table, ospf6);
    }
...
ospf6_asbr_lsentry_add(struct ospf6_route *asbr_entry, struct ospf6 *ospf6)
{
    ...
    if (! CHECK_FLAG(asbr_entry->flag, OSPF6_ROUTE_BEST))
    {
        ...
        inet_ntop(AF_INET, &ADV_ROUTER_IN_PREFIX(&asbr_entry->prefix),
                  buf, sizeof(buf));
        zlog_debug(OSPF6_DBG_LSA_DEBUG, "ignore non-best path: lsentry %s add", buf);
        return;
    }
...
#if 0
static void
ospf6_asbr_routemap_set(struct ospf6 *ospf6, int type, const char *mapname)
{
...
#endif
static void
ospf6_asbr_routemap_unset(struct ospf6 *ospf6, int type)
{
    if (ospf6->rmap[type].name)
    {
        free(ospf6->rmap[type].name);
...
static void
ospf6_asbr_redistribute_set(struct ospf6 *ospf6, int source, int instance ,int type ,int metric)
{
    ospf6_zebra_redistribute(ospf6, source, instance, type, metric);
}

static void
ospf6_asbr_redistribute_unset(struct ospf6 *ospf6, int source , int instance)
{
    struct ospf6_route *route = NULL;
    struct ospf6_external_info *info = NULL;
    ospf6_zebra_no_redistribute(ospf6, source, instance);
            ...
            route = ospf6_route_next(route))
    {
        ...
        ospf6_asbr_redistribute_remove(ospf6, info->type, info->instance, route->nexthop[0].ifindex,
                                       &route->prefix);
    }
			
	ospf6_asbr_routemap_unset(ospf6, source);
            ...
            route = ospf6_route_next(route))
    {
        ...
        ospf6_asbr_redistribute_remove(ospf6, info->type, route->nexthop[0].ifindex,
                                       &route->prefix);
    }
#endif
}
...
void
ospf6_asbr_redistribute_add(struct ospf6 *ospf6, struct ase_ipv6 api, ifindex_t ifindex, struct prefix *prefix,
                            u_int nexthop_num, struct in6_addr *nexthop)
{
    ...
    if (! (redist = ospf6_redist_lookup(ospf6, type , api.instance)))
    {
        return;
    }
    ...
    if (IS_OSPF6_DEBUG_ASBR)
    {
        ...
        zlog_debug(OSPF6_DBG_ASBR, "Redistribute %s (%s)", pbuf, ZROUTE_NAME(type));
    }
    ...
    if (match)
    {
		...
		match->path.origin.id = htonl(info->id[ospf6->ospf_id -1]);		
		match->path.metric_type = redist->type;
		match->path.cost = redist->metric;
		ospf6_as_external_lsa_originate(ospf6, match);
	...
	{
		zlog_debug(OSPF6_DBG_ASBR, "%s %d ", __func__,__LINE__);
	}
...
void
ospf6_asbr_redistribute_remove(struct ospf6 *ospf6, int type, int instance, ifindex_t ifindex,
                               struct prefix *prefix)
{
    struct ospf6_route *match;
    struct ospf6_external_info *info = NULL;
    ...
    if (info->instance != instance)
    {
        ...
        {
            prefix2str(prefix, pbuf, sizeof(pbuf));
            zlog_debug(OSPF6_DBG_ASBR, "Original protocol instance mismatch: %s", pbuf);
        }
    ...
    if (IS_OSPF6_DEBUG_ASBR)
    {
        ...
        zlog_debug(OSPF6_DBG_ASBR, "Withdraw %s (AS-External Id:%s)", pbuf, ibuf);
    }

    lsa = ospf6_lsdb_lookup(htons(OSPF6_LSTYPE_AS_EXTERNAL),
                            htonl(info->id[ospf6->ospf_id-1]), ospf6->router_id, ospf6->lsdb);
    ...
    prefix_id.u.prefix4.s_addr = htonl(info->id[ospf6->ospf_id-1]);
    node = route_node_lookup(ospf6->external_id_table, &prefix_id);
    assert(node);
    node->info = NULL;
    route_unlock_node(node);
	#endif
	if (om6->redis_count[info->type] == 0)
	{
     ...
     )
{
    ...
    source = proto_redistnum(AFI_IP6, argv[0]);
    ...
    //ospf6_asbr_redistribute_unset(o, source, 0);
    ospf6_asbr_redistribute_set(o, source , 0 ,type , metric);
     ...
     )
{
    ...
    if (source < 0 || source == ZEBRA_ROUTE_OSPF6)
    {
        return CMD_WARNING;
    }

    ospf6_asbr_redistribute_unset(o, source, 0);
    //ospf6_asbr_routemap_set (o, source, argv[1]);
    ospf6_asbr_redistribute_set(o, source , 0 ,type, metric);
    return CMD_SUCCESS;
}

DEFUN(ospf6_redistribute_instance,
      ospf6_redistribute_instance_cmd,
      "redistribute " QUAGGA_REDIST_STR_OSPF6D_RI "<1-255>"
      " {metric <0-16777214>|type (1|2)}",
      "Redistribute\n"
      QUAGGA_REDIST_HELP_STR_OSPF6D_RI
      "instance num\n"
      "Metric for redistributed routes\n"
      "OSPF default metric\n"
      "OSPF exterior metric type for redistributed routes\n"
      ...
      "Set OSPF External Type 2 metrics\n")
{
	...
	if (ROUTE_PROTO_OSPF6 == source  && instance == o->ospf_id)
    {
    ...
    //ospf6_asbr_redistribute_unset(o, source, instance);
    ospf6_asbr_redistribute_set(o, source, instance, type, metric);
    return CMD_SUCCESS;
}


DEFUN(no_ospf6_redistribute_instance,
      no_ospf6_redistribute_instance_cmd,
      "no redistribute " QUAGGA_REDIST_STR_OSPF6D_RI "<1-255>",
      NO_STR
      "Redistribute\n"
      QUAGGA_REDIST_HELP_STR_OSPF6D
      "instance num\n")
{
    ...
    if (ROUTE_PROTO_OSPF == source  && instance == o->ospf_id)
    {
        vty_error_out(vty, "The instance can not redistribute the routes of its own.%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
     ...
     )
{
    ...
    ospf6_asbr_redistribute_unset(o, type, 0);
    return CMD_SUCCESS;
}


int
ospf6_redistribute_config_write(struct ospf6 *ospf6, struct vty *vty)
{
	struct ospf6_redist *redist = NULL;
    struct listnode *node = NULL;
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->redist, node, redist))
	{
		if(redist->instance == 0)
		{
			if((redist->type == 2) && (redist->metric == 1))
				vty_out(vty, " redistribute %s%s",
	                    ZROUTE_NAME(redist->source), VNL);	
			else if(redist->type == 2)
				vty_out(vty, " redistribute %s metric %d%s",
	                    ZROUTE_NAME(redist->source), redist->metric, VNL);	
			else
				vty_out(vty, " redistribute %s type %d%s",
	                    ZROUTE_NAME(redist->source), redist->type, VNL);
		}
		else
		{
			if((redist->type == 2) && (redist->metric == 1))
				vty_out(vty, " redistribute %s %d%s",
	                    ZROUTE_NAME(redist->source), redist->instance, VNL);	
			else if(redist->type == 2)
				vty_out(vty, " redistribute %s %d metric %d%s",
	                    ZROUTE_NAME(redist->source), redist->instance, redist->metric, VNL);	
			else
				vty_out(vty, " redistribute %s %d type %d%s",
	                    ZROUTE_NAME(redist->source), redist->instance, redist->type, VNL);
		}
...
static void
ospf6_redistribute_show_config(struct ospf6 *ospf6, struct vty *vty)
{
    int type;
    int nroute[ROUTE_PROTO_MAX];
    int total;
...
route_map_command_status(struct vty *vty, int ret)
{
    ...
    switch (ret)
    {
        case RMAP_RULE_MISSING:
            vty_error_out(vty, "OSPF6 Can't find rule.%s", VNL);
            break;
        case RMAP_COMPILE_ERROR:
            vty_error_out(vty, "OSPF6 Argument is malformed.%s", VNL);
            break;
        default:
            vty_error_out(vty, "OSPF6 route-map add set failed.%s", VNL);
            break;
    }
...
ospf6_routemap_init(void)
{
    ...
    install_element(RMAP_NODE, &ospf6_routemap_match_address_prefixlist_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &ospf6_routemap_no_match_address_prefixlist_cmd, CMD_SYNC);
    /* Match interface */
    install_element(RMAP_NODE, &ospf6_routemap_match_interface_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &ospf6_routemap_no_match_interface_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &ospf6_routemap_no_match_interface_val_cmd, CMD_SYNC);
    /* ASE Metric Type (e.g. Type-1/Type-2) */
    install_element(RMAP_NODE, &ospf6_routemap_set_metric_type_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &ospf6_routemap_no_set_metric_type_cmd, CMD_SYNC);
    /* ASE Metric */
    install_element(RMAP_NODE, &set_metric_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &no_set_metric_cmd, CMD_SYNC);
    /* ASE Metric */
    install_element(RMAP_NODE, &ospf6_routemap_set_forwarding_cmd, CMD_SYNC);
    install_element(RMAP_NODE, &ospf6_routemap_no_set_forwarding_cmd, CMD_SYNC);
#endif
}
...
static void
ospf6_asbr_external_route_show(struct vty *vty, struct ospf6_route *route, struct ospf6 *ospf6)
{
    struct ospf6_external_info *info = route->route_option;
    char prefix[64], id[16], forwarding[64];
    u_int32_t tmp_id;
    prefix2str(&route->prefix, prefix, sizeof(prefix));
    tmp_id = ntohl(info->id[ospf6->ospf_id]);
...
DEFUN(show_ipv6_ospf6_redistribute,
      ...
      "show ipv6 ospf6 [<1-255>] redistribute",
      SHOW_STR
      IP6_STR
      OSPF6_STR
      OSPF6_INSTANCE
     ...
     )
{
    ...
    ospf6_redistribute_show_config(o, vty);

    for (route = ospf6_route_head(o->external_table); route;
            route = ospf6_route_next(route))
    {
        ospf6_asbr_external_route_show(vty, route, o);
    }
...
ospf6_asbr_init(void)
{
    ...
    install_element(CONFIG_NODE, &show_ipv6_ospf6_redistribute_cmd, CMD_LOCAL);
    install_element(OSPF6_NODE, &show_ipv6_ospf6_redistribute_cmd, CMD_LOCAL);
	
    install_element(OSPF6_NODE, &ospf6_redistribute_cmd, CMD_SYNC);
    //install_element(OSPF6_NODE, &ospf6_redistribute_routemap_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_ospf6_redistribute_cmd, CMD_SYNC);
	install_element(OSPF6_NODE, &ospf6_redistribute_instance_cmd, CMD_SYNC);
    install_element(OSPF6_NODE, &no_ospf6_redistribute_instance_cmd, CMD_SYNC);
...
ospf6_asbr_redistribute_reset(struct ospf6 *ospf6)
{
    ...
    for (type = 0; type < ROUTE_PROTO_MAX; type++)
    {
        ...
        ////if (ospf6_zebra_is_redistribute (type))
        {
            ospf6_asbr_redistribute_unset(ospf6, type , 0);
        }
    }
}
...
install_element_ospf6_debug_asbr()
{
    ...
    install_element(CONFIG_NODE, &no_debug_ospf6_asbr_cmd, CMD_SYNC);
}
#endif
