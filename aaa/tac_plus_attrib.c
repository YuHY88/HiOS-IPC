/* tac_plus_attrib.c - Procedures for handling internal list of attributes
...
void tac_add_attrib_pair(struct tac_attrib **attr, char *name, char sep, char *value) {
    ...
    if (total_len > 255) {
        zlog_debug(AAA_DBG_TACACS, "%s: attribute `%s' total length exceeds 255 characters, skipping\n",\
            __FUNCTION__, name);
        return;
    }
