...
#if 1
int
p2p_adj_level_check (struct isis_adjacency *adj, struct isis_area * area)
{
  struct listnode * anode, *lnode;
  struct area_addr *addr;
  struct area_addr *addrl;
  int flag = 0;

  if(adj->area_addrs && listcount(adj->area_addrs))
  { 	   	
	for (ALL_LIST_ELEMENTS_RO (adj->area_addrs, lnode, addrl))
   	{  
   	  for (ALL_LIST_ELEMENTS_RO (area->area_addrs, anode, addr))
   	  {	   		
		if(addr->addr_len == addrl->addr_len
		     && memcmp(addr->area_addr, addrl->area_addr,addr->addr_len) == 0)
		{
			flag = 1;
			break;
		}
	      ...
	      struct isis_circuit *circuit)
{
  ...
  if (adj == NULL)
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"Out of memory!");
        return NULL;
    }
		       ...
		       const char *reason)
{
  ...
  if(adj->circuit == NULL)
  {
  	zlog_err("%20s[%d]:bad error that circuit is null.",__FUNCTION__,__LINE__);
  	return;
  }
  if(adj->circuit->area == NULL)
  {
  	zlog_err("%20s[%d]:bad error that area is null.",__FUNCTION__,__LINE__);
  	return;
  }
  ...
  circuit = adj->circuit;
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Adj (%s): Adjacency state change %d->%d: %s",
  ...
  if (circuit->area->log_adj_changes)
  {
	...
	adj_name = sysid_print (adj->sysid);
        ...
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"%%ADJCHANGE: Adjacency to %s (%s) changed from %s to %s, %s",
                    adj_name,
  ...
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
  {
      for (level = IS_LEVEL_1; level <= IS_LEVEL_2; level++)
      {
        ...
        else if (new_state == ISIS_ADJ_DOWN)
        {
          ...
          if(listcount(circuit->u.bc.adjdb[level - 1]) == 0)
          {
            if(level == IS_LEVEL_1)
...
isis_adj_print (struct isis_adjacency *adj)
{
  ...
  u_char ip6[INET6_ADDRSTRLEN];
    ...
    dyn = dynhn_find_by_id (adj->sysid);
    if (dyn)
        zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"%s", dyn->name.name);

    zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"SystemId %20s SNPA %s, level %d\nHolding Time %d",
                sysid_print (adj->sysid), snpa_print (adj->snpa),
                adj->level, adj->hold_time);
    if (adj->ipv4_addrs && listcount (adj->ipv4_addrs) > 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"IPv4 Address(es):");

        for (ALL_LIST_ELEMENTS_RO (adj->ipv4_addrs, node, ipv4_addr))
            zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"%s", inet_ntoa (*ipv4_addr));
    }

#if 1
  if (adj->ipv6_addrs && listcount (adj->ipv6_addrs) > 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"IPv6 Address(es):");
        for (ALL_LIST_ELEMENTS_RO (adj->ipv6_addrs, node, ipv6_addr))
        {
            inet_ntop (AF_INET6, ipv6_addr, (char *)ip6, INET6_ADDRSTRLEN);
            zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"%s", ip6);
        }
    }
#endif /* 0 */
    zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"Speaks: %s", nlpid2string (&adj->nlpids));

  return;
}

int
isis_adj_expire (void * argv)
{
  struct isis_adjacency *adj;
    ...
    //assert (adj);
    adj = (struct isis_adjacency * )argv;
    if(!adj)
    {
...
isis_adj_print_vty (struct isis_adjacency *adj, struct vty *vty, char detail)
{
 ...
 // else
    vty_out (vty, "  %-18s", sysid_print (adj->sysid));

  if (detail == ISIS_UI_LEVEL_BRIEF)
    {
      if (adj->circuit)
		vty_out (vty, "%-23s", adj->circuit->interface->name);
      else
		vty_out (vty, "NULL circuit!");

	 if(adj->level == 3)
	 {	
	   if(adj->circuit->is_type == IS_LEVEL_1_AND_2)
	   {
	      if(p2p_adj_level_check(adj, adj->circuit->area))
	      {
	         vty_out(vty, "1/2     ");
	      }
	      else
	  	...
	  	vty_out (vty, "%-4u", adj->circuit->is_type);
	}
	else
	  vty_out (vty, "  %-4u", adj->level);  /* level */
	  ...
	  {	
	  	if(adj->circuit->is_type == IS_LEVEL_1_AND_2)
	  		...
	  		vty_out (vty, "%-5u", adj->circuit->is_type);
	  }
	  else
      	vty_out (vty, "%-5u", adj->level);	/* level */
    #endif
      vty_out (vty, "%-7s", adj_state2string (adj->adj_state));
      now = time (NULL);
      if (adj->last_upd)
        vty_out (vty, "%-9llu",
	...
	vty_out (vty, "-        ");
      vty_out (vty, "%-12s", snpa_print (adj->snpa));
  ...
  if (detail == ISIS_UI_LEVEL_DETAIL)
	...
	{	
	  if(adj->circuit->is_type == IS_LEVEL_1_AND_2)
	  {
	  	...
	  	vty_out (vty, "%u", adj->circuit->is_type);
	}
	else
	  vty_out (vty, "  %u", adj->level);  /* level */
      ...
      if (adj->circuit && (adj->circuit->circ_type == CIRCUIT_T_BROADCAST))
      {
        ...
        //if (dyn)
         ...
         //     dyn->name.name, adj->lanid[ISIS_SYS_ID_LEN]);
          //vty_out (vty, ", LAN id: %s.%02x",sysid_print (adj->lanid),adj->lanid);              
        //else
          vty_out (vty, ", LAN id: %s.%02x",
      ...
      if (adj->ipv4_addrs && listcount (adj->ipv4_addrs) > 0)
	{
	  ...
	  for (ALL_LIST_ELEMENTS_RO (adj->ipv4_addrs, node, ip_addr))
	  {
	       ...
	       inet_ntop (AF_INET, ip_addr, (char *)ip4, INET_ADDRSTRLEN);
	       vty_out (vty, "      %s%s", ip4, VTY_NEWLINE);
	      //ip_addr_1.s_addr = ntohl(ip_addr->s_addr);
          //vty_out (vty, "      %s%s", inet_ntoa (ip_addr_1), VTY_NEWLINE);
      }
	}
#if 1
      if (adj->ipv6_addrs && listcount (adj->ipv6_addrs) > 0)
...
isis_adj_build_neigh_list (struct list *adjdb, struct list *list)
{
  ...
  if (!list)
    {
        zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"isis_adj_build_neigh_list(): NULL list");
        return;
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (adjdb, node, adj))
    {
        if (!adj)
        {
            zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"isis_adj_build_neigh_list(): NULL adj");
            return;
        }
...
isis_adj_build_up_list (struct list *adjdb, struct list *list)
{
  ...
  if (!list)
    {
        zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"-30%s[%d]:NULL list",__FUNCTION__, __LINE__);
        return;
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (adjdb, node, adj))
    {
        if (!adj)
        {
            zlog_debug (ISIS_DEBUG_TYPE_NEIGHBOR,"-30%s[%d]:NULL adj",__FUNCTION__, __LINE__);
            return;
        }
