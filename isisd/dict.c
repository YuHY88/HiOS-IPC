	...
	dnode_free_t fr, void *context)
{
	...
	if(!((al == NULL && fr == NULL) || (al != NULL && fr != NULL)))
	{
		zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
		return;
	}
...
static void dict_clear(dict_t *dict)
{
	...
	if(dict->nilnode.color != dnode_black)
	{
		zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
		return;
	}
...
dnode_t *dict_delete(dict_t *dict, dnode_t *delete)
{
	...
	while (child->color == dnode_black) {
		...
		if (sister->left->color == dnode_black
			...
			if(sister->left->color != dnode_red)
			{
				zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
				return NULL;
			}
		...
		if (sister->right->color == dnode_black
			...
			if(sister->right->color != dnode_red)
			{
				zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
				return NULL;
			}
...
dnode_t *dict_next(dict_t *dict, dnode_t *curr)
{
    ...
    if(curr == NULL)
    {
        //	zlog_debug(ISIS_DEBUG_TYPE_EVENTS," %-20s [%d] error: curr is null ", __FUNCTION__,__LINE__);
        return NULL;
    }
...
void dict_load_next(dict_load_t *load, dnode_t *newnode, const void *key)
{
	...
	if(!(dict->nodecount < dict->maxcount))
	{
		zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
		return ;
	}
    ...
    if (dict->nodecount > 0) 
	{
		...
		{    
			...
			if(!(dict->compare(nil->left->key, key) <= 0))
			{
				zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
				return ;
			}
...
{
    dict_load_t load;
    dnode_t *leftnode = dict_first(dest);
	dnode_t *rightnode = dict_first(source);

    //assert (dict_similar(dest, source));
	if(!dict_similar(dest, source))
	{
		zlog_err("%-20s[%d] error !",__FUNCTION__ , __LINE__ );
		return ;
	}
