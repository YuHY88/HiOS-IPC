...
isis_run_dr_l1 (void * argv)
{
  ...
  //printf("%s():%d\n", __FUNCTION__, __LINE__);
    ...
    if (circuit->u.bc.run_dr_elect[0])
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_run_dr(): run_dr_elect already set for l1");
...
isis_run_dr_l2 (void * argv)
{
    ...
    if (circuit->u.bc.run_dr_elect[1])
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_run_dr(): run_dr_elect already set for l2");
...
isis_dr_elect (struct isis_circuit *circuit, int level)
{
  ...
  if (!adjdb)
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_dr_elect() adjdb == NULL");
        list_delete (list);
        list = NULL;
        return ISIS_WARNING;
    }
	  ...
	  if (adj_dr)
	      ...
	      if (cmp_res < 0)
		{
		  adj_dr = adj;
                ...
                if (cmp_res == 0)
                    zlog_debug(ISIS_DEBUG_TYPE_EVENTS,"isis_dr_elect(): multiple adjacencies with same SNPA");
	    }
	  else
	    {
	      adj_dr = adj;
...
isis_dr_resign (struct isis_circuit *circuit, int level)
{
  u_char id[ISIS_SYS_ID_LEN + 2];
    ...
    if(listcount(circuit->area->area_addrs) == 0)
        return ISIS_OK;
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_dr_resign l%d", level);

    circuit->u.bc.is_dr[level - 1] = 0;
    circuit->u.bc.run_dr_elect[level - 1] = 0;
	
    THREAD_TIMER_OFF_NEW (circuit->u.bc.t_run_dr[level - 1]);
	circuit->u.bc.t_run_dr[level - 1] = 0;
  ...
  lsp_regenerate_schedule(circuit->area, level, 1);
...
// if (isis->debugs & DEBUG_EVENTS|| isis->debugs & ISIS_DEBUG_TYPE_ALL)
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_dr_commence l%d", level);
  ...
  if (level == 1)
      ...
      if (LSP_PSEUDO_ID (old_dr))
	{
      ...
      memcpy (circuit->u.bc.l1_desig_is, circuit->area->sysid, ISIS_SYS_ID_LEN);
      ...
      if (LSP_PSEUDO_ID (old_dr))
	{
      ...
      memcpy (circuit->u.bc.l2_desig_is, circuit->area->sysid, ISIS_SYS_ID_LEN);
