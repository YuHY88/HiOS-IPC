 ...
 */
int parse_tlvs_selfregenate(u_int32_t prefix_flag, char *areatag, u_char *stream, int size, u_int32_t *expected,
														u_int32_t *found, struct tlvs *tlvs, u_int32_t *auth_tlv_offset)
{
	...
	struct isis_lsp_entry *lsp_entry;
	...
	*found = 0;
	...
	while (pnt < stream + size - 2)
	{
		...
		{
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): TLV (type %d, length %d) exceeds packet "
																			"boundaries",
								 areatag, type, length);
			retval = ISIS_WARNING;
			break;
	   ...
	   */
			*found |= TLVFLAG_AREA_ADDRS;
			...
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "TLV Area Adresses len %d", length);
		...
		case IS_NEIGHBOURS:
			*found |= TLVFLAG_IS_NEIGHS;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IS Neighbours length %d",
								 areatag, length);
	   ...
	   */
			*found |= TLVFLAG_TE_IS_NEIGHS;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): Extended IS Neighbours length %d",
								 areatag, length);
	   ...
	   * :                                                               :
             */
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): ES Neighbours length %d",
								 areatag, length);
	   ...
	   * :                                                               :
			...
			*found |= TLVFLAG_LAN_NEIGHS;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): LAN Neigbours length %d",
								 areatag, length);
		...
		case PADDING:
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "TLV padding %d", length);
			pnt += length;
			break;
	   ...
	   * +-------+-------+-------+-------+-------+-------+-------+-------+
             */
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): LSP Entries length %d", areatag, length);
			*found |= TLVFLAG_LSP_ENTRIES;
			if (TLVFLAG_LSP_ENTRIES & *expected)
			{
				while (length > value_len)
				{
					lsp_entry = (struct isis_lsp_entry *)pnt;
	   ...
	   */
			*found |= TLVFLAG_CHECKSUM;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): Checksum length %d", areatag, length);
	   ...
	   */
			*found |= TLVFLAG_NLPID;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): Protocols Supported length %d",
								 areatag, length);
	   ...
	   */
			*found |= TLVFLAG_IPV4_ADDR;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IPv4 Address length %d",
								 areatag, length);
			if (*expected & TLVFLAG_IPV4_ADDR)
			{
				while (length > value_len)
				{
					...
					// zlog_debug (ISIS_DEBUG_TYPE_TLV,"ISIS-TLV (%s) : IP ADDR %s, pnt %p", areatag,
		...
		case AUTH_INFO:
			*found |= TLVFLAG_AUTH_INFO;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IS-IS Authentication Information",
								 areatag);
			if (*expected & TLVFLAG_AUTH_INFO)
			{
				...
				if (length == 0)
				{
					zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): TLV (type %d, length %d) "
																					"incorrect.",
										 areatag, type, length);
					return ISIS_WARNING;
				}
		...
		case DYNAMIC_HOSTNAME:
			*found |= TLVFLAG_DYN_HOSTNAME;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): Dynamic Hostname length %d",
								 areatag, length);
	   ...
	   */
			*found |= TLVFLAG_TE_ROUTER_ID;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): TE Router ID %d", areatag, length);
	   ...
	   */
			*found |= TLVFLAG_IPV4_INT_REACHABILITY;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IPv4 internal Reachability length %d",
								 areatag, length);
	   ...
	   */
			*found |= TLVFLAG_IPV4_EXT_REACHABILITY;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IPv4 external Reachability length %d",
								 areatag, length);
	   ...
	   */
			*found |= TLVFLAG_TE_IPV4_REACHABILITY;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IPv4 extended Reachability length %d",
								 areatag, length);
			endpnt = pnt + length;
			if (*expected & TLVFLAG_TE_IPV4_REACHABILITY)
			{
				while (length > value_len)
				{
					...
					{
						zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): invalid IPv4 extended reach"
		  ...
		  listnode_add (tlvs->te_ipv4_reachs, te_ipv4_reach);*/
					...
					if (prefix_flag == CIRCUIT_IPV4_FLAG)
					{
						if (!tlvs->te_circuit_ipv4_reachs)
							tlvs->te_circuit_ipv4_reachs = list_new();
						listnode_add(tlvs->te_circuit_ipv4_reachs, te_ipv4_reach);
					}
					else
					{
						if (!tlvs->te_ipv4_reachs)
					...
					value_len += 5 + ((te_ipv4_reach->control & 0x3F) ? ((((te_ipv4_reach->control & 0x3F) -
																																 1) >>
																																3) +
																															 1)
																														: 0);
	   ...
	   */
			*found |= TLVFLAG_IPV6_ADDR;
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): IPv6 Address length %d",
								 areatag, length);
			...
			*found |= TLVFLAG_IPV6_REACHABILITY;
			...
			if (*expected & TLVFLAG_IPV6_REACHABILITY)
			{
				while (length > value_len)
				{
					...
					{
						zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): invalid IPv6 extended reach"
																						"ability prefix length %d",
											 areatag,
	   ...
	   */
			*found |= TLVFLAG_GRACEFUL_RESTART;
			if (*expected & TLVFLAG_GRACEFUL_RESTART)
			{
	      	...
	      	{
	      		tlvs->gr_info->len = 3;
	      		memcpy(tlvs->gr_info->flag, pnt, 1);
	      		value_len += tlvs->gr_info.len + 1;
	      	}
	      	else
	      	{
	      		tlvs->gr_info.len = 9;
	      		memcpy(tlvs->gr_info->flag, pnt, 1);
	      		memcpy(tlvs->gr_info->hold_time, pnt + 1, 2);
	      		memcpy(tlvs->gr_info->sysid, pnt + 3, ISIS_SYS_ID_LEN);	      		
	      	}
#endif
				}
			}
		...
		default:
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "ISIS-TLV (%s): unsupported TLV type %d, length %d",
								 areatag, type, length);
...
int add_tlv(u_char tag, u_char len, u_char *value, struct stream *stream)
{
	if ((stream_get_size(stream) - stream_get_endp(stream)) <
			(((unsigned)len) + 2))
	{
		zlog_debug(ISIS_DEBUG_TYPE_TLV, "No room for TLV of type %d "
...
#ifdef EXTREME_DEBUG
	zlog_debug(ISIS_DEBUG_TYPE_TLV, "Added TLV %d len %d", tag, len);
#endif /* EXTREME DEBUG */
	return ISIS_OK;
}

int tlv_graceful_restart(u_char flag, u_int16_t hold_time, u_char *sysid, struct stream *stream)
{
	u_char value[255];
	u_char *pos = value;

	memcpy(pos, &flag, sizeof(u_char));
	pos++;

	if (hold_time && sysid != NULL)
	{
		memcpy(pos, &hold_time, sizeof(u_int16_t));
		pos = pos + sizeof(u_int16_t);

		memcpy(pos, sysid, ISIS_SYS_ID_LEN);
		pos = pos + ISIS_SYS_ID_LEN;
	}
	else
	{
		memset(pos, 0, 2);
	...
	return add_tlv(GRACEFUL_RESTART, pos - value, value, stream);
}
...
err:
	zlog_debug(ISIS_DEBUG_TYPE_TLV, "tlv_add_area_addrs(): TLV longer than 255");
	return ISIS_WARNING;
}

int tlv_add_is_neighs(struct list *is_neighs, struct stream *stream, struct isis_area *area)
{
	struct listnode *node;
...
int tlv_add_te_is_neighs(struct list *te_is_neighs, struct stream *stream, struct isis_area *area)
{
	struct listnode *node;
...
int tlv_add_ip_addrs(struct list *ip_addrs, struct stream *stream)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ip_addrs, node, ipv4))
	{
		if (pos - value + IPV4_MAX_BYTELEN > 255)
		{
			...
			zlog_debug(ISIS_DEBUG_TYPE_TLV, "tlv_add_ip_addrs(): cutting off at 63 IP addresses");
			break;
		}
		*(u_int32_t *)pos = htonl(ipv4->prefix.s_addr);
...
int tlv_add_in_addr(struct in_addr *addr, struct stream *stream, u_char tag)
{
	...
	addr_tmp.s_addr = htonl(addr->s_addr);
	memcpy(pos, &addr_tmp, IPV4_MAX_BYTELEN);
...
int tlv_add_circuit_lsp_entries(struct list *c_lsps, struct stream *stream)
{
	struct listnode *node;
	struct isis_lsp_entry *c_lsp_entry;
	u_char value[255];
	u_char *pos = value;
	int retval;

	for (ALL_LIST_ELEMENTS_RO(c_lsps, node, c_lsp_entry))
	{
		if (pos - value + LSP_ENTRIES_LEN > 255)
		{
			retval = add_tlv(LSP_ENTRIES, pos - value, value, stream);
		...
		*((u_int16_t *)pos) = c_lsp_entry->rem_lifetime;
		pos += 2;
		memcpy(pos, c_lsp_entry->lsp_id, ISIS_SYS_ID_LEN + 2);
		pos += ISIS_SYS_ID_LEN + 2;
		*((u_int32_t *)pos) = c_lsp_entry->seq_num;
		pos += 4;
		*((u_int16_t *)pos) = c_lsp_entry->checksum;
		pos += 2;
	}
...
int summary_match(const struct ip_summary *n, const struct ipv4_reachability *p)
{
	int offset;
	int shift;
	const u_char *np, *pp;
	...
	if (n->summary_p.prefixlen > ip_masklen(p->mask))
		return 0;

	/* Set both prefix's head pointer. */
	np = (const u_char *)&n->summary_p.prefix.s_addr;
	pp = (const u_char *)&p->prefix.s_addr;

	offset = n->summary_p.prefixlen / 8;
	shift = n->summary_p.prefixlen % 8;
  ...
  if(listcount(area->summary) > 0)
  {
    for(ALL_LIST_ELEMENTS_RO (area->summary, snode, sreach))
    {
        ...
        *pos = METRICS_UNSUPPORTED;
        ...
        masklen2ip (sreach->summary_p.prefixlen, &reach->mask);
		reach->prefix.s_addr = ((reach->mask.s_addr) &
									(sreach->summary_p.prefix.s_addr));
										...
										struct isis_area *area)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ipv4_reachs, node, reach))
	{
		...
		*pos = reach->metrics.metric_error;
		pos++;
		*(u_int32_t *)pos = htonl(reach->prefix.s_addr);
		pos += IPV4_MAX_BYTELEN;
		*(u_int32_t *)pos = htonl(reach->mask.s_addr);
		pos += IPV4_MAX_BYTELEN;
	}
...
int tlv_add_ipv4_int_reachs(struct list *ipv4_reachs, struct stream *stream,
														struct isis_area *area)
{
	return tlv_add_ipv4_reachs(IPV4_INT_REACHABILITY, ipv4_reachs, stream, area);
}

int tlv_add_ipv4_ext_reachs(struct list *ipv4_reachs, struct stream *stream,
														struct isis_area *area)
{
	return tlv_add_ipv4_reachs(IPV4_EXT_REACHABILITY, ipv4_reachs, stream, area);
}

int tlv_add_te_ipv4_reachs(struct list *te_ipv4_reachs, struct stream *stream, struct isis_area *area)
{
	u_char prefix_size;
	int retval;
	u_char value[255];
	...
	//TLV memory leak
	//  if(te_ipv4_reachs != NULL)
	//  {
	//  	  te_ipv4_reachs->del = free_tlv;
	//      list_delete_all_node (te_ipv4_reachs);
	//  }
							...
							struct isis_area *area)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (te_ipv4_reachs, node, te_reach))
  {
	  ...
	  if(count > 0)
	  {
      	for(ALL_LIST_ELEMENTS_RO (area->summary, snode, sreach))
      	{
      		if(summary_match(sreach,te_reach))
      		{      		
      			sreach->sflag++;
      			count++;
      			continue;
  ...
  for(ALL_LIST_ELEMENTS_RO (area->summary, snode, sreach))
  {
  	 if(sreach->sflag > 0)
  	 {
		 *(u_int32_t *) pos = sreach->metric;
		 pos += 4;
		 *pos = sreach->summary_p.prefixlen & 0x3F;
		 pos++; 	 
		 memcpy (pos, &sreach->summary_p.prefix.s_addr, prefix_size);
		 pos += prefix_size;
      	 sreach->sflag = 0;
     }
     else
...
/* 
	why i need two similar functions to build the same tlv ?
	the source structs are different when i build hello and lsp ,
	the struct under citcuit is prefix_ipv6 ,
	but the struct under lsp is in6_addr...
	I want to change the situation , but too many problems to do it 
*/
int tlv_add_ipv6_addrs_for_hello(struct list *ipv6_addrs, struct stream *stream , struct isis_area * area)
{
	struct listnode *node;
	...
	for (ALL_LIST_ELEMENTS_RO(ipv6_addrs, node, ipv6_prefix))
	{
		...
		memcpy(pos, &(ipv6_prefix->prefix.s6_addr), IPV6_MAX_BYTELEN);
...
int tlv_add_ipv6_addrs_for_lsp(struct list *ipv6_addrs, struct stream *stream , struct isis_area * area)
{
	struct listnode *node;
	...
	for (ALL_LIST_ELEMENTS_RO(ipv6_addrs, node, ipv6))
	{
		...
		memcpy(pos, &(ipv6->s6_addr), IPV6_MAX_BYTELEN);
		pos += IPV6_MAX_BYTELEN;
	}
...
int tlv_add_ipv6_reachs(struct list *ipv6_reachs, struct stream *stream, struct isis_area *area)
{
	struct listnode *node;
...
err:
	zlog_debug(ISIS_DEBUG_TYPE_TLV, "tlv_add_padding(): no room for tlv");
	return ISIS_WARNING;
}
