...
open_bpf_dev (struct isis_circuit *circuit)
{
  ...
  if (fd < 0)
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"Opened BPF device %s", bpfdev);

  memcpy (ifr.ifr_name, circuit->interface->name, sizeof (ifr.ifr_name));
  if (ioctl (fd, BIOCSETIF, (caddr_t) & ifr) < 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"open_bpf_dev(): failed to bind to interface: %s",
                    safe_strerror (errno));
        return ISIS_WARNING;
    }
  ...
  if (ioctl (fd, BIOCGBLEN, (caddr_t) & blen) < 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"failed to get BPF buffer len");
  ...
  if (readbuff == NULL)
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"BPF buffer len = %u", blen);
...
isis_recv_pdu_bcast (struct isis_circuit *circuit, u_char * ssnpa)
{
  ...
  if (ioctl (circuit->fd, FIONREAD, (caddr_t) & bytestoread) < 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_RECEIVE,"ioctl() FIONREAD failed: %s", safe_strerror (errno));
    }
  ...
  if (bytesread < 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_RECEIVE,"isis_recv_pdu_bcast(): read() failed: %s",
                    safe_strerror (errno));
        return ISIS_WARNING;
    }
  ...
  if(bpf_hdr->bh_caplen != bpf_hdr->bh_datalen)
  {
  	zlog_err("%-20s[%d] error: ",__FUNCTION__ ,  __LINE__);
	return ISIS_ERROR;
  }
  ...
  memcpy (ssnpa, readbuff + bpf_hdr->bh_hdrlen + ETHER_ADDR_LEN,
    ...
    if (ioctl (circuit->fd, BIOCFLUSH, &one) < 0)
        zlog_debug (ISIS_DEBUG_TYPE_RECEIVE,"Flushing failed: %s", safe_strerror (errno));
...
isis_send_pdu_bcast (struct isis_circuit *circuit, int level)
{
  ...
  if (buflen > sizeof (sock_buff))
    {
        zlog_debug (ISIS_DEBUG_TYPE_RECEIVE,"isis_send_pdu_bcast: sock_buff size %zu is less than "
  ...
  if (written < 0)
    {
      zlog_err("IS-IS bpf: could not transmit packet on %s: %s",
