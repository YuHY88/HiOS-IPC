                                 ...
                                 struct isis_area *area, int up)
{
  area->circuit_state_changes++;
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) circuit %s", area->area_tag, up ? "up" : "down");
...
// if (isis->debugs & DEBUG_EVENTS|| isis->debugs & ISIS_DEBUG_TYPE_ALL)
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) system type change %s -> %s", area->area_tag,
  ...
  for (ALL_LIST_ELEMENTS_RO (area->circuit_list, node, circuit))
  {
  	  if(circuit->circ_type == CIRCUIT_T_BROADCAST)
  	  {
		  ...
		  {
		  	if(circuit->u.bc.adjdb[level] != NULL && listcount(circuit->u.bc.adjdb[level]) != 0)
			{
				for (ALL_LIST_ELEMENTS (circuit->u.bc.adjdb[level], anode,aanode, adj))
				{
					if(adj != NULL && newtype == IS_LEVEL_1 && (adj->level == IS_LEVEL_2 || adj->level == IS_LEVEL_1_AND_2))
					{
						adj = isis_adj_lookup (adj->sysid, circuit->u.bc.adjdb[IS_LEVEL_2-1]);
						if(adj != NULL)
						{
					...
					if(adj != NULL && newtype == IS_LEVEL_2 && (adj->level == IS_LEVEL_1 || adj->level == IS_LEVEL_1_AND_2))
					{
						adj = isis_adj_lookup (adj->sysid, circuit->u.bc.adjdb[IS_LEVEL_1-1]);
						if(adj != NULL)
						{
							isis_adj_state_change(adj,ISIS_ADJ_DOWN, "adjacency level was changed");
						}
...
circuit_resign_level (struct isis_circuit *circuit, int level)
{
  int idx = level - 1;
    ...
    THREAD_TIMER_OFF_NEW (circuit->t_send_csnp[idx]);
	circuit->t_send_csnp[idx] = 0;
    THREAD_TIMER_OFF_NEW (circuit->t_send_psnp[idx]);
	circuit->t_send_psnp[idx] = 0;

    list_delete(circuit->same_sys_psnp[level - 1]);
    circuit->same_sys_psnp[level - 1] = NULL;
    if (circuit->circ_type == CIRCUIT_T_BROADCAST)
    {
        THREAD_TIMER_OFF_NEW (circuit->u.bc.t_send_lan_hello[idx]);
		circuit->u.bc.t_send_lan_hello[idx] = 0;
        THREAD_TIMER_OFF_NEW (circuit->u.bc.t_run_dr[idx]);
		circuit->u.bc.t_run_dr[idx] = 0;
        THREAD_TIMER_OFF_NEW (circuit->u.bc.t_refresh_pseudo_lsp[idx]);
		circuit->u.bc.t_refresh_pseudo_lsp[idx] = 0;
...
// if (isis->debugs & DEBUG_EVENTS || isis->debugs & ISIS_DEBUG_TYPE_ALL)
    ...
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) circuit type change %s -> %s",
  ...
  if (!(newtype & circuit->area->is_type))
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) circuit type change - invalid level %s because"
...
// if (isis->debugs & DEBUG_EVENTS|| isis->debugs & ISIS_DEBUG_TYPE_ALL)
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) Adjacency State change",
...
isis_event_dis_status_change (struct thread *thread)
{
  ...
  circuit = THREAD_ARG (thread);
    ...
    if (!circuit || !circuit->area)
        return 0;
    //if (isis->debugs & DEBUG_EVENTS|| isis->debugs & ISIS_DEBUG_TYPE_ALL)
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) DIS status change", circuit->area->area_tag);
...
{
    //if (isis->debugs & DEBUG_EVENTS|| isis->debugs & ISIS_DEBUG_TYPE_ALL)
    zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"ISIS-Evt (%s) Authentication failure %s from %s",
