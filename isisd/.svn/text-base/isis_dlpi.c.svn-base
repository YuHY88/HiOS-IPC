  ...
  if (rv == -1 && dbuf == NULL)
    {
         ...
         */
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"%s: putmsg: %s", __func__, safe_strerror (errno));
    }
  return rv;
}
...
isis_recv_pdu_bcast (struct isis_circuit *circuit, u_char * ssnpa)
{
  ...
  if (retv < 0)
    {
        zlog_debug (ISIS_DEBUG_TYPE_RECEIVE,"isis_recv_pdu_bcast: getmsg failed: %s",
                    safe_strerror (errno));
        return ISIS_WARNING;
    }
...
isis_send_pdu_bcast (struct isis_circuit *circuit, int level)
{
  ...
  *dstsap = buflen;
  ...
  if (rv < 0)
    {
      zlog_err("IS-IS dlpi: could not transmit packet on %s: %s",
