...
static int dyn_cache_cleanup (void *);
...
dyn_cache_init (void)
{
    if (dyn_cache == NULL)
        dyn_cache = list_new ();
    THREAD_TIMER_ON_NEW ((char *)"ISISDynCleanUpNoLoop", isis->t_dync_clean, dyn_cache_cleanup, NULL, 100);

    return;
}

static int
dyn_cache_cleanup (void * argv)
{
...
isis_dynhn_insert (const u_char * id, struct hostname *hostname, int level)
{
  ...
  if (!dyn)
    {
        zlog_debug (ISIS_DEBUG_TYPE_EVENTS,"isis_dynhn_insert(): out of memory!");
        return;
    }
