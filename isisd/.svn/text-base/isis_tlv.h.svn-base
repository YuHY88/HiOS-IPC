 ...
 * InterDomain Routing Protocol Information  131   n   y   y  RFC1195
...
#define	CIRCUIT_IPV4_FLAG	1	/* circuit->ip_addrs */
...
struct tlvs
{
  ...
  struct list *te_circuit_ipv4_reachs;	/*circuit ipv4*/
...
int
parse_tlvs_selfregenate (u_int32_t prefix_flag, char *areatag, u_char * stream, int size, u_int32_t * expected,
...
int tlv_add_ipv4_int_reachs (struct list *ipv4_reachs, struct stream *stream, struct isis_area *area);
int tlv_add_ipv4_ext_reachs (struct list *ipv4_reachs, struct stream *stream, struct isis_area *area);
