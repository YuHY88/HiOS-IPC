 ...
 */
int ip_same_subnet(struct prefix_ipv4 *ip1, struct in_addr *ip2)
{
  u_char *addr1, *addr2;
...
ip_match(struct list *left, struct list *right)
{
  ...
  if ((left == NULL) || (right == NULL))
  {
    if (left == NULL)
    {
      zlog_debug(ISIS_DEBUG_TYPE_HELLO, "circuit->ip_addrs is NULL!\n");
    }

    if (right == NULL)
    {
      zlog_debug(ISIS_DEBUG_TYPE_HELLO, "tlv->data->ip is NULL!\n");
    }
    return 0;
  }
  ...
  for (ALL_LIST_ELEMENTS_RO(left, node1, ip1))
  {
    ...
    {
      ip1_tmp = *ip1;
      ip1_tmp.prefix.s_addr = htonl(ip1->prefix.s_addr);
      if (ip_same_subnet(&ip1_tmp, ip2))
      {
        return 1; /* match */
      }
    }
...
static int
ip_cmp(struct list *left, struct list *right)
{
  struct prefix_ipv4 *ip1;
  struct in_addr ip1_tmp;
  struct in_addr *ip2;
  struct listnode *node1, *node2;
  u_char *addr1, *addr2;
  ...
  for (ALL_LIST_ELEMENTS_RO(left, node1, ip1))
  {
    for (ALL_LIST_ELEMENTS_RO(right, node2, ip2))
    {
      ip1_tmp.s_addr = htonl(ip1->prefix.s_addr);
      addr1 = (u_char *)&ip1_tmp.s_addr;
      //addr1 = (u_char *) & ip1->prefix.s_addr;
      addr2 = (u_char *)&ip2->s_addr;
      if (!memcmp(addr1, addr2, sizeof(struct in_addr)))
      {
        return 0;
      }
    }
                         ...
                         int level, struct isis_passwd *passwd)
{
  ...
  if (retval != ISIS_OK)
  {
    zlog_debug(ISIS_DEBUG_TYPE_EVENTS, "ISIS-Upd (%s): Parse failed L%d LSP %s, seq 0x%08x, "
 ...
 */
static int
process_p2p_hello(struct isis_circuit *circuit, const u_char *ssnpa)
{
  int retval = ISIS_OK;
  ...
  zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE,
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "p2p hello on non p2p circuit");
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "Packet too short");
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Rcvd P2P IIH from (%s) with "
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "parse_tlvs() failed");
    free_tlvs(&tlvs);
    return retval;
  };
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "No Area addresses TLV in P2P IS to IS hello");
    free_tlvs(&tlvs);
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "No supported protocols TLV in P2P IS to IS hello");
    free_tlvs(&tlvs);
    return ISIS_WARNING;
  }
  ...
  if (hdr->circuit_t != circuit->is_type)
  {
    if (!(hdr->circuit_t & circuit->is_type))
    {
      if (circuit->u.p2p.neighbor != NULL)
      {
        isis_adj_state_change(circuit->u.p2p.neighbor, ISIS_ADJ_DOWN, "level isn't matching");
        free_tlvs(&tlvs);
        return ISIS_OK;
      }
    }
  ...
  /* 
  if(hdr->circuit_t != circuit->is_type && 
  	(hdr->circuit_t == IS_LEVEL_1 || hdr->circuit_t == IS_LEVEL_2) )
  {
  	  if(hdr->circuit_t == IS_LEVEL_1 )
  	  {
		 dnode = dict_first (circuit->area->lspdb[ISIS_LEVEL2-1]);
		 while (dnode)
		 {
			next = dict_next (circuit->area->lspdb[ISIS_LEVEL2-1], dnode);
			lsp = dnode_get (dnode);
			if(lsp != NULL && !memcmp(lsp->lsp_header->lsp_id,hdr->source_id,ISIS_SYS_ID_LEN) )
			{
				lsp_destroy(lsp);
				dict_delete_free (circuit->area->lspdb[ISIS_LEVEL2-1], dnode);
			}
			dnode = next;
	     }
  	  }
	  else if(hdr->circuit_t == IS_LEVEL_2 )  	  
	  {
		 dnode = dict_first (circuit->area->lspdb[ISIS_LEVEL1-1]);
		 while (dnode)
		 {
			next = dict_next (circuit->area->lspdb[ISIS_LEVEL1-1], dnode);
			lsp = dnode_get (dnode);
			if(lsp != NULL && !memcmp(lsp->lsp_header->lsp_id,hdr->source_id,ISIS_SYS_ID_LEN) )
			{
				lsp_destroy(lsp);
				dict_delete_free (circuit->area->lspdb[ISIS_LEVEL1-1], dnode);
			}
			dnode = next;
	     }
  	  }
  ...
  */
  ...
  {
    if (circuit->ip_addrs != NULL && circuit->ip_addrs->count != 0)
      v4_usable = 1;
  }
   ...
   */
  if (!memcmp(hdr->source_id, circuit->area->sysid, ISIS_SYS_ID_LEN))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, 
				"ISIS-Adj (%s): it's own IIH PDU - discarded", circuit->area->area_tag);
    free_tlvs(&tlvs);
    return ISIS_WARNING;
  }
  ...
  if (adj)
  {
    ...
    {
      zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "hello source and adjacency do not match, set adj down\n");
  ...
  if (!adj || adj->level != hdr->circuit_t)
  {
    if (!adj)
    {
      adj = isis_new_adj(hdr->source_id, ssnpa, hdr->circuit_t, circuit);
      if (adj == NULL)
        return ISIS_ERROR;
      //adj->level = hdr->circuit_t & circuit->is_type;
    }
    else
    {
  ...
  THREAD_TIMER_OFF_NEW(adj->t_expire);
  adj->t_expire = 0;

  THREAD_TIMER_ON_NEW((char *)"ISISAdjExpire", adj->t_expire, isis_adj_expire, adj,
                      (long)adj->hold_time);
    ...
    if (circuit->area->is_type == IS_LEVEL_1)
    {
      ...
      case IS_LEVEL_2:
        if (adj->adj_state != ISIS_ADJ_UP)
        {
          ...
          zlog_debug(ISIS_DEBUG_TYPE_EVENTS, "wrongSystemType");
          free_tlvs(&tlvs);
    ...
    else
    {
      ...
      case IS_LEVEL_1:
        if (adj->adj_state != ISIS_ADJ_UP)
        {
          ...
          zlog_debug(ISIS_DEBUG_TYPE_EVENTS, "AreaMismatch");
          free_tlvs(&tlvs);
          return ISIS_WARNING; /* Reject */
        }
        else if (adj->adj_usage == ISIS_ADJ_LEVEL1)
  ...
  zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Rcvd P2P IIH from (%s), cir type %s,"
                                                              " cir id %02d, length %d",
             circuit->area->area_tag, circuit->interface->name,
...
process_lan_hello(int level, struct isis_circuit *circuit, const u_char *ssnpa)
{
  ...
  zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Rcvd L%d LAN IIH on %s, cirType %s, "
                                                              "cirID %u",
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "lan hello on non broadcast circuit");
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "level %d LAN Hello received over circuit with "
                                                                "externalDomain = true",
               level);
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Interface level mismatch, %s",
               circuit->area->area_tag, circuit->interface->name);
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): is type mismatch",
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Rcvd LAN IIH from (%s) with "
                                                                "invalid pdu length %d",
               circuit->area->area_tag, circuit->interface->name,
               hdr.pdu_len);
    return ISIS_WARNING;
  ...
  expected |= TLVFLAG_IPV4_ADDR;
  expected |= TLVFLAG_IPV6_ADDR;
  expected |= TLVFLAG_GRACEFUL_RESTART;
  ...
  if (!(found & TLVFLAG_AREA_ADDRS))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "No Area addresses TLV in Level %d LAN IS to IS hello",
               level);
    retval = ISIS_WARNING;
    goto out;
  }
  ...
  if (!(found & TLVFLAG_NLPID))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "No supported protocols TLV in Level %d LAN IS to IS hello",
               level);
    retval = ISIS_WARNING;
    goto out;
  }
  ...
  if (!memcmp(hdr.source_id, circuit->area->sysid, ISIS_SYS_ID_LEN))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): duplicate system ID on interface %s",
               circuit->area->area_tag, circuit->interface->name);
    return ISIS_WARNING;
  }
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Area mismatch, level %d IIH on %s",
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): it's own IIH PDU - discarded",
               circuit->area->area_tag);
  ...
  if (hdr.circuit_t == IS_LEVEL_2)
  {
    if (circuit->u.bc.adjdb[IS_LEVEL_1 - 1] != NULL && listcount(circuit->u.bc.adjdb[IS_LEVEL_1 - 1]) != 0)
    {
      adj = isis_adj_lookup(hdr.source_id, circuit->u.bc.adjdb[IS_LEVEL_1 - 1]);
      if (adj)
        isis_adj_state_change(adj, ISIS_ADJ_DOWN, "adjacency level was changed");
    }
  }

  if (hdr.circuit_t == IS_LEVEL_1)
  {
    if (circuit->u.bc.adjdb[IS_LEVEL_2 - 1] != NULL && listcount(circuit->u.bc.adjdb[IS_LEVEL_2 - 1]) != 0)
    {
      adj = isis_adj_lookup(hdr.source_id, circuit->u.bc.adjdb[IS_LEVEL_2 - 1]);
      if (adj)
        isis_adj_state_change(adj, ISIS_ADJ_DOWN, "adjacency level was changed");
    }
  }
  ...
  if (found & TLVFLAG_IPV4_ADDR)
  {
    if (!ip_cmp(circuit->ip_addrs, tlvs.ipv4_addrs))
      v4_usable = 0;
    else if (ip_match(circuit->ip_addrs, tlvs.ipv4_addrs))
      v4_usable = 1;
    else
      zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj: IPv4 addresses present but no overlap "
                                                                  "in LAN IIH from %s\n",
                 circuit->interface->name);
  }
  else /* !(found & TLVFLAG_IPV4_ADDR) */
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj: no IPv4 in LAN IIH from %s "
  ...
  if (found & TLVFLAG_IPV6_ADDR)
  {
    ...
    if (!v6_usable)
      zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj: IPv6 addresses present but no link-local "
                                                                  "in LAN IIH from %s\n",
                 circuit->interface->name);
  }

  if (!(found & (TLVFLAG_IPV4_ADDR | TLVFLAG_IPV6_ADDR)))
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj: neither IPv4 nor IPv6 addr in LAN IIH from %s\n",
               circuit->interface->name);
#endif
...
#if 0
  if (found & TLVFLAG_GRACEFUL_RESTART)
  {
  	if(tlvs.gr_info.flag & 0x01)
  	{
  		if(adj != NULL && adj->adj_state == ISIS_ADJ_UP)
  		{
  			adj->hold_time = circuit->area->gr_interval;
  			circuit->area->gr = 1;
  	...
  	else if(tlvs.gr_info.flag & 0x02)
  	{
  ...
  if (found & TLVFLAG_LAN_NEIGHS)
  {
    ...
    else
    {
      ...
      if (found == 0)
      {
        //zlog_err("%s()%d",__FUNCTION__,__LINE__);
...
out:
  ...
  zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Adj (%s): Rcvd L%d LAN IIH from %s on %s, cirType %s, "
                                                              "cirID %u, length %lu",
             circuit->area->area_tag,
...
process_lsp(int level, struct isis_circuit *circuit, const u_char *ssnpa)
{
  ...
  zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): Rcvd L%d LSP on %s, cirType %s, cirID %u",
             circuit->area->area_tag, level, circuit->interface->name,
             circuit_t2string(circuit->is_type), circuit->circuit_id);
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "Packet too short");
    return ISIS_WARNING;
  }
  ...
  if (pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_LSP_HDR_LEN) ||
      ...
      pdu_len > stream_get_endp(circuit->rcv_stream))
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s invalid LSP length %d",
               circuit->area->area_tag,
  ...
  zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): Rcvd L%d LSP %s, seq 0x%08x, cksum 0x%04x, "
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s invalid LSP is type %x",
               circuit->area->area_tag,
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s invalid LSP checksum 0x%04x",
               circuit->area->area_tag,
  ...
  if (circuit->ext_domain)
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s received at level %d over circuit with "
                                                              "externalDomain = true",
               circuit->area->area_tag,
  ...
  if (!accept_level(level, circuit->is_type))
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s received at level %d over circuit of"
                                                              " type %s",
               circuit->area->area_tag,
               rawlspid_print(hdr->lsp_id),
  ...
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
  {
    ...
    if (!adj)
    {
      zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "(%s): DS ======= LSP %s, seq 0x%08x, cksum 0x%04x, "
                                                                "lifetime %us on %s",
                 circuit->area->area_tag,
                 rawlspid_print(hdr->lsp_id),
                 ntohl(hdr->seq_num),
...
dontcheckadj:
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): LSP %s seq 0x%08x with confused checksum received.",
  ...
  if (hdr->rem_lifetime == 0)
  {
    ...
    {
      if (memcmp(hdr->lsp_id, circuit->area->sysid, ISIS_SYS_ID_LEN))
      {
      ...
      else if (lsp->lsp_header->rem_lifetime != 0)
      {
        ...
        if (comp == LSP_NEWER)
        {
		  zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS (%s):LSP 0x%08x received generated by myself,increase the seq send again",
					   circuit->area->area_tag,
					   ntohl(hdr->seq_num));
        ...
        zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS (%s): re-originating LSP %s new seq 0x%08x",
                   circuit->area->area_tag,
                   rawlspid_print(hdr->lsp_id),
  ...
  if (memcmp(hdr->lsp_id, circuit->area->sysid, ISIS_SYS_ID_LEN) == 0)
  {
    if (!lsp)
    {
    ...
    if (ntohl(hdr->seq_num) > ntohl(lsp->lsp_header->seq_num))
    {
      ...
      zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Upd (%s): (2) re-originating LSP %s new seq "
                                                                "0x%08x",
                 circuit->area->area_tag,
                 rawlspid_print(hdr->lsp_id),
                 ntohl(lsp->lsp_header->seq_num));
  ...
  else
  {
    ...
    if ((!lsp || comp == LSP_NEWER))
    {
      ...
      if (LSP_FRAGMENT(hdr->lsp_id) != 0)
      {
        ...
        if (!lsp0)
        {
          zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_RECEIVE, "Got lsp frag, while zero lsp not in database");
          return ISIS_OK;
        }
      }
      /* i */
            ...
            const u_char *ssnpa)
{
  ...
  struct isis_lsp *lsp;
  struct isis_lsp_entry *entry;
  struct listnode *node, *nnode;
  struct listnode *node2, *nnode2;
  struct tlvs tlvs;
  struct list *lsp_list = NULL;
  struct isis_passwd *passwd;
  struct isis_lsp_entry *p_entry;
  ...
  if (snp_type == ISIS_SNP_CSNP_FLAG)
  {
    ...
    if (pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_CSNP_HDRLEN) ||
        ...
        pdu_len > stream_get_endp(circuit->rcv_stream))
    {
      zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_RECEIVE, "Received a CSNP with bogus length %d", pdu_len);
      return ISIS_WARNING;
    }
  }
  else
  {
    ...
    if (pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_PSNP_HDRLEN) ||
        ...
        pdu_len > stream_get_endp(circuit->rcv_stream))
    {
      zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_RECEIVE, "Received a PSNP with bogus length %d", pdu_len);
      return ISIS_WARNING;
    }
  }
  ...
  if (circuit->ext_domain)
  {
    ...
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Snp (%s): Rcvd L%d %cSNP on %s, "
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Snp (%s): Rcvd L%d %cSNP from %s on %s, "
  ...
  else
  {
    if (!circuit->u.p2p.neighbor)
    {
      zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "no p2p neighbor on circuit %s", circuit->interface->name);
      return ISIS_OK; /* Silently discard */
    }
  }
  ...
  if (retval > ISIS_WARNING)
  {
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "something went very wrong processing SNP");
    free_tlvs(&tlvs);
    return retval;
  }
  ...
  if (tlvs.lsp_entries)
  {
    ...
    {
      zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Snp (%s):         %cSNP entry %s, seq 0x%08x,"
  ...
  if (tlvs.lsp_entries)
  {
    for (ALL_LIST_ELEMENTS_RO(tlvs.lsp_entries, node, entry))
    {
      ...
      own_lsp = !memcmp(entry->lsp_id, circuit->area->sysid, ISIS_SYS_ID_LEN);
      if (lsp)
      {
      ...
      else
      {
        ...
        //memcmp (entry->lsp_id, circuit->area->sysid, ISIS_SYS_ID_LEN))
        {
          if (memcmp(entry->lsp_id, circuit->area->sysid, ISIS_SYS_ID_LEN))
          {
          ...
          {
            p_entry = XCALLOC(MTYPE_ISIS_TLV, sizeof(struct isis_lsp_entry));

            memcpy(p_entry->lsp_id, entry->lsp_id, ISIS_SYS_ID_LEN + 2);
            p_entry->checksum = entry->checksum;
            p_entry->rem_lifetime = entry->rem_lifetime;
            p_entry->seq_num = 0;

            listnode_add(circuit->same_sys_psnp[level - 1], p_entry);
          }
        }
      }
    }
...
process_csnp(int level, struct isis_circuit *circuit, const u_char *ssnpa)
{
  ...
  zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Snp (%s): Rcvd L%d CSNP on %s, cirType %s, cirID %u",
             circuit->area->area_tag, level, circuit->interface->name,
             circuit_t2string(circuit->is_type), circuit->circuit_id);
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_RECEIVE, "Packet too short ( < %d)", ISIS_CSNP_HDRLEN);
    return ISIS_WARNING;
  }
...
process_psnp(int level, struct isis_circuit *circuit, const u_char *ssnpa)
{
  ...
  zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_RECEIVE, "ISIS-Snp (%s): Rcvd L%d PSNP on %s, cirType %s, cirID %u",
             circuit->area->area_tag, level, circuit->interface->name,
             circuit_t2string(circuit->is_type), circuit->circuit_id);
  //      if (isis->debugs & DEBUG_PACKET_DUMP)
  //    zlog_dump_data (STREAM_DATA (circuit->rcv_stream),
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_RECEIVE, "Packet too short ( < %d)", ISIS_PSNP_HDRLEN);
    return ISIS_WARNING;
  }
...
isis_handle_pdu(struct isis_circuit *circuit, u_char *ssnpa)
{
  ...
  if (hdr->version1 != 1)
  {
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "Unsupported ISIS version %u", hdr->version1);
    return ISIS_WARNING;
  }
  /* either 6 or 0 */
  ...
  if (hdr->version2 != 1)
  {
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "Unsupported ISIS version %u", hdr->version2);
    return ISIS_WARNING;
  }

  if (circuit->is_passive)
  {
    zlog_debug(ISIS_DEBUG_TYPE_RECEIVE, "Received ISIS PDU on passive circuit %s",
               circuit->interface->name);
    return ISIS_WARNING;
  }
  ...
  case L2_LAN_HELLO:
    ...
    break;
  case P2P_HELLO:
    retval = process_p2p_hello(circuit, ssnpa);
    break;
  case L1_LINK_STATE:
    retval = process_lsp(ISIS_LEVEL1, circuit, ssnpa);
    isis_sta.isis_lsp_recv++;
    break;
  case L2_LINK_STATE:
...
int isis_receive(struct ipc_mesg_n *pmsg, int imlen)
{
  struct isis_circuit *circuit;
  struct pkt_buffer *rcv_buf = NULL;
  ...
  /*
     while(j)
     {
    	...
    	if (!IFM_IS_SUBPORT(rcv_buf->in_ifindex))
    	{
    		if(rcv_buf->svlan != 0)
    		{
    			zlog_err("ISIS Receive Packet,receive %0x from sub %d.", rcv_buf->in_ifindex, rcv_buf->svlan);
    			continue;
    		}
    	}
    */
  ...
  // if(revln <= imlen)
  // {
  //	 if(pmsg->msghdr.msg_type == IPC_TYPE_FTM)
  // {

  //	if(pmsg->msg_data == NULL)
  //	{
  //		zlog_err(" isis receive null data from ftm \n");
  //		return 0;
  //	}

  rcv_buf = (struct pkt_buffer *)(pmsg->msg_data);
  if (rcv_buf == NULL)
  {
  ...
  APP_RECV_PKT_FROM_FTM_DATA_SET(rcv_buf);

  if (rcv_buf->data == NULL)
  {
    zlog_err("%-15s[%d]: pkt->data is NULL! data_len = %d", __FUNCTION__, __LINE__, rcv_buf->data_len);
    return -1;
  }
  ...
  // if(rcv_buf == NULL)
  //	 {
  //	 zlog_debug(ISIS_DEBUG_TYPE_MSG,"isis recv msg: datalen error, data_len=%d, msgrcv len = %d\n", revln, imlen);
  //	 return 0;
  // }

  /*检查能否根据pkt_buffer中的in_index值查找到circuit*/
  ifm_get_name_by_ifindex(rcv_buf->in_ifindex, ifname_tmp);
  ...
  for (i = 0; i < 6; i++)
  {
    ssnpa[i] = rcv_buf->cb.ethcb.smac[i];
  }

  if (!memcmp(ssnpa, circuit->interface->hw_addr, 6))
  {
    //continue;
    return 0;
  }

  if (circuit->rcv_stream->size < (size_t)(rcv_buf->data_len - 3))
  {
    //continue;
    return 0;
  }
  ...
  stream_write(circuit->rcv_stream, (unsigned char *)rcv_buf->data + 3, rcv_buf->data_len - 3);
  ...
  //thread_add_event (master, isis_receive, NULL, 0); /* thread to receive packet */
  return 0;
}
...
void fill_fixed_hdr(struct isis_fixed_hdr *hdr, u_char pdu_type)
{
  ...
  case L2_PARTIAL_SEQ_NUM:
    ...
    break;
  default:
    zlog_debug(ISIS_DEBUG_TYPE_SEND, "fill_fixed_hdr(): unknown pdu type %d", pdu_type);
    return;
  }
  hdr->length += ISIS_FIXED_HDR_LEN;
  hdr->pdu_type = pdu_type;
  hdr->version1 = 1;
  hdr->id_len = 6; /* ISIS_SYS_ID_LEN -  0==6 */
  hdr->version2 = 1;
  hdr->max_area_addrs = 3; /* isis->max_area_addrs -  0==3 */
}
...
int send_hello(struct isis_circuit *circuit, int level)
{
  ...
  if (isis_circuit_pdu_size(circuit) == 0)
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "circuit has zero MTU");
    return ISIS_WARNING;
  }
  ...
  memcpy(hello_hdr.source_id, circuit->area->sysid, ISIS_SYS_ID_LEN);
  ...
  if(circuit->area->gr)
  {
    if(tlv_graceful_restart(0x02, circuit->area->gr_interval ,u_char * sysid, circuit->snd_stream))
      return ISIS_WARNING;
  }
#endif
#if 1
  ...
  {
    /* 
      only add link-local address in hello packets
      rfc 5308
    */
    if (circuit->ipv6_link && listcount(circuit->ipv6_link) > 0)
    {
      if (tlv_add_ipv6_addrs_for_hello(circuit->ipv6_link, circuit->snd_stream, NULL))
        return ISIS_WARNING;
    }
  }
  ...
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "ISIS-Adj (%s): Sending L%d LAN IIH on %s, length %zd",
               circuit->area->area_tag, level, circuit->interface->name,
               length);
  }
  else
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "ISIS-Adj (%s): Sending P2P IIH on %s, length %zd",
               circuit->area->area_tag, circuit->interface->name,
               length);
  }

  /* for sending hello */
  if (!(circuit->tx) || !(circuit->rx))
  {
  ...
  if (retval != ISIS_OK)
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "ISIS-Adj (%s): Send L%d IIH on %s failed",
               circuit->area->area_tag, level, circuit->interface->name);

  return retval;
}

int send_lan_l1_hello(void *argv)
{
  struct isis_circuit *circuit;
  int retval;
  ...
  if (!(circuit->area->is_type & IS_LEVEL_1))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "ISIS-Hello (%s): Trying to send L1 IIH in L2-only area",
               circuit->area->area_tag);
    return 1;
  }
  ...
  THREAD_TIMER_ON_NEW((char *)"ISISSendL1Hello", circuit->u.bc.t_send_lan_hello[0],
...
int send_lan_l2_hello(void *argv)
{
  struct isis_circuit *circuit;
  int retval;
  circuit = (struct isis_circuit *)argv;
  ...
  if (!(circuit->area->is_type & IS_LEVEL_2))
  {
    zlog_debug(ISIS_DEBUG_TYPE_HELLO | ISIS_DEBUG_TYPE_SEND, "ISIS-Hello (%s): Trying to send L2 IIH in L1 area",
               circuit->area->area_tag);
    return 1;
  }
  ...
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendL2Hello", circuit->u.bc.t_send_lan_hello[1],
...
int send_p2p_hello(void *argv)
{
  ...
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendP2PHello", circuit->u.p2p.t_send_p2p_hello,
           ...
           struct isis_circuit *circuit)
{
  ...
  stream_put(circuit->snd_stream, circuit->area->sysid, ISIS_SYS_ID_LEN);
...
int send_csnp(struct isis_circuit *circuit, int level)
{
  ...
  while (loop)
  {
    ...
    if (retval != ISIS_OK)
    {
      zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Build L%d CSNP on %s failed",
                 circuit->area->area_tag, level, circuit->interface->name);
      list_delete(list);
      list = NULL;
      return retval;
    ...
    zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Sending L%d CSNP on %s, length %lu",
               circuit->area->area_tag, level, circuit->interface->name,
               stream_get_endp(circuit->snd_stream));
    for (ALL_LIST_ELEMENTS_RO(list, node, lsp))
    {
      zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s):         CSNP entry %s, seq 0x%08x,"
                                                              " cksum 0x%04x, lifetime %us",
                 circuit->area->area_tag,
    ...
    if (retval != ISIS_OK)
    {
      zlog_debug(ISIS_DEBUG_TYPE_CSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Send L%d CSNP on %s failed",
                 circuit->area->area_tag, level,
                 circuit->interface->name);
      list_delete(list);
      list = NULL;
...
int send_l1_csnp(void *argv)
{
  ...
  if ((circuit->circ_type == CIRCUIT_T_BROADCAST && circuit->u.bc.is_dr[0]) ||
      (circuit->circ_type == CIRCUIT_T_P2P && circuit->area->area_passwd.len != 0))
  {
    send_csnp(circuit, 1);
  }
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendL1CsnpNoLoop", circuit->t_send_csnp[0], send_l1_csnp, circuit,
                      isis_jitter(circuit->csnp_interval[0], CSNP_JITTER));

  return retval;
}

int send_l2_csnp(void *argv)
{
  struct isis_circuit *circuit;
  int retval = ISIS_OK;
  ...
  if ((circuit->circ_type == CIRCUIT_T_BROADCAST && circuit->u.bc.is_dr[1]) ||
      (circuit->circ_type == CIRCUIT_T_P2P && circuit->area->domain_passwd.len != 0))
  {
    send_csnp(circuit, 2);
  }
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendL2CsnpNoLoop", circuit->t_send_csnp[1], send_l2_csnp, circuit,
...
static int
build_psnp(int level, struct isis_circuit *circuit, struct list *lsps, struct list *c_lsp_list)
{
  struct isis_fixed_hdr fixed_hdr;
  unsigned long lenp;
  u_int16_t length;
  ...
  if (circuit->same_sys_psnp[level - 1] != NULL && listcount(circuit->same_sys_psnp[level - 1]) != 0)
  {
    retval = tlv_add_circuit_lsp_entries(circuit->same_sys_psnp[level - 1], circuit->snd_stream);
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s):         PSNP entry %s, seq 0x%08x,"
                                                            " cksum 0x%04x, lifetime %us",
               circuit->area->area_tag,
               rawlspid_print(lsp->lsp_header->lsp_id),
               ntohl(lsp->lsp_header->seq_num),
...
send_psnp(int level, struct isis_circuit *circuit)
{
  ...
  while (1)
  {
    ...
    if (listcount(list) == 0 && listcount(circuit->same_sys_psnp[level - 1]) == 0)
    {
      list_delete(list);
      list = NULL;
      return ISIS_OK;
    ...
    retval = build_psnp(level, circuit, list, circuit->same_sys_psnp[level - 1]);
    if (retval != ISIS_OK)
    {
      zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Build L%d PSNP on %s failed",
                 circuit->area->area_tag, level, circuit->interface->name);
      list_delete(list);
      list = NULL;
      list_delete_all_node(circuit->same_sys_psnp[level - 1]);
      return retval;
    }

    // if (isis->debugs & DEBUG_SNP_PACKETS)

    zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Sending L%d PSNP on %s, length %lu",
               circuit->area->area_tag, level,
               circuit->interface->name,
               stream_get_endp(circuit->snd_stream));
    // if (isis->debugs & DEBUG_PACKET_DUMP)
    ...
    if (retval != ISIS_OK)
    {
      zlog_debug(ISIS_DEBUG_TYPE_PSNP | ISIS_DEBUG_TYPE_SEND, "ISIS-Snp (%s): Send L%d PSNP on %s failed",
                 circuit->area->area_tag, level,
                 circuit->interface->name);
      list_delete(list);
      list = NULL;
...
int send_l1_psnp(void *argv)
{
  ...
  send_psnp(1, circuit);
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendL1PsnpNoLoop", circuit->t_send_psnp[0], send_l1_psnp, circuit,
...
int send_l2_psnp(void *argv)
{
  ...
  /* set next timer thread */
  THREAD_TIMER_ON_NEW((char *)"ISISSendL2PsnpNoLoop", circuit->t_send_psnp[1], send_l2_psnp, circuit,
...
int send_lsp(struct thread *thread)
{
  ...
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_SEND, "ISIS-Upd (%s): Can't send L%d LSP %s, seq 0x%08x,"
                                                           " cksum 0x%04x, lifetime %us on %s. LSP Size is %lu"
  ...
  // if (isis->debugs & DEBUG_UPDATE_PACKETS)
  //   {
  zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_SEND, "ISIS-Upd (%s): Sending L%d LSP %s, seq 0x%08x, cksum 0x%04x,"
                                                         " lifetime %us on %s",
             circuit->area->area_tag, lsp->level,
             rawlspid_print(lsp->lsp_header->lsp_id),
             ntohl(lsp->lsp_header->seq_num),
  ...
  if (retval != ISIS_OK)
  {
    zlog_debug(ISIS_DEBUG_TYPE_LSP | ISIS_DEBUG_TYPE_SEND, "ISIS-Upd (%s): Send L%d LSP on %s failed %s",
               circuit->area->area_tag, lsp->level,
               circuit->interface->name,
               (retval == ISIS_WARNING) ? "temporarily" : "permanently");
  }
  isis_sta.isis_lsp_send++;
out:
            ...
            int level)
{
  ...
  retval = circuit->tx(circuit, level);
  if (retval != ISIS_OK)
    zlog_debug(ISIS_DEBUG_TYPE_LSP, "ISIS-Upd (%s): Send L%d LSP PSNP on %s failed",
               circuit->area->area_tag, level,
               circuit->interface->name);
