...
//	printf("%s %d \n",__FUNCTION__, __LINE__);
//	debug_printf(req->header_line, bytes_to_write);
#ifdef ZEBRA	
	pthread_mutex_lock(&g_web_syn_lock);
	bytes_written = web_tcp_data_write(req->fd, req->header_line, bytes_to_write);
...
//	debug_printf(req->header_line, bytes_written);
