...
//	printf("%s : %d req->request_uri %s req->pathname %s\n", __FUNCTION__, __LINE__, req->request_uri,req->pathname);
...
//	printf("%s : %d req->mmap_entry_var %s  \n", __FUNCTION__, __LINE__,req->mmap_entry_var->mmap);
...
int process_get(request * req)
{
    ...
    if (sigsetjmp(env, 1) == 0) {
        ...
        bytes_written = web_tcp_data_write(req->fd, req->data_mem + req->filepos,
                              bytes_to_write);
...
//		debug_printf(req->data_mem + req->filepos, bytes_written);
        handle_sigbus = 0;
        /* OK, SIGBUS **after** this point is very bad! */
    } else {
        /* sigbus! */
