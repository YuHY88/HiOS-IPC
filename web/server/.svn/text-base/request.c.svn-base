...
#endif
{
    ...
    request *conn;              /* connection */
	    ...
	    remote_addr.S_FAMILY = 0xdead;
			...
			fd = web_tcp_accept(&session_info );
			pthread_mutex_unlock(&g_web_syn_lock);
		    if (fd == -1) 
			{		
//				printf("%s %d no tcp accept\n",__FUNCTION__, __LINE__);
		            /* no requests */
		            pending_requests = 0;
		        return;
		    }
...
/* Pointed out 3-Oct-1999 by Paul Saab <paul@mu.org> */
    ...
    conn->kacount = ka_max;
	#ifdef ZEBRA
			strcpy(conn->local_ip_addr, inet_ntoa(session_info.dst_ip));
    ...
    if (system_bufsize < sockbufsize) {
        if (setsockopt
			...
			strcpy(conn->remote_ip_addr, inet_ntoa(session_info.src_ip));
			/* for possible use by CGI programs */
	        conn->remote_port = session_info.src_port;
...
//		printf("%s : %d current : %p fd : %d\n", __FUNCTION__, __LINE__,current, current->fd);
...
//	printf("%s : %d req->client_stream %s req->request_uri %s \n", __FUNCTION__, __LINE__,req->client_stream, req->request_uri);
