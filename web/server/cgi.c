...
#ifdef ZEBRA	
    my_add_cgi_env(req, "SYSLOG_DEBUG", simple_itoa(syslog_debug));
#endif
    if (req->path_info)
...
int init_cgi(request * req)
{
    ...
    int pipes[2];
    int use_pipes = 0;
//	printf("%s : %d req->request_uri %s req->pathname %s req->header_line : %s\n\n", __FUNCTION__, __LINE__, req->request_uri,req->pathname,req->header_line);

//#ifdef ZEBRA
//		int sfd ;
//		req->stdout= dup(STDOUT_FILENO);
//		printf("=================req->stdout : %d===================\n",req->stdout);
//#endif
