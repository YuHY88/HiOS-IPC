...
#include "read_config.h"       /* for "internal" definitions */
#include "utilities.h"
...
extern int snmp_init_config(void);
...
void print_config_files()
{
    struct config_files *ctmp = config_files;

    printf( "print_config_files\n" );
    for (; ctmp != NULL; ctmp = ctmp->next)
    {
        printf( "%s\n", ctmp->fileHeader);
    }
    printf( "print_config_files end\n" );
}
                         ...
                         const char *token)
{
    ...
    for (lptr = line_handlers; lptr != NULL; lptr = lptr->next) {
        printf( "%s %d, %s\n", lptr->config_token, strlen(lptr->config_token), token );
    }
    printf("read_config_find_handler end\n\n");
#endif
...
int write_configs(char *token, char *cptr)
{
	const char *type;
	struct config_line *ctmp;
	struct config_line *lptr;
	char  *cp;
	char   line[STRINGMAX];
	...
	ctmp = read_config_get_handlers(type);
	if (NULL == ctmp)
	{
		return SNMPERR_GENERR;
	}
	lptr = read_config_find_handler(ctmp, token);
	if (lptr != NULL)
	{
		memset(line, 0, STRINGMAX);	
		strcpy(line, cptr);

		cp = &(line[strlen(line)-1]);		//不可省略
		while (isspace(*cp)) 
		{
			*(cp--) = '\0';
		}
		...
		//lptr->parse_line调用时不能是静态数据空间，所以再此处用line做了一个转换，替换了cptr
		(*(lptr->parse_line)) (token, line);		
...
/*
	...
	write_configs("rocommunity", 	 "hios");
	write_configs("rocommunity", 	 "public");
	write_configs("rwcommunity", 	 "private");
	...
	write_configs("gruop", 			"admingrp usm hios");
...
read_premib_configs(void)
{
	...
	//这个函数也可以注释掉，因为我们的配置文件中不会放mib
	//另外，因为我们后面不会传递-c的参数，所以这句话不会执行到
    /*if (NULL != optional_config)
        read_configs_optional(optional_config, PREMIB_CONFIG);*/
