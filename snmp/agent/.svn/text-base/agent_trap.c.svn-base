                    ...
                    char *com, int version, int pdutype)
{
    ...
    session.is_ipc_trap = 1 ; //added by lidingcheng
                          ...
                          char * context, int flags)
{
    ...
    for (sink = sinks; sink; sink = sink->next) {
        ...
        if (sink->version == SNMP_VERSION_1) {
            if (template_v1pdu) {
                ...
                netsnmp_ipc_addr_pair *addr_pair;

                addr_pair = (netsnmp_ipc_addr_pair *)malloc(sizeof(netsnmp_ipc_addr_pair));				
                netsnmp_sockaddr_in_ipc(&client_addr,sink->sesp->peername, sink->sesp->remote_port);
	            memset(addr_pair, 0, sizeof(netsnmp_ipc_addr_pair));				
                memcpy(&(addr_pair->remote_addr), &client_addr, sizeof(struct sockaddr_in));		

                template_v1pdu->transport_data = addr_pair;
                template_v1pdu->transport_data_length = sizeof(netsnmp_ipc_addr_pair);
            ...
            if (template_v2pdu) {
                ...
                template_v2pdu->transport_data = addr_pair;
                template_v2pdu->transport_data_length = sizeof(netsnmp_ipc_addr_pair);
	...
	for (sink = sinks; sink; sink = sink->next) {
			...
			if (sink->version == SNMP_VERSION_1) {
				if (template_v1pdu) {
					...
					if (NULL == template_v1pdu->transport_data)
					{
						addr_pair = (netsnmp_ipc_addr_pair *)malloc(sizeof(netsnmp_ipc_addr_pair));
						template_v1pdu->transport_data = addr_pair;
					}
					...
					{
						addr_pair = template_v1pdu->transport_data;
					}
					template_v1pdu->transport_data_length = sizeof(netsnmp_ipc_addr_pair);
				...
				if (template_v2pdu) {
					...
					if (NULL == template_v2pdu->transport_data)
					{
						addr_pair = (netsnmp_ipc_addr_pair *)malloc(sizeof(netsnmp_ipc_addr_pair));
						template_v2pdu->transport_data = addr_pair;
					}
					...
					{
						addr_pair = template_v2pdu->transport_data;
					}
					template_v2pdu->transport_data_length = sizeof(netsnmp_ipc_addr_pair);
    ...
    if (template_v1pdu!=NULL)
    {
        snmp_free_pdu(template_v1pdu);
    }		 
    if (template_v2pdu!=NULL)	
    {
        snmp_free_pdu(template_v2pdu);
    }
